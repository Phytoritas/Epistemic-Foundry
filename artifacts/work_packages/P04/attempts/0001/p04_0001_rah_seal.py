#!/usr/bin/env python3
"""Append P04-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "P04-0001"
WORK_PACKAGE_ID = "P04"
EXPECTED_PARENT = "000144-c758cb46"
EXPECTED_PARENT_COUNT = 144
EXPECTED_PARENT_EVIDENCE = "E0146"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "b07c9e70363054531ea8f1c6c4905a017c0730a26d3ba4be3daf4b99e3073e4e"
)
CORE_EVIDENCE_ID = "E0147"
FINAL_EVIDENCE_ID = "E0148"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_p04_0001_evidence as evidence  # noqa: E402
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required P04-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "P04-0001 PASS core verification: the judge and independent "
        "attestation gate is implemented in "
        "python/epistemic_foundry/parliament/adjudication with exact P02-0001 "
        "and P03-0001 dependency bindings and a P03-0001 regression baseline. "
        "Both vocabularies are read from schemas/adjudication.schema.json and "
        "schemas/attestation.schema.json. A majority cannot promote: the "
        "recommendation is bounded above by the deterministic P02 ceiling, a "
        "unanimous panel cannot lift it, the majority is recorded as "
        "non-load-bearing and cannot be flipped by rehashing, a sustained veto "
        "or an unanswered cross-examination forces BLOCK, every open challenge "
        "must appear as an unresolved issue, the strongest dissent must be "
        "cited, and a deterministic gate override attempt is refused. The "
        "attestor reads structure, not persuasion: the pack carries ids, "
        "hashes, statuses, ceilings and counts, every persuasive field is "
        "refused anywhere in the pack or in the attestation's own checks, the "
        "judge may not attest to itself, and an attestation must rest on "
        "exactly the pack it was given with checks in canonical shape. Ruff "
        "lint/format, majority_independence_test 22/22, attestation_test "
        "30/30, targeted 52/52, auditors regression 42/42, cross-exam "
        "regression 43/43, full Python 1261/1261, full Node 982/982 across 91 "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"P04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". P03-0001, P02-0001, P01-0001, and every prior RAH generation "
        "remain immutable. P04 bounds and attests an adjudication that arrives "
        "as an input; it does not produce the judge's reasoning, cannot verify "
        "that the attestor ran the checks it lists or check its signature "
        "cryptographically, and does not commit promotion. The P-phase "
        "x01-x04 packages are PASS, but T04, B05, C05, R05, source coverage, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("P04-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "P04-0001 judge and independent attestation gate is evidence-sealed "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P04-0001 parent is not the sealed P03-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("P04 preflight requires exactly 144 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("P04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("P04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("P04 core did not append exactly E0147")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("P04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("P04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("P04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored P04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("P04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("P04 final did not append exactly E0148")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("P04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("P04 core tail is not E0147")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 146:
        raise SystemExit(f"unexpected P04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with P04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored P04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
