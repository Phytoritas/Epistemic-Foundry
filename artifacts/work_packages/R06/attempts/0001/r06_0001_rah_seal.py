#!/usr/bin/env python3
"""Append R06-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000220-f47b7e6d"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 220
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0222"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "9a9676dabce366c17d77301c4f76089b9b92a097fe7d8a9078277afb446be506"
)
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0223"
FINAL_EVIDENCE_ID = "E0224"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_r06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after R06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} causal, measurement and scope crossover safety integration gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "R06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required R06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: intake namespace is pinned so a change to the composed I05 surface (or the
    #: parent marker) is caught here too.
    relatives = sorted(
...
def core_summary() -> str:
    ...
    return (
        "R06-0001 PASS core verification: causal, measurement and scope crossover safety "
        "integration gate, implemented in src/epistemic_foundry/reasoning/v4_r06 by "
        "a bounded implementation subagent and reviewed by the sealing agent "
        "(actor-independent from the author) under exact dependency bindings "
        "(R05 R05-0001, composing R04 causal identification) and a W05-0001 regression baseline. The "
        "gate composes the sealed R05 typed-crossover surface and R04 causal "
        "identification output without restating them, then re-derives each "
        "crossover axis from the parents' own artifacts: causal identification, "
        "measurement compatibility, scope comparability and unit compatibility "
        "must each be compatible, and a compatibility report that disagrees with "
        "the derivation or is not an unconditional allow is refused; no candidate, "
        "model, prompt, backend or hook acquires evaluator, holdout or promotion authority. "
        "Twenty-four finding codes name exact refusals and a refusal surveys every "
        "failure at once. Every admit or refuse decision re-derives byte for "
        "byte from its own published fields with no clock or random draw, and "
        "nothing scores, ranks, selects, promotes or evaluates. Ruff lint/"
        "format, schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the R05 dependency regression, full Python and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"R06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. R06 re-derives each crossover axis and "
        "records a deterministic allow/refuse receipt only: it "
        "does not score, rank, select, promote or evaluate any candidate, grants "
        "no authority at the gate, makes no DSSAT or plant-model parity claim, and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("R06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"R06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("R06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("R06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"R06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("R06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("R06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("R06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored R06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("R06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"R06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("R06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"R06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected R06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with R06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored R06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
