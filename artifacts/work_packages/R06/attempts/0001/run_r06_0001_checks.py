#!/usr/bin/env python3
"""Run the R06-0001 crossover safety integration gate acceptance checks.

Required checks from manifests/development_manifest.yaml (R06):
...
gate ships under src/epistemic_foundry/reasoning/v4_r06; the ``reasoning``
package marker already existed before this attempt (created by R05), so the
level that could regress here is the new ``reasoning/v4_r06`` marker, and
``packaging-discovery`` proves it actually reaches the wheel rather than only
importing from the checkout.

R06 depends on R05, whose typed operator registry the gate composes: the gate
reads the canonical genome properties and the mutable-kind vocabulary through
``epistemic_foundry.reasoning.v4_r05.operators``.  R05's product tests live in
its own attempt directory, so the single dependency regression exercises that
sealed suite, and a regression in the composed R05 surface fails here rather
than only inside the full suite.
...
# R06's write scope is src/epistemic_foundry/reasoning/v4_r06/**; the enclosing
# ``reasoning`` package marker pre-dates this attempt and is owned by R05, so
# lint/format run only over the R06-owned subtree plus this attempt directory.
REASONING_V4_R06 = "src/epistemic_foundry/reasoning/v4_r06"
PRODUCT_TESTS = {
...
# R05 owns the typed operator registry whose canonical genome properties and
# mutable-kind vocabulary the gate composes.  A change in R05 must fail here, so
# its sealed suite (which lives in R05's attempt directory) runs as a
...
def ruff_check() -> int:
    return run(
        "ruff-check",
        ["uv", "run", "--locked", "ruff", "check", REASONING_V4_R06, ATTEMPT_DIR],
    )
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/reasoning/v4_r06 is a new package marker, so the
...
def targeted() -> int:
    return _pytest("targeted-r06-gate", ATTEMPT_DIR)
