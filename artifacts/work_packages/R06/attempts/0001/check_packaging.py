#!/usr/bin/env python3
"""Prove the R06 crossover safety gate is discoverable by the packaging config.

``src/epistemic_foundry/reasoning/v4_r06`` is a new package marker authorized by
the R06 write scope in ``manifests/development_manifest.yaml``, so there is a
scope decision to prove rather than assume: a marker that never reached the
...
change there fails here. ``reasoning`` itself was authorized by R05 and is
verified again so a regression there surfaces at this gate too.
"""
...
EXPECTED = ("epistemic_foundry.reasoning", "epistemic_foundry.reasoning.v4_r06")
