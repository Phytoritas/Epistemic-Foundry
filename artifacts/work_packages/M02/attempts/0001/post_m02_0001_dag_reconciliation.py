#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed M02-0001 PASS."""
...
OUTPUT = ATTEMPT / "post-m02-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-m02-0001-dag-review.md"
EXPECTED_M02_REPORT_HASH = (
    "sha256:2834ac5c90e6b4ebf03f6df03329b602dfcfd27ca590b9c91bca802a89c7d493"
)
...
def reconcile() -> dict[str, Any]:
    ...
    m02 = states["M02"]
    if not isinstance(m02.get("report"), str):
        raise SystemExit("M02 report is missing")
    m02_report = base.read_json(ROOT / m02["report"])
    rah = m02_report.get("rah_state")
    if not (
        m02["selected_attempt_id"] == "M02-0001"
        and m02["current_state"] == "PASS"
        and m02["report_sha256"] == EXPECTED_M02_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0056"
        and rah.get("core_generation") == "000056-277d83f7"
        and rah.get("final_closeout_evidence_id") == "E0057"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("M02-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "M02_attempt_id": "M02-0001",
            "M02_core_evidence_id": "E0056",
            "M02_core_generation": "000056-277d83f7",
            "M02_final_closeout_evidence_id": "E0057",
            "M02_final_generation": "000057-e5778c8c",
            "M02_report": m02["report"],
            "M02_report_sha256": m02["report_sha256"],
            "status": "PASS",
        },
        ...
        "external_resume_inspection": {
            ...
            "observed_state_generation": "000057-e5778c8c",
            "parse_errors": {},
            "ralph_status": "active",
            "status": "PASS",
        },
        ...
        "projection": "POST_M02_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 55
        and result["waiting_package_count"] == 97
        and result["ready_packages_manifest_order"] == ["M03", "N01", "T01", "A06"]
        and result["next_package"] == "M03"
    ):
        raise SystemExit(
            "post-M02 DAG differs from evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-M02-0001 live DAG reconciliation
