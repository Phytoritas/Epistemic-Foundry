#!/usr/bin/env python3
"""Append the verified post-M02-0001 DAG reconciliation to active RAH state."""
...
import m02_0001_rah_seal as m02_seal  # noqa: E402
import post_m02_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000057-e5778c8c"
EXPECTED_PARENT_MANIFEST_HASH = (
    "f6e33d5cec2003eac82df9af845909fcff13967181c0c4b7aad8229bc8cb95f2"
)
EXPECTED_EVIDENCE_ID = "E0058"
NEXT_ACTIONS = [
    "Execute M03-0001 query personalization, risk, and change impact under its exact manifest scope.",
    "Run M03 personalization_test and blast_radius_test with upstream regressions and separate primary-session review.",
    "Seal M03 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-m02-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 55
        and stored.get("waiting_package_count") == 97
        and stored.get("ready_packages_manifest_order") == ["M03", "N01", "T01", "A06"]
        and stored.get("next_package") == "M03"
    ...
    review_path = ATTEMPT / "post-m02-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_m02_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-M02-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "M02-0001 final evidence E0057 / 000057-e5778c8c. The canonical external "
        ...
        "no unknown dependency and no cycle; 55 packages are PASS, 4 are "
        "dependency-ready, and 97 wait on unmet dependencies. Manifest-order READY "
        "is M03, N01, T01, A06, so M03 is next. Highest numeric attempt selection "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed M02 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0057" or len(base.numbered_generations(ralph_root)) != 57:
        raise SystemExit("post-M02 DAG seal requires exact E0057 / 57-generation state")
    ...
    note = (
        "The sealed post-M02 DAG selects M03 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0057":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0058 summary differs from current DAG artifacts")
    result = m02_seal.verify_generation_store(58)
