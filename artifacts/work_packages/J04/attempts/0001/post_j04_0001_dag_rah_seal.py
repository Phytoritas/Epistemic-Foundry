#!/usr/bin/env python3
"""Append the verified post-J04-0001 DAG reconciliation to active RAH state."""
...
import post_j04_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000027-d83e42d6"
EXPECTED_EVIDENCE_ID = "E0028"
NEXT_ACTIONS = [
    "Execute K01-0001 canonical local artifact store under its exact write scope.",
    "Run K01 required checks, full regressions, and a separate primary-session review.",
    "Seal K01 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-j04-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 45
        and stored.get("ready_packages_manifest_order")
        == ["K01", "L01", "N01", "T01", "A06"]
        and stored.get("next_package") == "K01"
    ...
    review_path = ATTEMPT / "post-j04-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_j04_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-J04-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "J04-0001 final evidence E0027 / 000027-d83e42d6. The active development "
        "manifest has 156 unique packages, no unknown dependency and no cycle; "
        "45 packages are currently PASS, 5 are dependency-ready, and 106 wait on "
        "unmet dependencies. Manifest-order READY is K01, L01, N01, T01, A06, so "
        "K01 is the next bounded package. Package state is selected from the highest "
        "numeric attempt, preventing an older PASS from hiding a newer non-PASS or "
...
def commit(summary: str) -> str:
    ...
    identifiers = evidence_ids(payloads)
    if identifiers[-1] != "E0027" or len(numbered_generations(ralph_root)) != 27:
        raise SystemExit("post-J04 DAG seal requires the exact E0027 / 27-generation state")
    ...
    note = (
        "The sealed post-J04 DAG selects K01 as the earliest dependency-ready "
...
def main() -> int:
    ...
    ralph_root, current, payloads = current_state()
    identifiers = evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0027":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0028 summary differs from current DAG artifacts")
    result = verify_generation_store(28)
