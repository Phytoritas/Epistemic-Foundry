#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed J04-0001 PASS.

For every work package the highest numeric attempt directory is authoritative.
An older PASS therefore cannot hide a newer non-PASS or incomplete attempt.
"""
...
OUTPUT = ATTEMPT / "post-j04-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-j04-0001-dag-review.md"
EXPECTED_PACKAGE_COUNT = 156
EXPECTED_J04_REPORT_HASH = (
    "sha256:602c3dc98643608d93304c31c2f0232c449c778b586dded1ffbe416871341e16"
)
...
def reconcile() -> dict[str, Any]:
    ...
    j04 = states["J04"]
    if not isinstance(j04.get("report"), str):
        raise SystemExit("J04 report is missing")
    j04_report = read_json(ROOT / j04["report"])
    rah = j04_report.get("rah_state")
    if not (
        j04["selected_attempt_id"] == "J04-0001"
        and j04["current_state"] == "PASS"
        and j04["report_sha256"] == EXPECTED_J04_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0026"
        and rah.get("core_generation") == "000026-ca8416cb"
        and rah.get("final_closeout_evidence_id") == "E0027"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("J04-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "J04_attempt_id": "J04-0001",
            "J04_core_evidence_id": "E0026",
            "J04_core_generation": "000026-ca8416cb",
            "J04_final_closeout_evidence_id": "E0027",
            "J04_report": j04["report"],
            "J04_report_sha256": j04["report_sha256"],
            "status": "PASS",
        },
        ...
        "projection": "POST_J04_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 45
        and result["blocked_package_count"] == 106
        and result["ready_packages_manifest_order"]
        == ["K01", "L01", "N01", "T01", "A06"]
        and result["next_package"] == "K01"
    ):
        raise SystemExit(
            "post-J04 DAG differs from the expected evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-J04-0001 live DAG reconciliation
