#!/usr/bin/env python3
"""Append J04-0001 core and closeout evidence to active RAH state."""
...
import build_j04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_INITIAL_PARENT = "000025-8d5d617e"
EXPECTED_INITIAL_EVIDENCE = "E0025"
NEXT_ACTIONS = [
    "Recompute and seal the live 156-package DAG after J04-0001 PASS.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required J04 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "J04-0001 PASS: post-compaction recovery verifies ContextCapsule integrity, "
        "external sealed-receipt binding, and J03 freshness in that order before "
        "projecting phase, blockers, RunSpec, policy, included artifacts, or "
        "exclusions. Untrusted prose is never read as authority; direct tamper, "
        "attacker rehash, missing/changed/unaccounted artifacts, phase/RunSpec/policy "
        "drift, excluded-content escalation, accessors, and unknown fields fail "
        "closed. J04 targeted is 10/10, unchanged J03 capsule regression is 21/21, "
        "full Node is 470/470, and full Python is 990/990. The preserved one-case "
        "pytest-executable collection diagnostic is explicitly non-authoritative. "
        f"Verification sha256:{hashes['compaction-recovery-verification.json']}; "
        f"receipt sha256:{hashes['compaction-recovery-verification.artifact-receipt.json']}; "
        f"regression sha256:{hashes['full-regression-impact.json']}; scope sha256:"
        f"{hashes['write-scope-verification.json']}; dependency sha256:"
        f"{hashes['dependency-status.json']}; targeted JUnit sha256:"
        f"{hashes['targeted-j04-suite.junit.xml']}; J03 JUnit sha256:"
        f"{hashes['j03-capsule-regression.junit.xml']}; Node JUnit sha256:"
        f"{hashes['full-node-suite.junit.xml']}; Python JUnit sha256:"
        f"{hashes['full-python-suite.junit.xml']}; diagnostic sha256:"
        f"{hashes['full-python-suite.collection-diagnostic.junit.xml']}; review sha256:"
        f"{hashes['review.md']}; builder sha256:{hashes['build_j04_0001_evidence.py']}; "
        f"sealer sha256:{hashes['j04_0001_rah_seal.py']}. Review is primary-session "
        "separate with actor_independence=false. Global implementation_gate=fail "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"J04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"regression sha256:{hashes['full-regression-impact.json']}; scope sha256:"
        f"{hashes['write-scope-verification.json']}; dependency sha256:"
        f"{hashes['dependency-status.json']}; RAH integrity sha256:"
        f"{hashes['rah-core-integrity.json']}; builder sha256:"
        f"{hashes['build_j04_0001_evidence.py']}; sealer sha256:"
        f"{hashes['j04_0001_rah_seal.py']}. Every prior attempt and generation is "
        "preserved. J04-0001 is immutable PASS; the live 156-package DAG must now "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("J04 requires a contiguous evidence ledger")
    if (
        ...
        raise SystemExit("J04 preflight requires active/fail/completion_ready=false")
...
def commit_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "J04-0001 post-compaction recovery is evidence-sealed PASS. The live "
        "156-package DAG and downstream packages remain; implementation_gate=fail "
        "and completion_ready=false."
    )
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("J04 report is already RAH-bound; use verify")
    identifiers = evidence_ids(payloads)
    if not (
        generation == EXPECTED_INITIAL_PARENT
        and identifiers[-1] == EXPECTED_INITIAL_EVIDENCE
        and len(numbered_generations(ralph_root)) == 25
    ):
        raise SystemExit("J04 preflight is not at the sealed post-B04 DAG state")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("J04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, core_id]:
        raise SystemExit("J04 core did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("J04 core did not preserve every prior generation")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("J04 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("J04 evidence identifiers differ from the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("J04 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("J04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("J04 final did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J04 final evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("J04 final did not preserve every prior generation")
    return {
