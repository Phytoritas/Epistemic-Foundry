#!/usr/bin/env python3
"""Append Y03-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000328-2e8eecfb"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 328
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0330"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "a5adae4bafc161a8e12b3da9538d615e0bb2a88d55036dfc11b2eb159f672ac0"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0331"
FINAL_EVIDENCE_ID = "E0332"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_y03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after Y03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} backup, disaster-recovery and operational-runbook gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "Y03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Y03-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  Both Y03
    #: write roots -- the operational runbooks and their acceptance tree -- are
    #: pinned so a change to a runbook, the lint contract, or a test is caught.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for root in ("ops/runbooks", "tests/recovery/production")
...
def core_summary() -> str:
    ...
    return (
        "Y03-0001 PASS core verification: the backup, disaster-recovery and "
        "operational-runbook gate, implemented in ops/runbooks and "
        "tests/recovery/production by a bounded implementation agent and reviewed "
        "by this seal-prep session (a distinct actor, actor-independent from the "
        "author) over the sealed Y01-0001 package through the sealed D02 SQLite "
        "state store, D03 content-addressed artifact store and D04 recovery "
        "fixtures, with no new production dependency and no external database, "
        "container or Docker daemon. runbook_lint is fail-closed: it requires the "
        "exact ordered sections Metadata/Preconditions/Procedure/Verification/"
        "Rollback/Escalation, a well-formed id/severity/review-date and RPO/RTO as "
        "measured durations, a 1..N imperative procedure where every step carries a "
        "Verify: line, and rejects any placeholder token, proven by eleven negative "
        "fixtures. disaster_recovery_drill executes the documented RB-Y03-DR-RESTORE "
        "runbook end to end: a coordinated online SQLite backup plus canonical "
        "artifact snapshot, post-backup writes, corruption of both primaries, and a "
        "hash- and manifest-checked staged restore. No loss is hidden by reset: "
        "every pre-backup record and artifact is restored byte-exact into recovered "
        "ACTIVE integrity-ok stores, post-backup writes are asserted ABSENT as "
        "bounded expected loss within RPO, the corrupt primaries stay SAFE_MODE with "
        "digests unchanged as evidence, and a tampered backup refuses restore and "
        "publishes nothing; the measured RPO window and RTO are each within the "
        "15-/30-minute budgets parsed from the runbook. Ruff lint/format, "
        f"disaster_recovery_drill {evidence.EXPECTED_DISASTER_RECOVERY_COUNT}/"
        f"{evidence.EXPECTED_DISASTER_RECOVERY_COUNT}, runbook_lint "
        f"{evidence.EXPECTED_RUNBOOK_LINT_COUNT}/{evidence.EXPECTED_RUNBOOK_LINT_COUNT}, "
        f"targeted {evidence.EXPECTED_TARGETED_COUNT}/{evidence.EXPECTED_TARGETED_COUNT}, "
        f"full Python {evidence.EXPECTED_PYTHON_COUNT}/{evidence.EXPECTED_PYTHON_COUNT}, "
        f"full Node {evidence.EXPECTED_NODE_COUNT}/{evidence.EXPECTED_NODE_COUNT} across "
        f"{evidence.EXPECTED_NODE_FILE_COUNT} files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Y03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Y01-0001 and every prior RAH generation remain immutable. Y03 "
        "documents and qualifies coordinated backup, corruption response, and "
        "hash-checked disaster recovery over the durable state and artifact stores "
        "with measured RPO/RTO; a shipped backup service, cross-region or physical "
        "disaster recovery, production RPO/RTO under load, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Y03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y03-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"Y03 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Y03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Y03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"Y03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Y03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Y03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Y03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Y03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Y03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"Y03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Y03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"Y03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected Y03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Y03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Y03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
