#!/usr/bin/env python3
"""Run the E02-0001 receipt-bound effect coordination acceptance checks.

Required checks from manifests/development_manifest.yaml (E02):
``effect_reconciliation_test`` and ``idempotency_test``.  Both are Node test
files under ``packages/foundry-kernel/src/effects``, alongside the effect
coordinator and its deterministic test support, so they also join the full Node
inventory rather than being targeted only.  The coordinator composes its durable
substrate on the sealed E01 append-only Noetic Ledger, which itself builds on the
sealed D04 SQLite state store and the sealed C04 content-addressed artifact
store; the whole upstream Node surface is the regression that matters, so it is
covered by the full Node suite rather than a bespoke dependency-regression run.
The ``effect_reconciliation_test`` suite also cross-checks each emitted
ActionIntent and EffectReceipt against the canonical Draft 2020-12
``schemas/action-intent.schema.json`` and ``schemas/effect-receipt.schema.json``
through ``uv run python``, so the Python toolchain must be resolvable for the
Node suites to pass.
"""
...
ATTEMPT_SCRIPTS = "artifacts/work_packages/E02/attempts/0001"
...
def ruff_check() -> int:
    # The E02 surface is Node; ruff still guards this attempt's own scripts.
    return run(
...
def effect_reconciliation_test() -> int:
    return _node(
...
def idempotency_test() -> int:
    return _node("idempotency-test", [f"{COMPONENT}/idempotency.test.mjs"])
...
def main() -> int:
    ...
    checks = {
        ...
        "effect-reconciliation-test": effect_reconciliation_test,
