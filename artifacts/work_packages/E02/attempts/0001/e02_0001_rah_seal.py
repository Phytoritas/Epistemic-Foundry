#!/usr/bin/env python3
"""Append E02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000312-6c600ce8"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 312
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0314"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "16b7825d25c2164558a0452f17e8ea36d8da0e4425f264425312bf9f78c3e6b7"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0315"
FINAL_EVIDENCE_ID = "E0316"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after E02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} receipt-bound effect coordination gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "E02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: effects namespace is pinned so a change to the effect coordinator, its
    #: deterministic test support, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/effects").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "E02-0001 PASS core verification: the receipt-bound effect coordinator that "
        "binds ActionIntent, Attempt, and EffectReceipt, implemented in "
        "packages/foundry-kernel/src/effects by a bounded implementation agent and "
        "reviewed by this seal-prep session (a distinct actor, actor-independent "
        "from the author) on the sealed E01-0001 append-only Noetic Ledger over the "
        "sealed D04-0001 SQLite state store and C04-0001 content-addressed artifact "
        "store with no new production dependency. ActionIntent and EffectReceipt are "
        "sealed as canonical JSON whose intent_hash and receipt_hash re-derive from "
        "their canonical bytes; a mutated record fails closed with "
        "EFFECT_RECORD_MUTATED, a tampered or hostile-accessor or proxy candidate is "
        "rejected before execution, and emitted documents validate against the "
        "canonical Draft 2020-12 action-intent and effect-receipt schemas. The same "
        "idempotency key with the same request replays one registration, a different "
        "request fails with IDEMPOTENCY_KEY_REUSED, an intent-id rebind fails with "
        "INTENT_ID_CONFLICT, beginAttempt grants execution exactly once even under "
        "concurrent same-attempt callers, and older attempt and receipt replays stay "
        "idempotent. An UNKNOWN receipt blocks a blind retry with "
        "EFFECT_RECONCILIATION_REQUIRED, a crash before any receipt stays unresolved "
        "rather than narrated, an unsealed narrated receipt is rejected, "
        "reconciliation preserves the external operation identity, and a record "
        "missing its ledger event or checkpoint is repaired by exact replay without "
        "a second execution. Ruff lint/format, effect_reconciliation_test 10/10, "
        "idempotency_test 9/9, targeted 19/19, full Python 1261/1261, full Node "
        "1253/1253 across 111 files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"E02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". E01-0001, D04-0001, C04-0001, and every prior RAH generation remain "
        "immutable. E02 provides receipt-bound ActionIntent, Attempt, and "
        "EffectReceipt coordination with idempotent retry and exact reconciliation; "
        "capability leases, fencing, and approval policy (E03), the wider scheduler "
        "and replay surface, source coverage, PRD, review, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"E02 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"E02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"E02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"E02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected E02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
