#!/usr/bin/env python3
"""Append W01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "W01-0001"
WORK_PACKAGE_ID = "W01"
EXPECTED_PARENT = "000112-797cf7a1"
EXPECTED_PARENT_COUNT = 112
EXPECTED_PARENT_EVIDENCE = "E0114"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "083877cc8b2d085bd07168ef9a38b10c0da6644f953e31a65f593787e6cfa844"
)
CORE_EVIDENCE_ID = "E0115"
FINAL_EVIDENCE_ID = "E0116"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_w01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Retry A06 as A06-0002 under the recorded product-owner authority.",
    "After A06, recompute the 156-package DAG and continue the frontier serially.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required W01-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "W01-0001 PASS core verification: the workflow compiler and "
        "NodeContract validator are implemented in "
        "packages/foundry-kernel/src/workflows/compiler with exact D04, E04, "
        "F04, and N04 PASS dependency bindings. Node vocabulary derives from "
        "schemas/node-contract.schema.json (registry hash-bound); unknown "
        "executors, unresolved schema references, non-canonical documents, "
        "and unordered undeclared write-scope sharing fail closed; DAG and "
        "resource semantics delegate to the sealed scheduler compiler and "
        "the hash-bound memory_recall projection compiles to an eight-node "
        "plan. Repo structure/boundaries, targeted Node 26/26, full Python "
        "1156/1156, full Node 860/860 across 84 files, and git diff --check "
        "pass. "
        ...
        "A06-0002 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"W01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". T01-0002, O03-0001, and every prior RAH generation remain "
        "immutable. W01 is PASS, but A06, the wider 156-package DAG, source "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("W01-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "W01-0001 workflow compiler and NodeContract validator are "
        "evidence-sealed PASS. A06-0002 is next; implementation_gate=fail "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("W01-0001 parent is not the sealed T01-0002 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("W01 preflight requires exactly 112 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("W01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("W01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("W01 core did not append exactly E0115")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("W01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("W01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("W01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored W01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("W01 final did not append exactly E0116")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("W01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("W01 core tail is not E0115")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 114:
        raise SystemExit(f"unexpected W01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with W01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored W01 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
