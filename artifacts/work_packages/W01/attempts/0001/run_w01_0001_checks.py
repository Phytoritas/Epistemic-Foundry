#!/usr/bin/env python3
"""Run the W01-0001 workflow-compiler acceptance checks (attempt-local outputs)."""
...
def targeted_node() -> int:
    node = node_binary()
    if node is None:
        write_run_result("targeted-workflow-node", ["node", "--test"], 127)
        return 127
    return run(
        ...
        junit_from_stdout=ATTEMPT / "targeted-workflow-node.junit.xml",
    )
