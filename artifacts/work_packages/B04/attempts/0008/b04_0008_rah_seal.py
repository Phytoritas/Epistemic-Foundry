#!/usr/bin/env python3
"""Append B04-0008 PASS evidence while the global goal remains active."""
...
import build_b04_0008_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the live 156-package DAG from the sealed B04-0008 PASS evidence.",
    "Select the earliest dependency-ready package in manifest order.",
    "Continue packages serially under their bounded write scopes while completion_ready remains false until the external goal is terminal.",
]
...
def next_evidence_id(payloads: dict[str, Any]) -> str:
    ...
    if high_water != observed:
        raise SystemExit("RAH evidence high-water mark does not match ledger")
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B04-0008 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "B04-0008 recovery-core PASS preserves initial durable core E0022 / "
        "000022-6e053d7e, whose post-commit local integrity summary stopped on an "
        "undefined WORK_PACKAGE_ID before report binding. No prior evidence or "
        "generation was changed. The exact sealed C04-0003 "
        "dependency is bound at E0020/000020-4032536e and E0021; root authority "
        "projects 126 schemas plus OpenAPI 3.1.1/33 operations to 127 package "
        f"resources at {evidence.EXPECTED_SOURCE_BUNDLE_HASH}, "
        f"{evidence.EXPECTED_SNAPSHOT_BUNDLE_HASH}, and "
        f"{evidence.EXPECTED_REGISTRY_HASH}. Missing, extra, mismatch, duplicate-ID, "
        "reverse-sync, and fallback counts are zero. Targeted B04 is 41/41, full "
        "Python 990/990, and Node footer 460/460 across 52 files. Two clean wheel/"
        "sdist builds, sdist-to-wheel, installed-only, arbitrary-cwd, missing-resource, "
        "tamper rejection, boundaries, exact backend pins, and deterministic rebuild "
        f"pass. Projection sha256:{hashes['canonical-projection-verification.json']}; "
        ...
        f"runner sha256:{hashes['run_b04_0008_checks.py']}; builder sha256:"
        f"{hashes['build_b04_0008_evidence.py']}; sealer sha256:"
        f"{hashes['b04_0008_rah_seal.py']}. The corrected recovery core supersedes "
        "only the incomplete verification state of E0022, not its history. "
        "B04-0008 changes no product file. The "
        "156-package DAG must still be recomputed and downstream packages remain, so "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"B04-0008 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"projection receipt sha256:{hashes['projection.artifact-receipt.json']}; "
        f"wheel receipt sha256:{hashes['wheel.artifact-receipt.json']}; sdist receipt "
        f"sha256:{hashes['sdist.artifact-receipt.json']}; phase reconciliation "
        ...
        f"{hashes['targeted-projection.junit.xml']}; Python JUnit sha256:"
        f"{hashes['full-python-suite.junit.xml']}; Node JUnit sha256:"
        f"{hashes['full-node-suite.junit.xml']}; RAH integrity sha256:"
        f"{hashes['rah-core-integrity.json']}; runner sha256:"
        f"{hashes['run_b04_0008_checks.py']}; builder sha256:"
        f"{hashes['build_b04_0008_evidence.py']}; sealer sha256:"
        f"{hashes['b04_0008_rah_seal.py']}. E0022 / 000022-6e053d7e remains "
        "immutable post-commit-verification-incomplete history; this corrected "
        "core/final pair is authoritative. B04-0008 is immutable PASS and the "
        "next action is live 156-package DAG recomputation. Every prior attempt and "
        "generation remains preserved; implementation_gate=fail and "
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "B04-0008 final post-C04 packaging is evidence-sealed PASS. The live "
        "156-package DAG must now be recomputed and downstream packages remain; "
        "implementation_gate=fail and completion_ready=false."
    )
...
def verify_generation_store(expected_count: int | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("RAH is not active/fail with six current flat snapshots")
    identifiers = evidence_ids(payloads)
    return {
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("B04-0008 requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("B04-0008 report is already RAH-bound; use verify")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("B04-0008 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match B04-0008 summary")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("core seal did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("B04-0008 report does not bind current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("B04-0008 evidence IDs do not match the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("B04-0008 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("B04-0008 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match B04-0008 closeout")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("final seal did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("B04-0008 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("ledger does not end with B04-0008 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("B04-0008 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored B04-0008 final evidence differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
