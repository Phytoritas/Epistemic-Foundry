#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed B04-0008 PASS.

The current state of a package is selected from its highest numeric attempt
directory.  A historical root report or an older PASS can therefore never hide
a newer FAIL, BLOCKED, SPEC_GAP, or incomplete attempt.
"""
...
OUTPUT = ATTEMPT / "post-b04-0008-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-b04-0008-dag-review.md"
EXPECTED_PACKAGE_COUNT = 156
EXPECTED_B04_REPORT_HASH = (
    "sha256:9c5ee50f3be039c44209a248a142b12f4ba46796673597b147e6197011b042b7"
)
...
def report_state(package_id: str) -> dict[str, Any]:
    ...
    historical_report_count = int(root_report.is_file())
    ...
    if selection == "HIGHEST_NUMERIC_ATTEMPT" and reported_attempt != selected_attempt:
        raise SystemExit(
            f"attempt identity mismatch for {package_id}: {reported_attempt!r} != {selected_attempt!r}"
        )
...
def reconcile() -> dict[str, Any]:
    ...
    b04 = states["B04"]
    b04_report = read_json(ROOT / str(b04["report"]))
    rah = b04_report.get("rah_state")
    if not (
        b04["selected_attempt_id"] == "B04-0008"
        and b04["current_state"] == "PASS"
        and b04["report_sha256"] == EXPECTED_B04_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0023"
        and rah.get("core_generation") == "000023-fd7399f0"
        and rah.get("final_closeout_evidence_id") == "E0024"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("B04-0008 is not the exact sealed final packaging PASS")
    ...
    return {
        "attempt_binding": {
            "B04_attempt_id": "B04-0008",
            "B04_core_evidence_id": "E0023",
            "B04_core_generation": "000023-fd7399f0",
            "B04_final_closeout_evidence_id": "E0024",
            "B04_report": b04["report"],
            "B04_report_sha256": b04["report_sha256"],
            "initial_incomplete_core_preserved": {
                "evidence_id": "E0022",
                "generation": "000022-6e053d7e",
            },
            "status": "PASS",
        },
        "blocked_package_count": len(blocked),
        "blocked_packages": blocked_details,
        ...
        "projection": "POST_B04_0008_LIVE_RECOMPUTATION",
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-B04-0008 live DAG reconciliation
...
- Earliest next package: `{next_package}`
