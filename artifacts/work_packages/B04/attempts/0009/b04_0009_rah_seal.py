#!/usr/bin/env python3
"""Append B04-0009 PASS evidence to the active append-only RAH state."""
...
ATTEMPT_ID = "B04-0009"
EXPECTED_PARENT = "000100-a34b23bb"
EXPECTED_PARENT_EVIDENCE = "E0102"
CORE_EVIDENCE_ID = "E0103"
FINAL_EVIDENCE_ID = "E0104"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_b04_0009_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Execute O02-0002 retrieval implementation and acceptance after B04-0009 PASS.",
    "Run C04-0004 only after O02-0002 PASS, then the next-unused final B04 packaging attempt.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B04-0009 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "B04-0009 PASS core verification: root authority and derived package "
        "snapshot converge at 127 schemas plus one OpenAPI document, 128 "
        "resources total. Source, snapshot, registry, wheel, and sdist hashes "
        "are bound; missing, extra, mismatch, duplicate-ID, fallback, and root "
        "mutation counts are zero. Targeted projection is 41/41, full Python "
        "is 1073/1073, and authoritative full Node is 819/819 across 79 files. "
        ...
        "O02-0002 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"B04-0009 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". B04-0002 through B04-0008, C02-0004, and every prior RAH "
        "generation remain immutable. O02-0002 becomes dependency-ready, but "
        "O02, C04-0004, final packaging, and the wider DAG remain; "
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "B04-0009 is evidence-sealed PASS and O02-0002 is next. The global "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("B04-0009 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("B04-0009 parent is not generation 100 / E0102")
    generations = numbered_generations(ralph_root)
    if len(generations) != 100 or generations[-1] != generation:
        raise SystemExit("preflight requires exactly 100 preserved generations")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("B04-0009 report is already RAH-bound")
    return {
        **checked,
        "completion_ready": False,
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("B04-0009 report does not bind current core generation")
    if (
        ...
        raise SystemExit("B04-0009 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored B04-0009 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("final did not append exactly E0104")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != 102:
    ...
    return {
        ...
        "mode": "final",
        "parent_generation": parent,
        "state_verification": verify_generation_store(102),
        "status": "active",
    }
...
def run_verify() -> dict[str, Any]:
    ...
    if count == 101:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("B04-0009 core tail is not E0103")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != 102 or generation_number(generation) != 102:
        raise SystemExit(f"unexpected B04-0009 generation count: {count}")
    identifiers = evidence_ids(payloads)
