#!/usr/bin/env python3
"""Append B04-0010 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "B04-0010"
WORK_PACKAGE_ID = "B04"
EXPECTED_PARENT = "000106-01ed585f"
EXPECTED_PARENT_COUNT = 106
EXPECTED_PARENT_EVIDENCE = "E0108"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "4b94568aad82e0ccabf5b51a41ac7c7d1169919338741ee7ac7d7cf56bc40c01"
)
CORE_EVIDENCE_ID = "E0109"
FINAL_EVIDENCE_ID = "E0110"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_b04_0010_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the live 156-package DAG from sealed B04-0010 PASS evidence.",
    "Select the earliest dependency-ready package in manifest order.",
    "Continue packages serially while completion_ready remains false until the external goal is terminal.",
]
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B04-0010 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "B04-0010 PASS core verification: exact sealed C04-0004 dependency "
        "E0107/E0108 is bound; root authority and derived package snapshot "
        "converge at 127 schemas plus OpenAPI 3.1.1/33 operations, 128 "
        "resources total. Source, snapshot, registry, wheel, and sdist hashes "
        "are bound; missing, extra, mismatch, duplicate-ID, root-mutation, "
        "fallback, and unrelated-write counts are zero. Targeted packaging is "
        "41/41, full Python is 1115/1115, and Node is 819/819 across 79 files. "
        "Two clean builds, sdist-to-wheel, installed-only, arbitrary-cwd, "
        "missing-resource, tamper, boundaries, and deterministic rebuild pass. "
        ...
        "The 156-package DAG must still be recomputed; implementation_gate=fail "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"B04-0010 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". B04-0002 through B04-0009, C04-0004, and every prior RAH "
        "generation remain immutable. B04 final packaging is PASS, but the live "
        "156-package DAG, source coverage, release gate, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("B04-0010 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "B04-0010 final post-C04 packaging is evidence-sealed PASS. The live "
        "156-package DAG must be recomputed; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("B04-0010 parent is not the sealed C04-0004 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("B04 preflight requires exactly 106 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("B04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("B04-0010 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("B04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("B04 core did not append exactly E0109")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("B04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("B04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("B04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored B04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("B04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("B04 final did not append exactly E0110")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("B04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("B04 core tail is not E0109")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 108:
        raise SystemExit(f"unexpected B04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with B04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored B04 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
