#!/usr/bin/env python3
"""Append S02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000314-6f1f3e4a"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 314
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0316"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "9f0cf1ba889adf01e22e09ae6e8abe96103a6fadec4e85d1b61ad7987112a454"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0317"
FINAL_EVIDENCE_ID = "E0318"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_s02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after S02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} execution security boundary gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "S02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required S02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: execution component is pinned so a change to the execution boundary or its
    #: tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/security/execution").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "S02-0001 PASS core verification: the execution security boundary, "
        "implemented in packages/foundry-kernel/src/security/execution by a "
        "bounded implementation agent and reviewed by this seal-prep session (a "
        "distinct actor, actor-independent from the author) on the sealed "
        "S01-0001 package with no new production dependency. Secret references "
        "are opaque, non-serializing frozen handles; "
        "assertSecretFreeBoundaryPayload denies handles, secret-bearing field "
        "names, and secret-shaped text at the prompt, evidence-artifact, log, "
        "export, and network-request boundaries using descriptor-only reads that "
        "never invoke Proxy traps or accessors. Last-mile secret use requires "
        "both an exact egress-policy origin and a handle independently bound to "
        "that canonical HTTPS origin, and the decision exposes no secret bytes, "
        "handle id, or vault id. Resource paths are portable relative paths "
        "canonicalized beneath an existing real root, operation-scoped, and "
        "validated with a per-segment no-follow lstat/realpath walk that denies "
        "symlinks, junctions, mount crossings, non-directory traversal, and "
        "missing parents; root device/inode/birth identity is revalidated on "
        "every request. Egress is disabled or an exact canonical HTTP(S) origin "
        "allowlist that denies URL credentials, fragments, and unauthorized "
        "redirect destinations. Authority is held in per-boundary WeakMap/WeakSet "
        "brands, so foreign-compartment or JSON-copied objects fail closed. Ruff "
        "lint/format, secret_exfiltration_test 9/9, path_escape_test 9/9, "
        "targeted 18/18, full Python 1261/1261, full Node 1274/1274 across 113 "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"S02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". S01-0001 and every prior RAH generation remain immutable. S02 "
        "provides the deterministic execution security boundary primitive; "
        "effect-time enforcement at future runtime call sites, the S04 red-team "
        "integration, T04 sandbox and external-tool adapter integration, "
        "exhaustive secret detection, container/process/network isolation, source "
        "coverage, PRD, review, and global closeout remain; "
        "implementation_gate=fail and completion_ready=false."
    )
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("S02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"S02 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("S02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("S02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"S02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("S02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("S02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("S02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored S02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("S02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"S02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("S02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"S02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected S02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with S02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored S02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
