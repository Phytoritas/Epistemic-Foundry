#!/usr/bin/env python3
"""Append H03-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed A01/A02/A03/B03 templates do.

H03 depends on H01.  Its product bytes are the two static hook declarations
``plugins/epistemic-foundry/hooks/tools.json`` and
``plugins/epistemic-foundry/hooks/delegation.json``, installed byte-for-byte
from the authority blueprint and attested without further edit.
"""
...
EXPECTED_PARENT = "000344-95ad9860"
EXPECTED_PARENT_COUNT = 344
EXPECTED_PARENT_EVIDENCE = "E0346"
EXPECTED_PARENT_MANIFEST_SHA256 = "cb5490fc42946c652a18e459abe400a294e2dd5db0efc04ea629653bfc286295"
CORE_EVIDENCE_ID = "E0347"
FINAL_EVIDENCE_ID = "E0348"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_h03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after H03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} tool and delegation hook declarations are evidence-sealed "
    f"PASS ({CORE_EVIDENCE_ID}/{FINAL_EVIDENCE_ID}). "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "H03-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required H03-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory: the two static hook
    #: declarations H03 installs from the authority blueprint (never edits after).
    relatives = sorted(PRODUCT_FILES)
    for relative in relatives:
        if not (ROOT / relative).is_file():
            raise SystemExit(f"product declaration missing: {relative}")
...
def core_summary() -> str:
    ...
    return (
        "H03-0001 PASS core verification: tool and delegation hook declarations, "
        "installed byte-for-byte from the authority blueprint by the bounded "
        "implementation agent(s) (H03 maker) that authored the Node contract "
        "harness h03-hook-contract-tests.mjs under artifacts/work_packages/H03/**, "
        "and reviewed by the sealing session as an independent contract-reviewer "
        "(actor-independent from the author). H03 installs "
        "plugins/epistemic-foundry/hooks/tools.json and delegation.json exactly "
        "as plugin_blueprint/epistemic-foundry/hooks/tools.json and "
        "delegation.json and makes no other edit. tool_hook_policy_test asserts "
        "tools.json routes PermissionRequest (matcher Bash|apply_patch|mcp__.*) to "
        "the H01 gateway hook-runner permission-request command and binds "
        "PreToolUse guardrails and PostToolUse effect receipts with symmetric "
        "coverage, failing closed on a direct allow, a timeout expansion, dropped "
        "coverage, an asymmetric matcher, and any extra event. "
        "subagent_result_gate_test asserts delegation.json binds SubagentStart -> "
        "RoleSpec and SubagentStop -> ResultEnvelope over every subagent (matcher "
        ".*), failing closed on a substituted accept-partial-result stop handler, "
        "a partial matcher, and a missing route. The maturity boundary is honest: "
        "the plugin manifest carries no hooks key and empty capabilities and no "
        "dist/hook-runner.mjs exists, so runtime enforcement (H04/N04/X01/G06) is "
        "correctly not claimed executable -- a specified-not-implemented posture, "
        "not a weakening. hook_contract_tests 8/8 (tool_hook_policy_test 4/4, "
        "subagent_result_gate_test 4/4), the full Python suite green, the live "
        "full Node suite green with zero failures, and git diff --check all pass. "
        "H03 depends on H01; the sealed H01-0001 attempt is the build dependency "
        "and H02-0001 is a sealed PASS regression baseline. "
        ...
        "actors; external actor-independent certification does not hold). The DAG "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        ...
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. H03 installs the two "
        "static tool and delegation hook declarations from the authority "
        "blueprint and attests them; it does not implement a hook runner, register "
        "plugin-manifest hooks, or add a capability, makes no product-maturity, "
        "runtime-executability or release-readiness claim, and asserts only that "
        "observed tools use policy and receipts and the subagent expected-count "
        "contract is bound at the static declaration and handler-binding "
        "boundary. Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("H03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H03-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"H03 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("H03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("H03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"H03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("H03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("H03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("H03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored H03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"H03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("H03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"H03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected H03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with H03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored H03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
