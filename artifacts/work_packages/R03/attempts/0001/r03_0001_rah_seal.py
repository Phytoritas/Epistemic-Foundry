#!/usr/bin/env python3
"""Append R03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "R03-0001"
WORK_PACKAGE_ID = "R03"
EXPECTED_PARENT = "000130-de10baca"
EXPECTED_PARENT_COUNT = 130
EXPECTED_PARENT_EVIDENCE = "E0132"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "328034ae1c32123741d811dc093dca9dd8d061ce42aa9e71acc3074e1b7e6be5"
)
CORE_EVIDENCE_ID = "E0133"
FINAL_EVIDENCE_ID = "E0134"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_r03_0001_evidence as evidence  # noqa: E402
...
NEXT_ACTIONS = [
    ...
    "Run the R04 causal identification and ArgumentGraph gate on the sealed R02/R03 reasoning.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required R03-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "R03-0001 PASS core verification: abduction, contradiction and "
        "moderator handling are implemented in "
        "python/epistemic_foundry/reasoning/aporia with an exact R01-0001 "
        "dependency and an R02-0001 regression baseline. Every conflicting "
        "pair is typed as a direct contradiction, a condition difference, a "
        "nested scope, or a measurement difference before it can be sealed, "
        "and a pair the engine cannot attribute is UNCLASSIFIED and fails "
        "closed rather than defaulting to contradiction; directions that "
        "assert no position can never manufacture a conflict. Each conflict "
        "must keep at least two standing kinds of explanation, restatements "
        "of one kind do not count as competing, an explanation with no "
        "discriminating test cannot compete, and a refuted explanation is "
        "retained with its cited refutation. R03 never selects: "
        "selected_explanation_id is null and adjudication belongs to the "
        "Evidence Parliament. Ruff lint/format, contradiction_fixture_test "
        "24/24, abduction_diversity_test 20/20, targeted 44/44, dependency "
        "component regression 44/44, full Python 1261/1261, full Node 903/903 "
        "across 87 files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"R03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". R02-0001, R01-0001, and every prior RAH generation remain "
        "immutable. R03 types and preserves explanations supplied by a caller "
        "rather than generating them, does not execute the discriminating "
        "tests it requires, and treats conditions as an opaque key/value map. "
        "R04, T03, the x05 wave, source coverage, PRD, review, and global "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("R03-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "R03-0001 abduction, contradiction and moderator engine is "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R03-0001 parent is not the sealed R02-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("R03 preflight requires exactly 130 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("R03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("R03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("R03 core did not append exactly E0133")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("R03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("R03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("R03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored R03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("R03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("R03 final did not append exactly E0134")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("R03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("R03 core tail is not E0133")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 132:
        raise SystemExit(f"unexpected R03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with R03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored R03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
