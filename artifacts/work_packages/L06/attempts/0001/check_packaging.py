#!/usr/bin/env python3
"""Prove the L06 gate is discoverable by the project's own packaging config.

``src/epistemic_foundry/memory`` already ships with the wheel and L05 added
``v4_l05`` beneath it, so no new package marker and no scope decision exist for
L06 either; this check proves that claim rather than assuming it.  The
...
EXPECTED = ("epistemic_foundry.memory", "epistemic_foundry.memory.v4_l06")
