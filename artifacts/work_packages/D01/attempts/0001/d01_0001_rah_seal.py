#!/usr/bin/env python3
"""Append D01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000280-207d371e"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 280
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0282"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "aed36b524800f5e383c32297a8096b855d3a550739660c5c669e77ab98838e92"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0283"
FINAL_EVIDENCE_ID = "E0284"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_d01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after D01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} SQLite WAL local canonical store gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "D01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required D01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: sqlite state namespace is pinned so a change to the canonical store or
    #: its tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/state/sqlite").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "D01-0001 PASS core verification: the SQLite WAL local canonical store, "
        "implemented in packages/foundry-kernel/src/state/sqlite by a bounded "
        "implementation agent and reviewed by this seal-prep session (a distinct "
        "actor, actor-independent from the author) under sealed B04-0002 and "
        "C04-0001 dependency bindings. The store opens the database in WAL mode "
        "through node:sqlite DatabaseSync with no new production dependency, "
        "commits every revisioned record inside a BEGIN IMMEDIATE transaction "
        "that survives reopen and rolls back every partial write on callback "
        "failure, advances state only through a compare-and-swap on the expected "
        "revision so a stale writer becomes a typed no-op rather than a silent "
        "overwrite, and fails closed into a read-only SAFE_MODE that denies every "
        "mutation path on physical corruption, schema drift, invalid persisted "
        "JSON, or runtime revision or schema-version drift. Hostile JavaScript "
        "values and Promise/thenable transaction callbacks are refused without "
        "granting execution or partial state. Ruff lint/format, sqlite_wal_test "
        "19/19, concurrent_revision_test 2/2, integrity-safe-mode 16/16, targeted "
        "37/37, full Python 1261/1261, full Node 1253/1253 across 111 files, and "
        "git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"D01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". B04-0002, C04-0001, and every prior RAH generation remain immutable. "
        "D01 provides the local canonical store; the team PostgreSQL store, "
        "tenant isolation, broader recovery lifecycle, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("D01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"D01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("D01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("D01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"D01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("D01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("D01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("D01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored D01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("D01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"D01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("D01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"D01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected D01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with D01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored D01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
