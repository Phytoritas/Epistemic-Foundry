#!/usr/bin/env python3
"""Append V02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000230-ce1a4e7c"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 230
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0232"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "643459574995054e468c636238eb2a8bb4b4a3c68454f57f3db06c91531f02bf"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0233"
FINAL_EVIDENCE_ID = "E0234"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_v02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after V02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} preregistered ValidationPlan and falsification rules is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "V02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required V02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: planning component is pinned so a change to any product or test file (or
    #: the V01 screen it composes through) is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "python/epistemic_foundry/validation/planning").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "V02-0001 PASS core verification: preregistered ValidationPlan and "
        "falsification rules, implemented in "
        "python/epistemic_foundry/validation/planning by a bounded implementation "
        "agent and reviewed by the sealing agent (actor-independent from the "
        "author) under an exact dependency binding (V01 V01-0001) and an R06-0001 "
        "regression baseline. The component preregisters a ValidationPlan against "
        "the sealed V01 ValidationTarget screen before any result exists: the "
        "sealed plan re-derives its observables and single falsification_rule from "
        "the prediction register rather than accepting them beside it, the receipt "
        "re-derives every published hash and binds the exact target manifest and "
        "canonical vocabulary it screened, and require_intact refuses any post-hoc "
        "edit (PREREGISTRATION_MUTATED) so a result cannot move the plan after the "
        "fact. Falsifiability is enforced not assumed: a qualitative prediction "
        "carries no refuting comparator, an exploratory prediction may not carry a "
        "criterion, a confirmatory one must, a stochastic target without a fixed "
        "seed or a plan without an identifiability warning is refused, and a "
        "register of only exploratory predictions is refused PLAN_UNFALSIFIABLE. "
        "Thirty-two finding codes each name an exact refusal with an actionable "
        "reason. Every seal re-derives byte for byte from its own published fields "
        "with no clock or random draw, and nothing scores, ranks, selects, "
        "promotes or evaluates; no candidate, model, prompt, backend or hook "
        "acquires evaluator, holdout or promotion authority. Ruff lint/format, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, the "
        "targeted component suite, the V01 dependency regression, full Python and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"V02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. V02 seals a "
        "preregistration and its falsification rules only: it does not run a plan "
        "or produce a result (V03 territory), does not score, rank, select, "
        "promote or evaluate any candidate, grants no authority at the gate, and "
        "refuses every post-hoc edit rather than honouring it. Author/reviewer "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("V02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"V02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("V02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("V02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"V02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("V02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("V02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("V02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored V02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("V02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"V02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("V02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"V02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected V02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with V02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored V02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
