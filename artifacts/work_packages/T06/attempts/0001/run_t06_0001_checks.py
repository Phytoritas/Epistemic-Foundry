#!/usr/bin/env python3
"""Run the T06-0001 external-backend qualification and fallback gate checks.

Required checks from manifests/development_manifest.yaml (T06):
...
report rather than in this runner.  The gate ships under
src/epistemic_foundry/adapters/v4_t06, a second versioned subpackage under the
``epistemic_foundry.adapters`` marker T05 introduced, so ``packaging-discovery``
proves both levels actually reach the wheel rather than only importing from the
checkout.

The single dependency regression covers the sealed surface this package
composes: T05's backend-adapter qualification (pinning, capability
qualification, the S05 executor binding and the imported-run boundary), whose
usable-verdict vocabulary and refusals T06 reads and propagates unwrapped.  It
is exercised through T05's own sealed test suite so a regression there fails
here rather than only inside the full suite.
...
#: The adapters package family (docstring/context); the owned surface T06 may
#: write and lint is the versioned subpackage ``GATE`` below.
COMPONENT = "src/epistemic_foundry/adapters"
GATE = "src/epistemic_foundry/adapters/v4_t06"
PRODUCT_TESTS = {
...
# T05 owns the backend-adapter qualification surface whose usable-verdict
# vocabulary, pinning, capability qualification, S05 executor binding and
# imported-run boundary T06 composes and propagates unwrapped.  A change there
# must fail here, so it runs its own sealed suite.
DEPENDENCY_TARGETS = {
...
def ruff_check() -> int:
    return run(
        "ruff-check", ["uv", "run", "--locked", "ruff", "check", GATE, ATTEMPT_DIR]
    )
...
def ruff_format_check() -> int:
    return run(
        "ruff-format-check",
        ["uv", "run", "--locked", "ruff", "format", "--check", GATE, ATTEMPT_DIR],
    )


def packaging_discovery() -> int:
    # src/epistemic_foundry/adapters/v4_t06 is a second subpackage under the
    # adapters marker; the wheel claim is proved here rather than assumed.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-t06-gate", ATTEMPT_DIR)


def dependency_regression_t05() -> int:
    return _pytest(
        "dependency-regression-t05", *DEPENDENCY_TARGETS["dependency-regression-t05"]
    )
