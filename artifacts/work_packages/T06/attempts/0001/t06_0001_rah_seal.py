#!/usr/bin/env python3
"""Append T06-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was authored as a bounded hand
that was instructed not to touch ``.rah/``, so it cannot read the live ledger to
pin the parent generation identifier, its manifest hash, the retained-generation
count, or the two evidence IDs this attempt will issue.  ``EXPECTED_PARENT``,
``EXPECTED_PARENT_MANIFEST_SHA256``, ``EXPECTED_PARENT_COUNT``,
``CORE_EVIDENCE_ID`` and ``FINAL_EVIDENCE_ID`` are therefore left as the sentinel
``FILL_FROM_LEDGER`` and the seal refuses to run until the parent fills all five
from the live ledger tail — the generation committed by the R05-0001 final
closeout, evidence ``E0206``.  ``EXPECTED_PARENT_EVIDENCE`` is known: R05-0001
sealed most recently, so the parent tail evidence is ``E0206``; verify it against
``.rah/ralph`` before running.  Everything else — summaries, hashes,
...
#: The generation the N06-0001 final closeout committed (its evidence is E0208).
#: Filled from the live `.rah/ralph` current pointer.
EXPECTED_PARENT = "000206-8c76a183"
EXPECTED_PARENT_COUNT = 206
#: N06-0001 sealed most recently, so the parent tail evidence is E0208.
EXPECTED_PARENT_EVIDENCE = "E0208"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "7d901772fdf468f2d7c53335b8d5e89f084a1c9d113e6066e9e50c6c19a91115"
)
CORE_EVIDENCE_ID = "E0209"
FINAL_EVIDENCE_ID = "E0210"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_t06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after T06-0001 seals and select the next "
...
#: is built from the evidence-ID constants so that filling those constants
#: updates the note without a second edit.
GATE_NOTE = (
    f"{ATTEMPT_ID} external-backend qualification and fallback integration gate "
...
def assert_parent_pins() -> None:
    if PARENT_PIN_SENTINEL in (
        ...
        raise SystemExit(
            "T06-0001 seal is not pinned: set EXPECTED_PARENT, "
            "EXPECTED_PARENT_MANIFEST_SHA256, EXPECTED_PARENT_COUNT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required T06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path: the entire
    #: adapters surface (v4_t05, v4_t06 and the shared marker) so a change to any
    #: adapter file after the core summary would break the final seal.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/adapters").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "T06-0001 PASS core verification: external-backend qualification "
        "lifecycle, declared fallback and disablement integration gate, "
        "implemented in src/epistemic_foundry/adapters/v4_t06 by a bounded "
        "implementation agent and reviewed by an independent reviewer "
        "(actor-independent from the author) dispatched under the product "
        "owner's explicit parallel-agent instruction, under an exact dependency "
        "binding (T05 T05-0001) and an R05-0001 regression baseline. The gate "
        "composes T05's sealed backend-adapter qualification and adds duration, "
        "ordered substitution and withdrawal: the usable serving verdicts are "
        "the intersection of the T05 schema enum and the shinka usable set "
        "computed per call, the six STANDING_* lifecycle words are proven "
        "disjoint from every canonical enum value (EF4-I22), a fallback may only "
        "narrow, every declared chain terminates in the inert domain-neutral "
        "core, and disabling a backend both withdraws its head qualification and "
        "marks the in-flight imports it left as requiring re-verification. Every "
        "record re-derives byte for byte from its own published fields with no "
        "clock or random draw, and nothing scores, selects, promotes or "
        "evaluates. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, packaging-discovery, the T05 dependency "
        "regression, full Python and full Node suites, and git diff --check all "
        "pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"T06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. T06 gates "
        "qualification duration, declared fallback and disablement only: it does "
        "not run, score, select, promote or evaluate any candidate, treats "
        "external backend output as untrusted inert data, falls back to the "
        "domain-neutral core unchanged, makes no DSSAT or plant-model parity "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("T06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T06-0001 parent is not the sealed R05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"T06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("T06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("T06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"T06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("T06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("T06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("T06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored T06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("T06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"T06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("T06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"T06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected T06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with T06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored T06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
