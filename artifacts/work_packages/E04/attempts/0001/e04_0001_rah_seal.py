#!/usr/bin/env python3
"""Append E04-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000326-fcc0e984"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 326
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0328"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "a3fbad4375b8e29d0a1fb12487f938a3eb4d727d96e22e208beee2682fe1fed5"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0329"
FINAL_EVIDENCE_ID = "E0330"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after E04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} strict and semantic E-phase replay gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "E04-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: replay/effects namespace is pinned so a change to the replay harness or
    #: either replay-gate test is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "tests/replay/effects").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "E04-0001 PASS core verification: the strict and semantic E-phase replay "
        "gate -- byte-identical reducer replay of the E02 effect and E03 "
        "capability-lease, fencing and approval events over the append-only "
        "E01-0001 Noetic Ledger, C04-0001 artifact store and D04-0001 SQLite state "
        "store, plus honest strict/semantic ReplayReports -- implemented in "
        "tests/replay/effects by a bounded implementation agent and reviewed by this "
        "seal-prep session (a distinct actor, actor-independent from the author) with "
        "no new production dependency. Rebuilding the run twice is deepEqual with a "
        "matching sha256 state_hash and identical event order; reopening both durable "
        "stores preserves stream and reducer identity; exact retries append no events; "
        "an EXACT report requires event_equivalence=EXACT, drift_classification=NONE, "
        "sixteen matching pinned artifacts, zero mismatches, integrity and Draft "
        "2020-12 schema; and a tampered or missing payload, a duplicated logical "
        "payload identity, or a forged event binding fails closed "
        "(PAYLOAD_HASH_MISMATCH, PAYLOAD_RESOLUTION_FAILED, "
        "E04_REPLAY_SEQUENCE_INVALID, E04_REPLAY_EVENT_BINDING_INVALID). Semantic "
        "drift is reported not erased: distinct identities are SEMANTICALLY_EQUIVALENT "
        "never falsely EXACT, model/prompt pin drift is MODEL/MULTIPLE with both pins "
        "retained, gate/verdict/state changes are explicit DRIFT, missing pins are "
        "NOT_COMPARABLE/UNKNOWN, and the report_hash excludes itself and rejects "
        "mutation (E04_REPLAY_REPORT_HASH_MISMATCH, E04_PIN_INVALID, "
        "E04_STRICT_IDENTITY_INVALID, E04_NON_CANONICAL_JSON). Ruff lint/format, "
        "strict_replay_test 8/8, semantic_replay_report 10/10, targeted 18/18, full "
        "Python 1261/1261, full Node 1291/1291 across 115 files, and git diff --check "
        "pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"E04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C04-0001, D04-0001, E01-0001, E02-0001, E03-0001, and every prior RAH "
        "generation remain immutable. E04 provides the E-phase strict and semantic "
        "replay gate; the wider scheduler, effects execution, promotion, and "
        "evolution surface, source coverage, PRD, review, and global closeout remain; "
        "implementation_gate=fail and completion_ready=false."
    )
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E04-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"E04 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"E04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"E04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"E04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected E04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
