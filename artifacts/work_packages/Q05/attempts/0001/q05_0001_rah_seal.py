#!/usr/bin/env python3
"""Append Q05-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000232-87816334"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 232
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0234"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "3e0b2194d0a50a166875631f70d7f9e80b79f8c56ab40b610113376de52c940a"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0235"
FINAL_EVIDENCE_ID = "E0236"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_q05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after Q05-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} multi-objective fitness, hidden-evaluation and selective-inference "
    f"admissibility integration gate is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "Q05-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Q05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: evaluation namespace is pinned so a change to a composed evaluation surface
    #: (or the parent marker) is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/evaluation").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Q05-0001 PASS core verification: multi-objective fitness, hidden-evaluation and "
        "selective-inference admissibility integration gate, implemented in "
        "src/epistemic_foundry/evaluation/v4_q05 by a bounded implementation "
        "subagent and reviewed by the sealing agent (actor-independent from the "
        "author) under exact dependency bindings (Q04 Q04-0001, O05 O05-0001, "
        "C05 C05-0001) and an R06-0001 regression baseline. The gate composes the "
        "sealed evaluation.fitness, verifier_firewall.firewall, "
        "statistics.search_record and statistics.selective surfaces without "
        "restating them: fitness is kept a vector and a scalar is refused, a "
        "score can never promote (may_promote_on_score required False), the "
        "hidden evaluation stays sealed and is bound by hash with disclosure "
        "gated on both an unblinding approval and holdout-read authority, leaked "
        "evaluator feedback invalidates the comparison, and an adaptive "
        "best-of-many selection without the complete, hash-consistent, "
        "verdict-cleared statistical record is refused; no candidate, model, "
        "prompt, backend or hook acquires evaluator, holdout or promotion "
        "authority. Fourteen finding codes name exact refusals and every one is "
        "exercised. Every admit or refuse decision re-derives byte for byte from "
        "its own published fields with no clock or random draw, and nothing "
        "scores, ranks, selects, promotes or evaluates. Ruff lint/format, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the Q04/O05/C05 dependency regressions, full "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Q05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Q05 keeps fitness a "
        "vector, keeps the hidden evaluation hidden, refuses an uncorrected "
        "adaptive selection, and records a deterministic admit/refuse receipt "
        "only: it does not score, rank, select, promote or evaluate any "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Q05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q05-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"Q05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Q05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Q05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"Q05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Q05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Q05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Q05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Q05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Q05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"Q05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Q05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"Q05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected Q05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Q05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Q05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
