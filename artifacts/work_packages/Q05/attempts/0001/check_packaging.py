#!/usr/bin/env python3
"""Prove the Q05 admissibility gate is discoverable by the packaging config.

``src/epistemic_foundry/evaluation/v4_q05`` is a new package marker authorized
by the Q05 write scope in ``manifests/development_manifest.yaml``, so there is a
scope decision to prove rather than assume: a marker that never reached the wheel
...
change there fails here. ``evaluation`` itself was authorized by an earlier
package and is verified again so a regression there surfaces at this gate too.
"""
...
EXPECTED = ("epistemic_foundry.evaluation", "epistemic_foundry.evaluation.v4_q05")
