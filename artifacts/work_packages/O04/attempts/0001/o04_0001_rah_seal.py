#!/usr/bin/env python3
"""Append O04-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "O04-0001"
WORK_PACKAGE_ID = "O04"
EXPECTED_PARENT = "000118-f17cfdfe"
EXPECTED_PARENT_COUNT = 118
EXPECTED_PARENT_EVIDENCE = "E0120"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "e95b05638430b10d72fc5993e05f73f5df1b6918c2a3c692657b2adbad7ed3d6"
)
CORE_EVIDENCE_ID = "E0121"
FINAL_EVIDENCE_ID = "E0122"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_o04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the dependency-ready frontier serially: T02, T03, W02, W03, B05, C05.",
    "Consult the Pro orchestrator on design-dense packages before implementation.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required O04-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "O04-0001 PASS core verification: the O-phase absence and "
        "completeness gate is implemented in "
        "python/epistemic_foundry/retrieval/completeness with exact O02 and "
        "O03 PASS dependency bindings and the A06-0002 regression baseline. "
        "Every absence or novelty claim is sealed against a deterministically "
        "recomputed SearchCompletenessCertificate with content-addressed ids "
        "and exact-reconstruction validation; SEARCHED_NONE is the only "
        "zero-evidence state while UNSEARCHED, PARTIAL, BLOCKED, and FAILED "
        "lanes are ignorance that grounds no claim; PARTIAL lanes support "
        "only scope-bounded claims at a demoted ceiling; and an EvidencePack "
        "whose ignorance lane is reported complete fails the gate. Ruff "
        "lint/format, targeted 18/18, full Python 1196/1196, full Node "
        "860/860 across 84 files, and git diff --check pass. "
        ...
        "T02-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"O04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". A06-0002, O03-0001, and every prior RAH generation remain "
        "immutable. The O-phase x04 gate is PASS, but T02, T03, W02, W03, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("O04-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "O04-0001 O-phase absence/completeness gate is evidence-sealed "
        "PASS. T02-0001 is next; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("O04-0001 parent is not the sealed A06-0002 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("O04 preflight requires exactly 118 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("O04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("O04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("O04 core did not append exactly E0121")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("O04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("O04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("O04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored O04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("O04 final did not append exactly E0122")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("O04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("O04 core tail is not E0121")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 120:
        raise SystemExit(f"unexpected O04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with O04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored O04 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
