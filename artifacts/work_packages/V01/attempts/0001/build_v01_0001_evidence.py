#!/usr/bin/env python3
"""Build and verify V01-0001 evidence: ValidationTarget manifests and eligibility.
...
ATTEMPT = ROOT / "artifacts/work_packages/V01/attempts/0001"
ATTEMPT_ID = "V01-0001"
WORK_PACKAGE_ID = "V01"
RECORDED_AT = "2026-08-02T19:00:00.000Z"
ATTEMPT_DIR = "artifacts/work_packages/V01/attempts/0001"
AUTHORITY_DECISION = None

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/V01/attempts/0001/run_v01_0001_checks.py": "6eaf1acb5fbd4c76281f622d923705c5a23f931defb04e177d256b5d9795eaa9",
    "python/epistemic_foundry/validation/targets/__init__.py": "d9e7b996d768d7efc9d57dc7073eeebe20d7d8fd00cf23fe21dfae1d6569629c",
    "python/epistemic_foundry/validation/targets/contracts.py": "65e2ce98ddca6314284a645a9c6a02a9b1ec1a6c85ba1326e12695cc8a16b1fa",
    "python/epistemic_foundry/validation/targets/fixtures.py": "ccb22a99956d6141d5f4d4144124a2ce1a4e8cfe1878003e4bd9074ef540980a",
    "python/epistemic_foundry/validation/targets/pytest.ini": "071558d996e8ff164084e63be2b65ad7b3b242f7903ba13e2b6fb20f020fcfb6",
    "python/epistemic_foundry/validation/targets/test_negative_adversarial.py": "6d3fc286ed08594b1022bfbf548261b3d6214f208155d4a8c7597610f07bf814",
    "python/epistemic_foundry/validation/targets/test_provenance_receipts.py": "410f6e5324fd2a1b3c89c55753e3b5e881b862c13ce0a4d53cd86a34d6c4d92b",
    "python/epistemic_foundry/validation/targets/test_schema_and_type.py": "df240fb231073d09691ffea2c9b89ee128b3a7173d5486c96be8778111dcbf88",
    "python/epistemic_foundry/validation/targets/test_unit_contract.py": "15208adfecabcae8824f1ca58bd3781b4cb3a4da84233e2dac66034ac0b23d97",
}
...
JUNIT_PATHS = {
    "dependency_regression_t04": ATTEMPT / "dependency-regression-t04.junit.xml",
    ...
    "targeted_v01_validation_targets": ATTEMPT
    / "targeted-v01-validation-targets.junit.xml",
    "unit_and_contract_tests": ATTEMPT / "unit-and-contract-tests.junit.xml",
}
_NODE_JUNITS = frozenset({"full_node_suite"})
RUN_RESULTS = (
    "dependency-regression-t04",
    "full-node-suite",
    "full-python-suite",
    "git-diff-check",
    ...
    "targeted-v01-validation-targets",
...
OUTPUT_NAMES = (
    "build_v01_0001_evidence.py",
    "commands.jsonl",
    "dependency-regression-t04.junit.xml",
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        (
            "dependency_regression_t04",
            pytest_summary(JUNIT_PATHS["dependency_regression_t04"]),
            115,
        ),
        ...
        (
            "targeted_v01_validation_targets",
            pytest_summary(JUNIT_PATHS["targeted_v01_validation_targets"]),
            92,
        ),
        (
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        ...
        "next_action": "SEAL_V01_0001_THEN_CONTINUE_DAG",
...
def write_scope_verification() -> dict[str, Any]:
    ...
    return {
        "approved_scope": [
            "python/epistemic_foundry/validation/targets/**",
            "artifacts/work_packages/V01/**",
        ],
        "attempt_id": ATTEMPT_ID,
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "declaring_sources": {
            "manifest": (
                "schemas/validation-target-manifest.schema.json (by path, referencing.Registry)"
            ),
            "scope": ("schemas/scope-vector.schema.json (20 axes, per-type boundness)"),
        },
        "exit_criteria": {
            "all_completion_and_external_effects_resolve_to_immutable_receipts": {
                "mechanism": (
                    "eligibility reports re-derive their own hashes, artifact hashes are derived from a path-to-hash map so the manifest and its pin set cannot disagree, and determinism holds under caller-supplied ids"
                ),
                "status": "PASS",
            },
            "governing_schemas_authority_boundaries_failure_states_exact": {
                "mechanism": (
                    "the component reads both canonical schemas by path with a referencing registry so relative refs resolve canonically, derives the highest safety class and the no-approval policy from the enum's own order with both ladders pinned, and asserts its two local tables key-for-key against the declaring enums as VOCABULARY_DRIFT"
                ),
                "status": "PASS",
            },
            "happy_negative_crash_resume_adversarial_coverage": {
                "mechanism": (
                    "construction refuses an undeclared entrypoint, duplicate port ids, ungrounded constraints and malformed hashes; eligibility screens approval coherence, type-appropriate reproducibility, declared actions and a non-vacuous scope, with counts reconciling exactly"
                ),
                "status": "PASS",
            },
            "no_candidate_model_prompt_backend_or_hook_acquires_authority": {
                "mechanism": (
                    "the component imports nothing from src (boundary check PASS), holds its tests component-local so the global suite count is unchanged, and screens rather than approves — per-invocation approval stays with T04's gate"
                ),
                "status": "PASS",
            },
        },
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_v01_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "V01_VALIDATION_TARGET_MANIFESTS",
        ...
        "next_package": "X01-0001",
        "not_claimed": [
            "validation planning or execution: V02/V03 territory",
            "schema caching: every call re-reads the schema files, mirroring T04's no-cache stance",
...
def build() -> dict[str, Any]:
    ...
    write_json("v01-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("V01-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "v01-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored V01-0001 report is not the deterministic document")
    return _summary()
