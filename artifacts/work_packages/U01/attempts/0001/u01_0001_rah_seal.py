#!/usr/bin/env python3
"""Append U01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "U01-0001"
WORK_PACKAGE_ID = "U01"
EXPECTED_PARENT = "000196-ad719b37"
EXPECTED_PARENT_COUNT = 196
EXPECTED_PARENT_EVIDENCE = "E0198"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "0b1da9530000d33a868182ec6fc4b11df9de2dafabb3caac8f2c893b5d3a9c83"
)
CORE_EVIDENCE_ID = "E0199"
FINAL_EVIDENCE_ID = "E0200"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_u01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward V01-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "U01-0001 openapi server and generated clients "
    "is evidence-sealed PASS (E0199/E0200). V01-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required U01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "packages/ui-api/src/openapi").rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        ),
        *sorted(
            path.relative_to(ROOT).as_posix()
            for path in (ROOT / "web/src/generated").rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        ),
    ]
...
def core_summary() -> str:
    ...
    return (
        "U01-0001 PASS core verification: openapi server and generated clients, "
        ...
        "session, under exact dependency bindings (C04 C04-0001, T04 T04-0001) "
        "and an S05-0001 regression baseline. the route table is projected from the canonical OpenAPI document — operation ids unique, responses schema-bound, verbs declared — and the generated client is emitted by a generator whose header binds the source document hash "
        ...
        "Key review finding: The canonical-hash helper is restated locally with the reason in the file header: plugin-host declares no package exports and the boundary policy forbids cross-package src imports — the restatement is the boundary-compliant option. "
        ...
        + ". V01-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"U01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a live HTTP server: the binding answers exactly one question — does the handler set match the declared operation set; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("U01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U01-0001 parent is not the sealed U01 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("U01 preflight requires exactly 196 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("U01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("U01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("U01 core did not append exactly E0199")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("U01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("U01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("U01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored U01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("U01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("U01 final did not append exactly E0200")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("U01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("U01 core tail is not E0199")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 198:
        raise SystemExit(f"unexpected U01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with U01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored U01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
