#!/usr/bin/env python3
"""Append Y02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000320-44bc6324"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 320
#: The last evidence id in the ledger at that tail, e.g. "Y02xx".
EXPECTED_PARENT_EVIDENCE = "E0322"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "842d03f95d4356339d581448ffeddc564e4fb01148129a7859dccd77a0461f45"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0323"
FINAL_EVIDENCE_ID = "E0324"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_y02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after Y02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} observability, SLO and privacy-safe telemetry gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "Y02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Y02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: observability namespace is pinned so a change to any span, redaction,
    #: SLO, or primitive module or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/observability").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Y02-0001 PASS core verification: the observability subsystem -- "
        "OpenTelemetry-style spans over W3C trace context correlated to hashed "
        "effect receipts, privacy-safe log/telemetry redaction, and honest SLO "
        "result states -- implemented in "
        "packages/foundry-kernel/src/observability by a bounded implementation "
        "agent and reviewed by this seal-prep session (a distinct actor, "
        "actor-independent from the author) on the sealed Y01-0001 package with no "
        "new production dependency. emitSpan builds a span well-formed by "
        "construction (16-byte non-zero trace_id, 8-byte non-zero span_id, "
        "monotonic timing, canonical kind/status, W3C traceparent, sha256 "
        "span_hash) and fails closed on all-zero or malformed identifiers, an end "
        "before start, or a self-parent (SPAN_INVALID); correlateReceipt ties a "
        "span's trace_id/span_id to an effect receipt by id and sha256 hash and "
        "fails closed on mismatch or a missing reference "
        "(RECEIPT_CORRELATION_MISMATCH, RECEIPT_CORRELATION_MISSING). redactRecord "
        "drops sensitive-keyed values whole and rewrites secret/PII-shaped "
        "substrings (email, Bearer, JWT, AWS, GitHub, provider key, SSN, card), "
        "fails closed on malformed input or an unapplied required redaction "
        "(REDACTION_INPUT_INVALID, REDACTION_REQUIRED_MISSING), and "
        "assertNoResidualSecrets re-scans the output so no literal secret survives "
        "serialization; emitSpan redacts span and event attributes before storage "
        "through the same path. evaluateSlo reports UNKNOWN with no samples and a "
        "span with an exception event cannot claim OK (DISHONEST_SPAN_STATUS). "
        "Records are validated by the module's own fail-closed guards and "
        "canonical sha256 hashing; no canonical observability schema exists in the "
        "schemas/ registry, disclosed as a non-blocking observation. Ruff "
        "lint/format, otel_trace_test 13/13, log_redaction_test 8/8, targeted "
        "21/21, full Python 1261/1261, full Node 1274/1274 across 113 files, and "
        "git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Y02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Y01-0001 and every prior RAH generation remain immutable. Y02 "
        "provides the observability primitives, spans, receipt correlation, "
        "redaction, and honest SLO states; the wider Y-phase operations, scale, "
        "backup, and disaster-recovery surface, source coverage, PRD, review, and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Y02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"Y02 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Y02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Y02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"Y02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Y02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Y02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Y02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Y02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Y02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"Y02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Y02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"Y02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected Y02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Y02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Y02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
