#!/usr/bin/env python3
"""Append N05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "N05-0001"
WORK_PACKAGE_ID = "N05"
EXPECTED_PARENT = "000188-7fffd145"
EXPECTED_PARENT_COUNT = 188
EXPECTED_PARENT_EVIDENCE = "E0190"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "bb5f6b80d246d88007f916f5f4db2af8ad05c12b35f048f02605907ce49d5335"
)
CORE_EVIDENCE_ID = "E0191"
FINAL_EVIDENCE_ID = "E0192"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_n05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward T05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "N05-0001 bounded asynchronous proposal/evaluation/persistence lanes and scheduler "
    "is evidence-sealed PASS (E0191/E0192). T05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required N05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "src/epistemic_foundry/scheduler/v4_n05").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "N05-0001 PASS core verification: bounded asynchronous proposal/evaluation/persistence lanes and scheduler, "
        ...
        "session, under exact dependency bindings (E05 E05-0001, F05 F05-0001, N04 N04-0001) "
        "and an S05-0001 regression baseline. lane identities are derived from the reconciliation module's own stages so lane ledgers are reconciliation input with no translation, phase-node bindings must be graph-declared, unshared and transitively ordered, and three colliding literals were renamed rather than allowlisted so the repository discipline gate passes 5/5 "
        ...
        "Key review finding: The three EF4-I22 collisions (FAIL, contract, verdict) were resolved by renaming (LANE_FAIL, binding_contract, schedule verdict) without self-registering as a declaring owner — the correct precedence set by E05. "
        ...
        + ". T05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"N05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "real async execution: no threads, tasks or locks exist here; a runtime emitting a schedule unlike what its threads did is outside detection (N06 territory); author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("N05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("N05-0001 parent is not the sealed N05 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("N05 preflight requires exactly 188 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("N05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("N05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("N05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("N05 core did not append exactly E0191")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("N05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("N05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("N05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("N05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored N05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("N05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("N05 final did not append exactly E0192")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("N05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("N05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("N05 core tail is not E0191")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 190:
        raise SystemExit(f"unexpected N05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with N05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored N05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
