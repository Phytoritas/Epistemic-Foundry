#!/usr/bin/env python3
"""Append J03-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000348-ddcc8c15"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 348
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0350"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "033235cbe13c62f82b71abdbec9de76f9fcffb67ba11492e7b9eac3fc05fe5c8"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0351"
FINAL_EVIDENCE_ID = "E0352"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_j03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after J03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} ContextCapsule assembly and exclusion gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "J03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required J03-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: The five J03 product bytes outside the attempt directory, bound by path.
    return {
...
def core_summary() -> str:
    ...
    return (
        "J03-0001 PASS core verification: the deterministic content-addressed "
        "ContextCapsule assembler with explicit exclusions and a fail-closed "
        "freshness gate, implemented in packages/context-capsule by a bounded "
        "implementation agent and reviewed by this seal-prep session (a distinct "
        "actor, actor-independent from the author) on the sealed J01 parent skill "
        "router with no new production dependency. assembleContextCapsule turns an "
        ...
        "random id, filesystem discovery, or previous-capsule fallback, replaying "
        "the same snapshot yields identical bytes, and set-like input order cannot "
        "change the bytes or hash. Exclusions are explicit and cannot smuggle "
        "excluded content or its summary; every included artifact binds a source "
        "hash and nonblank summary whose summary_hash binds its bytes. "
        "requireFreshContextCapsule fails closed on session, phase, RunSpec, policy, "
        "expiry, not-yet-valid, changed, missing, or newly visible unaccounted "
        "artifacts and on capsule or summary tamper. Proxies, accessor getters, "
        "sparse arrays, custom prototypes, invalid hashes, invalid phases, and "
        "non-canonical JSON fail closed; the ContextCapsule boundary is bound to the "
        "generated contracts registry at module load and fails closed on drift, and "
        "the emitted capsule validates against the canonical Draft 2020-12 "
        "schemas/context-capsule.schema.json, which J03 did not modify. Ruff "
        "lint/format, capsule_hash_test 11/11, stale_capsule_test 10/10, targeted "
        "21/21, full Python 1261/1261, full Node 1291/1291 across 115 files, and git "
        "diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"J03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". J01-0001 and every prior RAH generation remain immutable. J03 provides "
        "the deterministic ContextCapsule assembler with explicit exclusions and a "
        "fail-closed freshness gate; post-compaction recovery, the J04 "
        "compaction-resume gate, live-ledger reading, source coverage, PRD, review, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("J03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J03-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"J03 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("J03 parent generation manifest hash mismatch")
    if (
        read_json(ATTEMPT / "report.json").get("rah_state", {}).get("status")
        == "active"
    ):
        raise SystemExit("J03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"J03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("J03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("J03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("J03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored J03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("J03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"J03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("J03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"J03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected J03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with J03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored J03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
