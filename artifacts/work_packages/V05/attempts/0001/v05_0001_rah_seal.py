#!/usr/bin/env python3
"""Append V05-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This is a seal-PREP artifact.  It was
authored as a bounded implementation hand that was instructed not to touch
``.rah/``, so it cannot read the live ledger to pin the parent generation
identifier, its manifest hash, or the exact evidence identifiers this seal will
issue.  ALL SIX ledger-derived pins are therefore left as the sentinel
``FILL_FROM_LEDGER`` and the seal refuses to run until the parent fills them
from the live ledger tail:

* ``EXPECTED_PARENT`` - the current committed RAH generation (e.g.
  ``000257-XXXXXXXX``), the tail the V05 core generation will branch from.
...
* ``EXPECTED_PARENT_EVIDENCE`` - the latest evidence id at that tail (the final
  closeout evidence of the package sealed immediately before V05).
* ``EXPECTED_PARENT_MANIFEST_SHA256`` - the sha256 of that tail generation's
  ``generation-manifest.json``.
* ``CORE_EVIDENCE_ID`` - the next contiguous evidence id the V05 core commits.
* ``FINAL_EVIDENCE_ID`` - the following evidence id the V05 closeout commits.
...
exactly as the R04/H05/R05 templates do.  Do not run any mode until the six pins
are filled; ``assert_parent_pins`` fails closed while any remains a sentinel.
"""
...
#: The current committed RAH generation the V05 core will branch from.
#: Fill from `.rah/ralph` current pointer before running, e.g. "000257-XXXXXXXX".
EXPECTED_PARENT = "000260-f23d67e6"
#: The number of preserved generations at that tail.
EXPECTED_PARENT_COUNT = 260
#: The latest evidence id at that tail (final closeout of the prior package).
EXPECTED_PARENT_EVIDENCE = "E0262"
#: The sha256 of that tail generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "39241a9cbf3a6fdca984e21085c4e9b0fd48db8243615c16b86a8c95d3f48a32"
#: The next contiguous evidence id the V05 core commits.
CORE_EVIDENCE_ID = "E0263"
#: The following evidence id the V05 closeout commits.
FINAL_EVIDENCE_ID = "E0264"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_v05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after V05-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} validation cascade, OOD challenge, independent replication "
    f"and promotion-ceiling advancement gate is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "V05-0001 seal is not pinned: set "
            + ", ".join(unfilled)
            + " from the live ledger tail (the generation and evidence ids "
            "committed by the package sealed immediately before V05) before "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required V05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/validation").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "V05-0001 PASS core verification: the validation-cascade / OOD-challenge "
        "/ statistical-admissibility / replication-ceiling advancement gate, "
        "implemented in src/epistemic_foundry/validation/v4_v05 by the primary "
        "session across bounded turns and reviewed by an independent "
        "contract-reviewer subagent (actor-independent from the author) "
        "dispatched under the product owner's explicit parallel-agent "
        "instruction, under exact dependency bindings (V04 V04-0001, S05 "
        "S05-0001, Q05 Q05-0001, R05 R05-0001) and an X04-0001 regression "
        "baseline. The gate advances a claim only when S05's cascade aggregates "
        "to the promotion-decision schema's own passing token, the Red Queen "
        "out-of-distribution challenge is survived by that surface's own "
        "all-matches-won predicate, Q05's statistical-admissibility receipt is "
        "authentic (produced by the admissibility gate, re-derives its own hash, "
        "names this candidate) and admitted the candidate, and the replication "
        "owner's ceiling reaches the configured promotion level. Every stage, "
        "challenge, admissibility receipt and replication plan is bound to one "
        "candidate id and one cascade plan id first, so a bundle assembled from "
        "another candidate's artifacts is refused. A candidate-generating "
        "requesting role is refused with the set the verifier firewall declares. "
        "Every advance-or-refuse decision re-derives byte for byte from its own "
        "published fields with no clock or random draw, no input is mutated, and "
        "nothing scores, selects, promotes or evaluates. Ruff lint/format, "
        "wire-literal-discipline, schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, V04/S05/Q05/R05 dependency regressions, full "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"V05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. V05 composes sealed "
        "verdicts and records a replayable advancement decision only: it does "
        "not score, select, promote or evaluate any candidate, makes no DSSAT or "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("V05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V05-0001 parent is not the sealed frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"V05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("V05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("V05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"V05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("V05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("V05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("V05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored V05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("V05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"V05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("V05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"V05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected V05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with V05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored V05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
