#!/usr/bin/env python3
"""Prove the V05 advancement gate is discoverable by the packaging config.

``src/epistemic_foundry/validation`` and ``.../validation/v4_v05`` are new
package markers authorized by the V05 write scope in
``manifests/development_manifest.yaml``, so there is a scope decision to prove
rather than assume: a marker that never reached the wheel would let the gate
...
EXPECTED = (
    "epistemic_foundry.validation",
    "epistemic_foundry.validation.v4_v05",
)
