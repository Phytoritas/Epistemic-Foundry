#!/usr/bin/env python3
"""Append A05-0003 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "A05-0003"
WORK_PACKAGE_ID = "A05"
EXPECTED_PARENT = "000114-474a0f88"
EXPECTED_PARENT_COUNT = 114
EXPECTED_PARENT_EVIDENCE = "E0116"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "0ecc772e41caae19ffeae5ddd800f94e0c12127e0d14124aefd7594420627e65"
)
CORE_EVIDENCE_ID = "E0117"
FINAL_EVIDENCE_ID = "E0118"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_a05_0003_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run A06-0002 with a fresh deterministic constitutional audit verifier.",
    "After A06-0002 PASS, recompute the 156-package DAG (the x05 wave becomes eligible).",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required A05-0003 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "A05-0003 PASS core verification: the constitutional runtime "
        "correction ordered by HD-EF4-A06-RM001-20260730-001 is implemented. "
        "The G00-G14 registry, charter 4.2 applicability matrix, pin-"
        "resolution contract, and attestor/approval independence checks live "
        "in src/epistemic_foundry/governance/evolution_authority; the "
        "canonical 23-node evolution_promotion workflow graph-enforces gate "
        "order, advisory-only llm outputs, one promotion:commit holder, and "
        "all authority artifact outputs; the chamber delegates promotion to "
        "that subworkflow (A06-F003/F004/F005 remediated at implementation "
        "level). The MASTER_SPEC table, spec pins, and J02 inventory cascade "
        "is bound to HD-EF4-A05-0003-SCOPE-20260801-001. All 24 negative and "
        "6 positive constitutional cases pass; ruff lint/format, targeted "
        "Python 40/40, targeted J02 Node 25/25, full Python 1196/1196, full "
        "Node 860/860 across 84 files, and git diff --check pass. "
        ...
        "A06-0002 independent re-audit is next; implementation_gate=fail and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"A05-0003 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". A05-0002, the immutable A06-0001 FAIL trigger, W01-0001, and "
        "every prior RAH generation remain unchanged. A05 correction is "
        "PASS, but the A06-0002 independent re-audit, the wider 156-package "
        "DAG, source coverage, PRD, review, and global closeout remain; "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("A05-0003 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "A05-0003 constitutional runtime correction is evidence-sealed PASS "
        "under HD-EF4-A06-RM001. A06-0002 independent re-audit is next; "
        "implementation_gate=fail and completion_ready=false."
    )
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("A05-0003 parent is not the sealed W01-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("A05 preflight requires exactly 114 preserved generations")
    ...
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("A05-0003 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("A05 core did not append exactly E0117")
...
def run_final() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("A05 final did not append exactly E0118")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("A05 core tail is not E0117")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 116:
    ...
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored A05 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
