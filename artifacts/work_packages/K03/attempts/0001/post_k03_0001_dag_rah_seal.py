#!/usr/bin/env python3
"""Append the verified post-K03-0001 DAG reconciliation to active RAH state."""
...
import post_k03_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000036-fa3c1178"
EXPECTED_EVIDENCE_ID = "E0037"
NEXT_ACTIONS = [
    "Execute K04-0001 corpus injection and ingest quality gates under its exact manifest write scope.",
    "Run K04 required checks, full regressions, and a separate primary-session review.",
    "Seal K04 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-k03-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 48
        and stored.get("blocked_package_count") == 103
        and stored.get("ready_packages_manifest_order")
        == ["K04", "L01", "N01", "T01", "A06"]
        and stored.get("next_package") == "K04"
    ...
    review_path = ATTEMPT / "post-k03-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_k03_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-K03-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "K03-0001 final evidence E0036 / 000036-fa3c1178. The active development "
        "manifest has 156 unique packages, no unknown dependency and no cycle; "
        "48 packages are currently PASS, 5 are dependency-ready, and 103 wait on "
        "unmet dependencies. Manifest-order READY is K04, L01, N01, T01, A06, so "
        "K04 is the next bounded package. Package state is selected from the "
...
def commit(summary: str) -> str:
    ...
    identifiers = evidence_ids(payloads)
    if identifiers[-1] != "E0036" or len(numbered_generations(ralph_root)) != 36:
        raise SystemExit("post-K03 DAG seal requires exact E0036 / 36-generation state")
    ...
    note = (
        "The sealed post-K03 DAG selects K04 as the earliest dependency-ready "
...
def main() -> int:
    ...
    ralph_root, current, payloads = current_state()
    identifiers = evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0036":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0037 summary differs from current DAG artifacts")
    result = verify_generation_store(37)
