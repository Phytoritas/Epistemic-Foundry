#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed K03-0001 PASS.
...
OUTPUT = ATTEMPT / "post-k03-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-k03-0001-dag-review.md"
...
def reconcile() -> dict[str, Any]:
    ...
    k03 = states["K03"]
    if not isinstance(k03.get("report"), str):
        raise SystemExit("K03 report is missing")
    k03_report = read_json(ROOT / k03["report"])
    rah = k03_report.get("rah_state")
    if not (
        k03["selected_attempt_id"] == "K03-0001"
        and k03["current_state"] == "PASS"
        and k03["report_sha256"] == EXPECTED_K03_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0035"
        and rah.get("core_generation") == "000035-b5901fdd"
        and rah.get("final_closeout_evidence_id") == "E0036"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("K03-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "K03_attempt_id": "K03-0001",
            "K03_core_evidence_id": "E0035",
            "K03_core_generation": "000035-b5901fdd",
            "K03_final_closeout_evidence_id": "E0036",
            "K03_report": k03["report"],
            "K03_report_sha256": k03["report_sha256"],
            "status": "PASS",
        },
        ...
        "projection": "POST_K03_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 48
        and result["blocked_package_count"] == 103
        and result["ready_packages_manifest_order"]
        == ["K04", "L01", "N01", "T01", "A06"]
        and result["next_package"] == "K04"
    ):
        raise SystemExit(
            "post-K03 DAG differs from the expected evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-K03-0001 live DAG reconciliation
