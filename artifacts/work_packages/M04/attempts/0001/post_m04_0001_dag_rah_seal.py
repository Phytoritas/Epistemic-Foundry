#!/usr/bin/env python3
"""Append the verified post-M04-0001 DAG reconciliation to active RAH state."""
...
EXPECTED_PARENT = "000063-873f0251"
EXPECTED_PARENT_MANIFEST_HASH = (
    "2f0f23381d228414dbf1f4afe56be048b913447a1b6e772e5fb96125887bff52"
)
EXPECTED_EVIDENCE_ID = "E0064"
NEXT_ACTIONS = [
    "Execute N01-0001 scientific claim store under its exact manifest scope.",
    "Run N01 claim_link_integrity and passport_query_test with upstream regressions and separate primary-session review.",
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-m04-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 57
        and stored.get("waiting_package_count") == 95
        and stored.get("ready_packages_manifest_order") == ["N01", "O01", "T01", "A06"]
        and stored.get("next_package") == "N01"
    ...
    review_path = ATTEMPT / "post-m04-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_m04_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-M04-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "M04-0001 final evidence E0063 / 000063-873f0251. The canonical external "
        ...
        "no unknown dependency and no cycle; 57 packages are PASS, 4 are "
        "dependency-ready, and 95 wait on unmet dependencies. Manifest-order READY "
        "is N01, O01, T01, A06, so N01 is next. Highest numeric attempt selection "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed M04 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0063" or len(base.numbered_generations(ralph_root)) != 63:
        raise SystemExit("post-M04 DAG seal requires exact E0063 / 63-generation state")
    ...
    note = (
        "The sealed post-M04 DAG selects N01 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0063":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0064 summary differs from current DAG artifacts")
    result = m04_seal.verify_generation_store(64)
