#!/usr/bin/env python3
"""Append O05-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was authored as a bounded
hand instructed not to touch ``.rah/``, so it cannot read the live ledger to
pin the parent generation, its count, its manifest hash, or the two evidence
identifiers this attempt will append.  Every one of those five pins is
therefore left as the literal sentinel ``FILL_FROM_LEDGER`` and
``assert_parent_pins()`` refuses to run while any remains.  The parent fills
them from the live ledger tail at seal time:

* ``EXPECTED_PARENT`` — the current committed RAH generation (``NNNNNN-xxxxxxxx``).
* ``EXPECTED_PARENT_COUNT`` — the number of preserved numbered generations.
* ``EXPECTED_PARENT_MANIFEST_SHA256`` — sha256 of that generation's
  ``generation-manifest.json``.
* ``CORE_EVIDENCE_ID`` — the next contiguous evidence id after the live tail.
* ``FINAL_EVIDENCE_ID`` — the id immediately after ``CORE_EVIDENCE_ID``.

The parent's latest evidence id is *derived* from the live ledger rather than
pinned, so only these five constants need filling.  Everything else — summaries,
hashes, flat-snapshot verification, contiguity and preservation checks — is
derived exactly as the sealed templates do.
"""
...
#: Sentinel for the five pins that can only be read from the live ledger tail.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
#: The current committed RAH generation, e.g. "000208-abcdef01".
EXPECTED_PARENT = "000208-def05e5d"
#: The number of preserved numbered generations at seal time.
EXPECTED_PARENT_COUNT = 208
#: sha256 of the parent generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "944489b83466015c9458edf5c53b92a9858713047030e4bde4da0d8f0c6f155b"
)
#: The next contiguous evidence id after the live tail (this attempt's core).
CORE_EVIDENCE_ID = "E0211"
#: The evidence id immediately after CORE_EVIDENCE_ID (this attempt's closeout).
FINAL_EVIDENCE_ID = "E0212"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_o05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after O05-0001 seals and select the next "
...
#: appends; it is checked against this attempt before anything is written.  The
#: evidence pair is interpolated from the two evidence pins, so it reads
#: correctly only after the parent has filled them.
GATE_NOTE = (
    f"{ATTEMPT_ID} evolution evidence retrieval, layered novelty and "
    f"coverage-debt acquisition is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if pending:
        raise SystemExit(
            "O05-0001 seal is not pinned: set "
            + ", ".join(pending)
            + " from the live ledger tail (the current committed RAH generation "
            "and the next two contiguous evidence ids) before running."
        )
...
def _next_evidence_id(evidence_id: str) -> str:
    if re.fullmatch(r"E\d{4}", evidence_id) is None:
        raise SystemExit(f"malformed evidence id: {evidence_id}")
    return f"E{int(evidence_id[1:]) + 1:04d}"
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required O05-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  The rglob
    #: covers the whole retrieval package so a silent change to the enclosing
    #: marker or search_state is caught here, even though O05 owns only v4_o05.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/retrieval").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "O05-0001 PASS core verification: evolution evidence retrieval, layered "
        "novelty and coverage-debt acquisition, implemented in "
        "src/epistemic_foundry/retrieval/v4_o05 by a bounded implementation "
        "agent dispatched in parallel under the product owner's explicit "
        "aggressive-parallel-agent instruction, and reviewed independently by "
        "the sealing session as an actor-independent reviewer distinct from the "
        "implementer, under exact dependency bindings (O04 O04-0001, K05 "
        "K05-0001, C05 C05-0001) and an R05-0001 regression baseline. A "
        "retrieval plan is declared inside a pinned K05 snapshot and the "
        "prior-art boundary over it, so a document outside the bytes or after "
        "the as-of date is refused, and the two refusals stay separate. Lane "
        "receipts derive their search state from results rather than accepting "
        "it, and the six receipt states project onto four coverage states so no "
        "inconclusive lane reads as searched-and-empty. Layered novelty binds "
        "the evaluation owner's per-layer vector and K05's corpus-bounded ladder "
        "to one boundary and one plan, inheriting the capped status and ceiling; "
        "coverage debt is ranked from the sealed M05 niche map, and a selected "
        "adversarial lane never conclusively searched is refused. Every record "
        "re-derives byte for byte from its own content with no clock or random "
        "draw, and nothing scores, selects, promotes or evaluates. Ruff "
        "lint/format, schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, O04/K05/C05 dependency regressions, full Python "
        "and full Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by the sealing session as an independent reviewer with "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"O05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. O05 retrieves "
        "evolution evidence, assesses layered novelty and ranks coverage debt "
        "only: it does not score, select, promote or evaluate any candidate, "
        "keeps novelty, coverage debt and evidence strength as separate "
        "dimensions, makes no DSSAT or plant-model parity claim, and promotion "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("O05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT:
        raise SystemExit("O05-0001 parent is not the pinned live ledger tail")
    if (
        _next_evidence_id(latest) != CORE_EVIDENCE_ID
        or _next_evidence_id(CORE_EVIDENCE_ID) != FINAL_EVIDENCE_ID
    ):
        raise SystemExit(
            "O05-0001 core/final evidence ids are not the next two contiguous "
            f"ids after the ledger tail {latest}"
        )
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"O05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("O05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("O05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("O05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"O05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("O05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("O05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("O05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored O05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("O05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"O05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("O05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"O05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected O05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with O05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored O05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
