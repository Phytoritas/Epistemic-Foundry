#!/usr/bin/env python3
"""Append F06-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000234-eb50bea2"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 234
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0236"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "98c3a43893af27154f5693121112a99f0d7e622f166031391577d59989ef82c2"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0237"
FINAL_EVIDENCE_ID = "E0238"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_f06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after F06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} FORGE-EVOLVE lifecycle and replay integration gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "F06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required F06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: evolution namespace is pinned so a change to the composed F05 surface (or
    #: the parent marker) is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/evolution").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "F06-0001 PASS core verification: FORGE-EVOLVE lifecycle and replay "
        "integration gate, implemented in src/epistemic_foundry/evolution/v4_f06 "
        "by a bounded implementation subagent and reviewed by the sealing agent "
        "(actor-independent from the author) under exact dependency bindings "
        "(F05 F05-0001, I05 I05-0001, R05 R05-0001) and an R06-0001 regression "
        "baseline. The gate composes the sealed F05 EVOLVE state machine, I05 "
        "genome intake and R05 typed operator registry without restating them, "
        "then refuses any run whose lifecycle transitions or stop certificate are "
        "inconsistent, whose checkpoints bind more than one evaluator bundle "
        "hash, whose seed population or candidate set does not reconcile, whose "
        "applied operator is undeclared, or whose own replay report is not a "
        "strict, exact, drift-free byte-for-byte reproduction; a replay report "
        "that flatters itself is refused as dishonest. Fifteen finding codes name "
        "exact refusals. Every admit or refuse decision re-derives byte for byte "
        "from its own published fields with no clock or random draw, and nothing "
        "scores, ranks, selects, promotes or evaluates; no candidate, model, "
        "prompt, backend or hook acquires evaluator, holdout or promotion "
        "authority. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, packaging-discovery, the F05, I05 and R05 "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"F06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. F06 composes the sealed "
        "lifecycle, intake and operator surfaces and records a deterministic "
        "admit/refuse lifecycle-replay receipt only: it does not score, rank, "
        "select, promote or evaluate any candidate, grants no authority at the "
        "gate, makes no DSSAT or plant-model parity claim, and promotion remains "
        "a governance decision. Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("F06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("F06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"F06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("F06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("F06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("F06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"F06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("F06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("F06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("F06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("F06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored F06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("F06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"F06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("F06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("F06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"F06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected F06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with F06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored F06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
