#!/usr/bin/env python3
"""Append G04-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000330-41e3fe43"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 330
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0332"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "887c2ce43857ed9000dcec3ebefaa222b6318a343bd67515a2a34d322f0f633e"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0333"
FINAL_EVIDENCE_ID = "E0334"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_g04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after G04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} local marketplace fresh-install and clean-uninstall gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "G04-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required G04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: local-marketplace namespace is pinned so a change to the lifecycle
    #: harness or its test is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "tests/install/local-marketplace").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "G04-0001 PASS core verification: the local marketplace fresh-install and "
        "clean-uninstall gate implemented in tests/install/local-marketplace by a "
        "bounded implementation agent and reviewed by this seal-prep session (a "
        "distinct actor, actor-independent from the author) over the sealed "
        "G02-0001/G03-0001 epistemic-foundry plugin package shell with no new "
        "production dependency. The single g04-lifecycle.test.mjs test drives "
        "lifecycle-harness.mjs against the real codex host: it builds an isolated, "
        "uniquely named OS-temp root with a disposable local marketplace, an "
        "isolated CODEX_HOME, an isolated user profile, and an empty cwd under "
        "spaced non-ASCII paths; installs the plugin into the isolated cache as a "
        "byte-for-byte hash-equal copy of the four source files; observes "
        "enable/disable/re-enable through the host; proves the installed cache "
        "survives marketplace-source detachment; invokes the absolute installed "
        "dispatcher from an empty PATH and empty cwd so it fails closed naming the "
        "installed dist target with zero repository-checkout fallback and no "
        "fabricated command success; then removes the plugin and marketplace with "
        "zero residue while the real user's ~/.codex/config.toml and selector cache "
        "stay byte-identical before and after. Both required checks -- "
        "fresh_install_test and clean_uninstall_test -- are asserted in that "
        "1/1 module. Ruff lint/format, the lifecycle module 1/1, full Python "
        "1261/1261, full Node 1291/1291 across 115 files, and git diff --check "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"G04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". G02-0001, G03-0001, and every prior RAH generation remain immutable. "
        "G04 gates local-marketplace fresh install and clean uninstall of the "
        "capability-free plugin shell; the T03 CLI payload and command semantics, "
        "remote marketplace publication, OS-enforced sandboxing, release "
        "readiness, source coverage, PRD, review, and global closeout remain; "
        "implementation_gate=fail and completion_ready=false."
    )
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("G04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G04-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"G04 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("G04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("G04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"G04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("G04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("G04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("G04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored G04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("G04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"G04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("G04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"G04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected G04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with G04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored G04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
