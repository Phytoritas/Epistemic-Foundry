#!/usr/bin/env python3
"""Append O02-0002 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "O02-0002"
EXPECTED_PARENT = "000102-345fe339"
EXPECTED_PARENT_COUNT = 102
EXPECTED_PARENT_EVIDENCE = "E0104"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "d7d85cce4a37e6a478bb2fbd4b312c5ae558048c6b1c89762dea1122b7df3195"
)
CORE_EVIDENCE_ID = "E0105"
FINAL_EVIDENCE_ID = "E0106"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_o02_0002_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run C04-0004 full conformance only after O02-0002 evidence-sealed PASS.",
    "Run the next-unused final B04 packaging attempt only after C04-0004 PASS.",
    "Recompute the 156-package DAG and continue the earliest dependency-ready package while completion_ready remains false.",
]
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required O02-0002 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "O02-0002 PASS core verification: provider-neutral request, response, "
        "candidate, snapshot, receipt, direction, exact dedupe, stable rank, "
        "RRF(k=60), typed terminal, replay, metadata boundary, and non-vector "
        "release contracts pass. All 11 benchmark lanes meet independent "
        "Recall@20 and nDCG@20 thresholds, fused Recall@20 and 4/4 critical "
        "must-find gates pass, and live network/LLM calls are zero. O02-only is "
        "42/42, O02+O01 is 83/83, and Node is 819/819. The first Python run's "
        "sole D04 PostgreSQL startup race is preserved, isolated D04 passes "
        "1/1, and the complete recheck passes 1115/1115. "
        ...
        "C04-0004 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"O02-0002 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". O02-0001 remains immutable SPEC_GAP history and every prior RAH "
        "generation is preserved. C04-0004 is dependency-ready, but C04, final "
        "B04 packaging, the wider DAG, source coverage, PRD, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("O02 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "O02-0002 is evidence-sealed PASS and C04-0004 is next. The global "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("O02-0002 parent generation/evidence is not the sealed B04-0009 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("O02 preflight requires exactly 102 preserved generations")
    ...
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("O02-0002 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("O02 core did not append exactly E0105")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("O02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("O02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("O02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored O02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("O02 final did not append exactly E0106")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("O02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("O02 core tail is not E0105")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit(f"unexpected O02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with O02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored O02 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
