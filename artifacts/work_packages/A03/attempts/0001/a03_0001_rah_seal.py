#!/usr/bin/env python3
"""Append A03-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed A01/A02 templates do.

A03 depends on A01.  Its product bytes are the architecture decision records
under ``docs/adr/`` and the plugin-architecture boundary map
``docs/v4_plugin_architecture.md``, which it attests without editing their
...
#: The current committed generation (e.g. "000260-abcd1234"); its retained
...
EXPECTED_PARENT = "000274-c24be22d"
EXPECTED_PARENT_COUNT = 274
EXPECTED_PARENT_EVIDENCE = "E0276"
EXPECTED_PARENT_MANIFEST_SHA256 = "c8f184868febf082c207e8bc2ba2227fe5f689e3569902a45871d58a70404345"
CORE_EVIDENCE_ID = "E0277"
FINAL_EVIDENCE_ID = "E0278"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_a03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after A03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} architecture decision records and boundary map is "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "A03-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required A03-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory: the ADR tree plus the
    #: plugin-architecture boundary map A03 attests (never edits).
    relatives = sorted(
        [
        ...
        + [ARCH_DOC.relative_to(ROOT).as_posix()]
    )
    for relative in relatives:
        if not (ROOT / relative).is_file():
            raise SystemExit(f"product document missing: {relative}")
...
def core_summary() -> str:
    ...
    return (
        "A03-0001 PASS core verification: architecture decision records and "
        "boundary map, attested by the bounded implementation agent(s) (A03 "
        "maker) that authored the ADR-034 governance record refining ADR-032 "
        "rule 5, the refined boundary_cycle_policy_check, the adr_index_check, "
        "and the adversarial fail-closed negative suite under "
        "artifacts/work_packages/A03/**, and reviewed by the sealing session as "
        "an independent contract-reviewer (actor-independent from the author). "
        "A03 attests the ADR tree docs/adr/** and the boundary map "
        "docs/v4_plugin_architecture.md exactly as they currently are and makes "
        "no edit to their load-bearing content. adr_index_check asserts the ADR "
        "index and files agree on a unique, contiguous ADR-031..034 record set "
        "continuing legacy ADR-030 with no dangling reference or orphan file, "
        "every record structurally complete, and the exit-criterion anchors "
        "ADR-031 and ADR-032 present. boundary_cycle_policy_check parses the real "
        "src/epistemic_foundry imports and enforces, per ADR-032 rule 5 as "
        "refined by ADR-034, layer discipline, no authority/adapter in any cycle "
        "at any granularity, a strict module-slice DAG, and a closed "
        "fingerprinted two-entry top-level L3<->L3 exemption list "
        "(operators<->security, evidence<->retrieval), preserving the "
        "documented-policy anchors, inward ordering, public-package-api-only and "
        "forbidden duplicate-implementation policies. The negative suite proves "
        "the refined predicate is fail-closed on every forbidden shape and passes "
        "only the two pinned exemptions -- a genuine tightening of ADR-032, not a "
        "weakening. adr_index_check 4/4, boundary_cycle_policy_check 6/6, "
        "boundary_cycle_policy_negative 12/12, the full Python suite green, the "
        "live full Node suite green with zero failures, and git diff --check all "
        "pass. A03 depends on A01; the sealed A01-0001 attempt is the build "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"A03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. A03 attests the "
        "architecture decision records and boundary map; it does not re-author "
        "the documents, makes no product-maturity, runtime-executability or "
        "release-readiness claim, and asserts only that the plugin shell and "
        "kernel authority are separated and the component import boundaries are "
        "documented. ADR-034 refines ADR-032 rule 5 into a strictly stronger, "
        "fingerprinted obligation and leaves rules 1-4, 6, 7 intact; no "
        "authority or adapter participates in any tolerated cycle. Author/"
        "reviewer separation holds while external actor-independent "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("A03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("A03-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"A03 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("A03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("A03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("A03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"A03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("A03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("A03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("A03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("A03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored A03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("A03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"A03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("A03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("A03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"A03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected A03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with A03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored A03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
