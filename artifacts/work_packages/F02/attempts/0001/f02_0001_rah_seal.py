#!/usr/bin/env python3
"""Append F02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000338-16ad3354"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 338
#: The last evidence id in the ledger at that tail, e.g. "E0xxx".
EXPECTED_PARENT_EVIDENCE = "E0340"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "e5436295bad1113a71fa10b6db9a6757a09ae6a6611a0d2fc45c9f692c7aa792"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0341"
FINAL_EVIDENCE_ID = "E0342"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_f02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after F02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} FORGE FSM and legal return-edge gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "F02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required F02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: forge/fsm namespace is pinned so a change to the FORGE FSM, its test
    #: support, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/forge/fsm").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "F02-0001 PASS core verification: the deterministic FORGE finite-state "
        "machine -- a class-derived legal transition graph, legal return edges, and "
        "target-inclusive downstream staleness -- implemented in "
        "packages/foundry-kernel/src/forge/fsm by a bounded implementation agent and "
        "reviewed by this seal-prep session (a distinct actor, actor-independent from "
        "the author) over the sealed F01-0003 epistemic-work classification with no "
        "new production dependency. compileForgePlan derives the I/F/O/R/G/E graph "
        "solely from the F01 hash-bound required_phases projection; a forged "
        "projection fails INVALID_CLASSIFICATION_PROJECTION and a tampered "
        "classification fails CLASSIFICATION_INTEGRITY_FAILED. describeForgeTransition "
        "classifies every phase pair as FORWARD/RETURN/CLOSE, "
        "PHASE_NOT_REACHABLE_FOR_CLASSIFICATION, or ILLEGAL_FORGE_TRANSITION. "
        "reduceForgeTransition refuses illegal, stale-revision, wrong-from-phase, "
        "cross-session, and non-transitionable requests before mutating "
        "(ILLEGAL_FORGE_TRANSITION, STALE_REVISION, FROM_PHASE_MISMATCH, "
        "SESSION_MISMATCH, FORGE_SESSION_NOT_TRANSITIONABLE); it is immutable, "
        "revision-bound, and hash-bound over request, event, prior/current state, "
        "plan, and phase sets, and strict event replay reproduces the direct reducer "
        "chain. On a RETURN edge projectReturnStaleness marks the target and every "
        "downstream execution phase STALE with re-derived PAS-STALE-<digest> ids and "
        "a fresh set_hash, emits the superseded sets, and records stale_phases and "
        "stale_artifact_ids, while source sets stay immutable and FORWARD/CLOSE stale "
        "nothing; cross-session, unretained, and tampered sets fail closed. Ruff "
        "lint/format, fsm_property_test 8/8, stale_propagation_test 6/6, targeted "
        "14/14, full Python 1261/1261, full Node 1291/1291 across 115 files, and git "
        "diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"F02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". F01-0003 and every prior RAH generation remain immutable. F02 provides "
        "the deterministic FORGE FSM, its class-derived legal graph, and "
        "return-edge staleness; F03 receipt resolution, policy, capability, "
        "approval, and veto admission gates, source coverage, PRD, review, and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("F02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("F02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"F02 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("F02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("F02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("F02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"F02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("F02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("F02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("F02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("F02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored F02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("F02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"F02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("F02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("F02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"F02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected F02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with F02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored F02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
