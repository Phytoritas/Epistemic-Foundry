#!/usr/bin/env python3
"""Prove the P05 promotion-Parliament gate is discoverable by the packaging config.

``src/epistemic_foundry/parliament`` and ``.../parliament/v4_p05`` are new package
markers: ``parliament`` did not exist before this package, and the P05 write scope
authorizes ``parliament/v4_p05`` while the parent marker is the minimum needed to
reach it.  A marker that never reached the wheel would let the gate import from a
checkout and vanish from an installed distribution, so this is a scope decision to
...
EXPECTED = ("epistemic_foundry.parliament", "epistemic_foundry.parliament.v4_p05")
