#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed L04-0001 PASS."""
...
OUTPUT = ATTEMPT / "post-l04-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-l04-0001-dag-review.md"
EXPECTED_PACKAGE_COUNT = 156
EXPECTED_L04_REPORT_HASH = (
    "sha256:35bb257c90665b2351ab3e9e28a769edd3984aed410c608a2d34ef1713a413d5"
)
...
def reconcile() -> dict[str, Any]:
    ...
    l04 = states["L04"]
    if not isinstance(l04.get("report"), str):
        raise SystemExit("L04 report is missing")
    l04_report = read_json(ROOT / l04["report"])
    rah = l04_report.get("rah_state")
    if not (
        l04["selected_attempt_id"] == "L04-0001"
        and l04["current_state"] == "PASS"
        and l04["report_sha256"] == EXPECTED_L04_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0050"
        and rah.get("core_generation") == "000050-2135fbf0"
        and rah.get("final_closeout_evidence_id") == "E0051"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("L04-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "L04_attempt_id": "L04-0001",
            "L04_core_evidence_id": "E0050",
            "L04_core_generation": "000050-2135fbf0",
            "L04_final_closeout_evidence_id": "E0051",
            "L04_final_generation": "000051-ef410595",
            "L04_report": l04["report"],
            "L04_report_sha256": l04["report_sha256"],
            "status": "PASS",
        },
        ...
        "external_resume_inspection": {
            ...
            "observed_state_generation": "000051-ef410595",
            "parse_errors": {},
            "ralph_status": "active",
            "status": "PASS",
        },
        ...
        "projection": "POST_L04_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 53
        and result["waiting_package_count"] == 99
        and result["ready_packages_manifest_order"] == ["M01", "N01", "T01", "A06"]
        and result["next_package"] == "M01"
    ):
        raise SystemExit(
            "post-L04 DAG differs from evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-L04-0001 live DAG reconciliation
