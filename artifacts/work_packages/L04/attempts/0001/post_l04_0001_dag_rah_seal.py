#!/usr/bin/env python3
"""Append the verified post-L04-0001 DAG reconciliation to active RAH state."""
...
import l04_0001_rah_seal as l04_seal  # noqa: E402
import post_l04_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000051-ef410595"
EXPECTED_PARENT_MANIFEST_HASH = (
    "bf0c25e32954f826cd6b6a008cde503d1731960284eef26112b1b4af037a6f6e"
)
EXPECTED_EVIDENCE_ID = "E0052"
NEXT_ACTIONS = [
    "Execute M01-0001 typed inventory and dependency extraction under its exact manifest scope.",
    "Run M01 map_inventory_test and edge_resolution_test with regressions and separate primary-session review.",
    "Seal M01 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-l04-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 53
        and stored.get("waiting_package_count") == 99
        and stored.get("ready_packages_manifest_order") == ["M01", "N01", "T01", "A06"]
        and stored.get("next_package") == "M01"
    ...
    review_path = ATTEMPT / "post-l04-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_l04_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-L04-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "L04-0001 final evidence E0051 / 000051-ef410595. The canonical external "
        ...
        "no unknown dependency and no cycle; 53 packages are PASS, 4 are "
        "dependency-ready, and 99 wait on unmet dependencies. Manifest-order READY "
        "is M01, N01, T01, A06, so M01 is next. Highest numeric attempt selection "
...
def active_helpers() -> tuple[Any, Any]:
    return l04_seal.sealed_base, l04_seal


def commit(summary: str) -> str:
    base, _ = active_helpers()
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed L04 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0051" or len(base.numbered_generations(ralph_root)) != 51:
        raise SystemExit("post-L04 DAG seal requires exact E0051 / 51-generation state")
    ...
    note = (
        "The sealed post-L04 DAG selects M01 as the earliest dependency-ready "
...
def main() -> int:
    ...
    base, wrapper = active_helpers()
    ralph_root, current, payloads = base.current_state()
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0051":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0052 summary differs from current DAG artifacts")
    result = wrapper.verify_generation_store(52)
