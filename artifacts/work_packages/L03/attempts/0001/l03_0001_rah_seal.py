#!/usr/bin/env python3
"""Append L03-0001 core and closeout evidence to the active RAH state."""
...
import build_l03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_INITIAL_PARENT = "000046-71abfb48"
EXPECTED_INITIAL_EVIDENCE = "E0046"
EXPECTED_INITIAL_GENERATION_COUNT = 46
NEXT_ACTIONS = [
    "Recompute and seal the live 156-package DAG after L03-0001 PASS.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required L03 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "L03-0001 PASS: explicit source-bound UTF-8 byte-span redaction emits "
        "immutable derived artifacts and rejects unresolved profiles, stale "
        "source hashes, overlap, invalid boundaries, no-op, accessor/proxy, and "
        "tamper inputs. Exact source-hash dedupe selects score-desc/memory-ID-asc "
        "representatives without score or count amplification and binds every "
        "exclusion to its selected representative. Forget/delete emits an "
        "immutable terminal revision, canonical ActionIntent, EventRecord, and "
        "EffectReceipt; tombstone retention requires sealed policy-and-law "
        "authority. Matching active legal hold preserves source state with "
        "NOT_EXECUTED, while cache eviction cannot masquerade as canonical "
        "forget. Replay is request, policy, lineage, event, and receipt bound. "
        "Targeted L03 is 44/44 with memory-lifecycle.mjs coverage 94.53% lines, "
        "79.95% branches, and 94.79% functions; canonical effect/ledger schema "
        "and hash interop is 3/3; L01 is 27/27; L02 is 41/41; full Node is "
        "588/588 over 61 files; full Python is 1064/1064; codegen is 126/126. "
        f"Verification sha256:{hashes['memory-lifecycle-verification.json']}; "
        f"receipt sha256:{hashes['memory-lifecycle-verification.artifact-receipt.json']}; "
        ...
        f"{hashes['targeted-l03-node.junit.xml']}; coverage sha256:"
        f"{hashes['targeted-l03-coverage.stdout.log']}; L01 JUnit sha256:"
        f"{hashes['predecessor-l01-node.junit.xml']}; L02 JUnit sha256:"
        f"{hashes['adjacent-l02-node.junit.xml']}; Node JUnit sha256:"
        ...
        f"builder sha256:{hashes['build_l03_0001_evidence.py']}; sealer sha256:"
        f"{hashes['l03_0001_rah_seal.py']}. Review is primary-session separate "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"L03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"{hashes['build_l03_0001_evidence.py']}; sealer sha256:"
        f"{hashes['l03_0001_rah_seal.py']}. Every prior attempt, report, and "
        "generation is preserved. L03-0001 is immutable PASS and the live "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("L03 requires a contiguous evidence ledger")
    if not (
        ...
        raise SystemExit("L03 preflight requires active/fail/completion_ready=false")
...
def commit_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "L03-0001 redaction, dedupe, forget, deletion, and legal-hold semantics "
        "is evidence-sealed PASS. The live DAG and downstream packages remain; "
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("L03 report is already RAH-bound; use verify")
    identifiers = evidence_ids(payloads)
    if not (
        ...
        raise SystemExit("L03 preflight is not at the sealed post-L02 DAG state")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("L03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, core_id]:
        raise SystemExit("L03 core did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("L03 core did not preserve every prior generation")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("L03 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("L03 evidence identifiers differ from the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("L03 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("L03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("L03 final did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L03 final evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("L03 final did not preserve every prior generation")
    return {
