#!/usr/bin/env python3
"""Append the verified post-L03-0001 DAG reconciliation to active RAH state."""
...
import l03_0001_rah_seal as l03_seal  # noqa: E402
import post_l03_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000048-8f4410ea"
EXPECTED_PARENT_MANIFEST_HASH = (
    "52b806db0e2176e2c97c85731d2ee1c5a8ea107f383ec9d7da45adc7010805eb"
)
EXPECTED_EVIDENCE_ID = "E0049"
NEXT_ACTIONS = [
    "Execute L04-0001 recall quality and cross-workspace privacy gates under its exact manifest scope.",
    "Run L04 required checks, regressions, and a separate primary-session review.",
    "Seal L04 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-l03-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 52
        and stored.get("blocked_package_count") == 99
        and stored.get("ready_packages_manifest_order")
        == ["L04", "M01", "N01", "T01", "A06"]
        and stored.get("next_package") == "L04"
    ...
    review_path = ATTEMPT / "post-l03-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_l03_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-L03-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "L03-0001 final evidence E0048 / 000048-8f4410ea. The canonical external "
        ...
        "packages, no unknown dependency and no cycle; 52 packages are currently "
        "PASS, 5 are dependency-ready, and 99 wait on unmet dependencies. "
        "Manifest-order READY is L04, M01, N01, T01, A06, so L04 is the next "
...
def commit(summary: str) -> str:
    ralph_root, parent, payloads = l03_seal.current_state()
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed L03 parent generation manifest hash changed")
    identifiers = l03_seal.evidence_ids(payloads)
    if identifiers[-1] != "E0048" or len(l03_seal.numbered_generations(ralph_root)) != 48:
        raise SystemExit("post-L03 DAG seal requires exact E0048 / 48-generation state")
    ...
    note = (
        "The sealed post-L03 DAG selects L04 as the earliest dependency-ready "
...
def main() -> int:
    ...
    ralph_root, current, payloads = l03_seal.current_state()
    identifiers = l03_seal.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0048":
        before = l03_seal.numbered_generations(ralph_root)
        generation = commit(summary)
        after = l03_seal.numbered_generations(ralph_root)
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0049 summary differs from current DAG artifacts")
    result = l03_seal.verify_generation_store(49)
