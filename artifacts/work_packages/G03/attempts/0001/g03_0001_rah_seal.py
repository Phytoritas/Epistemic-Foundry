#!/usr/bin/env python3
"""Append G03-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000322-8573ace1"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 322
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0324"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "3e03164cf5f63f10642acf915b597aa196421e7fe18a7bedc43f98c655deb05a"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0325"
FINAL_EVIDENCE_ID = "E0326"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_g03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after G03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} plugin path authority gate is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "G03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required G03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: paths namespace is pinned so a change to the resolver or its tests is
    #: caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/plugin-host/src/paths").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "G03-0001 PASS core verification: the deterministic plugin path resolver, "
        "implemented in packages/plugin-host/src/paths by a bounded implementation "
        "agent and reviewed by this seal-prep session (a distinct actor, "
        "actor-independent from the author) on the sealed G01-0001 plugin package "
        "scaffold with no new production dependency. resolvePluginPaths accepts only "
        "caller-supplied absolute roots and never consults cwd, HOME, environment "
        "variables, a repository checkout, or a PATH fallback; missing, relative, "
        "accessor, or Proxy inputs fail closed before any filesystem access, and the "
        "returned resolution record is frozen. Spaces and non-ASCII (Hangul) roots "
        "and portable children resolve through realpathSync.native to their exact "
        "canonical path. assertDisjoint holds PLUGIN_ROOT (read-only installed code), "
        "PLUGIN_DATA (writable), and the workspace boundary pairwise disjoint and "
        "non-nesting; CREATE against a read-only root is denied BOUNDARY_WRITE_DENIED. "
        "Traversal, absolute, mixed-separator, //, stream, Windows-reserved, "
        "trailing dot/space, and NUL inputs fail INVALID_PATH/PATH_ESCAPE_DENIED; "
        "symlinks, junctions, reparse points, and mount crossings are denied by lstat "
        "no-follow plus realpath canonical equality; a root replaced after resolution "
        "fails BOUNDARY_ROOT_CHANGED; and a copied resolution loses authority through "
        "the WeakMap record. Ruff lint/format, path_resolution_test 8/8, "
        "path_traversal_test 5/5, targeted 13/13, full Python 1261/1261, full Node "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"G03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". G01-0001 and every prior RAH generation remain immutable. G03 "
        "provides deterministic plugin path authority separating installed code "
        "from writable data; the G04 local-marketplace fresh-install gate, "
        "OS-enforced sandboxing, race-free durable file-handle capability, source "
        "coverage, PRD, review, and global closeout remain; implementation_gate=fail "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("G03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G03-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"G03 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("G03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("G03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"G03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("G03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("G03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("G03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored G03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("G03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"G03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("G03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"G03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected G03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with G03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored G03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
