#!/usr/bin/env python3
"""Run the I05-0001 hypothesis genome intake acceptance checks.

Required checks from manifests/development_manifest.yaml (I05):
...
report.  The engine ships under src/epistemic_foundry/intake/v4_i05 behind a
new package marker, so ``wire-literal-discipline`` runs the repository's
EF4-I22 gate and ``packaging-discovery`` proves the marker actually reaches the
wheel.  The dependency regressions cover the sealed surfaces this package
composes: F05's EVOLVE state machine, which the seed genome ids are produced
for, and C05's schema bundle, whose sealed family index declares the mutable
search space intake enforces.
"""
...
ATTEMPT_ID = "I05-0001"
COMPONENT = "artifacts/work_packages/I05/attempts/0001"
ENGINE = "src/epistemic_foundry/intake/v4_i05"
DEPENDENCY_TARGETS = {
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/intake is a new package marker, so the wheel claim
...
def targeted() -> int:
    return _pytest("targeted-i05-intake", COMPONENT)
