#!/usr/bin/env python3
"""Append I05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "I05-0001"
WORK_PACKAGE_ID = "I05"
EXPECTED_PARENT = "000180-98172378"
EXPECTED_PARENT_COUNT = 180
EXPECTED_PARENT_EVIDENCE = "E0182"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "36befb701743e5ab3c21f621e3bba496f4905c6c05a34b445fd50eda99878c62"
)
CORE_EVIDENCE_ID = "E0183"
FINAL_EVIDENCE_ID = "E0184"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_i05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward K05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "I05-0001 hypothesisgenome intake, seed population bootstrap and eligibility screening "
    "is evidence-sealed PASS (E0183/E0184). K05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required I05-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "I05-0001 PASS core verification: hypothesisgenome intake, seed population bootstrap and eligibility screening, "
        ...
        "session, under exact dependency bindings (C05 C05-0001, F05 F05-0001, I04 I04-0001) "
        "and an S05-0001 regression baseline. every genome validates against its canonical schema, screened field names are verified against the schema's own properties on every call (GENOME_CONTRACT_DRIFT), and search-space membership is re-read from the sealed C05 index fail-closed "
        ...
        "Key review finding: The eligibility field is named admitted (eligible is a canonical insight-card enum value) and the contract check reads the schema's properties rather than its required list, with required-ness asserted from the test side. "
        ...
        + ". K05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"I05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a specific F05 call-site: the sealed machine evaluates runs, and intake hands seed ids to the run spec rather than calling into it; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("I05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I05-0001 parent is not the sealed I05 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("I05 preflight requires exactly 180 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("I05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("I05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("I05 core did not append exactly E0183")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("I05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("I05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("I05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored I05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("I05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("I05 final did not append exactly E0184")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("I05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("I05 core tail is not E0183")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 182:
        raise SystemExit(f"unexpected I05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with I05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored I05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
