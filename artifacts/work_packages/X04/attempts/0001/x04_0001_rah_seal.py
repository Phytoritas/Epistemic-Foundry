#!/usr/bin/env python3
"""Append X04-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed R05/X01/X02/X03 templates do.
"""
...
#: The current committed generation (e.g. "000245-30a14b5f"); its retained
...
EXPECTED_PARENT = "000248-2a73282c"
EXPECTED_PARENT_COUNT = 248
EXPECTED_PARENT_EVIDENCE = "E0250"
EXPECTED_PARENT_MANIFEST_SHA256 = "ef5826910b4d589a5b19efd86932f104424ede3bc745b1a5205929414d09be49"
CORE_EVIDENCE_ID = "E0251"
FINAL_EVIDENCE_ID = "E0252"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after X04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} cross-provider parity and diversity gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "X04-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X04-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  Cache
    #: directories pytest/CPython scatter through the component are excluded.
    cache_names = {"__pycache__", ".pytest_cache"}
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "evals/provider_parity").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X04-0001 PASS core verification: cross-provider parity and diversity "
        "gate, implemented in evals/provider_parity by a bounded implementation "
        "agent under the product owner's explicit parallel-execution instruction "
        "and reviewed by the sealing session as an independent contract-reviewer "
        "(actor-independent from the author), under exact X02-0001 and X03-0001 "
        "dependency bindings and a G06-0001 live latest-sealed regression "
        "baseline. Parity is measured over the sealed Codex (X01) and Claude Code "
        "(X02) adapter surfaces against the canonical role set read from "
        "manifests/role_registry.yaml: the role set aligns on both adapters, "
        "every role carries the registry output-schema reference, the host agent "
        "type and Claude surface are uniform, and write-capable roles stay "
        "isolation-tracked; a dropped role, a rebound output schema, an unnamed "
        "host descriptor, a non-uniform surface, and isolation that stops "
        "tracking a write scope are each refused. Provider diversity is a number, "
        "not an assumption: the 2x2 error contingency and phi coefficient are "
        "recomputed from the raw synthetic trials, the observed joint-error rate "
        "exceeds what independence would predict, and a fixture asserting "
        "independence or a provider presented as live is refused; the committed "
        "results artifact is re-derived from the sources and any edit breaks its "
        "own hash. No eval, adapter or fixture acquires evaluator, holdout or "
        "promotion authority, and no live provider is invoked. "
        "provider_parity_eval 8/8, error_correlation_eval 8/8, the full Python "
        "and full Node suites green, and git diff --check all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. X04 measures parity "
        "over sealed adapter surfaces and reports vendor diversity as a "
        "correlation number only: it invokes no live provider, asserts no "
        "statistical independence between providers, does not score, select, "
        "promote or evaluate any candidate, and acquires no evaluator/holdout/"
        "promotion authority. Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X04-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"X04 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"X04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"X04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"X04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected X04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
