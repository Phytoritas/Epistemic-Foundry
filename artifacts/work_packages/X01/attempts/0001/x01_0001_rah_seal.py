#!/usr/bin/env python3
"""Append X01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "X01-0001"
WORK_PACKAGE_ID = "X01"
EXPECTED_PARENT = "000200-7c068e0e"
EXPECTED_PARENT_COUNT = 200
EXPECTED_PARENT_EVIDENCE = "E0202"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "2e9f2529b06efd4e7fd2043f16c3769a71f8ad914a969a461fee4a77d0fe07f5"
)
CORE_EVIDENCE_ID = "E0203"
FINAL_EVIDENCE_ID = "E0204"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward R05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "X01-0001 codex plugin, hooks and subagent adapter "
    "is evidence-sealed PASS (E0203/E0204). R05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "adapters/codex").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X01-0001 PASS core verification: codex plugin, hooks and subagent adapter, "
        ...
        "session, under exact dependency bindings (G04 G04-0001, N04 N04-0001, T04 T04-0001, W04 W04-0001) "
        "and an S05-0001 regression baseline. the host value lives only in the adapter's declaration JSON and is admitted by filtering the gateway's own host vocabulary for the declared value — zero or multiple matches refuse; per-module tests assert no quoted host literal exists in any product module "
        ...
        "Key review finding: The manifest grants plugins/epistemic-foundry/** but the skill tree is hash-bound by the sealed G05 surface; the adapter treats the payload as read-only reference and the G05 gate re-ran 20/20 after the attempt — the scope narrowing is recorded here as the sealed-surface-preserving reading. "
        ...
        + ". R05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "plugin installation, hook execution or subagent launching; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X01-0001 parent is not the sealed X01 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("X01 preflight requires exactly 200 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("X01 core did not append exactly E0203")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("X01 final did not append exactly E0204")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("X01 core tail is not E0203")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 202:
        raise SystemExit(f"unexpected X01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
