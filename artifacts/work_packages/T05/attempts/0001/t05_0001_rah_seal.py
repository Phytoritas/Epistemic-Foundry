#!/usr/bin/env python3
"""Append T05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "T05-0001"
WORK_PACKAGE_ID = "T05"
EXPECTED_PARENT = "000190-71998414"
EXPECTED_PARENT_COUNT = 190
EXPECTED_PARENT_EVIDENCE = "E0192"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "35ffb9460a213a32e53ac8f503c9f2d2279a8d41297f7f78512275102c03d74f"
)
CORE_EVIDENCE_ID = "E0193"
FINAL_EVIDENCE_ID = "E0194"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_t05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward D06-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "T05-0001 evolution cli/mcp tools, sandbox executors and shinka backend adapter "
    "is evidence-sealed PASS (E0193/E0194). D06-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required T05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "src/epistemic_foundry/adapters/v4_t05").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "T05-0001 PASS core verification: evolution cli/mcp tools, sandbox executors and shinka backend adapter, "
        ...
        "session, under exact dependency bindings (G05 G05-0001, S05 S05-0001, T04 T04-0001) "
        "and an S05-0001 regression baseline. backend manifests and qualifications validate against their canonical schemas, the verdict vocabulary is read positionally, pipeline stages come from the reconciliation module, and the adapter's own 749-value scan proves zero held enum values "
        ...
        "Key review finding: One own EF4-I22 violation (accepted) found and renamed by the author before reporting. "
        ...
        + ". D06-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"T05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "running ShinkaEvolve: no build was fetched or exercised; capability tests are recorded claims the gate cross-checks, not behavioural evidence; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("T05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T05-0001 parent is not the sealed T05 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("T05 preflight requires exactly 190 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("T05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("T05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("T05 core did not append exactly E0193")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("T05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("T05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("T05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored T05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("T05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("T05 final did not append exactly E0194")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("T05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("T05 core tail is not E0193")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 192:
        raise SystemExit(f"unexpected T05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with T05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored T05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
