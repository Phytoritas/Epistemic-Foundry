#!/usr/bin/env python3
"""Prove the Y05 operations surface is discoverable by packaging.

``src/epistemic_foundry/operations`` is a new package marker this work package
adds, and ``operations.v4_y05`` is the surface under it, so there is a scope
decision to prove rather than assume: a marker that never reached the wheel would
let the surface import from a checkout and vanish from an installed distribution.
The discovery mode is read from pyproject instead of assumed, so a change there
fails here.
"""
...
EXPECTED = ("epistemic_foundry.operations", "epistemic_foundry.operations.v4_y05")
