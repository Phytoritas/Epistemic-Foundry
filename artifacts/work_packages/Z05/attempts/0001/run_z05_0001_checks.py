#!/usr/bin/env python3
"""Run the Z05-0001 zero-trust release composition checks.

Required checks from manifests/development_manifest.yaml (Z05):
...
``review.md`` outside this runner.  The surface ships under ``release/v4_z05`` —
...
Z05 is a composition gate: it composes sealed surfaces — the Z04 final release
reconciliation (read from its frozen sealed report), the Z02/B05 release-provenance
surface (signing status derived fail-closed UNSIGNED), the 288-lens audit, the S05
security controls, the T05 tool/sandbox surface and the Y05 operations gate — into
one zero-trust release verdict that refuses to present the release as complete or
production-ready.  Two protective dependency regressions run composed dependency
attempt suites directly, so a break in a composed surface fails here rather than
only in its own package: ``dependency-regression-security`` runs the sealed S05
attempt suite and ``dependency-regression-operations`` runs the sealed Y05 attempt
suite.  (The Z04 final gate is repo-state-dependent by design — it fails while
Z05/Z06 are open — so Z04 is composed here through its frozen sealed report rather
than by re-running its live gate.)  ``wire-literal-discipline`` re-runs the
wire-literal discipline suite (the release module reads its status token
positionally from schema, derives the signing status through the sealed provenance
surface, reads the maturity floor from the acceptance matrix and grounds the
promotion authority in the imported capability, holding no canonical enum literal).
...
S05_ATTEMPT_DIR = "artifacts/work_packages/S05/attempts/0001"
...
def targeted() -> int:
    return _pytest("targeted-z05-release", ATTEMPT_DIR)
...
def dependency_regression_security() -> int:
    return _pytest("dependency-regression-security", S05_ATTEMPT_DIR)
...
def main() -> int:
    ...
    checks = {
        ...
        "targeted": targeted,
        "dependency-regression-security": dependency_regression_security,
