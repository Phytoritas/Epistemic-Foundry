#!/usr/bin/env python3
"""Append Z05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z05-0001"
WORK_PACKAGE_ID = "Z05"
EXPECTED_PARENT = "000362-d6656b6d"
EXPECTED_PARENT_COUNT = 362
EXPECTED_PARENT_EVIDENCE = "E0364"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "8c7a06ca17dd0505d54da14895f1095ed96fe7c649e9f15d0a0abb31d8b5cac7"
)
CORE_EVIDENCE_ID = "E0365"
FINAL_EVIDENCE_ID = "E0366"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward Z06-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Z05-0001 zero-trust v4 release, 288-lens audit, migration and signing provenance "
    "is evidence-sealed PASS (E0365/E0366). Z06-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "release/v4_z05").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Z05-0001 PASS core verification: zero-trust v4 release, 288-lens audit, migration and signing provenance, "
        ...
        "session, under exact dependency bindings (B05 B05-0001, S05 S05-0001, T05 T05-0001, Y05 Y05-0001, Z04 Z04-0001) "
        "and an S05-0001 regression baseline. the release verdict is built over the canonical attestation and plugin-release-provenance schemas, the passing status token is read positionally from the attestation enum and the provenance is validated by the sealed release-provenance surface so a reshaped schema fails closed, and the module holds no canonical enum value as a literal "
        "happy-path (unit/contract and real-artifact composition), negative and adversarial (every one of the thirteen FINDING_CODES provoked and its exact code asserted), and crash/resume is degenerate because every gate is a pure, clock-free, entropy-free function whose receipts are byte-identical on re-run "
        "require_no_release_authority_capture refuses any candidate, model, prompt, backend or hook that acquires promotion, holdout or evaluator authority or binds a numeric score into a promotion decision, grounding the promotion capability in the imported registry constant "
        "Key review finding: The honesty/maturity floor is load-bearing: the release hard-refuses completion_ready=true or production_ready=true and fixes release_level to the acceptance-matrix floor SPEC_BUNDLE, which cannot be overridden to present the release as production-ready. "
        "Repository gates at seal time: EF4-I22 discipline 5/5, structure "
        "and boundaries PASS, full Python 1312/1312, full Node 1740/1740 "
        "across the unified 140-file inventory, git diff --check clean. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Z06-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "that the v4 plugin or release is executable, validated or production-ready: the release stays an UNVERIFIED reference bundle and completion_ready stays false; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z05-0001 parent is not the sealed Z05 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z05 preflight requires exactly 362 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z05 core did not append exactly E0365")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z05 final did not append exactly E0366")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z05 core tail is not E0365")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 364:
        raise SystemExit(f"unexpected Z05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
