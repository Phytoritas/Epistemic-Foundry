#!/usr/bin/env python3
"""Build and verify Z05-0001 evidence: Zero-trust v4 release, 288-lens audit, migration and signing provenance.
...
ATTEMPT = ROOT / "artifacts/work_packages/Z05/attempts/0001"
ATTEMPT_ID = "Z05-0001"
WORK_PACKAGE_ID = "Z05"
RECORDED_AT = "2026-08-02T20:45:00.000Z"
ATTEMPT_DIR = "artifacts/work_packages/Z05/attempts/0001"
AUTHORITY_DECISION = None

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/Z05/attempts/0001/fixtures.py": "4abef475a879a55a3c25305fe4120f888e52b932837a36f5c8a956c8fe768974",
    "artifacts/work_packages/Z05/attempts/0001/pytest.ini": "eb32a7f919b938caca0021ebff523b384a83c77ea15610e39e1f5d00cde367e3",
    "artifacts/work_packages/Z05/attempts/0001/run_z05_0001_checks.py": "27844e8a1db81280d18ef3ebbf9afb105e2eb3f4f8d475e6bde05662dfdd3286",
    "artifacts/work_packages/Z05/attempts/0001/test_negative_adversarial.py": "4dee3b9a9db5f5a0a8e4c910b929f7f11a2388b9dbf9c6b315552b41062ed543",
    "artifacts/work_packages/Z05/attempts/0001/test_provenance_receipts.py": "cae043c98b912ac179fa65c941f46a9b2593a44a2ac8ce7f45cc43a60acd7b97",
    "artifacts/work_packages/Z05/attempts/0001/test_schema_and_type.py": "b222f72c657596209ffc05b4677598fbae67fa853147c691cfeca0c9cd274af7",
    "artifacts/work_packages/Z05/attempts/0001/test_unit_contract.py": "305c0f68e7ac2ea00b6822bed000f254e13a41023940be2516648672777a78be",
    "release/v4_z05/__init__.py": "42e1f7be4124a5995cc94cf299ed1dad46ee57256d567a2e411039485b3d13e0",
    "release/v4_z05/zero_trust_release.py": "4ba735f82d3330b54de9400cb69745bd96876404a224009acc202cca12bc0612",
}
EXPECTED_DEPENDENCY_HASHES = {
    ...
    "artifacts/work_packages/Y05/attempts/0001/report.json": "8809e6f104787a1babc293be916e274e65c670618e01d477726720e18f401956",
    "artifacts/work_packages/Z04/attempts/0001/report.json": "45be43f74a96c4dd246d71f206f5cacff6901cc7aa2e8b6167779ed54248e381",
}

JUNIT_PATHS = {
    "dependency_regression_operations": ATTEMPT
    / "dependency-regression-operations.junit.xml",
    "dependency_regression_security": ATTEMPT
    / "dependency-regression-security.junit.xml",
    ...
    "targeted_z05_release": ATTEMPT / "targeted-z05-release.junit.xml",
...
RUN_RESULTS = (
    ...
    "dependency-regression-security",
    "full-node-suite",
    "full-python-suite",
    "git-diff-check",
...
OUTPUT_NAMES = (
    ...
    "pytest.ini",
    "review.md",
    "run_z05_0001_checks.py",
    "schema-and-type-check.junit.xml",
    "targeted-z05-release.junit.xml",
    ...
    "z05-verification.json",
    "z05_0001_rah_seal.py",
)
...
EXIT_CRITERIA = {
    "all_completion_and_external_effects_resolve_to_immutable_receipts": "every gate emits a content-addressed receipt (receipt_id is a prefix plus sha256 of the body, receipt_hash via hash_excluding) and the sealed ZTR- verdict binds every sub-receipt by hash so it cannot be forged without reproducing each composed gate",
    "governing_schemas_authority_boundaries_failure_states_exact": "the release verdict is built over the canonical attestation and plugin-release-provenance schemas, the passing status token is read positionally from the attestation enum and the provenance is validated by the sealed release-provenance surface so a reshaped schema fails closed, and the module holds no canonical enum value as a literal",
    "happy_negative_crash_resume_adversarial_coverage": "happy-path (unit/contract and real-artifact composition), negative and adversarial (every one of the thirteen FINDING_CODES provoked and its exact code asserted), and crash/resume is degenerate because every gate is a pure, clock-free, entropy-free function whose receipts are byte-identical on re-run",
    "no_candidate_model_prompt_backend_or_hook_acquires_authority": "require_no_release_authority_capture refuses any candidate, model, prompt, backend or hook that acquires promotion, holdout or evaluator authority or binds a numeric score into a promotion decision, grounding the promotion capability in the imported registry constant",
}
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        ...
        (
            "dependency_regression_security",
            pytest_summary(JUNIT_PATHS["dependency_regression_security"]),
            62,
        ),
        ...
        (
            "targeted_z05_release",
            pytest_summary(JUNIT_PATHS["targeted_z05_release"]),
            50,
        ),
        (
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        ...
        "dependencies": {
            ...
            "Y05": _sealed_dependency("Y05", "Y05-0001", "E0353", "E0354"),
            "Z04": _sealed_dependency("Z04", "Z04-0001", "E0363", "E0364"),
        },
        "next_action": "SEAL_Z05_0001_THEN_CONTINUE_DAG",
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "declaring_sources": {
            "attestation_status": (
                "schemas/attestation.schema.json overall_status enum (the PASS token is read positionally, never held)"
            ),
            "maturity_floor": (
                "manifests/acceptance_matrix.yaml status_of_this_bundle (SPEC_BUNDLE floor, read not restated)"
            ),
            "promotion_capability": (
                "epistemic_foundry.governance.evolution_authority.registry.PROMOTION_COMMIT_CAPABILITY (imported, never a literal)"
            ),
            "signing_provenance": (
                "schemas/plugin-release-provenance.schema.json, validated by the composed epistemic_foundry.release.provenance surface"
            ),
        },
        "exit_criteria": {
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_z05_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "Z05_ZERO_TRUST_V4_RELEASE_288_LENS_AUDIT_GATE",
        ...
        "next_package": "Z06-0001",
        "not_claimed": [
            "that the v4 plugin or release is executable, validated or production-ready: the release stays an UNVERIFIED reference bundle and completion_ready stays false",
            "that the release is signed: signing stays fail-closed UNSIGNED and signing identity is modeled as unavailable",
            "a real 288-lens re-execution: the 288-lens audit result is composed by reading the frozen sealed audit artifact, not re-run",
...
def build() -> dict[str, Any]:
    ...
    write_json("z05-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("Z05-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "z05-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored Z05-0001 report is not the deterministic document")
    return _summary()
