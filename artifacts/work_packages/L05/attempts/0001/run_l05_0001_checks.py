#!/usr/bin/env python3
"""Run the L05-0001 evolution memory retention acceptance checks.

Required checks from manifests/development_manifest.yaml (L05):
...
report.  The engine ships under src/epistemic_foundry/memory/v4_l05, so
...
The D05 store regression drives a real PostgreSQL container; mock-only
substitution is forbidden by that suite itself.
"""
...
ATTEMPT_ID = "L05-0001"
COMPONENT = "artifacts/work_packages/L05/attempts/0001"
ENGINE = "src/epistemic_foundry/memory/v4_l05"
# L04's recall evaluations are the Node face of the memory surface, D05 is the
# store the lineage and archive actually persist to, and the Python regression
# covers the composed memory-policy, capsule and archive modules.
DEPENDENCY_TARGETS = {
    ...
    "dependency-regression-recall": ("tests/evals/recall",),
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/memory already ships, so no new marker and no
...
def targeted() -> int:
    return _pytest("targeted-l05-retention", COMPONENT)


def dependency_regression_memory() -> int:
    return _pytest(
        "dependency-regression-memory",
        *DEPENDENCY_TARGETS["dependency-regression-memory"],
    )
...
def dependency_regression_recall() -> int:
    name = "dependency-regression-recall"
    files = sorted(
    ...
    if not files:
        ...
        print("the recall gate declares no Node test file", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    checks = {
        ...
        "dependency-regression-recall": dependency_regression_recall,
