#!/usr/bin/env python3
"""Append L05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "L05-0001"
WORK_PACKAGE_ID = "L05"
EXPECTED_PARENT = "000168-4c9cd4de"
EXPECTED_PARENT_COUNT = 168
EXPECTED_PARENT_EVIDENCE = "E0170"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "d01a0c7b9724ec0f40f74f8b4d06b47c565723efd12ea6b4025a38fdc697704d"
)
CORE_EVIDENCE_ID = "E0171"
FINAL_EVIDENCE_ID = "E0172"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_l05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the M05 epistemic niche mapper, lineage map and evolution "
    "blast-radius cartography on the sealed M04/D05 state.",
    "Continue the x05 wave toward S05 and the U01/V01/X01 integration layer.",
...
GATE_NOTE = (
    "L05-0001 lineage memory, negative-result retention and evolution "
    "forget/export policies is evidence-sealed PASS (E0171/E0172). M05-0001 "
    "is next; implementation_gate=fail and completion_ready=false."
)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required L05-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: The engine ships outside this attempt directory, so the seal binds
    #: the product bytes by repository path rather than by artifact name.
    result: dict[str, str] = {}
    for relative in sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/memory/v4_l05").rglob("*")
        if path.is_file() and "__pycache__" not in path.parts
    ):
        result[relative] = sha256(ROOT / relative)
    if len(result) != 2:
        raise SystemExit(f"unexpected L05 product file set: {sorted(result)}")
    return result
...
def core_summary() -> str:
    ...
    return (
        "L05-0001 PASS core verification: the evolution memory retention "
        "engine is implemented under src/epistemic_foundry/memory/v4_l05, "
        "under exact L04-0001 and D05-0001 dependency bindings and a "
        "G05-0001 regression baseline, entirely inside the manifest write "
        "grant. The engine decides what the archive and the recall policy "
        "never did: what survives an actual forget and what an export may "
        "carry out. Capacity pressure can never erase any of the five "
        "protected classes; an external obligation reduces them at most to "
        "a tombstone keeping the class, reason, hash, lineage id and "
        "generation; erasure eligibility iterates to fixpoint so no "
        "surviving record is ever stranded without its ancestry; and an "
        "export that keeps results while dropping negative knowledge is "
        "refused as survivorship bias. Two real defects were found and "
        "fixed during this attempt with regression proof: a bound-method "
        "identity comparison that made the lineage cycle check "
        "unreachable, and erasure eligibility computed against the "
        "requested set rather than the erased set. Ten failure classes "
        "are refused by name. Nothing is restated: entry classes come "
        "from the canonical schema with the archive partition verified on "
        "every use, and export scope is delegated to "
        "require_recall_permitted. The D05 store regression ran against "
        "real PostgreSQL via the pinned container, 84/84. Ruff "
        "lint/format, EF4-I22 wire-literal discipline 5/5, packaging "
        "discovery, schema_and_type_check 11/11, unit_and_contract 20/20, "
        "negative_and_adversarial 29/29, provenance_and_receipt 12/12, "
        "targeted 72/72, memory regression 61/61, recall regression "
        "25/25, full Python 1261/1261, full Node 1063/1063 across 95 "
        ...
        "M05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"L05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". G05-0001 and every prior RAH generation remain immutable. The "
        "plan decides what may be erased; executing the deletion against "
        "the D05 store and emitting its effect receipt belong to the "
        "runtime that owns the transaction. External-sync dispositions "
        "are recorded in the manifest rather than interpreted, because "
        "their vocabulary belongs to the memory policy owner. The "
        "composed archive, policy and capsule modules are read-only "
        "inputs and were not modified. M05, S05, Q04, D06, E06, U01, V01, "
        "X01, source coverage, PRD, review, and global closeout remain; "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("L05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("L05-0001 parent is not the sealed G05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("L05 preflight requires exactly 168 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("L05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("L05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("L05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("L05 core did not append exactly E0171")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("L05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("L05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("L05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored L05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("L05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("L05 final did not append exactly E0172")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("L05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("L05 core tail is not E0171")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 170:
        raise SystemExit(f"unexpected L05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with L05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored L05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
