#!/usr/bin/env python3
"""Prove the X06 provider-diversity gate is discoverable by packaging.

``src/epistemic_foundry/providers`` is an existing package marker (neutrality,
untrusted and v4_x05 ship under it) and ``providers.v4_x06`` is the new marker
this package adds, so there is a scope decision to prove rather than assume: a
marker that never reached the wheel would let the surface import from a checkout
and vanish from an installed distribution. The discovery mode is read from
pyproject instead of assumed, so a change there fails here.
"""
...
EXPECTED = ("epistemic_foundry.providers", "epistemic_foundry.providers.v4_x06")
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the surface would import from a checkout but be absent "
                        "from the wheel"
                    ),
                    "status": "FAIL",
                },
