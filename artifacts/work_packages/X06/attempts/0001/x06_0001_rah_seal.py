#!/usr/bin/env python3
"""Append X06-0001 PASS evidence while global completion remains false.
...
generation count, the parent's latest evidence ID, or the two evidence IDs this
seal will mint the way a live-ledger seal does.  All six pins are therefore left
as the sentinel ``FILL_FROM_LEDGER`` (``EXPECTED_PARENT_COUNT`` as ``0``) and the
seal refuses to run until the parent fills them from the live ledger tail — the
generation committed by X06's parent final closeout, whichever package most
recently sealed on the frontier.  Everything else — summaries, hashes,
flat-snapshot verification — is derived exactly as the live-ledger seal does.
"""
...
#: The generation X06's parent final closeout committed, and the evidence and
#: manifest that identify it.  Fill all six from `.rah/ralph` before running:
#: the parent generation ("000NNN-XXXXXXXX"), its generation count, its latest
#: evidence ID, its manifest sha256, and the two evidence IDs this seal mints.
EXPECTED_PARENT = "000266-af53986a"
EXPECTED_PARENT_COUNT = 266
EXPECTED_PARENT_EVIDENCE = "E0268"
EXPECTED_PARENT_MANIFEST_SHA256 = "62d63582eae5114fb9cc940da6a22afce569925a3a61d94ad39895e6be78cc09"
CORE_EVIDENCE_ID = "E0269"
FINAL_EVIDENCE_ID = "E0270"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after X06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} provider diversity, cost, safety and reward-attribution "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "X06-0001 seal is not pinned: set "
            + ", ".join(sorted(unfilled))
            + " from the live ledger tail (the generation committed by X06's "
            "parent final closeout) before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/providers/v4_x06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X06-0001 PASS core verification: provider diversity, cost, safety and "
        "reward-attribution integration gate, implemented in "
        "src/epistemic_foundry/providers/v4_x06 by a bounded implementation agent "
        "and reviewed by an independent seal-prep session (a distinct actor from "
        "the author) under the product owner's explicit parallel-agent "
        "instruction, under an exact dependency binding (X05 X05-0001). The gate "
        "composes the sealed X05 cross-provider routing, safe delayed-reward "
        "bandit, neutral fallback and external-backend boundary, attests a routed "
        "set is diverse and cost-accounted, attributes a reward only through the "
        "safe bandit from a validated basis (a promotion-routed reward is "
        "refused), keeps every fallback provider-neutral and every backend "
        "advisory, and binds the sealed sub-decisions into one re-derivable "
        "integration receipt a tampered sub-decision cannot be laundered into. "
        "Cost is descriptive bookkeeping, never a threshold authority, and the "
        "gate restates none of the sealed vocabularies (EF4-I22): every canonical "
        "token is read positionally from the schema that declares it or delegated "
        "to the module that owns it. Every decision re-derives byte for byte from "
        "its own content with no clock or random draw, and nothing scores, "
        "selects, promotes or evaluates. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, packaging-discovery, the X05 dependency "
        ...
        + ". Review is by an independent seal-prep session with "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. X06 attests "
        "provider diversity and cost, attributes a safe reward and binds one "
        "re-derivable integration receipt only: it does not score, select, "
        "promote or evaluate any candidate, makes no DSSAT or plant-model parity "
        "claim, and promotion remains a governance decision. Author/reviewer "
        "separation holds (a bounded implementation agent authored, an "
        "independent seal-prep session reviewed as a distinct actor) while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X06-0001 parent is not the sealed frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"X06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"X06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"X06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"X06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected X06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
