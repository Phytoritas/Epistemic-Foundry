#!/usr/bin/env python3
"""Append Z04-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z04-0001"
WORK_PACKAGE_ID = "Z04"
EXPECTED_PARENT = "000360-307b5f75"
EXPECTED_PARENT_COUNT = 360
EXPECTED_PARENT_EVIDENCE = "E0362"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "a64954508dd2216cf72937f8335270979d7d413059f186760a82b6a8c8043484"
)
CORE_EVIDENCE_ID = "E0363"
FINAL_EVIDENCE_ID = "E0364"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward Z05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Z04-0001 final independent release gate and architecture freeze "
    "is evidence-sealed PASS (E0363/E0364). Z05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "reports/release").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Z04-0001 PASS core verification: final independent release gate and architecture freeze, "
        ...
        "session, under exact dependency bindings (Z02 Z02-0001, Z03 Z03-0001) "
        "and an S05-0001 regression baseline. three content-addressed record schemas (z04-release-reconciliation/v1, z04-independent-attestation/v1, z04-manifest-hash-reconciliation/v1) are each sealed with a canonical-JSON record_sha256 and every failure state is a typed refusal whose reason exceeds fifty characters "
        "the release gate iterates all 156 manifest packages against the ledger seal predicate plus a Kahn topological check of every depends_on edge, the attestation covers every completion-ready source and every acceptance conditional, and the manifest-hash check covers all four canonical manifests plus the compatibility-matrix pin "
        "reconciliation reads only canonical files and the ledger read-only, derives all counts rather than restating them, promotes nothing, and refuses fail-closed via typed codes; completion_ready stays false and no release label claiming production readiness is accepted "
        "Key review finding: Exact count reconciliation: expected 156 = sealed(153) + remaining(3), counts balance, remaining = {Z04,Z05,Z06} each owned with a >50-char reason, and ledger_orphans / unaccounted / orphan_owners / remaining_unowned are all empty; the sealed set is derived from the ledger (not hardcoded) and dropping an owner flips the gate to FAIL. "
        ...
        + ". Z05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "that Z05 or Z06 are complete: the gate passes because they are named and owned remaining items, not because they are done; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z04-0001 parent is not the sealed Z04 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z04 preflight requires exactly 360 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z04 core did not append exactly E0363")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z04 final did not append exactly E0364")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z04 core tail is not E0363")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 362:
        raise SystemExit(f"unexpected Z04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
