#!/usr/bin/env python3
"""Prove the I03 ontology-resolver component's packaging posture is intentional.

Unlike the ``src/`` gates that must reach the built wheel, I03 ships under the
...
3. the component imports exactly as its own test modules consume it -- the
   ``ontology`` package marker exists while no intermediate parent carries an
   ``__init__.py``, so pytest's prepend import mode roots the package at
   ``ontology`` with ``epistemic_foundry/`` on ``sys.path`` and the
   ``from .resolver`` relative import resolves; the import is exercised for real
   and the public entrypoint ``resolve_construct`` must resolve
...
COMPONENT_REL = "python/epistemic_foundry/ontology"
IMPORT_ROOT = ROOT / "python/epistemic_foundry"
PACKAGE = "ontology"
#: Parents that must stay marker-free so the component's import root is
#: ``epistemic_foundry/`` and the package is exactly ``ontology`` (as pytest
#: resolves it).
MARKER_FREE_PARENTS = (
    "python",
...
def main() -> int:
    ...
    if not (component / "__init__.py").is_file():
        return _fail({"code": "COMPONENT_PACKAGING_DRIFT",
                      "note": "ontology package marker missing"})
    # It must not have leaked into the shipped src tree.
    if (ROOT / "src/epistemic_foundry/ontology").exists():
        return _fail({"code": "COMPONENT_PACKAGING_DRIFT",
                      "note": "component must not be shipped under src"})
    # Intermediate parents must stay marker-free so the import root is
    # epistemic_foundry/.
    leaked = sorted(
    ...
    if not callable(getattr(module, "resolve_construct", None)):
        return _fail({"code": "COMPONENT_NOT_IMPORTABLE",
                      "note": "resolve_construct is not exported"})
