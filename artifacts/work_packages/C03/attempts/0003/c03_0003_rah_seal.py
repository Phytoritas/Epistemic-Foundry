#!/usr/bin/env python3
"""Append C03-0003 PASS evidence while the global repair goal stays active."""
...
import build_c03_0003_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Start F04-0002 and reconcile the approved fixture paths before J02-0003.",
    "Run J02-0003 only after F04-0002 is evidence-sealed PASS, then run S04-0003.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C03-0003 artifact is missing: {name}")
...
def core_summary() -> str:
    names = (
        ...
        "review.md",
        "build_c03_0003_evidence.py",
        "c03_0003_rah_seal.py",
    )
    hashes = artifact_hashes(names)
    return (
        "C03-0003 PASS under HD-EF4-B04-SG002-20260730-001: canonical "
        "GateDecision, EvaluatorBundle, and HoldoutManifest runtime migration "
        "passes deterministic hash, replay identity, evaluator/holdout binding, "
        "default-deny access, leakage, forged-hash, and defensive-copy checks. "
        "Targeted runtime migration is 174/174. All 51 prior GateDecision drift "
        "nodes and 15 prior HoldoutManifest drift nodes from B04-0006 are present "
        "and passing. Full Python is explicitly not green: 986 pass and exactly "
        "one pre-existing J02 exact-tiktoken dependency failure remains, with "
        "C03-caused failures zero. Verification sha256:"
        f"{hashes['c03-runtime-migration-verification.json']}; regression sha256:"
        f"{hashes['full-regression-impact.json']}; targeted JUnit sha256:"
        ...
        f"builder sha256:{hashes['build_c03_0003_evidence.py']}; sealer sha256:"
        f"{hashes['c03_0003_rah_seal.py']}. Review is primary-session separate "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C03-0003 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"{hashes['build_c03_0003_evidence.py']}; sealer sha256:"
        f"{hashes['c03_0003_rah_seal.py']}. C03-0003 is PASS and F04-0002 is "
        "dependency-ready; all prior attempt and RAH history remains preserved, "
        "the repository-wide implementation gate remains failed because J02 and "
        "later ordered gates remain, and completion_ready=false."
    )
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C03-0003 runtime migration is evidence-sealed PASS; F04-0002 is next. "
        "The repository-wide implementation gate remains failed and "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("C03-0003 preflight requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("C03-0003 report is already RAH-bound; use verify")
    generations = numbered_generations(ralph_root)
    if generations[-1] != generation:
        raise SystemExit("C03-0003 preflight generation pointer mismatch")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C03-0003 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match the C03-0003 summary")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("C03-0003 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("C03-0003 report evidence IDs do not match the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("C03-0003 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C03-0003 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match the C03-0003 closeout seal")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("C03-0003 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("sealed ledger does not end with the C03 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("sealed C03 final generation does not follow the core generation")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored C03 final evidence differs from closeout artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
