#!/usr/bin/env python3
"""Prove the I06 intake gate is discoverable by the packaging config.

``src/epistemic_foundry/intake/v4_i06`` is a new package marker authorized by
the I06 write scope in ``manifests/development_manifest.yaml``; the ``intake``
parent marker was authorized earlier by I05. Both must reach the wheel, so there
is a scope decision to prove rather than assume: a marker that never reached the
distribution would let the gate import from a checkout and vanish once
installed. The discovery mode is read from pyproject instead of assumed, so a
...
EXPECTED = ("epistemic_foundry.intake", "epistemic_foundry.intake.v4_i06")
