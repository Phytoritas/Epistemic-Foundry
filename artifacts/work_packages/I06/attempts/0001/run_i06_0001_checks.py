#!/usr/bin/env python3
"""Run the I06-0001 genome intake integration-gate acceptance checks.

Required checks from manifests/development_manifest.yaml (I06):
...
gate ships under src/epistemic_foundry/intake/v4_i06 behind a new ``v4_i06``
package marker (the ``intake`` parent marker pre-exists from I05), so
``packaging-discovery`` proves the marker actually reaches the wheel rather than
only importing from the checkout.

The dependency regressions cover the two sealed surfaces this package composes:
I05's intake screen, whose eligibility judgement the gate defers to entirely and
never re-derives, and R05's sealed operator registry, which shares the same
intake surface and canonical genome/scope/falsifier/prediction schemas the gate
resolves references across.  I05 has no importers under ``tests/``, so its
regression runs the sealed I05 attempt suite directly; R05's regression runs the
sealed R05 attempt suite.  A regression in either fails here rather than only
...
#: The write-scope component: v4_i06 only.  The ``intake`` parent marker
#: pre-exists from I05 and is out of this package's write scope, so lint and the
#: write-scope survey stay pinned to v4_i06 rather than the whole intake tree.
V4_I06 = "src/epistemic_foundry/intake/v4_i06"
PRODUCT_TESTS = {
...
# I05 owns the intake screen the gate composes and defers eligibility to; its
# sealed suite lives in the I05 attempt directory (no tests/ importer exists).
# R05 owns the sealed operator registry that shares the same intake surface and
# canonical schemas; its sealed suite lives in the R05 attempt directory.  A
# change in either must fail here, so each runs its own sealed suite.
DEPENDENCY_TARGETS = {
...
def ruff_check() -> int:
    return run(
        "ruff-check", ["uv", "run", "--locked", "ruff", "check", V4_I06, ATTEMPT_DIR]
    )
...
def ruff_format_check() -> int:
    return run(
        "ruff-format-check",
        ["uv", "run", "--locked", "ruff", "format", "--check", V4_I06, ATTEMPT_DIR],
    )


def packaging_discovery() -> int:
    # src/epistemic_foundry/intake/v4_i06 is a new package marker, so the wheel
...
def targeted() -> int:
    return _pytest("targeted-i06-gate", ATTEMPT_DIR)
