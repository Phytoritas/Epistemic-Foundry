#!/usr/bin/env python3
"""Append X03-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed R05/X01 templates do.
"""
...
#: The current committed generation (e.g. "000221-94e507f2"); its retained
...
EXPECTED_PARENT = "000224-360c3732"
EXPECTED_PARENT_COUNT = 224
EXPECTED_PARENT_EVIDENCE = "E0226"
EXPECTED_PARENT_MANIFEST_SHA256 = "7ae5df04c11a2e27eb0032dad2ebf6308cbe3f70d44663d6f47ddc9d4cd37659"
CORE_EVIDENCE_ID = "E0227"
FINAL_EVIDENCE_ID = "E0228"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after X03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} model routing and fallback policy is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "X03-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "packages/role-router/src/routing").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X03-0001 PASS core verification: model routing and fallback policy, "
        "implemented in packages/role-router/src/routing by a bounded "
        ...
        "under an exact X01-0001 dependency binding and an R06-0001 live "
        "latest-sealed regression baseline. Each task class routes over a "
        "declared, hash-bound route order whose selection is driven by the "
        "declared reward basis and per-route estimated cost/latency; every "
        "emitted ModelRoutingReceipt re-derives its own content hash and the "
        "policy/reward_basis vocabularies are read from the canonical "
        "model-routing-receipt schema rather than restated. Fallback is "
        "recorded and policy-approved: the route order terminates in exactly "
        "one safe default that can never be declared unavailable, skipped "
        "candidates form an ordered fallback_chain with a policy-approved RFB "
        "decision id, and distinct fallback depths yield distinct decision ids. "
        "No route acquires evaluator, holdout or promotion authority; an "
        "authority-aliasing task class is refused, an undeclared task class is "
        "refused, an unknown route candidate is refused, and floating or "
        "tampered references are refused. routing_policy_test 14/14, "
        "fallback_provenance_test 10/10, the full Node suite green with the two "
        "X03 routing modules inside the inventory, and git diff --check all "
        "pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. X03 plans and "
        "records replayable model routing and fallback provenance only: it does "
        "not score, select, promote or evaluate any candidate, acquires no "
        "evaluator/holdout/promotion authority, and makes no provider-parity or "
        "cross-vendor independence claim (X04 scope). Author/reviewer separation "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X03-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"X03 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"X03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"X03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"X03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected X03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
