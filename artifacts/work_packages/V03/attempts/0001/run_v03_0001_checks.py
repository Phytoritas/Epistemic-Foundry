#!/usr/bin/env python3
"""Run the V03-0001 capability-controlled execution and receipt acceptance checks.

Required checks from manifests/development_manifest.yaml (V03):
``validation_execution_test`` and ``effect_receipt_test``.  Both are answered by
the component suites under ``python/epistemic_foundry/validation/execution``:
capability-controlled execution is covered by ``schema-and-type-check`` and
``unit-and-contract-tests``, receipt resolution by
``negative-and-adversarial-tests`` and ``provenance-and-receipt-audit``, and
``targeted`` runs the whole component so no suite can be green only in isolation.

The component carries its own ``pytest.ini`` and sits outside the repository
``testpaths``, exactly as the sealed V01 component does, so the repository-wide
``pytest tests`` collection is unchanged by this package.  V01 is the sealed
dependency whose ValidationTarget manifests and eligibility vocabulary this
package's execution and receipts read, so its component suite runs as the
...
ATTEMPT_ID = "V03-0001"
COMPONENT = "python/epistemic_foundry/validation/execution"
ATTEMPT_DIR = "artifacts/work_packages/V03/attempts/0001"
# V03 depends only on V01.  V01's component defines the ValidationTarget
# manifests and eligibility surface this package's capability-controlled
# execution and receipts read, so its component suite is the regression that
# would actually break if this package moved that contract.
...
def targeted() -> int:
    return _pytest("targeted-v03-validation-execution", COMPONENT)
