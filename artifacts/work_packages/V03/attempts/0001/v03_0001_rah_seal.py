#!/usr/bin/env python3
"""Append V03-0001 PASS evidence while global completion remains false.
...
ATTEMPT = ROOT / "artifacts/work_packages/V03/attempts/0001"
...
EXPECTED_PARENT = "000236-f1398b98"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 236
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0238"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "997fc22d94038580c28ddb479150f5ba06d191d6a761995c49d847273be623dd"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0239"
FINAL_EVIDENCE_ID = "E0240"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_v03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after V03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} capability-controlled execution and receipts is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "V03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required V03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "python/epistemic_foundry/validation/execution").rglob("*")
...
ATTEMPT_ARTIFACTS = (
    ...
    "run_v03_0001_checks.py",
    "schema-and-type-check.junit.xml",
    "targeted-v03-validation-execution.junit.xml",
    "unit-and-contract-tests.junit.xml",
    "v03_0001_rah_seal.py",
)
...
def core_summary() -> str:
    ...
    return (
        "V03-0001 PASS core verification: capability-controlled execution and "
        "receipts, implemented in "
        "python/epistemic_foundry/validation/execution by a bounded "
        "implementation agent under the parent architect's delegated V03 write "
        "scope and reviewed by an independent reviewer distinct from the author, "
        "under exact dependency bindings (V01 V01-0001) and an R06-0001 "
        "regression baseline. authorize_execution evaluates all ten "
        "authorization criteria without short-circuiting and returns DENIED "
        "unless the exact intent and arguments verify, the sealed lease is "
        "current, every requested capability and write scope is leased, no "
        "fencing token is stale, an approval names each controlled_effect or "
        "high_risk action, the principal is fenceable, the policy hash is "
        "pinned and the run environment is bound, so an unleased capability or "
        "scope is refused rather than executed. seal_run_environment pins the "
        "seed, environment and resource envelope, build_run_capture requires "
        "all four capture channels before a receipt may seal, and "
        "reconcile_effects publishes exact arithmetic that raises an incident "
        "on any unexpected effect regardless of exit status while UNKNOWN keeps "
        "reconciliation open, so every completion and external effect resolves "
        "to an immutable receipt. The EXECUTION_GATE_LADDER keeps FAILED_RUN "
        "distinct from INCIDENT and DENIED so a real negative validation result "
        "reads as evidence, and nothing scores, selects, promotes or evaluates "
        "any candidate. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, whole-component targeted, the V01 "
        "dependency regression, full Python 1261/1261, full Node 1641/1641 "
        "across the unified 132-file inventory, and git diff --check all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"V03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. V03 executes "
        "validation targets only under a capability lease and sandbox and "
        "records every effect as an immutable receipt: it does not score, "
        ...
        "Author/reviewer separation holds with actor independence while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("V03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V03-0001 parent is not the sealed frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"V03 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("V03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("V03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"V03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("V03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("V03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("V03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored V03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("V03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"V03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("V03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"V03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected V03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with V03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored V03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
