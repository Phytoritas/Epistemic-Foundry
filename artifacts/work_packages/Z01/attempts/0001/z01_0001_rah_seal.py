#!/usr/bin/env python3
"""Append Z01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z01-0001"
WORK_PACKAGE_ID = "Z01"
EXPECTED_PARENT = "000354-e7a6229c"
EXPECTED_PARENT_COUNT = 354
EXPECTED_PARENT_EVIDENCE = "E0356"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "6214d1201ce5fee55e3f3067a1a746b255ca3b4c8abaddba304184fd444c3ac9"
)
CORE_EVIDENCE_ID = "E0357"
FINAL_EVIDENCE_ID = "E0358"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward Z02-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Z01-0001 fresh-install, compatibility and uninstall matrix "
    "is evidence-sealed PASS (E0357/E0358). Z02-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "tests/install").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Z01-0001 PASS core verification: fresh-install, compatibility and uninstall matrix, "
        ...
        "session, under exact dependency bindings (G04 G04-0001, H04 H04-0001, L04 L04-0001, M04 M04-0001, P04 P04-0001, Q04 Q04-0001, S04 S04-0005, T04 T04-0001, U04 U04-0001, V04 V04-0001, W04 W04-0001, X04 X04-0001, Y04 Y04-0001) "
        "and an S05-0001 regression baseline. the host and platform lists, payload layout and sealed-host provenance are read only from manifests/compatibility_matrix.yaml and never restated, the matrix is fail-closed (every host cell UNVERIFIED or ADAPTER_REQUIRED and every install decision REFUSED until the recorded fresh-install, lifecycle-hook, MCP, payload, upgrade, rollback and uninstall evidence exists), and compatibility-matrix-lint enforces that closed shape "
        "install_matrix_test proves fresh-install and host/OS compatibility as declared-matrix lifecycle proofs over the in-repo payload and refuses any cell claiming SUPPORTED without recorded evidence, while uninstall_data_test proves the uninstall lifecycle preserves the user-data policy and refuses a plan that would delete retained user data "
        "the harness spawns no host and mutates nothing on disk; the one real single-host marketplace lifecycle is composed from the sealed G04-0001 gate rather than re-implemented, and no matrix cell can be promoted to SUPPORTED from inside this package "
        "Key review finding: The compatibility matrix is a fail-closed UNVERIFIED reference: every host cell is UNVERIFIED or ADAPTER_REQUIRED and every install decision REFUSED until recorded evidence, so no cell presents the v4 plugin as executable, validated or production-ready. "
        "Repository gates at seal time: EF4-I22 discipline 5/5, structure "
        "and boundaries PASS, full Python 1285/1285, full Node 1740/1740 "
        "across the unified 140-file inventory, git diff --check clean. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Z02-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a real multi-OS install: every host/OS cell except the composed G04 single-host lifecycle is a declared-matrix proof over the in-repo payload, not an executed install; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z01-0001 parent is not the sealed Z01 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z01 preflight requires exactly 354 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z01 core did not append exactly E0357")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z01 final did not append exactly E0358")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z01 core tail is not E0357")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 356:
        raise SystemExit(f"unexpected Z01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
