#!/usr/bin/env python3
"""Append X02-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed X01/X03 templates do.
"""
...
EXPECTED_PARENT = "000242-0c5f84ba"
EXPECTED_PARENT_COUNT = 242
EXPECTED_PARENT_EVIDENCE = "E0244"
EXPECTED_PARENT_MANIFEST_SHA256 = "636a87ae25e50136b38aca109725b486fd06e8df7a3bcf7f31a83925d5cc06e5"
CORE_EVIDENCE_ID = "E0245"
FINAL_EVIDENCE_ID = "E0246"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after X02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} Claude Code skills, agents and worktree adapter is "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "X02-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "adapters/claude-code").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X02-0001 PASS core verification: Claude Code skills, agents and worktree "
        "adapter, implemented in adapters/claude-code by a bounded implementation "
        ...
        "(actor-independent from the author), under an exact X01-0001 dependency "
        "binding and a V03-0001 live latest-sealed regression baseline. Each "
        "declared RoleSpec generates exactly one custom agent definition whose "
        "frontmatter and tool grants are derived from manifests/role_registry.yaml "
        "and cross-checked row-for-row against adapters/claude-code/"
        "role_mapping.yaml; the Claude Code host binding is read from "
        "claude-binding.json rather than invented, and every emitted binding and "
        "worktree-plan receipt re-derives its own content hash. Parallel writes "
        "are isolated: each parallel write yields a worktree plan with a disjoint "
        "branch, path and write scope, overlapping scopes are refused, and the "
        "plan never claims isolation it does not prove. No adapter, role or "
        "worktree plan acquires evaluator, holdout or promotion authority; an "
        "undeclared role is refused, an unknown role candidate is refused, and "
        "the adapter launches no custom agent and creates no git worktree. "
        "claude-schema-check 12/12, claude-adapter-test 10/10, "
        "claude-adversarial-tests 21/21, claude-worktree-isolation-test 9/9, "
        "claude-receipts 10/10, the sealed X01 Codex adapter dependency "
        "regression 68/68, the full Node suite green with the five X02 Claude "
        "Code modules inside the inventory, and git diff --check all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. X02 plans and records "
        "replayable role-to-custom-agent bindings and disjoint parallel-write "
        "worktree plans only: it launches no custom agent, creates no git "
        "worktree, acquires no evaluator/holdout/promotion authority, and makes "
        "no model-routing, provider-parity or cross-vendor independence claim "
        "(X03/X04 scope). Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X02-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"X02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"X02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"X02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"X02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected X02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
