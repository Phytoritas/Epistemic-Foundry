#!/usr/bin/env python3
"""Append C05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "C05-0001"
WORK_PACKAGE_ID = "C05"
EXPECTED_PARENT = "000152-e434b604"
EXPECTED_PARENT_COUNT = 152
EXPECTED_PARENT_EVIDENCE = "E0154"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "58c4434f67c22ceaf5d400413a3b6183cd7d1c70fde59d709498ff47c8e3baf2"
)
CORE_EVIDENCE_ID = "E0155"
FINAL_EVIDENCE_ID = "E0156"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_c05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the Q02 parser, Claim and grounding evaluation on the sealed Q01 corpus.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C05-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C05-0001 PASS core verification: the evolution genome, evaluator, "
        "archive, statistics and adapter schema families are implemented as a "
        "generated composition bundle under schemas/v4_c05, under exact "
        "A06-0002 and C04-0004 dependency bindings and a B05-0001 regression "
        "baseline. The 127 sealed canonical schemas are untouched and remain "
        "the single declaring source: every composite is pure $ref structure "
        "with no enum, const, pattern or format of its own, and a composite "
        "edited to smuggle vocabulary is refused as VOCABULARY_SMUGGLED. The "
        "mutable search space is structural: a document is a candidate if and "
        "only if it is one of the four genome kinds, evaluator, holdout, "
        "promotion-shaped, archive and backend documents are refused by the "
        "same oneOf, and a bundle edited to widen or narrow the space is "
        "refused as AUTHORITY_IN_MUTABLE_SPACE or MUTABLE_SPACE_MISMATCH. "
        "EF4-I53 and EF4-I63 validate structurally: adaptive-search evidence "
        "requires its multiplicity, sequential and selective-inference "
        "records, and an imported run requires the pinned backend manifest "
        "and its qualification under the canonical consts. One minimal "
        "instance of every one of the 42 members is synthesized from the "
        "schema itself and validates against both its own schema and its "
        "family composite; the synthesizer refuses constructs it does not "
        "model. family-index.json receipts the generator, all 42 members and "
        "all five composites by content hash and regenerates byte-identically. "
        "Ruff lint/format, schema_and_type_check 26/26, unit_and_contract "
        "71/71, negative_and_adversarial 27/27, provenance_and_receipt 8/8, "
        "targeted 132/132, A06 constitutional regression 40/40, C04 contract "
        "regression 62/62, full Python 1261/1261, full Node 982/982 across 91 "
        ...
        "Q02-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". B05-0001 and every prior RAH generation remain immutable. The "
        "composites bind documents, not runtime behaviour: enforcement of the "
        "mutable-space boundary in the running chamber belongs to E05/F05 and "
        "the kernel; generated types and fixtures belong to C06; the "
        "synthesizer models only the constructs the canonical family uses; "
        "and schemas/v4_c05 is regenerable byte-for-byte from the canonical "
        "sources. Q02, Q03, C06, the remaining x05 wave, U01, V01, X01, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("C05-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C05-0001 evolution genome, evaluator, archive, statistics and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("C05-0001 parent is not the sealed B05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("C05 preflight requires exactly 152 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("C05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("C05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("C05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("C05 core did not append exactly E0155")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("C05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("C05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("C05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored C05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("C05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("C05 final did not append exactly E0156")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("C05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("C05 core tail is not E0155")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 154:
        raise SystemExit(f"unexpected C05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with C05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored C05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
