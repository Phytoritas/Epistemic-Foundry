#!/usr/bin/env python3
"""Run the C05-0001 evolution schema bundle acceptance checks.

Required checks from manifests/development_manifest.yaml (C05):
...
report.  ``schemas/v4_c05`` is emitted by the recorded ``bundle-emit`` step
and re-verified by ``bundle-verify``.
"""
...
ATTEMPT_ID = "C05-0001"
COMPONENT = "artifacts/work_packages/C05/attempts/0001"
# A06's constitutional surface and C04's contract-conformance surface are the
# two dependencies; each regression runs the tests that package itself owns.
DEPENDENCY_TARGETS = {
    "dependency-regression-a06": ("tests/governance/a05",),
...
def bundle_emit() -> int:
    return run(
        ...
        [
            ...
            "python",
            "-B",
            f"{COMPONENT}/c05_schema_bundle.py",
            "emit",
        ],
    )
...
def targeted() -> int:
    return _pytest("targeted-c05-bundle", COMPONENT)


def dependency_regression_a06() -> int:
    return _pytest(
        "dependency-regression-a06", *DEPENDENCY_TARGETS["dependency-regression-a06"]
    )


def dependency_regression_c04() -> int:
    return _pytest(
        "dependency-regression-c04", *DEPENDENCY_TARGETS["dependency-regression-c04"]
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "bundle-emit": bundle_emit,
        "bundle-verify": bundle_verify,
        ...
        "targeted": targeted,
        "dependency-regression-a06": dependency_regression_a06,
        "dependency-regression-c04": dependency_regression_c04,
