#!/usr/bin/env python3
"""Append X05-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was authored as a seal-prep
hand that was instructed not to touch ``.rah/``, so it cannot read the live
ledger to pin the parent generation identifier, its manifest hash, the parent
count, the parent evidence id, or this attempt's own core/final evidence ids.
All SIX of those pins are therefore left as sentinels and ``assert_parent_pins``
refuses to run until the parent fills every one of them from the live ledger
tail (the generation committed by the current latest-sealed final closeout).
Everything else — summaries, hashes, flat-snapshot verification — is derived
exactly as the templates do.

Prep-time observation (verify against the live ledger before filling; the
frontier may have moved).  At prep the ledger tail was ``E0260`` (the U04-0001
final closeout), the current generation was ``000258-c2f1262c`` and there were
258 preserved generations.  If X05-0001 is the next seal on that unchanged
frontier, the parent would fill EXPECTED_PARENT=``000258-c2f1262c`` (after
reconfirming its manifest hash), EXPECTED_PARENT_COUNT=258,
EXPECTED_PARENT_EVIDENCE=``E0260``, CORE_EVIDENCE_ID=``E0261`` and
FINAL_EVIDENCE_ID=``E0262``.  These are guidance only — reconfirm each against
``.rah/ralph`` before running.
"""
...
#: Sentinels for the pins that can only be read from the live ledger tail.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
PARENT_COUNT_SENTINEL = -1
#: All six seal pins are left as sentinels for the parent (see the authoring
#: note); the seal refuses until every one is filled from the live ledger tail.
EXPECTED_PARENT = "000262-226696fe"
EXPECTED_PARENT_COUNT = 262
EXPECTED_PARENT_EVIDENCE = "E0264"
EXPECTED_PARENT_MANIFEST_SHA256 = "f61f3a79cf00a243e9575dd77ac80760364ea4f113977fb38017fccbe11db7e2"
CORE_EVIDENCE_ID = "E0265"
FINAL_EVIDENCE_ID = "E0266"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_x05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after X05-0001 seals and select the next "
...
#: is built from the evidence-id pins so it stays consistent whether they are
#: sentinels or filled.
GATE_NOTE = (
    f"{ATTEMPT_ID} cross-provider mutation routing, safe delayed-reward bandit "
    f"and fallback is evidence-sealed PASS ({CORE_EVIDENCE_ID}/{FINAL_EVIDENCE_ID}). "
...
def assert_parent_pins() -> None:
    unfilled = [
        ...
        for name, value, sentinel in (
            ...
            ("EXPECTED_PARENT_COUNT", EXPECTED_PARENT_COUNT, PARENT_COUNT_SENTINEL),
    ...
    if unfilled:
        raise SystemExit(
            "X05-0001 seal is not pinned: set "
            + ", ".join(unfilled)
            + " from the live ledger tail (the generation committed by the "
            "current latest-sealed final closeout) before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required X05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/providers/v4_x05").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "X05-0001 PASS core verification: cross-provider mutation routing, safe "
        "delayed-reward bandit and fallback, implemented in "
        "src/epistemic_foundry/providers/v4_x05 by a bounded implementation agent "
        "and reviewed by the sealing session (actor-independent from the author) "
        "dispatched under the product owner's explicit parallel-agent "
        "instruction, under exact dependency bindings (X04 X04-0001, N05 "
        "N05-0001, T05 T05-0001) and a U04-0001 regression baseline. Routing may "
        "select only a provider the X04 parity gate made eligible; an adapting "
        "policy must be one this surface bounds as safe with a validated reward "
        "basis, never the immediate proxy a candidate can game; a fallback that "
        "alters a canonical result field is refused through the sealed neutrality "
        "check; a bandit reward is refused with no applied statistical correction "
        "and refused outright when routed at a promotion; provider fan-in is "
        "reconciled through the N05 schedule gate; and an external backend used "
        "as a provider never becomes Foundry authority through the T05 boundary. "
        "Every decision re-derives byte for byte from its own published content "
        "with no clock or random draw, and nothing scores, selects, promotes or "
        "evaluates. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, packaging-discovery, full Python and full "
        "Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by the sealing session with actor_independence=true "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"X05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. X05 routes mutations, "
        "checks provider neutrality, admits search-only bandit rewards and "
        "reconciles fan-in only: it does not score, select, promote or evaluate "
        "any candidate, does not execute or dispatch any provider or external "
        "backend, makes no DSSAT or plant-model parity claim, and promotion "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("X05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X05-0001 parent is not the sealed latest-frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"X05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("X05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("X05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("X05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"X05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("X05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("X05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("X05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored X05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("X05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"X05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("X05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("X05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"X05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected X05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with X05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored X05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
