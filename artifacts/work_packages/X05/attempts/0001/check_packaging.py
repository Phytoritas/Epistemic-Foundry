#!/usr/bin/env python3
"""Prove the X05 routing surface is discoverable by packaging.

``src/epistemic_foundry/providers`` is an existing package marker (neutrality
and untrusted ship under it) and ``providers.v4_x05`` is the new marker this
package adds, so there is a scope decision to prove rather than assume: a marker
that never reached the wheel would let the surface import from a checkout and
vanish from an installed distribution. The discovery mode is read from pyproject
instead of assumed, so a change there fails here.
"""
...
EXPECTED = ("epistemic_foundry.providers", "epistemic_foundry.providers.v4_x05")
