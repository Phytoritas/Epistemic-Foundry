#!/usr/bin/env python3
"""Run the X05-0001 cross-provider routing and safe-bandit acceptance checks.

Required checks from manifests/development_manifest.yaml (X05):
...
routing surface ships under src/epistemic_foundry/providers/v4_x05 as a new
package under the existing ``providers`` marker, so ``packaging-discovery``
proves the ``providers`` and ``providers.v4_x05`` markers actually reach the
wheel rather than only importing from the checkout.

X05 is an integration surface: it composes sealed dependencies (X04's
cross-provider parity/diversity gate, N05's bounded schedule fan-in, T05's
external-backend adapter boundary) and the sealed ``providers/neutrality.py``
and ``evaluation/bandits.py`` modules.  Those composed surfaces are exercised
through ``full-python-suite`` rather than through per-dependency regression
runners, so a regression in any of them still fails here.
...
SURFACE = "src/epistemic_foundry/providers/v4_x05"
PRODUCT_TESTS = {
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/providers is an existing package marker and
    # providers.v4_x05 is the new marker this package adds, so the wheel claim
...
def targeted() -> int:
    return _pytest("targeted-x05-routing", ATTEMPT_DIR)
