#!/usr/bin/env python3
"""Append E03-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000310-b76a7f25"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 310
#: The last evidence id in the ledger at that tail, e.g. "E03xx".
EXPECTED_PARENT_EVIDENCE = "E0312"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "e8e3bb960d0cc8eb209059de2212f481143eb3cb3bd942fc99915d865f87928f"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0313"
FINAL_EVIDENCE_ID = "E0314"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after E03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} capability lease, fencing and approval gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "E03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: capabilities namespace is pinned so a change to the capability authority,
    #: its test support, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/capabilities").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "E03-0001 PASS core verification: the capability authority -- scoped, "
        "expiring CapabilityLeases, monotonic fencing tokens, and explicit "
        "server-derived approval -- implemented in "
        "packages/foundry-kernel/src/capabilities by a bounded implementation agent "
        "and reviewed by this seal-prep session (a distinct actor, actor-independent "
        "from the author) on the sealed E01-0001 append-only Noetic Ledger over the "
        "C04-0001 content-addressed artifact store and D04-0001 SQLite state store "
        "with no new production dependency. issueLease binds each lease to a sealed "
        "PolicyBundle subject, its declared run and resource scopes, and a strictly "
        "positive lifetime; a zero or negative lifetime fails LEASE_ALREADY_EXPIRED "
        "before any state mutation, and a run/scope drift fails "
        "LEASE_RUN_SCOPE_MISMATCH or LEASE_SUBJECT_SCOPE_MISMATCH. commitWithLease "
        "fails closed before invoking or persisting the callback on an expired, "
        "not-yet-valid, revoked, policy-changed, forged, or stale lease "
        "(LEASE_EXPIRED, LEASE_NOT_YET_VALID, LEASE_REVOKED, LEASE_POLICY_MISMATCH, "
        "LEASE_HASH_MISMATCH, STALE_FENCING_TOKEN); an in-callback clock regression, "
        "expiry, async callback, or thrown callback rolls the protected mutation and "
        "event outbox back atomically, and an exact operation retry is idempotent. A "
        "newer overlapping lease increments the monotonic fencing token so no two "
        "holders act; approval is explicit (REQUIRED_APPROVAL_MISSING, "
        "SELF_APPROVAL_DENIED, UNTRUSTED_AUTHORITY_GRANT_DENIED) with a "
        "server-derived role, and a committed lease survives an E01 outage and "
        "reconciles its event exactly once. Emitted leases and approvals validate "
        "against the canonical Draft 2020-12 capability-lease and approval-record "
        "schemas. Ruff lint/format, lease_expiry_test 11/11, fencing_test 19/19, "
        "targeted 30/30, full Python 1261/1261, full Node 1253/1253 across 111 "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"E03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C04-0001, D04-0001, E01-0001, and every prior RAH generation remain "
        "immutable. E03 provides capability leases, fencing, and the approval "
        "policy; the E-phase strict and semantic replay gate (E04), the wider "
        "scheduler, effects execution, and promotion surface, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E03-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"E03 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"E03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"E03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"E03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected E03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
