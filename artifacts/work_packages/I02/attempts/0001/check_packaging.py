#!/usr/bin/env python3
"""Prove the I02 frame-compiler component's packaging posture is intentional.

Unlike the ``src/`` gates that must reach the built wheel, I02 ships under the
...
3. the component imports exactly as its own test modules consume it -- the
   ``frame`` package marker exists while no intermediate parent carries an
   ``__init__.py``, so pytest's prepend import mode roots the package at
   ``frame`` with ``intake/`` on ``sys.path`` and the ``from .compiler`` relative
   import resolves; the import is exercised for real and the public entrypoint
   ``compile_frame`` must resolve (COMPONENT_NOT_IMPORTABLE otherwise).
"""
...
def main() -> int:
    ...
    if not (component / "__init__.py").is_file():
        return _fail({"code": "COMPONENT_PACKAGING_DRIFT",
                      "note": "frame package marker missing"})
    # It must not have leaked into the shipped src tree.
    if (ROOT / "src/epistemic_foundry/intake/frame").exists():
        return _fail({"code": "COMPONENT_PACKAGING_DRIFT",
    ...
    if not callable(getattr(module, "compile_frame", None)):
        return _fail({"code": "COMPONENT_NOT_IMPORTABLE",
                      "note": "compile_frame is not exported"})
