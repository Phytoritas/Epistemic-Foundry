#!/usr/bin/env python3
"""Append I02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000286-f2921102"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 286
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0288"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "3cea3860b6f252395a4f95cf6ab1bde491076f27f6c528e2c4901f8a37e554a3"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0289"
FINAL_EVIDENCE_ID = "E0290"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_i02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after I02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} InsightCard, falsifier and ScopeVector compiler is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "I02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required I02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: frame component is pinned so a change to any product or test file is
    #: caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "python/epistemic_foundry/intake/frame").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "I02-0001 PASS core verification: InsightCard, falsifier and ScopeVector "
        "compiler, implemented in python/epistemic_foundry/intake/frame by a "
        ...
        "(I01); the compiler imports the standard library alone "
        "(jsonschema/referencing enter at test time only), so there is no "
        "composed code surface. Falsifiability is the load-bearing gate: "
        "falsifiers are checked before every other required field and an absent, "
        "empty, blank, or scalar falsifier list is refused FALSIFIER_REQUIRED, "
        "while predictions and mechanism_path are likewise mandatory, so no "
        "unfalsifiable frame reaches the council. Scope normalization preserves "
        "unknowns without inference: a missing, explicit-null, or blank scalar "
        "position becomes a canonical null (list/map positions become the "
        "canonical empty [] / {}) and a typed ScopeUnknown sidecar records "
        "ABSENT, EXPLICIT_NULL, or BLANK_STRING; an eligible card that retains a "
        "required domain/population/unit_of_analysis unknown or an undefined "
        "construct is refused FRAME_ELIGIBILITY_CONFLICT, and inbox/withdrawn "
        "cards keep their unknowns but never claim council readiness. Supplied "
        "identifiers, timestamps and registration hashes are validated for "
        "canonical form and preserved byte-for-byte but never generated. Eight "
        "finding codes each name an exact refusal (FALSIFIER_REQUIRED, "
        "FRAME_FIELD_REQUIRED, FRAME_FIELD_UNKNOWN, FRAME_INPUT_INVALID, "
        "FRAME_ELIGIBILITY_CONFLICT, SCOPE_INPUT_INVALID, SCOPE_FIELD_UNKNOWN, "
        "SCOPE_INTERVENTION_NAME_REQUIRED) and nothing scores, ranks, selects, "
        "promotes or evaluates a candidate. The two required suites are green "
        "(frame_gold_test 19/19, falsifier_gate_test 12/12), ruff check and the "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        ...
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. I02 seals a "
        "deterministic InsightCard/falsifier/ScopeVector compiler and "
        "council-readiness gate only: it validates but does not generate "
        "identifiers, timestamps or registration-hash content (their canonical "
        "authority stays outside I02), does not implement ontology, "
        "measurement identity, intake UI or export (I03/I04 territory), does not "
        "add or modify any canonical schema or workflow, does not score, rank, "
        "select, promote or evaluate any candidate, and refuses adversarial "
        "input with an exact finding code rather than degrading silently. "
        "ScopeUnknown is a component-local typed sidecar, not a new canonical "
        "artifact. Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("I02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"I02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("I02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("I02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"I02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("I02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("I02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("I02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored I02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"I02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("I02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"I02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected I02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with I02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored I02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
