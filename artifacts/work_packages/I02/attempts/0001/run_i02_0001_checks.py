#!/usr/bin/env python3
"""Run the I02-0001 frame-compiler checks.

Required checks from manifests/development_manifest.yaml (I02):
``frame_gold_test`` and ``falsifier_gate_test``.  Each maps to one component test
module under ``python/epistemic_foundry/intake/frame``: ``frame_gold_test`` ->
``test_frame_gold.py`` and ``falsifier_gate_test`` -> ``test_falsifier_gate.py``.
Together the two modules are the whole component's test surface (there is no
third module), so they are the targeted coverage rather than a subset of it.

The component sits under the standalone ``python/`` tree, carries its own
``frame`` package marker, and lies outside the repository ``testpaths``
...
I02 depends on I01 at the manifest layer only; the compiler imports the standard
library plus ``jsonschema``/``referencing`` at test time alone, so there is no
...
ATTEMPT_ID = "I02-0001"
COMPONENT = "python/epistemic_foundry/intake/frame"
...
def frame_gold_test() -> int:
    return _pytest("frame-gold-test", f"{COMPONENT}/test_frame_gold.py")


def falsifier_gate_test() -> int:
    return _pytest("falsifier-gate-test", f"{COMPONENT}/test_falsifier_gate.py")
...
def main() -> int:
    ...
    checks = {
        "frame-gold-test": frame_gold_test,
        "falsifier-gate-test": falsifier_gate_test,
