#!/usr/bin/env python3
"""Append Q01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Q01-0001"
WORK_PACKAGE_ID = "Q01"
EXPECTED_PARENT = "000146-375c11e2"
EXPECTED_PARENT_COUNT = 146
EXPECTED_PARENT_EVIDENCE = "E0148"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "c4dc344b2e98a68e791ba4529fad000270ec647d62dee93cd108dd8096975c69"
)
CORE_EVIDENCE_ID = "E0149"
FINAL_EVIDENCE_ID = "E0150"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_q01_0001_evidence as evidence  # noqa: E402
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Q01-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "Q01-0001 PASS core verification: the gold corpus and annotation "
        "protocol are implemented in evals/gold with the corpus-binding "
        "section added to docs/annotation_manual.md, under exact O04-0001, "
        "P04-0001 and R04-0001 dependency bindings and a P04-0001 regression "
        "baseline. All three case classes are required rather than merely "
        "present: at least three true, false, and boundary cases, with v1.0 "
        "carrying four of each over twelve cases. Every case cites a source "
        "span, a boundary case must name the condition that makes it one, and "
        "a non-boundary case may not carry that field. Adjudication is a "
        "record: two independent annotations per case, an unadjudicated "
        "disagreement fails, an annotator may not adjudicate its own "
        "disagreement, the resolution is canonical, the reason cites the rule, "
        "and a unanimous case carries none. Agreement is measured, not "
        "asserted: Fleiss' kappa is computed over the raw annotations and "
        "reported with the observed and expected agreement it derives from, "
        "measuring 0.749 against a floor of 0.60 that no corpus may declare "
        "weaker; a single-label corpus is reported undefined rather than "
        "perfect. Ruff lint/format, gold_dataset_validation 21/21, "
        "annotation_agreement_check 20/20, targeted 41/41, adjudication "
        "regression 52/52, causal regression 45/45, full Python 1261/1261, "
        "full Node 982/982 across 91 files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"Q01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". P04-0001 and every prior RAH generation remain immutable. The "
        "labels are the primary session's own and have not been validated by "
        "domain experts; the source spans are synthetic identifiers rather "
        "than real document locators; twelve cases exercise the protocol but "
        "do not set production thresholds, which the manual's release rule "
        "already holds conditional; and calibration and scoring belong to "
        "later Q-phase packages. T04, B05, C05, R05, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Q01-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "Q01-0001 gold corpus and annotation protocol is evidence-sealed "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q01-0001 parent is not the sealed P04-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Q01 preflight requires exactly 146 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Q01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Q01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Q01 core did not append exactly E0149")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Q01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Q01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Q01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Q01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Q01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Q01 final did not append exactly E0150")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Q01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Q01 core tail is not E0149")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 148:
        raise SystemExit(f"unexpected Q01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Q01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Q01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
