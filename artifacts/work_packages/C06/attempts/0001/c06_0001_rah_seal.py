#!/usr/bin/env python3
"""Append C06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "C06-0001"
WORK_PACKAGE_ID = "C06"
EXPECTED_PARENT = "000158-0ac12b08"
EXPECTED_PARENT_COUNT = 158
EXPECTED_PARENT_EVIDENCE = "E0160"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "de831b802d6966b244409e566ad101d808ab5ebfc9abfd0abc50fb9dcb6c85bf"
)
CORE_EVIDENCE_ID = "E0161"
FINAL_EVIDENCE_ID = "E0162"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_c06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the D05 lineage, quality-diversity archive, island and checkpoint "
    "transactional store on the sealed C05/C06 state.",
    "Continue the x05 wave toward E05/F05/G05 and the U01/V01/X01 integration layer.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C06-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C06-0001 PASS core verification: the generated types, fixtures and "
        "compatibility integration gate is implemented under schemas/v4_c06, "
        "under an exact C05-0001 dependency binding and a Q03-0001 regression "
        "baseline. Four independently produced receipts are reconciled and "
        "none derives from another: the sealed C05 family index and the "
        "TypeScript, Python and UI contract manifests C02 emits. All four must "
        "agree with the live canonical file for every one of the 42 members, "
        "and a disagreement is refused rather than resolved by picking a "
        "winner. Fixtures are proved against the repository: 84 validations "
        "over 42 members, every canonical example against its own schema and, "
        "where a C05 composite governs it, against that composite. The gate "
        "caught a real error during this attempt: an initial mapping bound the "
        "whole fifteen-member genome family to the candidate composite and "
        "candidate-generation-record failed; the composite was right and the "
        "mapping was wrong, so rather than widen the mutable space the mapping "
        "was corrected and the remaining eleven records are now checked from "
        "the hostile side, each required to be refused as a candidate, and "
        "recorded as schema-only with the reason (EF4-I41). Compatibility is "
        "bound as structure: a migration cannot validate without the "
        "compatibility matrix it applies under, and the binding declares no "
        "enum, const, pattern or format of its own. The canonical schemas, "
        "canonical examples, generated projections and sealed C05 bundle are "
        "read-only inputs, hashed into the receipt and verified unchanged. "
        "Ruff lint/format, schema_and_type_check 18/18, unit_and_contract "
        "61/61, negative_and_adversarial 34/34, provenance_and_receipt 11/11, "
        "targeted 124/124, C05 bundle regression 132/132, contract packaging "
        "regression 29/29, full Python 1261/1261, full Node 982/982 across 91 "
        ...
        "D05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C05-0001 and every prior RAH generation remain immutable. The gate "
        "reconciles receipts and fixtures, not the generators that produced "
        "them: regenerating the C02 projections is C02's own codegen check. "
        "The compatibility binding describes what a migration record must "
        "carry, not whether a proposed migration is correct, which C03 owns. "
        "schemas/v4_c06 is regenerable byte-for-byte from the canonical "
        "sources, the sealed C05 bundle and the generated manifests. D05, E05, "
        "F05, G05, S05, Q04, U01, V01, X01, source coverage, PRD, review, and "
        "global closeout remain; implementation_gate=fail and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("C06-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C06-0001 evolution genome, evaluator, archive, statistics and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("C06-0001 parent is not the sealed Q03-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("C06 preflight requires exactly 158 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("C06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("C06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("C06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("C06 core did not append exactly E0161")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("C06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("C06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("C06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored C06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("C06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("C06 final did not append exactly E0162")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("C06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("C06 core tail is not E0161")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 160:
        raise SystemExit(f"unexpected C06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with C06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored C06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
