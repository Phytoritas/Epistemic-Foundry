#!/usr/bin/env python3
"""Run the C06-0001 generated types, fixtures and compatibility checks.

Required checks from manifests/development_manifest.yaml (C06):
...
report.  ``schemas/v4_c06`` is emitted by the recorded ``gate-emit`` step and
re-verified by ``gate-verify``.
"""
...
ATTEMPT_ID = "C06-0001"
COMPONENT = "artifacts/work_packages/C06/attempts/0001"
# C05 is the sole dependency; its bundle is re-verified as a regression
# alongside the C02 packaging surface this gate reconciles against.
DEPENDENCY_TARGETS = {
...
def gate_emit() -> int:
    return run(
        ...
        [
            ...
            "python",
            "-B",
            f"{COMPONENT}/c06_integration_gate.py",
            "emit",
        ],
    )
...
def targeted() -> int:
    return _pytest("targeted-c06-gate", COMPONENT)


def dependency_regression_c05() -> int:
    # The C05 component ships its own pytest.ini, so it is run on its own
    # rather than folded into a shared invocation that would lose it.
    return _pytest(
