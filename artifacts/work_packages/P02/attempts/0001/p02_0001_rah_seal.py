#!/usr/bin/env python3
"""Append P02-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "P02-0001"
WORK_PACKAGE_ID = "P02"
EXPECTED_PARENT = "000140-22dc6ae1"
EXPECTED_PARENT_COUNT = 140
EXPECTED_PARENT_EVIDENCE = "E0142"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "7c669f42e7d8d0b674f76b6e018336f738666006e6aca4b2ef0353be6cf4cf3f"
)
CORE_EVIDENCE_ID = "E0143"
FINAL_EVIDENCE_ID = "E0144"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_p02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Implement P03 cross-examination and Minority Report on the sealed P01 blind dispatch.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required P02-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "P02-0001 PASS core verification: method, scope, causal and novelty "
        "audits with veto are implemented in "
        "python/epistemic_foundry/parliament/auditors with an exact P01-0001 "
        "dependency and a W04-0001 regression baseline. The promotion ladder "
        "is read from schemas/hypothesis-passport.schema.json rather than "
        "re-spelled. The combined ceiling is derived as the lowest any auditor "
        "set, is invariant to verdict order, cannot be raised by agreement "
        "from the other three, and a declared ceiling above it is refused and "
        "cannot be restored by rehashing. Method-incompatible evidence is "
        "never pooled: the method auditor reports one ceiling per stratum, its "
        "ceiling must equal the strongest stratum, a blended mid-range figure "
        "is refused, only it stratifies, and a sealed audit cannot be "
        "relabelled pooled. The veto belongs to the method auditor alone, must "
        "carry a reason, floors the ladder at INBOX when sustained, stays on "
        "the record when withdrawn, and cannot be removed from a rehashed "
        "audit. Ruff lint/format, veto_fixture_test 19/19, "
        "promotion_ceiling_test 23/23, targeted 42/42, blind regression "
        "37/37, causal regression 45/45, full Python 1261/1261, full Node "
        "982/982 across 91 files, and git diff --check pass. "
        ...
        "P03-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"P02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". W04-0001, P01-0001, and every prior RAH generation remain "
        "immutable. P02 combines auditor judgements that arrive as inputs; it "
        "does not produce them, does not own the evidence-class ceiling policy "
        "the ingest layer holds, cannot see a method incompatibility the "
        "auditor never stratified, and does not commit promotion. P03, T04, "
        "B05, C05, R05, source coverage, PRD, review, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("P02-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "P02-0001 method, scope, causal and novelty audits with veto is "
        "evidence-sealed PASS. P03-0001 is next; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P02-0001 parent is not the sealed W04-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("P02 preflight requires exactly 140 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("P02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("P02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("P02 core did not append exactly E0143")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("P02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("P02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("P02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored P02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("P02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("P02 final did not append exactly E0144")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("P02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("P02 core tail is not E0143")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 142:
        raise SystemExit(f"unexpected P02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with P02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored P02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
