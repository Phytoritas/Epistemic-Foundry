#!/usr/bin/env python3
"""Append B05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "B05-0001"
WORK_PACKAGE_ID = "B05"
EXPECTED_PARENT = "000150-435146de"
EXPECTED_PARENT_COUNT = 150
EXPECTED_PARENT_EVIDENCE = "E0152"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "8bc2d3a3c3a10f3f1edcec0157c89c1046cec597eac2d973117245fe9187e0e9"
)
CORE_EVIDENCE_ID = "E0153"
FINAL_EVIDENCE_ID = "E0154"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_b05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the C05 evolution genome, evaluator, archive, statistics and adapter "
    "schema gate on the sealed A06/C04 state.",
    "Continue the x05 wave toward R05 and the U01/V01/X01 integration layer.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B05-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "B05-0001 PASS core verification: the deterministic v4 build profile, "
        "dependency pinning proof and ShinkaEvolve optional-feature profile "
        "are implemented under exact A06-0002 and B04-0010 dependency "
        "bindings, a T04-0001 regression baseline, and the recorded scope "
        "decision HD-EF4-B05-SCOPE-20260801-001, which reads build/v4_b05 as "
        "generated, reproducible, untracked build output. The output derives "
        "from exactly five tracked inputs and re-emits byte-identically; the "
        "build manifest receipts the generator, every input and every output "
        "by content hash, and verification re-derives all of them from the "
        "live repository, so a re-sealed forgery still fails. Pinning is "
        "proved: the build backend is pinned exactly, all twenty locked "
        "external packages carry sha256 artifact hashes, the lock's Python "
        "floor equals the project's, every declared dependency and group "
        "entry resolves in the lock, and the canonical registry carries the "
        "sealed B04 counts of 127 schemas plus one OpenAPI document. The "
        "Shinka feature is DISABLED and UNQUALIFIED with every EF4-I63 "
        "authority flag false; a profile edited toward enablement, "
        "qualification, authority or an install extra is refused with the "
        "code that names the attempt, and a shinka dependency in pyproject or "
        "the lock is refused at derivation time. Ruff lint/format, "
        "schema_and_type_check 14/14, unit_and_contract_tests 11/11, "
        "negative_and_adversarial_tests 27/27, provenance_and_receipt_audit "
        "10/10, targeted 62/62, A06 constitutional regression 40/40, B04 "
        "packaging/contract/CLI regression 41/41, full Python 1261/1261, full "
        "Node 982/982 across 91 files, and git diff --check pass. "
        ...
        "C05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"B05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". T04-0001 and every prior RAH generation remain immutable. The "
        "ShinkaEvolve pin is a public observation, not a verified clone; the "
        "locked wheel hashes are trusted as the pin authority and no wheel "
        "rebuild was performed, which the B06 reproducible-build gate owns; "
        "enablement and qualification belong to B06 and T06; and build/v4_b05 "
        "stays untracked by design, reconstructible byte-for-byte from tracked "
        "sources. C05, R05, Q02, Q03, U01, V01, X01, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("B05-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "B05-0001 deterministic build, dependency pinning and Shinka "
        "optional-feature profile is evidence-sealed PASS. C05-0001 is next; "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("B05-0001 parent is not the sealed T04-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("B05 preflight requires exactly 150 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("B05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("B05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("B05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("B05 core did not append exactly E0153")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("B05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("B05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("B05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored B05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("B05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("B05 final did not append exactly E0154")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("B05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("B05 core tail is not E0153")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 152:
        raise SystemExit(f"unexpected B05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with B05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored B05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
