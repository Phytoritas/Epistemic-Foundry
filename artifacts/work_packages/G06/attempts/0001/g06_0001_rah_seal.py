#!/usr/bin/env python3
"""Append G06-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed X03/R05 templates do.
"""
...
#: The current committed generation (e.g. "000235-4954aeeb"); its retained
...
EXPECTED_PARENT = "000244-38556e9a"
EXPECTED_PARENT_COUNT = 244
EXPECTED_PARENT_EVIDENCE = "E0246"
EXPECTED_PARENT_MANIFEST_SHA256 = "e59dbaf8485ec45c221e130565c3b2cd31d74604e4a2e2db375ec550eeb89c33"
CORE_EVIDENCE_ID = "E0247"
FINAL_EVIDENCE_ID = "E0248"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_g06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after G06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} native plugin packaging and skill-discovery integration gate "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "G06-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required G06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes under the frozen G06 write scope, bound by path.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "plugin_blueprint/epistemic-foundry/v4_g06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "G06-0001 PASS core verification: the native plugin packaging and "
        "skill-discovery integration gate, implemented in "
        "plugin_blueprint/epistemic-foundry/v4_g06 by a bounded implementation "
        "agent under the product owner's explicit parallel-execution instruction "
        "and reviewed by the sealing session as an independent reviewer "
        "(actor-independent from the author), under exact G05-0001/H05-0001/"
        "T05-0001 dependency bindings and an H06-0001 live latest-sealed "
        "regression baseline. Packaging is a projection: the manifest may declare "
        ...
        "package configures, and understating any surface is refused as loudly as "
        "overstating it. Discovery derives from declared manifests: a bundled "
        "skill is discovered from the sealed inventory with its content hash, and "
        "a third-party skill enters the discoverable set only from a signed, "
        "hash-verified, approved and attested lockfile row whose permissions the "
        "package declares; unsigned, quarantined, unattested, over-permissioned "
        "and id-colliding rows are refused by name. Every receipt binds its "
        "declaring sources by digest and re-derives its own hash with the sealed "
        "gateway's canonical-JSON digest, carrying no clock and no randomness. No "
        "declared capability may name the evaluator, holdout or promotion "
        "authority the G05 surface denies, and promotion-bearing projected "
        "commands are recorded as such rather than laundered into a discoverable "
        "capability. The four required modules are green (54 packaging tests), the "
        "composed G05/H05/T05 dependency regressions green, the full Node suite "
        "green with the four G06 packaging modules inside the inventory, and git "
        "diff --check all pass. "
        ...
        "external actor-independent certification does not hold). The DAG "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"G06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. G06 validates, "
        "discovers and integrates plugin declarations only: it installs no "
        "plugin, activates no skill and executes no command, acquires no "
        "evaluator/holdout/promotion authority, and makes no release or "
        "production-readiness claim. Author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("G06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G06-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"G06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("G06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("G06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"G06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("G06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("G06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("G06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored G06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("G06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"G06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("G06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"G06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected G06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with G06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored G06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
