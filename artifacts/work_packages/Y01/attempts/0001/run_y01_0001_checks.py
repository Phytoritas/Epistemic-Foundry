#!/usr/bin/env python3
"""Run the Y01-0001 typed budget, adaptive fleet and performance control checks.

Required checks from manifests/development_manifest.yaml (Y01):
``budget_enforcement_test`` and ``adaptive_fleet_test``.  Both are Node test
files under ``packages/foundry-kernel/src/budget``, so they also join the full
Node inventory rather than being targeted only.  The budget subsystem composes
its vocabulary from the sealed contract registry and the canonical
...
ATTEMPT_SCRIPTS = "artifacts/work_packages/Y01/attempts/0001"
...
def ruff_check() -> int:
    # The Y01 surface is Node; ruff still guards this attempt's own scripts.
    return run(
...
def budget_enforcement_test() -> int:
    return _node(
        "budget-enforcement-test", [f"{COMPONENT}/budget-enforcement.test.mjs"]
    )


def adaptive_fleet_test() -> int:
    return _node("adaptive-fleet-test", [f"{COMPONENT}/adaptive-fleet.test.mjs"])
...
def main() -> int:
    ...
    checks = {
        ...
        "budget-enforcement-test": budget_enforcement_test,
        "adaptive-fleet-test": adaptive_fleet_test,
        "targeted": targeted,
