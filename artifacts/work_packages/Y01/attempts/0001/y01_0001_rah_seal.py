#!/usr/bin/env python3
"""Append Y01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000300-dcf015bb"
...
EXPECTED_PARENT_EVIDENCE = "E0302"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "f8139b64a604c2885b6c476c7d269506a9901ca6dccf9a94613d9b453f3c9467"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0303"
FINAL_EVIDENCE_ID = "E0304"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_y01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after Y01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} typed budgets and adaptive fleet gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "Y01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Y01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: budget namespace is pinned so a change to the typed budget, adaptive
    #: fleet, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/budget").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Y01-0001 PASS core verification: the typed budget subsystem and adaptive "
        "worker fleet, implemented in packages/foundry-kernel/src/budget by a "
        "bounded implementation agent and reviewed by this seal-prep session (a "
        "distinct actor, actor-independent from the author) on the sealed B04-0002, "
        "D04-0001, Q04-0001, W04-0001 and X04-0001 attempts with no new production "
        "dependency. The enforcement vocabulary is composed from the sealed "
        "BudgetEnvelope contract (schemas/budget-envelope.schema.json), never "
        "restated; only HARD_ labels bound spend. An over-budget reservation is "
        "refused with BUDGET_LIMIT_EXCEEDED and leaves usage unchanged rather than "
        "truncating; SOFT_ESTIMATE and UNMETERED stay truthfully non-bounding and "
        "never refuse; a mislabelled envelope, a tampered budget_hash, and a "
        "tampered contract are refused. The adaptive fleet sizes workers as "
        "clamp(backlog, min_workers, effective_max_workers) where the effective "
        "maximum is the declared maximum capped by the budget concurrency hard "
        "limit; a declared maximum exceeding a bounding concurrency limit is "
        "refused, an out-of-window request is refused, and an unbounded backlog "
        "cannot exceed the bound, so fan-out is bounded by construction. Both meter "
        "and fleet acquire no authority and emit only deterministic re-derivable "
        "receipts. Ruff lint/format, budget_enforcement_test 12/12, "
        "adaptive_fleet_test 8/8, targeted 20/20, full Python 1261/1261, full Node "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Y01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". B04-0002, D04-0001, Q04-0001, W04-0001, X04-0001 and every prior RAH "
        "generation remain immutable. Y01 provides typed budgets, the adaptive "
        "fleet, and their in-process performance controls; observability/SLO "
        "telemetry (Y02), backup/recovery runbooks (Y03), source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Y01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"Y01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Y01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Y01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Y01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"Y01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Y01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Y01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Y01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Y01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Y01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"Y01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Y01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Y01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"Y01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected Y01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Y01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Y01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
