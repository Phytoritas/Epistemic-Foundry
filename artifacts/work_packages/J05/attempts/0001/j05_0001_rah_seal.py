#!/usr/bin/env python3
"""Append J05-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was prepared by a bounded
agent that was instructed not to touch ``.rah/``, so it cannot read the live
ledger to pin the parent generation, its manifest hash, the retained-generation
count, the parent tail evidence id, or the two evidence identifiers this attempt
will mint.  The RAH frontier is moving under concurrent sealing (R05-0001 sealed
at evidence ``E0206``, and further packages sealed on top of it afterwards), so
the parent tail is NOT fixed at R05 and must be read live at seal time.  All of
``EXPECTED_PARENT``, ``EXPECTED_PARENT_MANIFEST_SHA256``,
``EXPECTED_PARENT_COUNT``, ``EXPECTED_PARENT_EVIDENCE``, ``CORE_EVIDENCE_ID``
and ``FINAL_EVIDENCE_ID`` are therefore left as the sentinel
``FILL_FROM_LEDGER``; ``assert_parent_pins`` refuses to run until the parent
fills every one of them from the live ledger tail (the current committed
generation, its evidence id, and the next two contiguous evidence ids this
attempt will mint).  Everything else — summaries, hashes, flat-snapshot
verification — is derived exactly as the R05 template does, and
``product_hashes`` binds the J05 product bytes under
``src/epistemic_foundry/operators``.
"""
...
#: Sentinel for the pins that can only be read from the live ledger tail.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
#: Fill every pin below from the live `.rah/ralph` tail immediately before
#: running: EXPECTED_PARENT is the current committed generation
#: "NNNNNN-XXXXXXXX"; EXPECTED_PARENT_COUNT its retained-generation count;
#: EXPECTED_PARENT_MANIFEST_SHA256 that generation's manifest sha256;
#: EXPECTED_PARENT_EVIDENCE the tail evidence id already committed; and
#: CORE_EVIDENCE_ID / FINAL_EVIDENCE_ID the next two contiguous evidence ids
#: this attempt will mint.  The frontier moved past R05-0001 (E0206) under
#: concurrent sealing, so none of these can be hardcoded here.
EXPECTED_PARENT = "000210-94d54c46"
EXPECTED_PARENT_COUNT = 210
EXPECTED_PARENT_EVIDENCE = "E0212"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "907e6de588a46623540994a8da6944b94bbef73be4a3af5bbdf505e9a09aedb0"
)
CORE_EVIDENCE_ID = "E0213"
FINAL_EVIDENCE_ID = "E0214"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_j05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after J05-0001 seals and select the next "
...
#: is built from the evidence ids so that filling the sentinels updates it too.
GATE_NOTE = (
    f"{ATTEMPT_ID} typed mutation-operator registry, prompt genomes and "
    f"quarantine workflow is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    if PARENT_PIN_SENTINEL in (
        ...
        raise SystemExit(
            "J05-0001 seal is not pinned: set EXPECTED_PARENT, "
            "EXPECTED_PARENT_MANIFEST_SHA256, EXPECTED_PARENT_COUNT, "
            "EXPECTED_PARENT_EVIDENCE, CORE_EVIDENCE_ID and FINAL_EVIDENCE_ID "
            "from the live ledger tail (the current committed generation and its "
            "evidence id) before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required J05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/operators").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "J05-0001 PASS core verification: typed mutation-operator registry, "
        "prompt genomes and quarantine workflow, implemented in "
        "src/epistemic_foundry/operators/v4_j05 by a bounded implementation "
        "agent and reviewed by an independent reviewer (actor-independent from "
        "the author) that did not author the subject code, under exact "
        "dependency bindings (I05 I05-0001, J04 J04-0001, C05 C05-0001) and an "
        "R05-0001 regression baseline. An operator may register only for genome "
        "kinds the sealed C05 index still marks mutable through the I05 intake "
        "surface, a prompt genome is born quarantined and cannot be constructed "
        "active, a prompt change is a proposal for a future sealed run built by "
        "the governance quarantine module, and an activation is refused unless "
        "the quarantine released the proposal, qualification evidence is bound, "
        "and the target is not the run that produced it. Authority-carrying and "
        "lifecycle fields cannot be edited, nothing scores, selects, promotes "
        "or evaluates, and every record re-derives its own digest with no clock "
        "or random draw. Ruff lint/format, schema-and-type-check, "
        "unit-and-contract-tests, negative-and-adversarial-tests, "
        "provenance-and-receipt-audit, packaging-discovery, I05/J04/C05 "
        "dependency regressions, full Python and full Node suites, and git diff "
        "--check all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"J05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. J05 types "
        "registrations, builds quarantined prompt proposals and records "
        "replayable lineage only: it does not score, select, promote or "
        "evaluate any candidate or prompt, releases no prompt genome from "
        "quarantine, makes no DSSAT or plant-model parity claim, and promotion "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("J05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J05-0001 parent is not the sealed R05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"J05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("J05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("J05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"J05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("J05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("J05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("J05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored J05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("J05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"J05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("J05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"J05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected J05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with J05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored J05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
