#!/usr/bin/env python3
"""Prove the J05 operator registry is discoverable by the packaging config.

``src/epistemic_foundry/operators`` is a package marker authorized under
HD-EF4-J05-SCOPE-20260802-001, so there is a scope decision to prove rather
than assume: a marker that never reached the wheel would let the registry
import from a checkout and vanish from an installed distribution. The discovery
mode is read from pyproject instead of assumed, so a change there fails here.
"""
...
EXPECTED = ("epistemic_foundry.operators", "epistemic_foundry.operators.v4_j05")
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the registry would import from a checkout but be absent "
                        "from the wheel"
                    ),
                    "status": "FAIL",
                },
