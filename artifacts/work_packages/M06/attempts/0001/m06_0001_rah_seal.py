#!/usr/bin/env python3
"""Append M06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT = ROOT / "artifacts/work_packages/M06/attempts/0001"
AUTOMATION = ROOT / ".rah/helpers/recursive-architecture-refactoring-auto/automation"
ATTEMPT_ID = "M06-0001"
WORK_PACKAGE_ID = "M06"
EXPECTED_PARENT = "000186-786903e1"
EXPECTED_PARENT_COUNT = 186
EXPECTED_PARENT_EVIDENCE = "E0188"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "b8f81e6eedbee2dabc70aef3381878f8052ded304ed5a9797ca6847d6becfec7"
)
CORE_EVIDENCE_ID = "E0189"
FINAL_EVIDENCE_ID = "E0190"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_m06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward N05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "M06-0001 map correctness, ranking separation and stale-propagation integration gate "
    "is evidence-sealed PASS (E0189/E0190). N05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required M06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "src/epistemic_foundry/cartography/v4_m06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "M06-0001 PASS core verification: map correctness, ranking separation and stale-propagation integration gate, "
        ...
        "session, under exact dependency bindings (M05 M05-0001) "
        "and an S05-0001 regression baseline. every niche and record in the fixtures is built through the sealed M05 builders, archive entries validate against their canonical schema, and authority schema names are resolved against the registry on every call "
        "cross-surface agreement is surveyed completely rather than first-failure — every divergent pair is named in context — and staleness cascades list every derived record that must be rebuilt from a stale revision "
        "a promotion request resting on a combined score, coverage ratio or entropy figure alone refuses PROMOTION_AUTHORITY_ABSENT in the EF4-I45 direction; a map figure may inform but never justify, and the actual decision is recorded as P-phase territory "
        "Key review finding: The raised code on a multi-finding survey is the alphabetically first, documented as deterministic rather than a severity claim; context carries the complete finding set. "
        ...
        + ". N05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"M06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a canonical promotion-request schema: none exists, so the shape is modeled and recorded as such; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("M06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("M06-0001 parent is not the sealed M06 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("M06 preflight requires exactly 186 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("M06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("M06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("M06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("M06 core did not append exactly E0189")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("M06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("M06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("M06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("M06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored M06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("M06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("M06 final did not append exactly E0190")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("M06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("M06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("M06 core tail is not E0189")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 188:
        raise SystemExit(f"unexpected M06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with M06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored M06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
