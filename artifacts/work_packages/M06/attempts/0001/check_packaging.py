#!/usr/bin/env python3
"""Prove the M06 gate is discoverable by the project's own packaging config.

Without ``src/epistemic_foundry/cartography/v4_m06/__init__.py`` the gate
imports from a source checkout but is silently absent from the built wheel,
because pyproject discovers packages with ``find_packages`` rather than the
namespace variant (HD-EF4-M05-SCOPE-20260802-001).  This check reads the
discovery mode from pyproject instead of assuming it, so a change there fails
here.
"""
...
EXPECTED = (
    "epistemic_foundry.cartography",
    "epistemic_foundry.cartography.v4_m06",
)
