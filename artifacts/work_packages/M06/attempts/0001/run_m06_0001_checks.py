#!/usr/bin/env python3
"""Run the M06-0001 cartography integration acceptance checks.

Required checks from manifests/development_manifest.yaml (M06):
...
report.  The gate ships under src/epistemic_foundry/cartography/v4_m06, so
``wire-literal-discipline`` runs the repository's EF4-I22 gate as its own
check, and ``packaging-discovery`` proves the new subpackage is not silently
absent from the wheel.

M06 is an integration gate over sealed surfaces rather than a new engine, so
its dependency regressions are the two suites it composes: M05's cartography
attempt (the niches, coverage map, diversity report and blast radius it reads)
and the archive suite that owns the entry contract it compares against.
"""
...
ATTEMPT_ID = "M06-0001"
COMPONENT = "artifacts/work_packages/M06/attempts/0001"
ENGINE = "src/epistemic_foundry/cartography/v4_m06"
# M05 owns every map record this gate reads, and the archive module owns the
# entry contract the map is compared against.  Both are sealed; a regression in
# either invalidates this gate's inputs rather than its logic.
DEPENDENCY_TARGETS = {
    "dependency-regression-archive": ("tests/test_epistemic_species_archive.py",),
    "dependency-regression-cartography": ("artifacts/work_packages/M05/attempts/0001",),
}
...
def packaging_discovery() -> int:
    # Without the v4_m06 package marker the gate imports from a checkout but
    # is absent from the wheel (HD-EF4-M05-SCOPE-20260802-001).
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-m06-integration", COMPONENT)


def dependency_regression_archive() -> int:
    return _pytest(
        "dependency-regression-archive",
        *DEPENDENCY_TARGETS["dependency-regression-archive"],
    )


def dependency_regression_cartography() -> int:
    return _pytest(
        "dependency-regression-cartography",
        *DEPENDENCY_TARGETS["dependency-regression-cartography"],
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "targeted": targeted,
        "dependency-regression-archive": dependency_regression_archive,
        "dependency-regression-cartography": dependency_regression_cartography,
