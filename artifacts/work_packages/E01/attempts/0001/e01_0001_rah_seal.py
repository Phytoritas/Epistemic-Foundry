#!/usr/bin/env python3
"""Append E01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000304-797394bc"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 304
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0306"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "25e958f4f22c345abff01bf4de0a7e17c6ad442e71311f952fad4aeb0d5b221d"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0307"
FINAL_EVIDENCE_ID = "E0308"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after E01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} append-only Noetic Ledger gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "E01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: ledger namespace is pinned so a change to the append-only Noetic Ledger,
    #: its reducer, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/ledger").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "E01-0001 PASS core verification: the append-only Noetic Ledger and its "
        "deterministic reducer, implemented in packages/foundry-kernel/src/ledger "
        "by a bounded implementation agent and reviewed by this seal-prep session "
        "(a distinct actor, actor-independent from the author) on the sealed "
        "C04-0001 content-addressed artifact store and D04-0001 SQLite state store "
        "with no new production dependency. Each append resolves and hash-verifies "
        "the payload artifact, assigns the next contiguous per-run sequence, links "
        "previous_event_hash to the verified run tail, and commits the immutable "
        "EventRecord and its run stream in one transaction; events are written once "
        "at revision 0 and never updated or deleted, an exact retry is idempotent, "
        "and rebinding an event id to different bytes fails with EVENT_ID_CONFLICT. "
        "verifyEventChain fails closed on sequence gaps or reorder, hash tamper, "
        "cross-run splice, payload drift, and a missing immutable tail; concurrent "
        "writers serialize through the run-stream compare-and-swap. rebuild "
        "re-resolves and hash-verifies every payload, runs the reducer twice "
        "asserting an identical canonical trace, rejects non-deterministic, async, "
        "mutating, and non-JSON reducers, and reproduces byte-identical state and "
        "state_hash after both durable stores are reopened. Emitted records "
        "validate against the canonical Draft 2020-12 event-record schema. Ruff "
        "lint/format, ledger_hash_chain_test 12/12, reducer_replay_test 9/9, "
        "targeted 21/21, full Python 1261/1261, full Node 1253/1253 across 111 "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"E01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C04-0001, D04-0001, and every prior RAH generation remain immutable. "
        "E01 provides the append-only Noetic Ledger, its hash chain, and "
        "deterministic rebuild; ActionIntent/Attempt/EffectReceipt (E02), the "
        "wider effects, capability, scheduler, and replay surface, source "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"E01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"E01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"E01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"E01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected E01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
