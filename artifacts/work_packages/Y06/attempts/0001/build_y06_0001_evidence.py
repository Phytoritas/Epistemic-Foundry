#!/usr/bin/env python3
"""Build and verify Y06-0001 evidence: 2,000-document evolution qualification and cost/latency integration gate.
...
ATTEMPT_ID = "Y06-0001"
WORK_PACKAGE_ID = "Y06"
RECORDED_AT = "2026-08-02T19:30:00.000Z"
ATTEMPT_DIR = "artifacts/work_packages/Y06/attempts/0001"
AUTHORITY_DECISION = None

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/Y06/attempts/0001/check_packaging.py": "408c73f60adc58084558dee1a675879dda94be4fe5f9ccbc56b13d405839a510",
    "artifacts/work_packages/Y06/attempts/0001/fixtures.py": "33067672e9d96d99de632e38104b4b2d7176c17057dcb2cd559defe9d04a7817",
    "artifacts/work_packages/Y06/attempts/0001/pytest.ini": "ca8fe220a8e00a311a9db275a2a371a7b2bf6826f7f86822ec857f1a9527b1a0",
    "artifacts/work_packages/Y06/attempts/0001/run_y06_0001_checks.py": "ebff0a8b411ef5b1d8b4a384d255120b76f207ee5c57467f0c93fd6ac417cf4b",
    "artifacts/work_packages/Y06/attempts/0001/test_negative_adversarial.py": "d9f88f3fc52c4620488bfc75941164aba630320330e3cce150ff554181c64daf",
    "artifacts/work_packages/Y06/attempts/0001/test_provenance_receipts.py": "25c5914f0d1adbbe3b15db333e64d1b6f62ac87026f7ebbc26add3048d7c6e5f",
    "artifacts/work_packages/Y06/attempts/0001/test_schema_and_type.py": "bef3386e261862626e5da92f05e4c7c45017dc5b3685d1ddc2c3c33957900ced",
    "artifacts/work_packages/Y06/attempts/0001/test_unit_contract.py": "4cd4eeae9bfd53c2ff79176b4a4c665f5a1c400720d218d6edd91beedbe5ed4c",
    "src/epistemic_foundry/operations/v4_y06/__init__.py": "7388faf4e6b4183a4641091e71ba3a13b7f0f46da24215d3571c4f0587b995ad",
    "src/epistemic_foundry/operations/v4_y06/qualification.py": "ebdea8d13cf99969532c280a8b8f77ffe0cab2863695ff89e5e3771fa849f1d8",
}
...
JUNIT_PATHS = {
    ...
    "targeted_y06_qualification": ATTEMPT / "targeted-y06-qualification.junit.xml",
...
OUTPUT_NAMES = (
    ...
    "pytest.ini",
    "review.md",
    "run_y06_0001_checks.py",
    "schema-and-type-check.junit.xml",
    "targeted-y06-qualification.junit.xml",
    ...
    "y06-verification.json",
    "y06_0001_rah_seal.py",
)
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        ...
        (
            "targeted_y06_qualification",
            pytest_summary(JUNIT_PATHS["targeted_y06_qualification"]),
            41,
        ),
        (
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        ...
        "next_action": "SEAL_Y06_0001_THEN_CONTINUE_DAG",
...
def write_scope_verification() -> dict[str, Any]:
    ...
    return {
        "approved_scope": [
            "src/epistemic_foundry/operations/v4_y06/**",
            "artifacts/work_packages/Y06/**",
        ],
        "attempt_id": ATTEMPT_ID,
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "declaring_sources": {
            "budget_dimensions": (
                "budgets/envelope.py LIMIT_DIMENSIONS and the Y05 bounded-budget attestation (composed)"
            ),
            "promotion_capability": (
                "governance/evolution_authority/registry.py PROMOTION_COMMIT_CAPABILITY (imported, never a literal)"
            ),
            "reconciliation_stages": (
                "evolution_chamber/reconciliation.py STAGES/TERMINAL_DISPOSITIONS (composed, not restated)"
            ),
            "surrogate_token": (
                "surrogate-triage-report schema acceptance token, read positionally"
            ),
        },
        "exit_criteria": {
            "all_completion_and_external_effects_resolve_to_immutable_receipts": {
                "mechanism": (
                    "the reconciliation, budget, surrogate and authority sub-receipts each re-derive their own hashes, the final verdict binds all four sub-receipt hashes, and every record is deterministic under caller-supplied ids with no clock or randomness"
                ),
                "status": "PASS",
            },
            "governing_schemas_authority_boundaries_failure_states_exact": {
                "mechanism": (
                    "reconciliation stages, budget dimensions, the surrogate acceptance token and the promotion-commit capability are read from their declaring owners (composed or imported, never restated), and the gate holds no canonical enum value as a literal after renaming three own collisions (capability_id, qualification_passed, accepted_count), proven by the package scan and the repository EF4-I22 gate 5/5"
                ),
                "status": "PASS",
            },
            "happy_negative_crash_resume_adversarial_coverage": {
                "mechanism": (
                    "caller-declared per-stage counts are reconciled against the composed E05 ledger (submitted == admitted + refused fan-in), and a silent partial fan-in, an orphan effect, a count declaration mismatch, an unbounded or overrun budget dimension, and a surrogate acceptance above its ceiling are each refused by their own code"
                ),
                "status": "PASS",
            },
            "no_candidate_model_prompt_backend_or_hook_acquires_authority": {
                "mechanism": (
                    "the scale run acquires no evaluator, holdout or promotion authority: promotion capability is grounded in the imported canonical constant, a mutable-search holder granted protected authority refuses SCALE_RUN_ACQUIRES_PROMOTION_AUTHORITY, and a numeric score bound into a promotion decision refuses SCORE_BOUND_INTO_PROMOTION_FIELD"
                ),
                "status": "PASS",
            },
        },
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_y06_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "Y06_SCALE_QUALIFICATION_COST_LATENCY_GATE",
        ...
        "next_package": "Z01-0001",
        "not_claimed": [
            "a real 2,000-document evolution run: the corpus, evaluator and backend are not executed and the gate reconciles a caller-declared manifest of counts and measurements",
            "independent classification of a holder as a search-space artifact: holder_is_search_space and protected_authority are declared manifest properties, hardened by the grounded capability constant and structural score detection but not independently adjudicated",
            "wall-clock or thread-level cost/latency measurement: cost and latency are caller-supplied measurements mapped onto the composed budget-envelope dimensions",
...
def build() -> dict[str, Any]:
    ...
    write_json("y06-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("Y06-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "y06-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored Y06-0001 report is not the deterministic document")
    return _summary()
