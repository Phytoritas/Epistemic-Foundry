#!/usr/bin/env python3
"""Append the verified post-N02-0001 DAG reconciliation to active RAH state."""
...
import post_n02_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000070-dc0826eb"
EXPECTED_PARENT_MANIFEST_HASH = (
    "007d9a14548c01ccd23a4c8735469679ccb1d2cea279ff9dcd0bb8f96290bc14"
)
EXPECTED_EVIDENCE_ID = "E0071"
NEXT_ACTIONS = [
    "Execute N03-0001 DAG scheduler, leases, retries and concurrency under its exact manifest scope.",
    "Run N03 scheduler_property_test and resource_conflict_test with upstream regressions and separate primary-session review.",
    "Seal N03 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-n02-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 59
        and stored.get("waiting_package_count") == 93
        and stored.get("ready_packages_manifest_order") == ["N03", "O01", "T01", "A06"]
        and stored.get("next_package") == "N03"
    ...
    review_path = ATTEMPT / "post-n02-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_n02_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-N02-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "N02-0001 final evidence E0070 / 000070-dc0826eb. The canonical external "
        ...
        "no unknown dependency and no cycle; 59 packages are PASS, 4 are "
        "dependency-ready, and 93 wait on unmet dependencies. Manifest-order READY "
        "is N03, O01, T01, A06, so N03 is next. Highest numeric attempt selection "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed N02 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0070" or len(base.numbered_generations(ralph_root)) != 70:
        raise SystemExit("post-N02 DAG seal requires exact E0070 / 70-generation state")
    ...
    note = (
        "The sealed post-N02 DAG selects N03 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0070":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0071 summary differs from current DAG artifacts")
    result = n02_seal.verify_generation_store(71)
