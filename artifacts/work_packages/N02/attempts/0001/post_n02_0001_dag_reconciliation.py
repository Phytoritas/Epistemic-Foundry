#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed N02-0001 PASS."""
...
OUTPUT = ATTEMPT / "post-n02-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-n02-0001-dag-review.md"
EXPECTED_N02_REPORT_HASH = (
    "sha256:7b63fb48c4a006a63ad33baa414711a5cfc1ac8d3c38f0fc53c68b05f0a41ebd"
)

sys.path.insert(0, str(N01_ATTEMPT))
import post_n01_0001_dag_reconciliation as previous  # noqa: E402
...
def reconcile() -> dict[str, Any]:
    ...
    n02 = states["N02"]
    if not isinstance(n02.get("report"), str):
        raise SystemExit("N02 report is missing")
    n02_report = base.read_json(ROOT / n02["report"])
    rah = n02_report.get("rah_state")
    if not (
        n02["selected_attempt_id"] == "N02-0001"
        and n02["current_state"] == "PASS"
        and n02["report_sha256"] == EXPECTED_N02_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0069"
        and rah.get("core_generation") == "000069-38d16ba1"
        and rah.get("final_closeout_evidence_id") == "E0070"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("N02-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "N02_attempt_id": "N02-0001",
            "N02_core_evidence_id": "E0069",
            "N02_core_generation": "000069-38d16ba1",
            "N02_final_closeout_evidence_id": "E0070",
            "N02_final_generation": "000070-dc0826eb",
            "N02_report": n02["report"],
            "N02_report_sha256": n02["report_sha256"],
            "status": "PASS",
        },
        ...
        "external_resume_inspection": {
            ...
            "observed_state_generation": "000070-dc0826eb",
            "parse_errors": {},
            "ralph_status": "active",
            "status": "PASS",
        },
        ...
        "projection": "POST_N02_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 59
        and result["waiting_package_count"] == 93
        and result["ready_packages_manifest_order"] == ["N03", "O01", "T01", "A06"]
        and result["next_package"] == "N03"
    ):
        raise SystemExit(
            "post-N02 DAG differs from evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-N02-0001 live DAG reconciliation
