#!/usr/bin/env python3
"""Append N06-0001 PASS evidence while global completion remains false.
...
read the live ledger to pin the parent generation, its manifest hash, the
preserved-generation count, or the two evidence identifiers this seal will
create.  ``EXPECTED_PARENT``, ``EXPECTED_PARENT_MANIFEST_SHA256``,
``EXPECTED_PARENT_COUNT``, ``CORE_EVIDENCE_ID`` and ``FINAL_EVIDENCE_ID`` are
therefore left as the sentinel ``FILL_FROM_LEDGER`` and the seal refuses to run
(``assert_parent_pins``) until the parent fills all five from the live ledger
tail (the current committed generation and the next two free evidence IDs).
The parent's tail evidence is derived from ``CORE_EVIDENCE_ID`` (it is the one
before it), so it is not a sixth pin.  Everything else — summaries, hashes,
flat-snapshot verification — is derived exactly as the templates do.
"""
...
#: The current committed generation (e.g. "000206-XXXXXXXX"); fill from
#: `.rah/ralph` current pointer before running.
EXPECTED_PARENT = "000204-aaa75b86"
#: The number of preserved generations at that tail; fill from
#: `.rah/ralph/generations` before running.
EXPECTED_PARENT_COUNT = 204
#: The two free evidence IDs this seal appends (core, then final closeout).
CORE_EVIDENCE_ID = "E0207"
FINAL_EVIDENCE_ID = "E0208"
#: The parent tail's generation-manifest sha256 (no "sha256:" prefix).
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "e0e56756428fa709640ac241df9db92f25ee1ce441d86098469ea37fa7056530"
)
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_n06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after N06-0001 seals and select the next "
...
GATE_NOTE = (
    "N06-0001 backpressure, missing-worker and resource-lock integration gate "
    f"is evidence-sealed PASS ({CORE_EVIDENCE_ID}/{FINAL_EVIDENCE_ID}). The next "
    "package is RECOMPUTE_DAG; implementation_gate=fail and "
...
def assert_parent_pins() -> None:
    ...
    if PARENT_PIN_SENTINEL in pins:
        raise SystemExit(
            "N06-0001 seal is not pinned: set EXPECTED_PARENT, "
            "EXPECTED_PARENT_MANIFEST_SHA256, EXPECTED_PARENT_COUNT, "
            "CORE_EVIDENCE_ID and FINAL_EVIDENCE_ID from the live ledger tail "
            "before running."
        )


def parent_count() -> int:
    return int(EXPECTED_PARENT_COUNT)


def parent_evidence_id() -> str:
    #: The parent tail's last evidence is the one immediately before the core
    #: evidence this seal appends.
    return f"E{int(CORE_EVIDENCE_ID[1:]) - 1:04d}"
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required N06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: scheduler package is hashed (marker plus v4_n05 and the v4_n06 gate this
    #: attempt adds) because the gate composes the sealed scheduler at import
    #: time, so the surface it stands on is part of the sealed evidence.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/scheduler").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "N06-0001 PASS core verification: backpressure, missing-worker and "
        "resource-lock integration gate, implemented in "
        "src/epistemic_foundry/scheduler/v4_n06 by a bounded implementation agent "
        "and reviewed by an independent reviewer that did not author the gate "
        "(actor-independent from the author), under an exact dependency binding "
        "(N05 N05-0001) and an R05-0001 regression baseline. The gate sits on top "
        "of the sealed N05 lane scheduler: it asks N05 for the verdict on every "
        "prefix of the schedule rather than walking the events a second time, "
        "holds no canonical schema enum value as a string literal and declares no "
        "lane, action or stage identity of its own (EF4-I22). It refuses the three "
        "modes N05 declares outside its own detection — work shed under "
        "backpressure (checked against the declared policy in both directions), a "
        "worker that stops producing events across a declared event horizon with "
        "no clock, and a resource overcommitted, retained, held unmet or "
        "deadlocked in a declared wait cycle — while a schedule that ends holding "
        "in-flight work stays N05's FANIN_INCOMPLETE rather than a second name. "
        "Nothing scores, selects, promotes or evaluates: integrated is the "
        "conjunction of a valid schedule and no finding, the signature takes no "
        "evaluator/holdout/fitness/promotion input, and every gated run re-derives "
        "byte for byte from its own published fields with no clock or random draw. "
        ...
        "packaging-discovery, the N05 dependency regression, full Python and full "
        "Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by an independent reviewer with actor_independence=true "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"N06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. N06 gates a declared "
        "run for backpressure, missing workers and resource locks and records a "
        "re-derivable integration record only: it does not score, select, promote "
        "or evaluate any candidate, does not observe locks or stalls a "
        "declaration did not state, does not swallow a partial fan-in, makes no "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("N06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        or evidence_ids(payloads)[-1] != parent_evidence_id()
    ):
        raise SystemExit("N06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != parent_count() or generations[-1] != generation:
        raise SystemExit(
            f"N06 preflight requires exactly {parent_count()} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("N06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("N06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
        "generation": generation,
        "latest_evidence_id": parent_evidence_id(),
        "mode": "preflight",
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("N06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"N06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("N06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != parent_count() + 1:
        raise SystemExit("N06 core did not preserve all prior generations")
    integrity = verify_generation_store(parent_count() + 1)
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("N06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("N06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored N06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("N06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"N06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("N06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != parent_count() + 2:
        raise SystemExit("N06 final did not preserve all prior generations")
    return {
        ...
        "mode": "final",
        "parent_generation": parent,
        "state_verification": verify_generation_store(parent_count() + 2),
        "status": "active",
    }
...
def run_verify() -> dict[str, Any]:
    ...
    if count == parent_count() + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"N06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        count != parent_count() + 2
        or generation_number(generation) != parent_count() + 2
    ):
        raise SystemExit(f"unexpected N06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with N06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored N06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
