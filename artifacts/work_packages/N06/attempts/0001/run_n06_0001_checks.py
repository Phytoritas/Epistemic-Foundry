#!/usr/bin/env python3
"""Run the N06-0001 backpressure, missing-worker and resource-lock gate checks.

Required checks from manifests/development_manifest.yaml (N06):
...
under this attempt directory (note the module is ``test_schema_type.py``); the
review is recorded in ``review.md`` and the report rather than in this runner.

The gate ships under src/epistemic_foundry/scheduler/v4_n06 and composes the
sealed N05 lane scheduler at import time, so ``packaging-discovery`` proves both
the scheduler package (its marker pre-exists under HD-EF4-N05-SCOPE-20260802-001)
and the v4_n06 subpackage reach the wheel, and that the composed v4_n05 is
discoverable too — a wheel carrying v4_n06 without v4_n05 would fail on the
first import rather than on the first use.

N06 depends on the sealed N05 scheduler, which it composes rather than forks, so
``dependency-regression-n05`` runs N05's sealed attempt suite unchanged: a change
to the lane scheduler that this gate silently absorbed would fail there before it
failed inside a gate test.  N05's suite lives in its attempt directory because
that is the only place the scheduler is exercised in the repository; there is no
separate tests/ mirror to target.
...
#: The package whose discoverability packaging-discovery proves; the gate lives
#: in its v4_n06 subpackage and the marker one level up pre-exists under N05.
COMPONENT = "src/epistemic_foundry/scheduler"
ENGINE = "src/epistemic_foundry/scheduler/v4_n06"
PRODUCT_TESTS = {
...
# N06 composes the sealed N05 lane scheduler; running N05's own sealed attempt
# suite unchanged is the regression that proves this gate consumed it rather than
# forked it.  N05 is exercised only in its attempt directory, so that is the
# target (no tests/ mirror of the scheduler exists).
DEPENDENCY_TARGETS = {
...
def ruff_check() -> int:
    return run(
        "ruff-check", ["uv", "run", "--locked", "ruff", "check", ENGINE, ATTEMPT_DIR]
    )
...
def ruff_format_check() -> int:
    return run(
        "ruff-format-check",
        ["uv", "run", "--locked", "ruff", "format", "--check", ENGINE, ATTEMPT_DIR],
    )


def packaging_discovery() -> int:
    # src/epistemic_foundry/scheduler is the package marker (pre-existing under
    # HD-EF4-N05-SCOPE-20260802-001); v4_n06 is the new subpackage and v4_n05 is
    # composed at import time.  This check proves all three ship in the wheel.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-n06-integration", ATTEMPT_DIR)


def dependency_regression_n05() -> int:
    return _pytest(
        "dependency-regression-n05", *DEPENDENCY_TARGETS["dependency-regression-n05"]
    )
