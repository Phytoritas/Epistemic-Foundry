#!/usr/bin/env python3
"""Append the verified post-K02-0001 DAG reconciliation to active RAH state."""
...
import post_k02_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000033-1de1c2f0"
EXPECTED_EVIDENCE_ID = "E0034"
NEXT_ACTIONS = [
    "Execute K03-0001 SourceSpan emission under its exact manifest write scope.",
    "Run K03 required checks, full regressions, and a separate primary-session review.",
    "Seal K03 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-k02-0001-dag-reconciliation.json"
    ...
    review_path = ATTEMPT / "post-k02-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_k02_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-K02-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "K02-0001 final evidence E0033 / 000033-1de1c2f0. The active development "
        "manifest has 156 unique packages, no unknown dependency and no cycle; "
        "47 packages are currently PASS, 5 are dependency-ready, and 104 wait on "
        "unmet dependencies. Manifest-order READY is K03, L01, N01, T01, A06, so "
        "K03 is the next bounded package. Package state is selected from the "
...
def commit(summary: str) -> str:
    ...
    identifiers = evidence_ids(payloads)
    if identifiers[-1] != "E0033" or len(numbered_generations(ralph_root)) != 33:
        raise SystemExit("post-K02 DAG seal requires exact E0033 / 33-generation state")
    ...
    note = (
        "The sealed post-K02 DAG selects K03 as the earliest dependency-ready "
...
def main() -> int:
    ...
    ralph_root, current, payloads = current_state()
    identifiers = evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0033":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0034 summary differs from current DAG artifacts")
    result = verify_generation_store(34)
