#!/usr/bin/env python3
"""Append K02-0001 core and closeout evidence to the active RAH state."""
...
import build_k02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_INITIAL_PARENT = "000031-46c5cc7c"
EXPECTED_INITIAL_EVIDENCE = "E0031"
NEXT_ACTIONS = [
    "Recompute and seal the live 156-package DAG after K02-0001 PASS.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required K02 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "K02-0001 PASS: caller-supplied immutable GROBID, Docling and fallback "
        "outputs require exact backend version/revision, executable digest, "
        "profile hash and adapter version. Malformed or unsafe output fails "
        "typed; no live backend, network, CWD or repository fallback is claimed. "
        "Primary FAIL/BLOCKED remains visible when fallback is used, successful "
        "primary output cannot be replaced, and cross-parser disagreements retain "
        "both provenance-bound observations without selecting truth. K02 targeted "
        "is 40/40, full Python is 1054/1054, full Node is 470/470 over 54 files, "
        "and codegen is 126/126. The initial 39/40 fixture diagnostic and rejected "
        "39-file/366-test Node inventory are preserved. "
        f"Parser sha256:{hashes['parser-verification.json']}; fallback sha256:"
        f"{hashes['fallback-status-verification.json']}; disagreement sha256:"
        f"{hashes['disagreement-verification.json']}; regression sha256:"
        ...
        f"{hashes['parser-verification.artifact-receipt.json']}; targeted JUnit "
        f"sha256:{hashes['targeted-k02-suite.junit.xml']}; Python JUnit sha256:"
        ...
        f"{hashes['build_k02_0001_evidence.py']}; sealer sha256:"
        f"{hashes['k02_0001_rah_seal.py']}. Review is primary-session separate "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"K02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        f"report sha256:{hashes['report.json']}; commands sha256:"
        f"{hashes['commands.jsonl']}; review sha256:{hashes['review.md']}; parser "
        f"sha256:{hashes['parser-verification.json']}; fallback sha256:"
        f"{hashes['fallback-status-verification.json']}; disagreement sha256:"
        f"{hashes['disagreement-verification.json']}; regression sha256:"
        f"{hashes['full-regression-impact.json']}; scope sha256:"
        f"{hashes['write-scope-verification.json']}; dependency sha256:"
        f"{hashes['dependency-status.json']}; receipt sha256:"
        f"{hashes['parser-verification.artifact-receipt.json']}; RAH integrity "
        f"sha256:{hashes['rah-core-integrity.json']}; builder sha256:"
        f"{hashes['build_k02_0001_evidence.py']}; sealer sha256:"
        f"{hashes['k02_0001_rah_seal.py']}. Every prior report and generation is "
        "preserved. K02-0001 is immutable PASS and the live 156-package DAG must "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("K02 requires a contiguous evidence ledger")
    if not (
        ...
        raise SystemExit("K02 preflight requires active/fail/completion_ready=false")
...
def commit_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "K02-0001 parser adapters are evidence-sealed PASS. The live 156-package "
...
def run_preflight() -> dict[str, Any]:
    ...
    if not (
        ...
        and len(numbered_generations(ralph_root)) == 31
    ):
        raise SystemExit("K02 preflight is not at the sealed post-K01 DAG state")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("K02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, core_id]:
        raise SystemExit("K02 core did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("K02 core did not preserve every prior generation")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("K02 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("K02 evidence identifiers differ from the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("K02 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("K02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("K02 final did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K02 final evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("K02 final did not preserve every prior generation")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if evidence_ids(payloads)[-2:] != [core_id, final_id]:
        raise SystemExit("RAH ledger does not end with K02 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("K02 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored K02 final summary differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
