#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed K02-0001 PASS.

The highest numeric attempt directory is authoritative for every package. An
older PASS can therefore never hide a newer non-PASS or incomplete attempt.
"""
...
OUTPUT = ATTEMPT / "post-k02-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-k02-0001-dag-review.md"
...
def reconcile() -> dict[str, Any]:
    ...
    k02 = states["K02"]
    if not isinstance(k02.get("report"), str):
        raise SystemExit("K02 report is missing")
    k02_report = read_json(ROOT / k02["report"])
    rah = k02_report.get("rah_state")
    if not (
        k02["selected_attempt_id"] == "K02-0001"
        and k02["current_state"] == "PASS"
        and k02["report_sha256"] == EXPECTED_K02_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0032"
        and rah.get("core_generation") == "000032-176398f3"
        and rah.get("final_closeout_evidence_id") == "E0033"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("K02-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "K02_attempt_id": "K02-0001",
            "K02_core_evidence_id": "E0032",
            "K02_core_generation": "000032-176398f3",
            "K02_final_closeout_evidence_id": "E0033",
            "K02_report": k02["report"],
            "K02_report_sha256": k02["report_sha256"],
            "status": "PASS",
        },
        ...
        "projection": "POST_K02_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 47
        and result["blocked_package_count"] == 104
        and result["ready_packages_manifest_order"]
        == ["K03", "L01", "N01", "T01", "A06"]
        and result["next_package"] == "K03"
    ):
        raise SystemExit(
            "post-K02 DAG differs from the expected evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-K02-0001 live DAG reconciliation
