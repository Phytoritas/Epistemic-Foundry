#!/usr/bin/env python3
"""Append C01-0007 PASS evidence while the global repair goal stays active."""
...
import build_c01_0007_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run C04-0002 full conformance after the C01-0007 GateDecision hash correction is evidence-sealed PASS.",
    "Run B04-0008 final packaging only after C04-0002 PASS.",
    "Recompute the 156-package DAG after B04-0008 PASS and continue the next dependency-ready package; keep implementation_gate=fail and completion_ready=false until the complete external goal is terminal.",
]
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C01-0007 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C01-0007 PASS: the C01-owned sample GateDecision decision_hash now "
        f"recomputes to {evidence.EXPECTED_DECISION_HASH} with the self field "
        "excluded, validates against the strict Draft 2020-12 schema, and rejects "
        "semantic tampering. The complete canonical inventory is 126 schemas and "
        "126 one-to-one validating examples with 126 unique schema IDs; OpenAPI "
        "remains 3.1.1 with 33 unique operations. B04-0007 projection freshness "
        "passes. Targeted contracts are 77/77, full Python is 990/990, and the "
        "authoritative Node footer is 460/460. Contract verification sha256:"
        f"{hashes['canonical-contract-verification.json']}; GateDecision hash "
        f"verification sha256:{hashes['gate-decision-hash-verification.json']}; "
        f"receipt sha256:{hashes['gate-decision.artifact-receipt.json']}; "
        f"projection freshness sha256:{hashes['projection-receipt-verification.json']}; "
        f"regression sha256:{hashes['full-regression-impact.json']}; dependencies "
        f"sha256:{hashes['dependency-status.json']}; JUnit normalization sha256:"
        ...
        f"{hashes['phase-artifact-reconciliation.json']}; targeted JUnit sha256:"
        ...
        f"builder sha256:{hashes['build_c01_0007_evidence.py']}; sealer sha256:"
        f"{hashes['c01_0007_rah_seal.py']}. Review is primary-session separate "
        "with actor_independence=false because Fleet/subagents are forbidden. "
        "C04-0002 and B04-0008 remain, so implementation_gate=fail and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C01-0007 PASS closeout is hash-sealed after core generation {parent}: "
        f"report sha256:{hashes['report.json']}; commands sha256:"
        f"{hashes['commands.jsonl']}; review sha256:{hashes['review.md']}; "
        f"contract sha256:{hashes['canonical-contract-verification.json']}; "
        f"GateDecision hash verification sha256:"
        f"{hashes['gate-decision-hash-verification.json']}; receipt sha256:"
        f"{hashes['gate-decision.artifact-receipt.json']}; projection freshness "
        f"sha256:{hashes['projection-receipt-verification.json']}; phase "
        ...
        f"status sha256:{hashes['dependency-status.json']}; normalization sha256:"
        f"{hashes['junit-normalization-verification.json']}; write scope sha256:"
        ...
        f"{hashes['build_c01_0007_evidence.py']}; sealer sha256:"
        f"{hashes['c01_0007_rah_seal.py']}. C01-0007 is PASS and C04-0002 is "
        "dependency-ready. All prior attempts and generations remain preserved; "
        "C04-0002 and B04-0008 remain, so implementation_gate=fail and "
        "completion_ready=false."
    )
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C01-0007 bounded GateDecision hash correction is evidence-sealed PASS. "
        "C04-0002 full conformance is next; B04-0008 final packaging remains, so "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("C01-0007 preflight requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("C01-0007 report is already RAH-bound; use verify")
    generations = numbered_generations(ralph_root)
    if generations[-1] != generation:
        raise SystemExit("C01-0007 preflight generation pointer mismatch")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C01-0007 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match the C01-0007 summary")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("C01-0007 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("C01-0007 report evidence IDs do not match the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("C01-0007 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C01-0007 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match C01-0007 closeout artifacts")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("C01-0007 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("sealed ledger does not end with C01-0007 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("sealed C01-0007 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored C01-0007 final evidence differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
