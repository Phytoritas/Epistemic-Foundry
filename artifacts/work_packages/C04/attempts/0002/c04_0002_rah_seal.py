#!/usr/bin/env python3
"""Append C04-0002 FAIL evidence while the repair goal stays active."""
...
import build_c04_0002_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run C02-0003 with the canonical generator to refresh all generated contract projections.",
    "Run C04-0003 full conformance only after C02-0003 is evidence-sealed PASS.",
    "Run B04-0008 final packaging only after C04 PASS; keep implementation_gate=fail and completion_ready=false until the external goal is terminal.",
]
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C04-0002 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C04-0002 FAIL with owner C02: canonical 126/126 validation, OpenAPI "
        "3.1.1/33 operations, B04-0007 projection freshness, Python 990/990, "
        "Node 460/460, and targeted contracts 287/287 pass, but the non-waivable "
        "generated-contract gate finds exactly seven stale generated files and "
        "the cross-language fixture reports exactly sample_gate_decision.json "
        "example hash mismatch. This is a generated projection defect, not a "
        "SPEC_GAP. Conformance sha256:"
        f"{hashes['c04-conformance-verification.json']}; regression sha256:"
        ...
        f"{hashes['full-node-suite.junit.xml']}; targeted JUnit sha256:"
        f"{hashes['targeted-contract-conformance.junit.xml']}; review sha256:"
        f"{hashes['review.md']}; builder sha256:{hashes['build_c04_0002_evidence.py']}; "
        f"sealer sha256:{hashes['c04_0002_rah_seal.py']}. C04 product changes are "
        "zero, review has actor_independence=false, C02-0003 is next, and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C04-0002 FAIL closeout is hash-sealed after core generation {parent}: "
        f"report sha256:{hashes['report.json']}; commands sha256:"
        f"{hashes['commands.jsonl']}; review sha256:{hashes['review.md']}; "
        f"conformance sha256:{hashes['c04-conformance-verification.json']}; "
        ...
        f"{hashes['build_c04_0002_evidence.py']}; sealer sha256:"
        f"{hashes['c04_0002_rah_seal.py']}. C04-0002 remains immutable FAIL, "
        "C02-0003 is repair-ready, every prior generation remains preserved, "
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C04-0002 is evidence-sealed FAIL because C02 generated projections are "
        "stale. C02-0003 is next; implementation_gate=fail and completion_ready=false."
    )
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("C04-0002 requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("C04-0002 report is already RAH-bound; use verify")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04-0002 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match C04-0002 summary")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("C04-0002 report does not bind current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("C04-0002 evidence IDs do not match live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("C04-0002 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04-0002 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match C04-0002 closeout")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("C04-0002 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("ledger does not end with C04-0002 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("C04-0002 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored C04-0002 final evidence differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
