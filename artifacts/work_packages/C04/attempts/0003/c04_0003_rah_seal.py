#!/usr/bin/env python3
"""Append C04-0003 PASS evidence while the global goal stays active."""
...
import build_c04_0003_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run B04-0008 final packaging against the C04-0003 conformant source tree.",
    "After B04-0008 PASS, recompute the 156-package DAG.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C04-0003 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C04-0003 PASS: canonical 126/126 validation, OpenAPI 3.1.1 with 33 "
        "operations, current B04-0007 projection of 127 resources, current C02 "
        "nine-file generated projection, Python 990/990, Node 460/460 across "
        "52 files, and corrected targeted contracts 287/287 all pass. All 24 "
        "C01 migration nodes pass, the migration allowlist is empty, document "
        "registration and phase-artifact reconciliation pass, active legacy "
        "enum hits are zero, and receipt-bound promotion crash/replay probes pass. "
        ...
        f"{hashes['review.md']}; builder sha256:{hashes['build_c04_0003_evidence.py']}; "
        f"sealer sha256:{hashes['c04_0003_rah_seal.py']}. C04 product changes are "
        "zero, review actor_independence=false, B04-0008 is next, global "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C04-0003 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"{hashes['build_c04_0003_evidence.py']}; sealer sha256:"
        f"{hashes['c04_0003_rah_seal.py']}. C04-0003 is immutable PASS, "
        "B04-0008 is dependency-ready, every prior generation is preserved, "
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C04-0003 full conformance is evidence-sealed PASS; B04-0008 final "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("C04-0003 requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("C04-0003 report is already RAH-bound; use verify")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04-0003 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match C04-0003 summary")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("C04-0003 report does not bind current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("C04-0003 evidence IDs do not match live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("C04-0003 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04-0003 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match C04-0003 closeout")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("C04-0003 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("ledger does not end with C04-0003 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("C04-0003 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored C04-0003 final evidence differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
