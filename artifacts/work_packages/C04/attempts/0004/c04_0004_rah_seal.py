#!/usr/bin/env python3
"""Append C04-0004 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "C04-0004"
WORK_PACKAGE_ID = "C04"
EXPECTED_PARENT = "000104-96fab768"
EXPECTED_PARENT_COUNT = 104
EXPECTED_PARENT_EVIDENCE = "E0106"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "b0b85bbeee2ab5aedabbcaefb63ed0e7ca38462659a750ac70aa65422eeb9bf3"
)
...
import build_c04_0004_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run B04-0010 final packaging only after C04-0004 evidence-sealed PASS.",
    "After B04-0010 PASS, recompute the 156-package DAG.",
    "Continue the earliest dependency-ready package serially while completion_ready remains false until the external goal is terminal.",
]
...
def evidence_ids(payloads: dict[str, Any]) -> list[str]:
    ...
    if (
        ...
        or int(ledger.get("issued_id_high_water", 0)) != len(identifiers)
    ):
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required C04-0004 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "C04-0004 PASS core verification: live canonical authority is 127 "
        "Draft 2020-12 schemas with 127 one-to-one examples; OpenAPI is 3.1.1 "
        "with 33 operations; RetrievalCandidate, the nine generated contract "
        "files, the 128-resource B04-0009 projection, installed-wheel bytes, "
        "runtime migration semantics, 15 promotion gates, O02 eleven-lane "
        "retrieval with RRF(k=60), and the F04 17-transition/14-artifact-set "
        "reconciliation pass. Python is 1115/1115 and Node is 819/819 across "
        "79 files; targeted contracts are clean; all 24 historical migration "
        "nodes pass; failures, errors, skips, xfails, todo, cancellation, and "
        "migration allowlist entries are zero. "
        ...
        "B04-0010 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"C04-0004 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C04-0001 through C04-0003 remain immutable history and every prior "
        "RAH generation is preserved. B04-0010 final packaging is dependency-ready, "
        "but the wider DAG, source coverage, release gate, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("C04 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "C04-0004 full conformance is evidence-sealed PASS; B04-0010 final "
        "packaging is next. The global implementation gate remains failed and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("C04-0004 parent generation/evidence is not the sealed O02 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("C04 preflight requires exactly 104 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("C04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("C04-0004 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("C04 core did not append exactly E0107")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("C04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("C04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("C04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored C04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("C04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("C04 final did not append exactly E0108")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("C04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("C04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("C04 core tail is not E0107")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 106:
        raise SystemExit(f"unexpected C04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with C04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored C04 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
