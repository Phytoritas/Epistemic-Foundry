#!/usr/bin/env python3
"""Append B02-0001 PASS evidence while global completion remains false.

Authoring note (read before running). This seal is authored as a bounded
seal-prep hand that is instructed not to touch ``.rah/``, so it cannot read the
...
the sealed A01/A02/A03/A04/B01 templates do.

B02 depends on B01. Its product bytes are ``pyproject.toml`` and ``uv.lock`` --
the pinned dependency manifest and its resolved lock -- which it attests without
editing.
"""
...
#: The current committed generation (e.g. "000298-abcd1234"); its retained
...
EXPECTED_PARENT = "000308-59389421"
EXPECTED_PARENT_COUNT = 308
EXPECTED_PARENT_EVIDENCE = "E0310"
EXPECTED_PARENT_MANIFEST_SHA256 = "1dadb6f8b497fa01236f90171aaca2eafb9cef89e98a20fe52c5483c8f4a16da"
CORE_EVIDENCE_ID = "E0311"
FINAL_EVIDENCE_ID = "E0312"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_b02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402

#: The product bytes B02 attests, outside the attempt directory.
PRODUCT_FILES = (
    "pyproject.toml",
    "uv.lock",
)
...
NEXT_ACTIONS = [
    "Recompute the DAG frontier after B02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} pinned toolchains, lockfiles and deterministic build is "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "B02-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B02-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory: the pinned dependency
    #: manifest and its resolved lock B02 attests (never edits).
...
def core_summary() -> str:
    ...
    return (
        "B02-0001 PASS core verification: pinned toolchains, lockfiles and "
        "deterministic build, attested by the bounded agent(s) that authored the "
        "B02 dependency-lock correction (exact skill-context tiktoken==0.13.0 in "
        "pyproject.toml and its resolved uv.lock closure) and the two "
        "attempt-local build/lock adapters, and reviewed by this seal-prep "
        "session as an independent contract-reviewer (actor-independent from the "
        "author). B02 attests pyproject.toml and uv.lock exactly as they "
        "currently are and makes ZERO edit to them. lockfile_check "
        "(uv lock --check + scripts/build/check_locks.py) confirms 21 locked "
        "packages, 20 registry packages with exact versions and sha256 artifact "
        "hashes, and the pinned setuptools==82.0.1 backend. "
        "double_build_comparison stages the exact current source inputs and "
        "yields two byte-identical build snapshots over 11 artifacts with zero "
        "mismatches via the attempt-local current-input adapter; the production "
        "helper scripts/build/double_build.py is preserved unmodified as a B04 "
        "integration handoff. tiktoken_exact_lock_check confirms uv.lock pins "
        "exactly tiktoken==0.13.0 as the sole skill-context dev-group member from "
        "the PyPI registry, never a runtime dependency. skill_context_frozen_sync "
        "(uv sync --frozen --group skill-context --offline) plus a structural "
        "old/new lock reconstruction proves the group added only tiktoken and its "
        "mandatory closure with zero unrelated dependency changes and zero "
        "runtime exposure. j02_tokenizer_vector_test is 20/20 green including the "
        "seven exact o200k_base vectors with the installed tiktoken 0.13.0. "
        "write_scope_audit attributes only pyproject.toml and uv.lock with zero "
        "violations. The scoped Python suite is 1261/1261 green, the live Node "
        "structure and boundary checks pass (10 components, 18 internal package "
        "edges), and git diff --check is clean. B02 depends on B01; the sealed "
        "B01-0001 attempt is the build dependency and regression baseline. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by this seal-prep session as an independent "
        "contract-reviewer with actor_independence=true (author and reviewer are "
        "distinct actors; external actor-independent certification does not "
        "hold). The stale production double-build helper is a disclosed, "
        "unmodified B04 scope-boundary, not a weakening. The DAG frontier "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"B02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. B02 attests the pinned "
        "dependency manifest and resolved lock; it does not re-author them, makes "
        "no product-maturity, runtime-executability or release-readiness claim, "
        "does not correct the stale production build helper (B04 scope), and "
        "asserts only that all shipped dependencies are pinned, clean builds are "
        "reproducible over 11 byte-identical artifacts, the skill-context group "
        "declares exactly tiktoken==0.13.0, the frozen offline sync adds no "
        "unrelated dependency, and the o200k_base tokenizer vectors pass. "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("B02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("B02-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"B02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("B02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("B02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"B02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("B02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("B02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("B02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored B02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"B02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("B02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"B02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected B02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with B02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored B02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
