#!/usr/bin/env python3
"""Append the verified post-N03-0001 DAG reconciliation to active RAH state."""
...
EXPECTED_PARENT = "000073-6df5ddba"
EXPECTED_PARENT_MANIFEST_HASH = (
    "549b8882c534f2053a2f01588d17ab283499ae81cec7621faad97f7e59ff053f"
)
EXPECTED_EVIDENCE_ID = "E0074"
NEXT_ACTIONS = [
    "Execute N04-0001 event bus and deterministic replay under its exact manifest scope.",
    "Run N04 required checks with upstream regressions and separate primary-session review.",
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-n03-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 60
        and stored.get("waiting_package_count") == 92
        and stored.get("ready_packages_manifest_order") == ["N04", "O01", "T01", "A06"]
        and stored.get("next_package") == "N04"
        ...
        and stored.get("attempt_binding", {}).get("N03_final_generation_manifest_sha256")
        == "sha256:" + EXPECTED_PARENT_MANIFEST_HASH
    ):
        raise SystemExit("DAG reconciliation does not identify the expected next step")
    review_path = ATTEMPT / "post-n03-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_n03_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-N03-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "N03-0001 final evidence E0073 / 000073-6df5ddba and generation manifest "
        ...
        "no unknown dependency and no cycle; 60 packages are PASS, 4 are "
        "dependency-ready, and 92 wait on unmet dependencies. Manifest-order READY "
        "is N04, O01, T01, A06, so N04 is next. Highest numeric attempt selection "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed N03 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0073" or len(base.numbered_generations(ralph_root)) != 73:
        raise SystemExit("post-N03 DAG seal requires exact E0073 / 73-generation state")
    ...
    note = (
        "The sealed post-N03 DAG selects N04 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0073":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0074 summary differs from current DAG artifacts")
    result = n03_seal.verify_generation_store(74)
