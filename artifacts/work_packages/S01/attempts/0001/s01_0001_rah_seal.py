#!/usr/bin/env python3
"""Append S01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000306-828a5130"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 306
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0308"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "724dfdd2fde6464b6bd090023659b84564d8de87c7b04d148b23b648079ec1c3"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0309"
FINAL_EVIDENCE_ID = "E0310"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_s01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after S01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} trust-zone enforcement gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "S01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required S01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: trust component is pinned so a change to the trust boundary or its tests
    #: is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/foundry-kernel/src/security/trust").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "S01-0001 PASS core verification: the trust-zone enforcement boundary, "
        "implemented in packages/foundry-kernel/src/security/trust by a bounded "
        "implementation agent and reviewed by this seal-prep session (a distinct "
        "actor, actor-independent from the author) on the sealed A04-0001 and "
        "B01-0001 packages with no new production dependency. Evidence and model "
        "output are sealed as immutable runtime-branded data segments whose frozen "
        "disposition sets instructionEligible and authorityEligible to false; the "
        "data-only allowlist is transforms only, so requesting an instruction use "
        "fails closed with UNTRUSTED_USE_DENIED and the assembled data-only context "
        "exposes neither an instruction nor a messages field. "
        "denyUntrustedAuthorityRequest returns a frozen DENY / UNTRUSTED_ORIGIN with "
        "empty capability, approval, policy, phase, and instruction identifiers for "
        "every request type including unknown future actions, and never inspects "
        "the content; sidecar authority fields are rejected with UNEXPECTED_FIELD. "
        "There is no promotion or authority-grant API: copied or JSON-round-tripped "
        "segments lose the WeakMap brand and are denied with UNSEALED_CONTENT, "
        "unknown source kinds fail closed with UNKNOWN_SOURCE_KIND, and Proxy record "
        "and array inputs are denied with PROXY_INPUT_DENIED without invoking their "
        "traps. Ruff lint/format, prompt_injection_suite 8/8, "
        "authority_escalation_test 9/9, targeted 17/17, full Python 1261/1261, full "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"S01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". A04-0001, B01-0001, and every prior RAH generation remain immutable. "
        "S01 provides the deterministic trust-zone enforcement primitive; provider "
        "prompt assembly, corpus ingest, context-capsule compilation, execution "
        "guards, the S04 red-team integration, exhaustive injection detection, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("S01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"S01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("S01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("S01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"S01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("S01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("S01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("S01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored S01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("S01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"S01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("S01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"S01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected S01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with S01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored S01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
