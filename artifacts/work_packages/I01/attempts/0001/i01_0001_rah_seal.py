#!/usr/bin/env python3
"""Append I01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000276-438545a0"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 276
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0278"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "800f824d3363cca3902d004c2579ed7136c3aa9fa18744fb15240a45908c443a"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0279"
FINAL_EVIDENCE_ID = "E0280"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_i01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after I01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} bounded Interview and contradiction scan is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "I01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required I01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: interview component is pinned so a change to any product or test file is
    #: caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "python/epistemic_foundry/intake/interview").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "I01-0001 PASS core verification: bounded Interview and contradiction "
        "scan, implemented in python/epistemic_foundry/intake/interview by a "
        "bounded implementation agent and reviewed by the sealing agent "
        "(actor-independent from the author) under the manifest-order dependencies "
        "(C04, F04); the engine imports the standard library alone, so there is no "
        "composed code surface. The planner emits a question only for a missing "
        "decision-critical dimension or a recorded critical contradiction: "
        "non-critical needs are recorded and deferred, duplicate needs on one "
        "dimension merge to a single stable question, question order is canonical "
        "rather than input order, and a known fact or an answered prior question "
        "resolves its dimension while an open prior is held pending rather than "
        "re-emitted and a prior blocker is sticky for the same revision. Every "
        "contradiction is recorded verbatim; a critical unresolved one routes a "
        "question and an accepted-as-blocker one sets an explicit blocker that is "
        "never silently downgraded. Nine finding codes each name an exact refusal "
        ...
        "PRIOR_QUESTION_STATE_INVALID) and nothing scores, ranks, selects, "
        "promotes or evaluates a candidate. The two required suites are green "
        "(interview_readiness_test 19/19, no_repeat_question_test 17/17), ruff "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        ...
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. I01 seals a bounded "
        "interview planner and contradiction recorder only: it does not compile "
        "insight, falsifier or scope framing (I02 territory), does not score, "
        "rank, select, promote or evaluate any candidate, and refuses adversarial "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("I01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"I01 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("I01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("I01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"I01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("I01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("I01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("I01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored I01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"I01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("I01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"I01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected I01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with I01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored I01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
