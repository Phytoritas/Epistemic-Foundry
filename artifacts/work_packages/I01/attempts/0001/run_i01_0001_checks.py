#!/usr/bin/env python3
"""Run the I01-0001 bounded-Interview and contradiction-scan checks.

Required checks from manifests/development_manifest.yaml (I01):
``interview_readiness_test`` and ``no_repeat_question_test``.  Each maps to one
component test module under ``python/epistemic_foundry/intake/interview``:
``interview_readiness_test`` -> ``test_interview_readiness.py`` and
``no_repeat_question_test`` -> ``test_no_repeat_question.py``.  Together the two
...
``interview`` package marker, and lies outside the repository ``testpaths``
...
I01 depends on C04 and F04 at the manifest layer only; the engine imports the
...
ATTEMPT_ID = "I01-0001"
COMPONENT = "python/epistemic_foundry/intake/interview"
...
def interview_readiness_test() -> int:
    return _pytest(
        "interview-readiness-test", f"{COMPONENT}/test_interview_readiness.py"
    )


def no_repeat_question_test() -> int:
    return _pytest(
        "no-repeat-question-test", f"{COMPONENT}/test_no_repeat_question.py"
    )
...
def main() -> int:
    ...
    checks = {
        "interview-readiness-test": interview_readiness_test,
        "no-repeat-question-test": no_repeat_question_test,
