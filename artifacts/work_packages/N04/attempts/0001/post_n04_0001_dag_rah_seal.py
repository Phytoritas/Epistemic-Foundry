#!/usr/bin/env python3
"""Append the verified post-N04-0001 DAG reconciliation to active RAH state."""
...
import n04_0001_rah_seal as n04_seal  # noqa: E402
import post_n04_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000077-41e5ef8e"
EXPECTED_PARENT_MANIFEST_HASH = (
    "b8c59e3010da25e50bb91d80a399843d42c2156787c6f2a88c5b27d043f1d0fa"
)
EXPECTED_EVIDENCE_ID = "E0078"
NEXT_ACTIONS = [
    "Execute O01-0001 QueryPlan and SearchLaneReceipt contracts under its exact manifest scope.",
    "Run query_plan_test and receipt_completeness_test with dependency regressions and separate primary-session review.",
    "Seal O01 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-n04-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 61
        and stored.get("waiting_package_count") == 91
        and stored.get("ready_packages_manifest_order") == ["O01", "T01", "W01", "A06"]
        and stored.get("next_package") == "O01"
        ...
        and stored.get("attempt_binding", {}).get("N04_final_generation_manifest_sha256")
        == "sha256:" + EXPECTED_PARENT_MANIFEST_HASH
    ):
        raise SystemExit("DAG reconciliation does not identify the expected next step")
    review_path = ATTEMPT / "post-n04-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_n04_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-N04-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "N04-0001 final evidence E0077 / 000077-41e5ef8e and generation manifest "
        ...
        "no unknown dependency and no cycle; 61 packages are PASS, 4 are "
        "dependency-ready, and 91 wait on unmet dependencies. Manifest-order READY "
        "is O01, T01, W01, A06, so O01 is next. Highest numeric attempt selection "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed N04 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0077" or len(base.numbered_generations(ralph_root)) != 77:
        raise SystemExit("post-N04 DAG seal requires exact E0077 / 77-generation state")
    ...
    note = (
        "The sealed post-N04 DAG selects O01 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0077":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0078 summary differs from current DAG artifacts")
    result = n04_seal.verify_generation_store(78)
    if result["latest_evidence_id"] != EXPECTED_EVIDENCE_ID:
        raise SystemExit("DAG evidence is not the live RAH tail")
    result["generation"] = generation
    result["next_package"] = "O01"
