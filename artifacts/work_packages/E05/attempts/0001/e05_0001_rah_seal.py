#!/usr/bin/env python3
"""Append E05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "E05-0001"
WORK_PACKAGE_ID = "E05"
EXPECTED_PARENT = "000162-de9126ea"
EXPECTED_PARENT_COUNT = 162
EXPECTED_PARENT_EVIDENCE = "E0164"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "bb513bd4ea8d7636924a1534da453e854947b9a36d6662cc17adc5744c111dec"
)
CORE_EVIDENCE_ID = "E0165"
FINAL_EVIDENCE_ID = "E0166"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the F05 EVOLVE subprotocol state machine, return edges and typed stop "
    "certificates on the sealed C05/D05/E05 state.",
    "Continue the x05 wave toward G05/L05/M05/S05 and the U01/V01/X01 "
    "integration layer.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E05-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "E05-0001 PASS core verification: the candidate action/effect, mutation "
        "receipt and count-reconciliation engine is implemented under "
        "src/epistemic_foundry/effects/v4_e05, under exact A06-0002, C05-0001 "
        "and E04-0001 dependency bindings, a D05-0001 regression baseline, and "
        "the recorded scope decision HD-EF4-E05-SCOPE-20260802-001 authorizing "
        "the single package marker outside the manifest grant. The engine adds "
        "the reconciliation nobody performed: the chamber accounts for candidate "
        "identities and the ledger mints receipts, but nothing checked the two "
        "against each other, which is where a side effect hides. Six failure "
        "classes are refused by name — orphan side effect, missing mutation "
        "receipt, mutation without provenance, dangling effect reference, "
        "unresolved effect, and ledger/pipeline disagreement — and a fan-out "
        "that does not reconcile is reported before its ledger consequences. "
        "Nothing is reimplemented: candidate accounting, receipt minting and "
        "the effect-status vocabulary are all imported from the modules that "
        "own them, and every receipt in the tests is minted by the runtime's "
        "own validated builders. EF4-I22 is enforced on this module by the "
        "repository's own gate running as a named check; it caught a real "
        "violation during this attempt and the duplicated count was removed "
        "rather than the module being registered as an owner it is not. The "
        "status-to-disposition table is data, verified on every use to cover "
        "the imported vocabulary exactly, so a new status fails as "
        "DISPOSITION_DRIFT. A named packaging-discovery check proves the engine "
        "stays discoverable by the project's own configuration. Ruff "
        "lint/format, schema_and_type_check 18/18, unit_and_contract 13/13, "
        "negative_and_adversarial 20/20, provenance_and_receipt 15/15, targeted "
        "66/66, wire-literal discipline 5/5, chamber regression 57/57, E04 "
        "replay regression 18/18, full Python 1261/1261, full Node 982/982 "
        "across 91 files, and git diff --check pass. "
        ...
        "F05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"E05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". D05-0001 and every prior RAH generation remain immutable. The "
        "engine reconciles ledgers that are handed to it; collecting them from "
        "a running system and wiring the engine into the EVOLVE loop belongs to "
        "F05. An UNKNOWN effect is reported as unresolved rather than resolved, "
        "because resolving it needs the external system this engine "
        "deliberately does not touch. The composed chamber and ledger modules "
        "are read-only inputs and were not modified. F05, G05, L05, M05, S05, "
        "Q04, U01, V01, X01, source coverage, PRD, review, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E05-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "E05-0001 evolution genome, evaluator, archive, statistics and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E05-0001 parent is not the sealed E05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("E05 preflight requires exactly 162 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("E05 core did not append exactly E0165")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("E05 final did not append exactly E0166")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("E05 core tail is not E0165")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 164:
        raise SystemExit(f"unexpected E05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
