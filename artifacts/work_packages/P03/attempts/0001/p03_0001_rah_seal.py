#!/usr/bin/env python3
"""Append P03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "P03-0001"
WORK_PACKAGE_ID = "P03"
EXPECTED_PARENT = "000142-62fd3dcf"
EXPECTED_PARENT_COUNT = 142
EXPECTED_PARENT_EVIDENCE = "E0144"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "8e3564c69d342e9d6fa67c910d2d885194608eb3525cf70c7c7fbb6867d70bae"
)
CORE_EVIDENCE_ID = "E0145"
FINAL_EVIDENCE_ID = "E0146"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_p03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Bind the T02 mutation ports to the kernel stores in T04.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required P03-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "P03-0001 PASS core verification: cross-examination and Minority "
        "Report are implemented in "
        "python/epistemic_foundry/parliament/cross_exam with an exact P01-0001 "
        "dependency and a P02-0001 regression baseline. Both vocabularies are "
        "read from schemas/cross-examination.schema.json and "
        "schemas/minority-report.schema.json rather than restated. Every "
        "attack must name a brief this round carries, an assertion inside it, "
        "and evidence its own P01 context contained; citing nothing, citing "
        "withheld evidence, attacking one's own brief, or attacking without a "
        "context is refused, so cross-examination cannot become a side channel "
        "around the Evidence ACL. An unanswered challenge stays open and "
        "carries no response, and the sealed round lists its open challenges. "
        "The strongest dissent is the report with the greatest expected "
        "information gain, ties broken deterministically; it must survive as "
        "required or preserved, may be superseded only by cited new evidence, "
        "and dropping, renaming, or stripping the evidence from it in a "
        "rehashed round fails closed because the strongest is recomputed. "
        "Every report is retained whatever its status. Cross-examination "
        "cannot run in the blind first round. Ruff lint/format, "
        "cross_exam_grounding_test 23/23, minority_preservation_test 20/20, "
        "targeted 43/43, blind regression 37/37, auditors regression 42/42, "
        "full Python 1261/1261, full Node 982/982 across 91 files, and git "
        "diff --check pass. "
        ...
        "T04-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"P03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". P02-0001, P01-0001, and every prior RAH generation remain "
        "immutable. P03 checks that a challenge is grounded, not that it is "
        "correct; challenges and dissents arrive as inputs rather than being "
        "generated; the unresolved test a dissent names is not executed; and "
        "adjudication belongs to the Parliament verdict. The P-phase x01-x03 "
        "packages are PASS, but T04, B05, C05, R05, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("P03-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "P03-0001 cross-examination and Minority Report is evidence-sealed "
        "PASS. T04-0001 is next; implementation_gate=fail and "
        "completion_ready=false."
    )
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P03-0001 parent is not the sealed P02-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("P03 preflight requires exactly 142 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("P03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("P03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("P03 core did not append exactly E0145")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("P03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("P03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("P03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored P03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("P03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("P03 final did not append exactly E0146")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("P03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("P03 core tail is not E0145")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 144:
        raise SystemExit(f"unexpected P03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with P03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored P03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
