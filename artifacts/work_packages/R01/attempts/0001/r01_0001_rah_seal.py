#!/usr/bin/env python3
"""Append R01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "R01-0001"
WORK_PACKAGE_ID = "R01"
EXPECTED_PARENT = "000126-6faec503"
EXPECTED_PARENT_COUNT = 126
EXPECTED_PARENT_EVIDENCE = "E0128"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "65f7258f6610b7dab03795732fa697062fecc8e410dce51b3acc5bf13185b85e"
)
CORE_EVIDENCE_ID = "E0129"
FINAL_EVIDENCE_ID = "E0130"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_r01_0001_evidence as evidence  # noqa: E402
...
NEXT_ACTIONS = [
    ...
    "Build R02 and R03 on the sealed R01 synthesis before the R04 identification gate.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required R01-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "R01-0001 PASS core verification: inductive synthesis and heterogeneity "
        "are implemented in python/epistemic_foundry/reasoning/induction with "
        "exact C04-0004 and O04-0001 dependency bindings and a T02-0001 "
        "regression baseline. Direction is weighted by the O03 "
        "dependency-adjusted independence of each finding rather than head "
        "count, and the recomputed effective independent count must equal the "
        "pack's own; the same weights scale the inverse-variance statistics so "
        "dependent replications cannot inflate precision. Heterogeneity is "
        "independence-weighted Cochran Q, I-squared, and DerSimonian-Laird "
        "tau-squared, banded with a strict upper bound so a boundary resolves "
        "into the more cautious band, and insufficient data is UNDETERMINED "
        "with a stated reason rather than LOW. Every observed moderator and "
        "level is retained whatever its status, a declared null without a "
        "finding fails closed, and the synthesis reports relation_kind "
        "ASSOCIATION with causal_identification NOT_ASSESSED. Ruff "
        "lint/format, induction_fixture_test 23/23, heterogeneity_test 21/21, "
        "targeted 44/44, dependency component regression 63/63, full Python "
        "1261/1261, full Node 903/903 across 87 files, and git diff --check "
        "pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"R01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". T02-0001, O04-0001, C04-0004, and every prior RAH generation "
        "remain immutable. R01 consumes findings supplied by a caller and does "
        "not extract them from sources; moderator discrimination is a "
        "direction-level screen, not a between-group significance test; causal "
        "identification remains R04. T03, R02, R03, the x05 wave, source "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("R01-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "R01-0001 inductive synthesis and heterogeneity is evidence-sealed "
        "PASS. T03-0001 is next; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R01-0001 parent is not the sealed T02-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("R01 preflight requires exactly 126 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("R01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("R01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("R01 core did not append exactly E0129")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("R01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("R01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("R01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored R01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("R01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("R01 final did not append exactly E0130")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("R01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("R01 core tail is not E0129")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 128:
        raise SystemExit(f"unexpected R01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with R01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored R01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
