#!/usr/bin/env python3
"""Append P01-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "P01-0001"
WORK_PACKAGE_ID = "P01"
EXPECTED_PARENT = "000134-1157aab2"
EXPECTED_PARENT_COUNT = 134
EXPECTED_PARENT_EVIDENCE = "E0136"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "51801252053b6addf533c02b2e41ac79ba1d8e0250d807f9c4af279e4d72f0ac"
)
CORE_EVIDENCE_ID = "E0137"
FINAL_EVIDENCE_ID = "E0138"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_p01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Implement T03 stable CLI JSON/error and PATH-less surfaces on the sealed T01/T02 contracts.",
    "Build P02 cross-examination on the sealed P01 blind dispatch.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required P01-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "P01-0001 PASS core verification: blind independent briefs and "
        "asymmetric dispatch are implemented in "
        "python/epistemic_foundry/parliament/blind with exact N04-0001, "
        "O04-0001, and R04-0001 dependency bindings and an R04-0001 regression "
        "baseline. Each role's Evidence ACL is read from "
        "manifests/role_registry.yaml rather than restated, an unregistered "
        "role fails closed, the assembled context carries only permitted "
        "classes, and withheld ids and per-class counts are named so their "
        "existence stays visible without being readable. A brief citing "
        "withheld evidence, evidence no context ever held, another role's "
        "context, or no falsifying condition is refused. First-round isolation "
        "is measured over every ordered pair of briefs: one cross-reference, a "
        "shared context manifest, or a non-blind brief fails the seal, and a "
        "sealed report that does not cover every brief is rejected as "
        "unmeasured. A panel briefed identically is refused as one opinion "
        "repeated. Fixtures validate against "
        "schemas/council-brief.schema.json. Ruff lint/format, blindness_test "
        "16/16, evidence_acl_test 21/21, targeted 37/37, causal regression "
        "45/45, completeness regression 18/18, full Python 1261/1261, full "
        "Node 903/903 across 87 files, and git diff --check pass. "
        ...
        "T03-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"P01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". R04-0001 and every prior RAH generation remain immutable. P01 "
        "assembles contexts and checks what briefs cite; it does not generate "
        "briefs and cannot enforce the ACL inside a model that already holds "
        "the context, so collusion outside the recorded artifacts is invisible "
        "to it. Cross-examination, adjudication, and minority reports are "
        "later P-phase packages. Note: the live implementation-gate note "
        "carried a stale R03 label after the R04 seal and is corrected here; "
        "the sealed R04 evidence summary itself named R04 correctly. T03, R05, "
        "the x05 wave, source coverage, PRD, review, and global closeout "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("P01-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "P01-0001 blind independent briefs and asymmetric dispatch is "
        "evidence-sealed PASS. T03-0001 is next; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P01-0001 parent is not the sealed R04-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("P01 preflight requires exactly 134 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("P01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("P01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("P01 core did not append exactly E0137")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("P01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("P01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("P01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored P01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("P01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("P01 final did not append exactly E0138")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("P01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("P01 core tail is not E0137")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 136:
        raise SystemExit(f"unexpected P01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with P01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored P01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
