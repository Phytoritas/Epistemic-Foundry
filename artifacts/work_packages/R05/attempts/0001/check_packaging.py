#!/usr/bin/env python3
"""Prove the R05 operator registry is discoverable by the packaging config.

``src/epistemic_foundry/reasoning`` is a new package marker authorized by the
R05 write scope in ``manifests/development_manifest.yaml``, so there is a scope
decision to prove rather than assume: a marker that never reached the wheel
would let the registry import from a checkout and vanish from an installed
...
EXPECTED = ("epistemic_foundry.reasoning", "epistemic_foundry.reasoning.v4_r05")
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the registry would import from a checkout but be "
