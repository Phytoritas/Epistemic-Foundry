#!/usr/bin/env python3
"""Append R05-0001 PASS evidence while global completion remains false.
...
read the live ledger to pin the parent generation identifier or its manifest
hash the way the R04/H05 templates do.  ``EXPECTED_PARENT`` and
``EXPECTED_PARENT_MANIFEST_SHA256`` are therefore left as the sentinel
``FILL_FROM_LEDGER`` and the seal refuses to run until the parent fills them
from the live ledger tail (the generation committed by the X01-0001 final
closeout, evidence ``E0204``).  ``EXPECTED_PARENT_COUNT`` is set to 202 on the
assumption that X01-0001's final closeout is the current ledger tail (X01 core
generation ``000201`` + one final generation); verify it against
``.rah/ralph/generations`` before running and adjust if the frontier moved.
Everything else — evidence IDs, summaries, hashes, flat-snapshot verification —
is derived exactly as the templates do.
"""
...
#: Sentinel for the two pins that can only be read from the live ledger tail.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
#: The generation the X01-0001 final closeout committed (its evidence is E0204).
#: Fill from `.rah/ralph` current pointer before running, e.g. "000202-XXXXXXXX".
EXPECTED_PARENT = "000202-fc002a63"
EXPECTED_PARENT_COUNT = 202
EXPECTED_PARENT_EVIDENCE = "E0204"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "ed1d2be854f577504cbc27dac4faa8ca80344500b5376e3866674ae679bc7078"
)
CORE_EVIDENCE_ID = "E0205"
FINAL_EVIDENCE_ID = "E0206"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_r05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after R05-0001 seals and select the next "
...
GATE_NOTE = (
    "R05-0001 scientific mutation, typed crossover, mechanism and Aporia "
    "operators is evidence-sealed PASS (E0205/E0206). The next package is "
    "RECOMPUTE_DAG; implementation_gate=fail and completion_ready=false."
)


def assert_parent_pins() -> None:
    if PARENT_PIN_SENTINEL in (EXPECTED_PARENT, EXPECTED_PARENT_MANIFEST_SHA256):
        raise SystemExit(
            "R05-0001 seal is not pinned: set EXPECTED_PARENT and "
            "EXPECTED_PARENT_MANIFEST_SHA256 from the live ledger tail (the "
            "generation committed by the X01-0001 final closeout, evidence "
            "E0204) before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required R05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/reasoning").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "R05-0001 PASS core verification: scientific mutation, typed crossover, "
        "mechanism and Aporia operators, implemented in "
        "src/epistemic_foundry/reasoning/v4_r05 by the primary session across "
        "bounded turns and reviewed by an independent contract-reviewer subagent "
        ...
        "bindings (R04 R04-0001, I05 I05-0001, C05 C05-0001) and an X01-0001 "
        "regression baseline. Each operator declares only genes the canonical genome "
        "schema still lists, targets only a kind the sealed C05 index still "
        "marks mutable through the I05 intake surface, and declares one of the "
        "Aporia Engine's two edge partitions. Four refusals hold: an undeclared "
        "gene, a touch on identity/lineage/authority (refused by path), a "
        "crossover whose parents disagree on kind or mechanism, and an Aporia "
        "operator citing no open question. Every application re-derives byte "
        "for byte from its own published fields with no clock or random draw, "
        "and nothing scores, selects, promotes or evaluates. Ruff lint/format, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, R04/I05/C05 dependency "
        "regressions, full Python and full Node suites, and git diff --check "
        "all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"R05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. R05 proposes typed "
        "variants and records replayable lineage only: it does not score, "
        "select, promote or evaluate any candidate, makes no DSSAT or "
        "plant-model parity claim, and promotion remains a governance "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("R05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R05-0001 parent is not the sealed X01-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"R05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("R05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("R05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"R05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("R05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("R05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("R05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored R05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("R05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"R05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("R05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"R05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected R05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with R05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored R05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
