#!/usr/bin/env python3
"""Run the R05-0001 scientific-operator registry acceptance checks.

Required checks from manifests/development_manifest.yaml (R05):
...
registry ships under src/epistemic_foundry/reasoning/v4_r05 behind a new
``reasoning`` package marker, so ``packaging-discovery`` proves the marker
actually reaches the wheel rather than only importing from the checkout.

The dependency regressions cover the sealed surfaces this package composes:
R04's argument/causal reasoning tree, whose Aporia edge partitions and
open-question view the operators read; I05's intake surface, whose sealed
mutable genome kind an operator's genome kind is validated against; and C05's
schema bundle, whose canonical genome schema an operator's declared gene fields
are checked against.  Each is exercised through the dependency's own sealed
...
REASONING = "src/epistemic_foundry/reasoning"
PRODUCT_TESTS = {
...
# R04 owns the argument/causal reasoning tree whose Aporia edge partitions and
# open-question view the operators read; I05 owns the intake surface whose
# sealed mutable genome kind is enforced; C05 owns the schema bundle whose
# canonical genome schema the declared gene fields are checked against.  A
# change in any of the three must fail here, so each runs its own sealed suite.
DEPENDENCY_TARGETS = {
    "dependency-regression-r04": ("python/epistemic_foundry/reasoning/causal",),
...
def ruff_check() -> int:
    return run(
        "ruff-check", ["uv", "run", "--locked", "ruff", "check", REASONING, ATTEMPT_DIR]
    )
...
def ruff_format_check() -> int:
    return run(
        "ruff-format-check",
        ["uv", "run", "--locked", "ruff", "format", "--check", REASONING, ATTEMPT_DIR],
    )


def packaging_discovery() -> int:
    # src/epistemic_foundry/reasoning is a new package marker, so the wheel
...
def targeted() -> int:
    return _pytest("targeted-r05-operators", ATTEMPT_DIR)


def dependency_regression_r04() -> int:
    return _pytest(
        "dependency-regression-r04", *DEPENDENCY_TARGETS["dependency-regression-r04"]
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "targeted": targeted,
        "dependency-regression-r04": dependency_regression_r04,
