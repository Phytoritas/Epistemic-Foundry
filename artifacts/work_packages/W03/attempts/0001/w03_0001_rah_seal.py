#!/usr/bin/env python3
"""Append W03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "W03-0001"
WORK_PACKAGE_ID = "W03"
EXPECTED_PARENT = "000122-3b01a7cc"
EXPECTED_PARENT_COUNT = 122
EXPECTED_PARENT_EVIDENCE = "E0124"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "000122867d50f010f4d4d0b6a59fb585f1f6caf7afcb5e970d3b0170b5afc07a"
)
CORE_EVIDENCE_ID = "E0125"
FINAL_EVIDENCE_ID = "E0126"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_w03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Implement T02 MCP mutating tools using the delivered Pro architecture recommendation.",
    "Record the narrow product-owner scope extension the T02 design requires before writing contracts.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required W03-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "W03-0001 PASS core verification: evidence-update propagation, "
        "staleness, and reassessment are implemented in "
        "python/epistemic_foundry/reassessment with exact W01-0001 dependency "
        "and W02-0001 regression baseline bindings. The blast radius is the "
        "full transitive dependent closure rather than one hop and terminates "
        "on provenance cycles; retraction voids reached Passports while "
        "correction leaves them stale and a new document prompts without "
        "invalidating; every affected Passport carries an explicit state, can "
        "never stay FRESH, and is marked into a new revision bound to the "
        "plan id, plan hash, and trigger event; an invalidating trigger whose "
        "only action is no_action fails closed. Ruff lint/format, targeted "
        "18/18, full Python 1196/1196, full Node 887/887 across 86 files, and "
        "git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"W03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". W02-0001, W01-0001, and every prior RAH generation remain "
        "immutable. The W-phase x01-x03 packages are PASS, but T02, T03, the "
        "x05 wave, source coverage, PRD, review, and global closeout remain; "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("W03-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "W03-0001 evidence-update staleness and reassessment is "
        "evidence-sealed PASS. T02-0001 is next; implementation_gate=fail "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("W03-0001 parent is not the sealed W02-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("W03 preflight requires exactly 122 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("W03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("W03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("W03 core did not append exactly E0125")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("W03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("W03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("W03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored W03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("W03 final did not append exactly E0126")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("W03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("W03 core tail is not E0125")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 124:
        raise SystemExit(f"unexpected W03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with W03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored W03 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
