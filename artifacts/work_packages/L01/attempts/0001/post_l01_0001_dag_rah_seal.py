#!/usr/bin/env python3
"""Append the verified post-L01-0001 DAG reconciliation to active RAH state."""
...
import l01_0001_rah_seal as l01_seal  # noqa: E402
import post_l01_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000042-4d990eef"
EXPECTED_EVIDENCE_ID = "E0043"
NEXT_ACTIONS = [
    "Execute L02-0001 memory indexing and scoped retrieval under its exact manifest scope.",
    "Run L02 required checks, full regressions, and a separate primary-session review.",
    "Seal L02 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-l01-0001-dag-reconciliation.json"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_l01_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-L01-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "L01-0001 final evidence E0042 / 000042-4d990eef. The active development "
        "manifest has 156 unique packages, no unknown dependency and no cycle; "
        "50 packages are currently PASS, 6 are dependency-ready, and 100 wait on "
        "unmet dependencies. Manifest-order READY is L02, L03, M01, N01, T01, "
        "A06, so L02 is the next bounded package. Package state is selected from "
        "the highest numeric attempt, preventing an older PASS from hiding a "
        f"newer non-PASS or incomplete attempt. DAG sha256:{hashes['dag']}; review "
...
def commit(summary: str) -> str:
    ralph_root, parent, payloads = l01_seal.current_state()
    if parent != EXPECTED_PARENT:
        raise SystemExit(f"unexpected RAH parent {parent}; expected {EXPECTED_PARENT}")
    identifiers = l01_seal.evidence_ids(payloads)
    if identifiers[-1] != "E0042" or len(l01_seal.numbered_generations(ralph_root)) != 42:
        raise SystemExit("post-L01 DAG seal requires exact E0042 / 42-generation state")
    ...
    note = (
        "The sealed post-L01 DAG selects L02 as the earliest dependency-ready "
...
def main() -> int:
    ...
    ralph_root, current, payloads = l01_seal.current_state()
    identifiers = l01_seal.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0042":
        before = l01_seal.numbered_generations(ralph_root)
        generation = commit(summary)
        after = l01_seal.numbered_generations(ralph_root)
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0043 summary differs from current DAG artifacts")
    result = l01_seal.verify_generation_store(43)
