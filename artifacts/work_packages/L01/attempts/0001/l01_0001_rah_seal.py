#!/usr/bin/env python3
"""Append L01-0001 core and closeout evidence to the active RAH state."""
...
import build_l01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_INITIAL_PARENT = "000040-d5e83580"
EXPECTED_INITIAL_EVIDENCE = "E0040"
EXPECTED_INITIAL_GENERATION_COUNT = 40
NEXT_ACTIONS = [
    "Recompute and seal the live 156-package DAG after L01-0001 PASS.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required L01 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "L01-0001 PASS: the deterministic admission policy uses the six closed "
        "canonical memory classes, exact-field plain objects, canonical SHA-256 "
        "seals, and immutable projections. Cross-workspace recall defaults to "
        "DENY and is limited to USER memory with policy permission, explicit "
        "opt-in, and purpose/data-class/class/policy/workspace-bound consent. "
        "Revoked, expired, retention-expired, future-dated, tampered, malformed, "
        "or scope-mismatched inputs fail closed at exact time boundaries. L01 "
        "stops at admission; L02 retains retrieval/receipt ownership and L03 "
        "retains deletion/redaction/deduplication/legal-hold ownership. Targeted "
        "L01 is 27/27 with 92.05% line, 80.89% branch, and 100% function "
        "coverage; canonical runtime schema validation is 2/2; predecessors are "
        "42/42; full Node is 503/503 over 57 files; full Python is 1064/1064; "
        "and codegen is 126/126. "
        f"Verification sha256:{hashes['consent-retention-verification.json']}; "
        f"receipt sha256:{hashes['consent-retention-verification.artifact-receipt.json']}; "
        ...
        f"{hashes['targeted-l01-node.junit.xml']}; coverage sha256:"
        f"{hashes['targeted-l01-coverage.stdout.log']}; predecessor JUnit sha256:"
        f"{hashes['predecessor-l01-node.junit.xml']}; Node JUnit sha256:"
        ...
        f"builder sha256:{hashes['build_l01_0001_evidence.py']}; sealer sha256:"
        f"{hashes['l01_0001_rah_seal.py']}. Review is primary-session separate "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"L01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"{hashes['build_l01_0001_evidence.py']}; sealer sha256:"
        f"{hashes['l01_0001_rah_seal.py']}. Every prior attempt, report, and "
        "generation is preserved. L01-0001 is immutable PASS and the live "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("L01 requires a contiguous evidence ledger")
    if not (
        ...
        raise SystemExit("L01 preflight requires active/fail/completion_ready=false")
...
def commit_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "L01-0001 memory consent and retention admission policy is "
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("L01 report is already RAH-bound; use verify")
    identifiers = evidence_ids(payloads)
    if not (
        ...
        raise SystemExit("L01 preflight is not at the sealed post-K04 DAG state")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("L01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, core_id]:
        raise SystemExit("L01 core did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("L01 core did not preserve every prior generation")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("L01 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("L01 evidence identifiers differ from the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("L01 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("L01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("L01 final did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("L01 final evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("L01 final did not preserve every prior generation")
    return {
