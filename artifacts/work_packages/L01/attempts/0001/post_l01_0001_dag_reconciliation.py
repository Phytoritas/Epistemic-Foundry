#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed L01-0001 PASS."""
...
OUTPUT = ATTEMPT / "post-l01-0001-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-l01-0001-dag-review.md"
EXPECTED_PACKAGE_COUNT = 156
EXPECTED_L01_REPORT_HASH = (
    "sha256:010ce588e3e0783012aea46ab2b4ab748cef16f83e16c46a7a47454be59ae68e"
)
...
def reconcile() -> dict[str, Any]:
    ...
    l01 = states["L01"]
    if not isinstance(l01.get("report"), str):
        raise SystemExit("L01 report is missing")
    l01_report = read_json(ROOT / l01["report"])
    rah = l01_report.get("rah_state")
    if not (
        l01["selected_attempt_id"] == "L01-0001"
        and l01["current_state"] == "PASS"
        and l01["report_sha256"] == EXPECTED_L01_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0041"
        and rah.get("core_generation") == "000041-0299bd60"
        and rah.get("final_closeout_evidence_id") == "E0042"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("L01-0001 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "L01_attempt_id": "L01-0001",
            "L01_core_evidence_id": "E0041",
            "L01_core_generation": "000041-0299bd60",
            "L01_final_closeout_evidence_id": "E0042",
            "L01_report": l01["report"],
            "L01_report_sha256": l01["report_sha256"],
            "status": "PASS",
        },
        ...
        "projection": "POST_L01_0001_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 50
        and result["blocked_package_count"] == 100
        and result["ready_packages_manifest_order"]
        == ["L02", "L03", "M01", "N01", "T01", "A06"]
        and result["next_package"] == "L02"
    ):
        raise SystemExit(
            "post-L01 DAG differs from the evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-L01-0001 live DAG reconciliation
