#!/usr/bin/env python3
"""Append H01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000336-313631db"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 336
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0338"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "a5fe051ebfffc576bad0de60946efc3e6c5142059d51170162cf355aa582376e"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0339"
FINAL_EVIDENCE_ID = "E0340"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_h01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after H01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} normalized hook gateway gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "H01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required H01-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: gateway namespace is pinned so a change to the hook gateway or its tests
    #: is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/plugin-host/src/hooks/gateway").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "H01-0001 PASS core verification: the normalized hook gateway, implemented "
        "in packages/plugin-host/src/hooks/gateway by a bounded implementation "
        "agent and reviewed by this seal-prep session (a distinct actor, "
        "actor-independent from the author) on the sealed E04-0001, G04-0001, and "
        "S02-0001 attempts with no new production dependency. The raw host payload "
        "is asserted canonical JSON before the decision callback runs -- cycles, "
        "sparse or accessor-backed arrays, Proxies, BigInt, NaN, -0, Infinity, lone "
        "surrogates, and prototype-polluted or symbol-keyed objects fail closed "
        "with NON_CANONICAL_JSON without invoking the callback or any accessor or "
        "Proxy trap -- then hashed under sha256:<hex> and cloned into a deep-frozen "
        "normalized_payload with sorted keys so insertion order changes no hash. "
        "Every emitted HookEventEnvelope carries exactly the schema required keys "
        "with additionalProperties false, canonical vocabulary, and sha256 hash "
        "patterns; tampering is rejected with HOOK_ENVELOPE_HASH_MISMATCH or "
        "HOOK_ENVELOPE_INVALID. timeout_ms is validated positive and platform "
        "bounded before the callback; a non-settling callback times out via "
        "Promise.race and AbortController.abort to a sealed ERROR "
        "(HOOK_DECISION_TIMEOUT), a rejecting callback to "
        "HOOK_DECISION_CALLBACK_ERROR without leaking its message, and invalid "
        "callback output to HOOK_DECISION_INVALID -- never coerced to ALLOW, and a "
        "late resolution cannot mutate the frozen result. Ruff lint/format, "
        "hook_schema_fixture_test 6/6, hook_timeout_test 5/5, targeted 11/11, full "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"H01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". E04-0001, G04-0001, S02-0001 and every prior RAH generation remain "
        "immutable. H01 provides the normalized hook gateway and event envelopes; "
        "H02/H03 host-specific response mapping and lifecycle/tool hook "
        "integration, exhaustive enforcement, source coverage, PRD, review, and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("H01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"H01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("H01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("H01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"H01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("H01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("H01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("H01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored H01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("H01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"H01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("H01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"H01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected H01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with H01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored H01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
