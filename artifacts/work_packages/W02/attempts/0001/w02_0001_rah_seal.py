#!/usr/bin/env python3
"""Append W02-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "W02-0001"
WORK_PACKAGE_ID = "W02"
EXPECTED_PARENT = "000120-6393ffc8"
EXPECTED_PARENT_COUNT = 120
EXPECTED_PARENT_EVIDENCE = "E0122"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "fb77ab922978a62120ebdd975c52cd9d1c8bbc17a29c3732eef17ac6190c6183"
)
CORE_EVIDENCE_ID = "E0123"
FINAL_EVIDENCE_ID = "E0124"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_w02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run W03-0001 evidence-update staleness and reassessment next.",
    "Fold the pending Pro orchestrator recommendation into T02 when it lands.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required W02-0001 artifact missing: {name}")
...
def core_summary() -> str:
    names = (
        ...
        "w02-verification.json",
        "write-scope-verification.json",
        "node-test-inventory.json",
        "junit-normalization-verification.json",
        "targeted-runtime-node.junit.xml",
        "full-python-suite.junit.xml",
        "full-node-suite.junit.xml",
        "review.md",
        "run_w02_0001_checks.py",
        "build_w02_0001_evidence.py",
        "w02_0001_rah_seal.py",
    )
    hashes = artifact_hashes(names)
    return (
        "W02-0001 PASS core verification: the checkpoint, pause, resume, and "
        "cancellation runtime is implemented in "
        "packages/foundry-kernel/src/workflows/runtime with exact W01-0001 "
        "dependency and O04-0001 regression baseline bindings. "
        "replay_verified is computed by replaying the sealed command log into "
        "a fresh scheduler and matching state hashes, not asserted; resume "
        "requires an APPROVE review by a non-author bound to the exact "
        "checkpoint hash plus exact replay identity; truncated, reordered, "
        "and unverified logs fail closed; cancellation reaches CANCELLED only "
        "when every LEASED/RUNNING/RECONCILING effect resolves to a terminal "
        "receipt, and a missing or UNKNOWN receipt yields "
        "CANCELLED_WITH_UNRESOLVED_EFFECTS with the exact unresolved list. "
        "Repo structure/boundaries, targeted Node 27/27, full Python "
        "1196/1196, full Node 887/887 across 86 files, and git diff --check "
        "pass. "
        ...
        "W03-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"W02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". O04-0001, W01-0001, and every prior RAH generation remain "
        "immutable. W02 is PASS, but W03, T02, T03, the x05 wave, source "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("W02-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "W02-0001 checkpoint/pause/resume/cancellation runtime is "
        "evidence-sealed PASS. W03-0001 is next; implementation_gate=fail "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("W02-0001 parent is not the sealed O04-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("W02 preflight requires exactly 120 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("W02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("W02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("W02 core did not append exactly E0123")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("W02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("W02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("W02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored W02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("W02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("W02 final did not append exactly E0124")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("W02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("W02 core tail is not E0123")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 122:
        raise SystemExit(f"unexpected W02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with W02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored W02 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
