#!/usr/bin/env python3
"""Build and verify W02-0001 checkpoint/pause/resume/cancel evidence.

W02-0001 implements `packages/foundry-kernel/src/workflows/runtime/**`: the
checkpoint, pause, resume, and cancellation runtime whose `replay_verified`
is computed by actually replaying the sealed command log, whose resume
requires an independent approved review plus exact replay identity, and whose
cancellation is complete only when every pending effect resolves to a
terminal receipt.  This builder verifies the executed checks and emits the
...
ATTEMPT_ID = "W02-0001"
WORK_PACKAGE_ID = "W02"
RECORDED_AT = "2026-08-01T03:20:00.000Z"

EXPECTED_TARGETED_NODE_COUNT = 27
...
EXPECTED_PRODUCT_HASHES = {
    f"{COMPONENT}/checkpoint-runtime.mjs": (
        "2a15fd024955fb9fde61b012c258af852a1dd4d8bc40f9ce14a87e023eda229c"
    ),
    f"{COMPONENT}/checkpoint-resume.test.mjs": (
        "3c1ff433e7ab1fb2de8d77abbe4638eb4310f512ff7aa4231a1d71afb46edb2d"
    ),
    f"{COMPONENT}/cancellation.test.mjs": (
        "620d163d1d39efb078ad68d85ecb8ab6f3e12b3e886d7b8c9a0e9b24c000d9b7"
    ),
}
EXPECTED_DEPENDENCY_HASHES = {
    ...
    "artifacts/work_packages/O04/attempts/0001/report.json": (
        "2fd0059a9e1bd66d383168472a644386c299f870e6815e7df291391005f79f0f"
    ),
}

JUNIT_PATHS = {
    "targeted_runtime_node": ATTEMPT / "targeted-runtime-node.junit.xml",
    "full_python": ATTEMPT / "full-python-suite.junit.xml",
    "full_node": ATTEMPT / "full-node-suite.junit.xml",
}
_NODE_JUNITS = frozenset({"targeted_runtime_node", "full_node"})
RUN_RESULTS = (
    "repo-structure",
    "repo-boundaries",
...
def regression_evidence() -> dict[str, Any]:
    targeted = node_summary(JUNIT_PATHS["targeted_runtime_node"])
    ...
    if (
        ...
        raise SystemExit(f"targeted W02 gate failed: {targeted}")
    if (
        python["collected"],
        python["passed"],
        python["failed"],
    ...
    return {
        "attempt_id": ATTEMPT_ID,
        "baseline_attempt": "O04-0001",
        "full_node": node,
        "full_python": python,
        "new_failure_count": 0,
        "prior_baseline_counts": {"full_node": 860, "full_python": 1196},
        "status": "PASS",
        "targeted_runtime_node": targeted,
...
def dependency_status() -> dict[str, Any]:
    ...
    baseline_path = ROOT / "artifacts/work_packages/O04/attempts/0001/report.json"
    baseline = read_json(baseline_path)
    rah = baseline.get("rah_state")
    if (
        baseline.get("status") != "PASS"
        or not isinstance(rah, dict)
        or rah.get("core_evidence_id") != "E0121"
        or rah.get("final_closeout_evidence_id") != "E0122"
    ):
        raise SystemExit("O04-0001 regression baseline is not the sealed PASS attempt")
    return {
        "attempt_id": ATTEMPT_ID,
        "dependencies": {
            "W01": {
        ...
        "next_action": "SEAL_W02_0001_THEN_CONTINUE_DAG",
        "regression_baseline": {
            "attempt_id": "O04-0001",
            "core_evidence_id": "E0121",
            "final_closeout_evidence_id": "E0122",
            "report": "artifacts/work_packages/O04/attempts/0001/report.json",
...
def write_scope_verification() -> dict[str, Any]:
    ...
    if component_files != sorted(EXPECTED_PRODUCT_HASHES):
        raise SystemExit(f"runtime component holds unexpected files: {component_files}")
    return {
        "approved_scope": [f"{COMPONENT}/**", "artifacts/work_packages/W02/**"],
        "attempt_id": ATTEMPT_ID,
...
def w02_verification(regression: dict[str, Any]) -> dict[str, Any]:
    return {
        "attempt_id": ATTEMPT_ID,
        "exit_criteria": {
            "cancellation_reconciles_effects": {
                "evidence": [f"{COMPONENT}/cancellation.test.mjs"],
                "status": "PASS",
            },
            "resume_from_reviewed_checkpoint": {
                "evidence": [f"{COMPONENT}/checkpoint-resume.test.mjs"],
                "status": "PASS",
            },
        },
        "gate_semantics": {
            ...
            "checkpoint_replay_verified_is_computed": True,
            "resume_requires_independent_approved_review": True,
            "unresolved_effect_states": ["LEASED", "RUNNING", "RECONCILING"],
        },
        "required_checks": {
            "cancellation_test": {
                "module": f"{COMPONENT}/cancellation.test.mjs",
                "status": "PASS",
            },
            "checkpoint_resume_test": {
                "module": f"{COMPONENT}/checkpoint-resume.test.mjs",
                "status": "PASS",
            },
        },
        "status": "PASS",
        "targeted_test_count": regression["targeted_runtime_node"]["collected"],
    }
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                "artifacts/work_packages/W02/attempts/0001/build_w02_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    artifacts = [
        {
            ...
            "path": f"artifacts/work_packages/W02/attempts/0001/{name}",
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "W02_CHECKPOINT_PAUSE_RESUME_AND_CANCELLATION",
        "completion_ready": False,
        "contract_status": "CONFORMANT",
        "dependency_state": dependencies,
        "exit_criteria": {
            "cancellation_reconciles_effects": "PASS",
            "resume_from_reviewed_checkpoint": "PASS",
        },
        ...
        "next_package": "W03-0001",
        "not_claimed": [
            "durable checkpoint persistence or ledger-backed resume",
            "evidence-update staleness and reassessment (W03 scope)",
            "runtime workflow execution against live executors",
...
def build() -> dict[str, Any]:
    ...
    verification = w02_verification(regression)
    write_json("dependency-status.json", dependencies)
    write_json("write-scope-verification.json", write_scope)
    write_json("w02-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("W02-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "w02-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored W02-0001 report is not the deterministic document")
    return {
        "attempt_id": ATTEMPT_ID,
        "completion_ready": False,
        "full_node": "887/887",
