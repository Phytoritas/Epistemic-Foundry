#!/usr/bin/env python3
"""Append U04-0001 PASS evidence while global completion remains false.
...
``assert_parent_pins()`` refuses to run any mode until the parent fills every one
from the live ledger tail (the generation committed by the current latest-sealed
final closeout).  Everything else — summaries, product and artifact hashes,
flat-snapshot verification, atomic core/final commits — is derived exactly as the
sealed U02/X03/X04 templates do.
...
#: The current committed generation (e.g. "000249-54650d1f"); its retained
...
EXPECTED_PARENT = "000256-a398ce69"
EXPECTED_PARENT_COUNT = 256
EXPECTED_PARENT_EVIDENCE = "E0258"
EXPECTED_PARENT_MANIFEST_SHA256 = "b562e696688759264fb12ee6c52f11a7f82ed4e50a6fbefb9b5b0efc56f899c3"
CORE_EVIDENCE_ID = "E0259"
FINAL_EVIDENCE_ID = "E0260"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_u04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after U04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} U-phase accessibility and packaged-path parity gate is "
...
def assert_parent_pins() -> None:
    ...
    if PARENT_PIN_SENTINEL in pins:
        raise SystemExit(
            "U04-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required U04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: approved write-scope tree tests/ui/**.  Cache directories are excluded.
    cache_names = {"__pycache__", ".pytest_cache"}
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "tests/ui").rglob("*")
        if path.is_file() and not (cache_names & set(path.parts))
    )
...
def core_summary() -> str:
    ...
    return (
        "U04-0001 PASS core verification: U-phase accessibility and packaged-path "
        "parity gate, implemented in tests/ui by a bounded implementation agent "
        "under the product owner's instruction and reviewed by a separate sealing "
        "agent (actor-independent from the author), under exact U02-0001 and "
        "U03-0001 dependency bindings and an X04-0001 live latest-sealed "
        "regression baseline. The manifest declares exactly two required checks. "
        ...
        "alone, and empty results render as text; each rule refuses a broken "
        "surface. It is a deterministic property of the HTML and frozen records "
        "with no running browser and no axe engine (WCAG critical failures zero). "
        "packaged_ui_parity_test (20/20): for every view the record and rendered "
        "HTML built through the packaged export-surface barrels are byte-identical "
        "to the source-path build with a re-derivable canonical hash, every "
        "barrel re-exports exactly the source implementations and adds nothing "
        "that traces to no source module, and the packaged client route table "
        "matches the recorded route manifest; a forked or invented barrel export "
        "is refused. No running server, site, or produced Vite/bundler dist is "
        "claimed (Vite and packaged paths behave identically). full-node-suite "
        "captured GREEN at zero failures and full-python-suite GREEN; those "
        "repository-wide totals are integration-owned and moved by concurrent "
        "in-flight packages, so the frozen JUnit is the deterministic evidence. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"U04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: full "
        "WCAG 2.x conformance beyond zero critical structural failures, any "
        "running browser, DOM or axe engine, any produced Vite/bundler dist "
        "bundle, a running web server or live HTTP endpoint, or authority "
        "acquisition by any UI surface, view, test or import path. Author/"
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("U04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U04-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"U04 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("U04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("U04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"U04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("U04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("U04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("U04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored U04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("U04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"U04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("U04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"U04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected U04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with U04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored U04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
