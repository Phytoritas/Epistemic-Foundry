#!/usr/bin/env python3
"""Append T02-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "T02-0001"
WORK_PACKAGE_ID = "T02"
EXPECTED_PARENT = "000124-b2497389"
EXPECTED_PARENT_COUNT = 124
EXPECTED_PARENT_EVIDENCE = "E0126"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "9c4501d6ed7e7af3cb512502c5c65396c814feff3be2fb1923b63d7585d6d08d"
)
CORE_EVIDENCE_ID = "E0127"
FINAL_EVIDENCE_ID = "E0128"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_t02_0001_evidence as evidence  # noqa: E402
...
NEXT_ACTIONS = [
    ...
    "Bind the T02 mutation ports to the kernel stores in T04/T05 rather than widening T02.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required T02-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "T02-0001 PASS core verification: nine MUTATING_EFFECT tools are "
        "composed onto the sealed T01 read and planning surface with exact "
        "T01-0002 dependency and W03-0001 regression baseline bindings. Each "
        "mutating name is declared exactly once in contracts/mcp/t02/"
        "tool-catalog.yaml; the sealed T01 catalog, its generated descriptor "
        "projection, and both shared envelope schemas are unchanged, and "
        "tools/list composes 13 + 9 = 22 through a catalog set that holds no "
        "tool-name literal. No effect is reachable without a policy-granted "
        "capability, verified non-self approvals, and an exact-scope "
        "capability lease revalidated immediately before execution; an "
        "unresolved effect answers UNKNOWN with committed=null and "
        "reconciliation_required rather than success or a false claim that "
        "nothing happened; a dry run records the intent and a NOT_EXECUTED "
        "receipt. Ruff lint/format, mcp_effect_test 24/24, approval_test "
        "18/18, targeted 65/65, sealed T01 regression 106/106, full Python "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"T02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". W03-0001, T01-0002, and every prior RAH generation remain "
        "immutable. T02 binds its authority and evidence ports by injection "
        "only; kernel binding is T04/T05. T03, the x05 wave, source coverage, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("T02-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "T02-0001 MCP mutating tools with intents, leases, and receipts is "
        "evidence-sealed PASS. T03-0001 is next; implementation_gate=fail "
        "and completion_ready=false."
    )
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T02-0001 parent is not the sealed W03-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("T02 preflight requires exactly 124 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("T02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("T02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("T02 core did not append exactly E0127")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("T02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("T02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("T02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored T02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("T02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("T02 final did not append exactly E0128")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("T02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("T02 core tail is not E0127")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 126:
        raise SystemExit(f"unexpected T02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with T02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored T02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
