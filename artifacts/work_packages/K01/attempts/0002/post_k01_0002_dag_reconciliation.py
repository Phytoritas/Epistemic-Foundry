#!/usr/bin/env python3
"""Recompute the live 156-package DAG after sealed K01-0002 PASS.
...
OUTPUT = ATTEMPT / "post-k01-0002-dag-reconciliation.json"
REVIEW = ATTEMPT / "post-k01-0002-dag-review.md"
...
def reconcile() -> dict[str, Any]:
    ...
    k01 = states["K01"]
    if not isinstance(k01.get("report"), str):
        raise SystemExit("K01 report is missing")
    k01_report = read_json(ROOT / k01["report"])
    rah = k01_report.get("rah_state")
    if not (
        k01["selected_attempt_id"] == "K01-0002"
        and k01["current_state"] == "PASS"
        and k01["report_sha256"] == EXPECTED_K01_REPORT_HASH
        and isinstance(rah, dict)
        and rah.get("core_evidence_id") == "E0029"
        and rah.get("core_generation") == "000029-4b4d41db"
        and rah.get("final_closeout_evidence_id") == "E0030"
        and rah.get("completion_ready") is False
    ):
        raise SystemExit("K01-0002 is not the exact sealed PASS")
    ...
    result = {
        "attempt_binding": {
            "K01_attempt_id": "K01-0002",
            "K01_core_evidence_id": "E0029",
            "K01_core_generation": "000029-4b4d41db",
            "K01_final_closeout_evidence_id": "E0030",
            "K01_report": k01["report"],
            "K01_report_sha256": k01["report_sha256"],
            "status": "PASS",
        },
        ...
        "projection": "POST_K01_0002_LIVE_RECOMPUTATION",
    ...
    if not (
        result["completed_package_count"] == 46
        and result["blocked_package_count"] == 104
        and result["ready_packages_manifest_order"]
        == ["K02", "K03", "L01", "N01", "T01", "A06"]
        and result["next_package"] == "K02"
    ):
        raise SystemExit(
            "post-K01 DAG differs from the expected evidence-derived state: "
...
def review_text(result: dict[str, Any]) -> str:
    ...
    return f"""# Post-K01-0002 live DAG reconciliation
