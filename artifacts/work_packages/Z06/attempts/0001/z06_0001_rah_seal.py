#!/usr/bin/env python3
"""Append Z06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z06-0001"
WORK_PACKAGE_ID = "Z06"
EXPECTED_PARENT = "000364-b2b7aaf2"
EXPECTED_PARENT_COUNT = 364
EXPECTED_PARENT_EVIDENCE = "E0366"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "6951a86e9465fa476112da869639e1fdd463293bcdffe3e0418c1869c864a726"
)
CORE_EVIDENCE_ID = "E0367"
FINAL_EVIDENCE_ID = "E0368"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward closeout and the remaining integration layers.",
...
GATE_NOTE = (
    "Z06-0001 independent release, clean extraction and truthful maturity gate "
    "is evidence-sealed PASS (E0367/E0368). closeout is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "release/v4_z06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "Z06-0001 PASS core verification: independent release, clean extraction and truthful maturity gate, "
        ...
        "session, under exact dependency bindings (B06 B06-0001, C06 C06-0001, F06 F06-0001, G06 G06-0001, K06 K06-0001, N06 N06-0001, P06 P06-0001, Q06 Q06-0001, S06 S06-0001, T06 T06-0001, V06 V06-0001, W06 W06-0001, Y06 Y06-0001, Z05 Z05-0001) "
        "and an S05-0001 regression baseline. the module holds no canonical enum literal — the passing status token and maturity floor are read through the composed Z05 surface, the fail-closed UNSIGNED status is derived through the sealed provenance surface, and the forbidden-maturity-claim vocabulary is proven disjoint from every schema enum "
        "happy path (unit and provenance), negative and adversarial (23 cases, one per finding code), crash/resume-style composition over the actually-sealed frozen Z05 report and the thirteen real *06 reports, and two dependency-regressions that fail here if a composed surface breaks "
        "no candidate, model, prompt, backend or hook can gain evaluator, holdout or promotion authority — the truthful-maturity gate refuses any release-level above floor, any completion claim, any executable/validated/production-ready/signed claim, and any unowned conditional in the accounting "
        "Key review finding: Truthful-maturity enforcement is the crux: the gate PASSES by proving honesty, not completion — release_level is pinned to the acceptance-matrix floor via release_level_floor() (never restated), completion_ready is hardcoded False in every emitted receipt and verdict, signing_status is the composed derived UNSIGNED, and FORBIDDEN_MATURITY_CLAIMS refuses executable / validated / production-ready / GA / signed / shippable / certified via boundary-anchored regex so 'ga' cannot fire inside 'organization'. "
        ...
        + ". closeout is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "that a working v4 runtime, qualified evaluator, hidden holdout, Shinka adapter, production database, sandbox, UI, security review or 2,000-document deployment exists: the bundle stays SPEC_BUNDLE / UNVERIFIED reference; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z06-0001 parent is not the sealed Z06 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z06 preflight requires exactly 364 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z06 core did not append exactly E0367")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z06 final did not append exactly E0368")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z06 core tail is not E0367")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 366:
        raise SystemExit(f"unexpected Z06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
