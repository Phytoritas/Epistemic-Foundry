#!/usr/bin/env python3
"""Prove the F05 machine is discoverable by the project's own packaging config.

Without ``src/epistemic_foundry/evolution/__init__.py`` the machine imports from a
source checkout but is silently absent from the built wheel, because pyproject
discovers packages with ``find_packages`` rather than the namespace variant
(HD-EF4-F05-SCOPE-20260802-001).  This check reads the discovery mode from
pyproject instead of assuming it, so a change there fails here.
"""
...
EXPECTED = ("epistemic_foundry.evolution", "epistemic_foundry.evolution.v4_f05")
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the machine would import from a checkout but be absent "
                        "from the wheel"
                    ),
                    "status": "FAIL",
                },
