#!/usr/bin/env python3
"""Run the F05-0001 EVOLVE subprotocol state machine acceptance checks.

Required checks from manifests/development_manifest.yaml (F05):
...
report.  The machine ships under src/epistemic_foundry/evolution/v4_f05, so
...
ATTEMPT_ID = "F05-0001"
COMPONENT = "artifacts/work_packages/F05/attempts/0001"
# The machine composes the chamber checkpoint surface, and F04's golden
# FORGE flows are the end-to-end regression the state machine must not
# disturb. The F01 classifier suites are the Node side of that gate.
DEPENDENCY_TARGETS = {
    ...
    "dependency-regression-golden": ("packages/foundry-kernel/src/forge/classifier",),
}
...
def packaging_discovery() -> int:
    ...
    # is absent from the wheel (HD-EF4-F05-SCOPE-20260802-001).
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-f05-machine", COMPONENT)
...
def dependency_regression_golden() -> int:
    name = "dependency-regression-golden"
    files = sorted(
    ...
    if not files:
        ...
        print("the golden gate declares no Node test file", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    checks = {
        ...
        "dependency-regression-golden": dependency_regression_golden,
