#!/usr/bin/env python3
"""Append Q04-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Q04-0001"
WORK_PACKAGE_ID = "Q04"
EXPECTED_PARENT = "000174-69c2b656"
EXPECTED_PARENT_COUNT = 174
EXPECTED_PARENT_EVIDENCE = "E0176"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "c15aab56d139e99c6992365f18c8a4a3566d00b2c154f3e577371b57411b02aa"
)
CORE_EVIDENCE_ID = "E0177"
FINAL_EVIDENCE_ID = "E0178"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_q04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward B06-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Q04-0001 q-phase time-sliced and adversarial benchmark gate "
    "is evidence-sealed PASS (E0177/E0178). B06-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Q04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "evals/time_sliced").rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        ),
        *sorted(
            path.relative_to(ROOT).as_posix()
            for path in (ROOT / "evals/adversarial").rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        ),
    ]
...
def core_summary() -> str:
    ...
    return (
        "Q04-0001 PASS core verification: q-phase time-sliced and adversarial benchmark gate, "
        ...
        "session, under exact dependency bindings (Q01 Q01-0001, Q02 Q02-0001, Q03 Q03-0001) "
        "and an S05-0001 regression baseline. gold labels and attack classes are read from their declaring datasets and never restated (AST-asserted); every dataset and results artifact is self-hashing and 46 typed refusal codes each carry their reason, with a coverage assertion that the exercised set equals the declared set "
        ...
        "Key review finding: Predictions are a committed synthetic fixture guarded by SYSTEM_OVERCLAIM; the gates measure the benchmark contract, not any component's accuracy, and both reports say so. "
        ...
        + ". B06-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Q04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "any real system's accuracy: predictions are a committed fixture and a dataset claiming otherwise is refused; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Q04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q04-0001 parent is not the sealed Q04 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Q04 preflight requires exactly 174 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Q04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Q04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Q04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Q04 core did not append exactly E0177")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Q04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Q04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Q04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Q04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Q04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Q04 final did not append exactly E0178")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Q04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Q04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Q04 core tail is not E0177")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 176:
        raise SystemExit(f"unexpected Q04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Q04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Q04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
