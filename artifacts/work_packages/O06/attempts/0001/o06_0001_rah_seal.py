#!/usr/bin/env python3
"""Append O06-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000250-2b586915"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 250
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0252"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "b702dd310db8b16834cdc1cc83317ddb9d69a6fe316489105f5074118664f14b"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0253"
FINAL_EVIDENCE_ID = "E0254"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_o06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after O06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} search-completeness, novelty-failure and prior-art integration "
    f"gate is evidence-sealed PASS "
    f"({CORE_EVIDENCE_ID}/{FINAL_EVIDENCE_ID}). The next package is {NEXT_PACKAGE}; "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "O06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required O06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: retrieval namespace is pinned so a change to a composed retrieval surface
    #: (or the parent marker) is caught here too.
    relatives = sorted(
...
def core_summary() -> str:
    ...
    return (
        "O06-0001 PASS core verification: search-completeness, novelty-failure and "
        "prior-art integration gate, implemented in "
        "src/epistemic_foundry/retrieval/v4_o06 by a bounded implementation "
        "subagent and reviewed by the sealing agent (actor-independent from the "
        "author) under exact dependency bindings (O05 O05-0001, Q05 Q05-0001) and "
        "an X04-0001 regression baseline. The gate composes the sealed O05 "
        "retrieval / layered-novelty / coverage surface, the K05/evaluation "
        "novelty owners and the Q05 selective-inference gate without restating "
        "them: it reconciles the eleven canonical O05 lane receipts into the "
        "canonical search-completeness-certificate with every lane state, the "
        "completion state and both claim ceilings derived rather than supplied, "
        "and it refuses a novelty claim that earned no novelty ceiling, a "
        "determination that left a required source unsearched, a "
        "candidate-generating role driving the decision, and a Q05 admissibility "
        "receipt that is not an untampered ADMIT for this candidate; novelty is "
        "earned only by a complete search and no score is taken from anywhere. "
        "Twenty-one finding codes name exact refusals and every one is exercised. "
        "Every certificate and every admit/refuse decision re-derives byte for "
        "byte from its own published fields with no clock or random draw, and "
        "nothing scores, ranks, selects, promotes or evaluates. Ruff lint/format, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the O05/Q05 dependency regressions, full Python and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"O06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. O06 earns novelty only "
        "by a complete search, refuses a prior-art determination that ignored a "
        "required source, composes the sealed Q05 ADMIT receipt by hash, and "
        "records a deterministic search-completeness certificate and admit/refuse "
        "receipt only: it does not score, rank, select, promote or evaluate any "
        "candidate, grants no authority at the gate, makes no DSSAT or "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("O06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("O06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"O06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("O06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("O06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("O06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"O06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("O06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("O06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("O06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored O06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("O06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"O06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("O06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"O06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected O06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with O06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored O06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
