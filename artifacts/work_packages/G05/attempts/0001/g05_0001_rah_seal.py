#!/usr/bin/env python3
"""Append G05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "G05-0001"
WORK_PACKAGE_ID = "G05"
EXPECTED_PARENT = "000166-29dc649f"
EXPECTED_PARENT_COUNT = 166
EXPECTED_PARENT_EVIDENCE = "E0168"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "86572e353aaa354c769472edc12309d588c9c661b42434a929312d0255db820a"
)
CORE_EVIDENCE_ID = "E0169"
FINAL_EVIDENCE_ID = "E0170"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_g05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run the L05 evolution telemetry and observability package on the "
    "sealed A06/C05 state.",
    "Continue the x05 wave toward M05/S05 and the U01/V01/X01 integration layer.",
...
GATE_NOTE = (
    "G05-0001 evolution plugin skills, CLI surface and progressive-disclosure "
    "routing is evidence-sealed PASS (E0169/E0170). L05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required G05-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: The surface ships outside this attempt directory, so the seal binds
    #: the product bytes by repository path rather than by artifact name.
    result: dict[str, str] = {}
    for relative in sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "plugin_blueprint/epistemic-foundry/v4_g05").rglob("*")
        if path.is_file()
    ):
        result[relative] = sha256(ROOT / relative)
    if len(result) != 8:
        raise SystemExit(f"unexpected G05 product file set: {sorted(result)}")
    return result
...
def core_summary() -> str:
    ...
    return (
        "G05-0001 PASS core verification: the evolution plugin surface is "
        "implemented under plugin_blueprint/epistemic-foundry/v4_g05, under "
        "exact A06-0002, C05-0001 and G04-0001 dependency bindings and a "
        "F05-0001 regression baseline, entirely inside the manifest write "
        "grant. The surface answers a question nothing else asked: the "
        "payload ships 29 skills and the sealed tool surface projects 22 "
        "commands, and until now nothing checked that the evolution skills "
        "and the CLI describe the same product. The 15 evolution skills are "
        "derived from the sealed J02 inventory by reference closure rather "
        "than listed, the 25 proposed evolution commands are parsed from "
        "MASTER_SPEC section 35, and the mutable search space is read from "
        "the sealed C05 index. All 25 proposed commands are absent from the "
        "projected surface and are recorded as unavailable rather than "
        "implied runnable; the 5 projected commands the skills may name are "
        "each published with their effect class. Eighteen failure classes "
        "are refused by name, including an invented command, a "
        "twice-claimed command, an unowned proposed command, a genome "
        "outside the sealed search space, a drifted inventory, a "
        "contradicted payload policy and an outgrown disclosure budget. "
        "Promotion authority is derived from the sealed catalog by effect "
        "class and object, no evolution skill may name it, and a predicate "
        "that matched nothing is refused as vacuous rather than satisfied. "
        "Progressive disclosure is bounded by J02's own budgets: the widest "
        "skill opens 11 references at depth 5 against limits of 12 and 5. "
        "Nothing is reimplemented: selection is delegated to the sealed J01 "
        "policy function and the receipt is hashed with its canonicalizer. "
        "Ruff lint/format, repository structure and boundary gates, "
        "schema_and_type_check 20/20, unit_and_contract 18/18, "
        "negative_and_adversarial 27/27, provenance_and_receipt 16/16, "
        "targeted 81/81, host regression 57/57, install regression 1/1, "
        "C05 regression 132/132, plugin regression 37/37, full Python "
        "1261/1261, full Node 1063/1063 across 95 files, and git diff "
        "--check pass. "
        ...
        "L05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"G05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". F05-0001 and every prior RAH generation remain immutable. The "
        "surface validates and routes; it executes nothing, and the "
        "blueprint package remains a reference package whose CLI still "
        "exits 78. The sealed routing policy, command projection, skill "
        "inventory and payload skills are read-only inputs and were not "
        "modified. Two claims are recorded as not derivable rather than "
        "asserted: which skill owns which proposed command is a declared "
        "judgment, and token counts come from the inventory that owns the "
        "tokenizer. L05, M05, S05, Q04, U01, V01, X01, source coverage, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("G05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G05-0001 parent is not the sealed F05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("G05 preflight requires exactly 166 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("G05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("G05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("G05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("G05 core did not append exactly E0169")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("G05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("G05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("G05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored G05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("G05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("G05 final did not append exactly E0170")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("G05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("G05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("G05 core tail is not E0169")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 168:
        raise SystemExit(f"unexpected G05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with G05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored G05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
