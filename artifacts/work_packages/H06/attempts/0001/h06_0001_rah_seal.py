#!/usr/bin/env python3
"""Append H06-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed H05/X03 templates do.
"""
...
#: The current committed generation (e.g. "000236-f1398b98"); its retained
...
EXPECTED_PARENT = "000238-aa09bffd"
EXPECTED_PARENT_COUNT = 238
EXPECTED_PARENT_EVIDENCE = "E0240"
EXPECTED_PARENT_MANIFEST_SHA256 = "11739826919e35a2cbb768b401858a957e9c0932fd9537f7c85f2cbb3f881229"
CORE_EVIDENCE_ID = "E0241"
FINAL_EVIDENCE_ID = "E0242"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_h06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after H06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} hook-disabled and hosted-tool degraded-mode integration gate "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "H06-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required H06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "plugin_blueprint/epistemic-foundry/hooks/v4_h06").rglob(
            "*"
        )
...
def core_summary() -> str:
    ...
    return (
        "H06-0001 PASS core verification: hook-disabled and hosted-tool "
        "degraded-mode integration gate, implemented in "
        "plugin_blueprint/epistemic-foundry/hooks/v4_h06 by a bounded "
        "implementation agent under the product owner's explicit parallel-"
        "execution instruction and reviewed by the sealing session as an "
        "independent contract reviewer (actor-independent from the author), "
        "under exact dependency bindings (H05 H05-0001, G05 G05-0001) and an "
        "F06-0001 live latest-sealed regression baseline. The gate stands "
        "between a host-declared capability state and any claim of hook-verified "
        "provenance: it holds no state, detects no degradation and grants no "
        "authority, reading the declared state, projecting sealed H05 "
        "HOOK_COVERAGE through the enabled-host set, and refusing every claim "
        "that outruns the declaration. Mode and capability vocabularies are read "
        "from degraded-mode-policy.json and the imported gateway/capability-probe "
        "surfaces rather than restated; a coverage order that ties refuses "
        "COVERAGE_RANK_AMBIGUOUS, a claim outside the gateway vocabulary refuses "
        "COVERAGE_UNDECLARED, a report or claim exceeding the enabled set refuses "
        "DEGRADED_OVERCLAIMED, an understated one refuses DEGRADED_UNDERSTATED, "
        "and an unrestored recovery refuses RECOVERY_COVERAGE_UNRESTORED. Every "
        "degraded receipt and step-provenance record hashes through the "
        "gateway's own sha256HookJson and validateDegradedModeReceipt re-derives "
        "its EFH06-DEGRADED-MODE id; no clock or randomness exists in any product "
        "path. schema-and-type-check 14/14, unit-and-contract 19/19, negative-"
        "and-adversarial 38/38, provenance-and-receipt 20/20 (targeted 91/91), "
        "the gateway/capability-probe/H05/G05 regressions green, the full Node "
        "suite green with the two H06 modules inside the 132-file inventory, and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"H06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. H06 plans and records "
        "a safe degraded mode only: it never detects degradation, never delivers "
        "or enforces hooks, fabricates no hook-verified provenance, and acquires "
        "no evaluator/holdout/promotion authority. Author/reviewer separation "
        "holds with actor_independence=true while external actor-independent "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("H06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H06-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"H06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("H06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("H06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"H06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("H06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("H06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("H06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored H06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("H06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"H06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("H06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"H06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected H06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with H06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored H06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
