#!/usr/bin/env python3
"""Recompute the 156-package DAG after the D04 core PASS evidence."""
...
D04_SOURCE_HASHES = {
    ...
    "artifacts/work_packages/D04/review.md": (
        "3ef7fc082571fdd3bb7efdf010f6aa2091104e0adf1339788fcd150c219a5623"
    ),
}
...
def verified_d04_core() -> dict[str, Any]:
    for relative, expected in D04_SOURCE_HASHES.items():
        actual = sha256(ROOT / relative)
        if actual != expected:
            raise SystemExit(
                f"D04 core source hash changed for {relative}: {actual} != {expected}"
            )
    ...
    if "Final verdict: `PASS`" not in review or "blocking findings: 0" not in review:
        raise SystemExit("D04 review is not an unblocked PASS")

    current = read_json(RAH_ROOT / "current.json")
    generation = current.get("generation")
    if generation != "000021-dc79580e":
        raise SystemExit(f"unexpected D04 core generation: {generation}")
    ledger = read_json(RAH_ROOT / "evidence_ledger.json")
    entries = ledger.get("entries")
    if not isinstance(entries, list) or not entries:
        raise SystemExit("RAH evidence ledger has no entries")
    latest = entries[-1]
    if not isinstance(latest, dict) or latest.get("id") != "E0022":
        raise SystemExit("D04 DAG reconciliation requires latest evidence E0022")
    summary = str(latest.get("summary", ""))
    if not summary.startswith("D04-0001 PASS core:"):
        raise SystemExit("E0022 is not the D04 core PASS evidence")
    generation_dir = RAH_ROOT / "generations" / generation
    generation_manifest = generation_dir / "generation-manifest.json"
    if not generation_manifest.is_file():
        raise SystemExit("D04 core generation manifest is missing")
    return {
        "attempt_id": "D04-0001",
        "core_evidence_id": "E0022",
        ...
        "review": "artifacts/work_packages/D04/review.md",
        "review_sha256": D04_SOURCE_HASHES["artifacts/work_packages/D04/review.md"],
        "source_hashes": D04_SOURCE_HASHES,
        "status": "PASS",
    }
...
def reconcile() -> dict[str, Any]:
    ...
    pass_evidence["D04"] = verified_d04_core()
    ...
    expected_ready = ["E01", "G01", "K01", "A06"]
    if ready != expected_ready:
        raise SystemExit(
            f"post-D04 manifest-order READY set changed: {ready} != {expected_ready}"
        )
    blocked = [
        package_id
        for package_id in order
    ...
    return {
        "schema_version": 1,
        "status": "PASS",
        "projection": "POST_D04_0001_CORE_PASS",
        "manifest": {
