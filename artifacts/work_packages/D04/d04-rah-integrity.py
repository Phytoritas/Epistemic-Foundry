#!/usr/bin/env python3
"""Read-only integrity verification for the D04 RAH core generation."""
...
CORE_GENERATION = "000021-dc79580e"
...
def verify(
    *,
    ...
    if ids[-1] != expected_evidence_id:
        raise SystemExit(f"latest evidence is not {expected_evidence_id}")
...
def main() -> int:
    ...
    parser.add_argument("--expected-generation-count", type=int, default=21)
    parser.add_argument("--expected-generation", default=CORE_GENERATION)
    parser.add_argument("--expected-generation-sequence", type=int, default=21)
    parser.add_argument("--expected-evidence-id", default=CORE_EVIDENCE_ID)
