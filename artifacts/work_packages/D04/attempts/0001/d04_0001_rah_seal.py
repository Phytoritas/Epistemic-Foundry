#!/usr/bin/env python3
"""Append D04-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000298-4f748359"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 298
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0300"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "5f2296cf31e603612b904ac5261d23cc3cc55627847a584dac7030a6de6a4974"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0301"
FINAL_EVIDENCE_ID = "E0302"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_d04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after D04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} backup/corruption/recovery gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "D04-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required D04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: recovery-gate test tree is pinned so a change to a fixture or test is
    #: caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "tests/recovery/state").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "D04-0001 PASS core verification: the D-phase backup, corruption and "
        "recovery gate, implemented in tests/recovery/state by a bounded "
        "implementation agent and reviewed by this seal-prep session (a distinct "
        "actor, actor-independent from the author) over the sealed D02-0001 SQLite "
        "state store and D03-0001 content-addressed artifact store with no new "
        "production dependency and no modification of either dependency. Crash and "
        "corruption fixtures recover safely: a committed WAL replays after SIGKILL "
        "into a recovered ACTIVE store that reads the sealed record with integrity "
        "ok, an interrupted transaction rolls back with no partial record, a "
        "corrupted SQLite header opens read-only SAFE_MODE and closes, and artifact "
        "crash residue stays quarantined while the store stays ACTIVE. No data loss "
        "is hidden by reset: the corrupted database sha256 is identical before and "
        "after open, crash residue is still present after recovery, and restore is "
        "staged, hash-verified, integrity-checked and atomically published so a "
        "damaged, hash-mismatched or raced backup fails closed with the target "
        "absent while source and backup are never mutated. Backup and restore "
        "round-trip faithfully point-in-time with exact bytes, the artifact "
        "snapshot excludes .staging/.mutation-lock, and a PostgreSQL logical custom "
        "dump is qualified in a disposable container. Ruff lint/format, "
        "crash_recovery_test 4/4, backup_restore_test 2/2, targeted 6/6, full "
        "Python 1261/1261, full Node 1253/1253 across 111 files, and git diff --check "
        "pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"D04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". D02-0001, D03-0001 and every prior RAH generation remain immutable. "
        "D04 qualifies backup, corruption, and recovery for the durable state and "
        "artifact stores; the broader recovery lifecycle beyond these fixtures, a "
        "production backup service, PostgreSQL physical backup/PITR, source "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("D04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D04-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"D04 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("D04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("D04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"D04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("D04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("D04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("D04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored D04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("D04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"D04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("D04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"D04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected D04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with D04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored D04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
