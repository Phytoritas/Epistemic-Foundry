#!/usr/bin/env python3
"""Append W05-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was authored as a bounded
review-and-seal hand that was instructed not to touch ``.rah/``, so it cannot
read the live ledger to pin the parent generation identifier, its manifest hash,
the retained generation count, or this attempt's own evidence identifiers.  All
five of ``EXPECTED_PARENT``, ``EXPECTED_PARENT_MANIFEST_SHA256``,
...
until the parent fills them from the live ledger tail.  The expected parent is
the generation committed by the R05-0001 final closeout (evidence ``E0206``),
which is the latest sealed attempt and this package's regression baseline; the
parent's last evidence id is pinned to ``E0206`` (``EXPECTED_PARENT_EVIDENCE``)
and should be verified against the live ledger tail before running.  Everything
else — summaries, hashes, flat-snapshot verification — is derived exactly as the
templates do.
"""
...
#: The generation the J05-0001 final closeout committed (its evidence is E0214).
#: Filled from the live `.rah/ralph` current pointer.
EXPECTED_PARENT = "000212-1f4b67fa"
#: The number of preserved generations at the live tail (J05-0001 sealed last).
EXPECTED_PARENT_COUNT = 212
#: The parent's last committed evidence id: J05-0001's final closeout, the
#: live ledger tail at seal time.
EXPECTED_PARENT_EVIDENCE = "E0214"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "9d27f71cc945e76d887c1f139f75812bf1eb4033dbc1067f91dcf6260e026246"
)
#: This attempt's own core and final evidence ids (the next two after the tail).
CORE_EVIDENCE_ID = "E0215"
FINAL_EVIDENCE_ID = "E0216"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_w05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after W05-0001 seals and select the next "
...
#: names this attempt's evidence via the pins so it stays correct once filled.
GATE_NOTE = (
    f"{ATTEMPT_ID} evolution checkpoint resume, cancel and evaluator-drift "
    f"reassessment workflow is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    if PARENT_PIN_SENTINEL in (
        ...
        raise SystemExit(
            "W05-0001 seal is not pinned: set EXPECTED_PARENT, "
            "EXPECTED_PARENT_MANIFEST_SHA256, EXPECTED_PARENT_COUNT, "
            "CORE_EVIDENCE_ID and FINAL_EVIDENCE_ID from the live ledger tail "
            "(the generation committed by the R05-0001 final closeout, evidence "
            "E0206) before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required W05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "src/epistemic_foundry/recovery").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "W05-0001 PASS core verification: evolution checkpoint resume, cancel and "
        "evaluator-drift reassessment workflow, implemented in "
        "src/epistemic_foundry/recovery/v4_w05 by a bounded implementation agent "
        "and reviewed by an independent reviewer agent (actor-independent from "
        "the author) dispatched under the product owner's explicit "
        "parallel-agent instruction, under exact dependency bindings (W04 "
        "W04-0001, D05 D05-0001, F05 F05-0001, N05 N05-0001) and an R05-0001 "
        "regression baseline. Resume is expressed as a return edge and handed to "
        "the F05 machine, whose verdict is the verdict; the resume record binds "
        "atomically only after require_valid_run passes and the machine's "
        "unsealed/partial refusals travel out unwrapped. Cancel derives the "
        "remaining map from the run's own accounting and reconciles proposed vs "
        "evaluated candidates and mapped vs assessed niches exactly, refusing a "
        "disclosure that hides or invents remaining work, then certifies through "
        "the canonical stop certificate which forces partial_results_visible. "
        "Evaluator drift is the firewall's content-recomputed judgment; affected "
        "comparisons are marked, never removed or re-scored, and the fix is a "
        "future-only quarantined proposal whose retroactive use is refused by "
        "quarantine's own rule. Canonical vocabulary is composed, not restated "
        "(EF4-I22), and nothing scores, selects, promotes or evaluates. Ruff "
        "lint/format, schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, W04/D05/F05/N05 dependency regressions, full "
        "Python and full Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by an independent reviewer agent with "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"W05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. W05 is the workflow "
        "logic over records already in hand: it does not score, select, promote "
        "or evaluate any candidate, recovers nothing physical (D06 owns the "
        "store), makes no DSSAT or plant-model parity claim, and promotion "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("W05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("W05-0001 parent is not the sealed R05-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"W05 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("W05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("W05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("W05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"W05 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("W05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("W05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("W05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored W05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("W05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"W05 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("W05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"W05 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected W05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with W05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored W05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
