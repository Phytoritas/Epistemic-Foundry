#!/usr/bin/env python3
"""Build and verify Z03-0001 evidence: Upgrade, downgrade, migration and rollback matrix.
...
ATTEMPT_ID = "Z03-0001"
WORK_PACKAGE_ID = "Z03"
RECORDED_AT = "2026-08-02T20:15:00.000Z"
ATTEMPT_DIR = "artifacts/work_packages/Z03/attempts/0001"
AUTHORITY_DECISION = None

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/Z03/attempts/0001/run_z03_0001_checks.py": "71c818145086b9f31877de671a82b32ddc045a217ceb799b35430a50f9fb5723",
    "docs/migration_v2_to_v4.md": "6255e6b93c068c252bf9ac11083902fa6755145b997d3106f697091886237dfb",
    "tests/migration/fixtures/upgrade_rollback_matrix.yaml": "eb11c1c4a83d89334c1fcc48e659483f1d7a6d2916a7c2c6253fac097d34d6f2",
    "tests/migration/test_rollback.py": "fc9f23a4fe19baa976e14e0586dd1b22596b0fafdba124413b21fae75a1083c0",
    "tests/migration/test_upgrade_matrix.py": "0754e49da4d1dd10280bcdbd3a7b46820444c928083839aee318efd71d80d1fa",
    "tests/migration/z03_migration_harness.py": "8c1d437b93aebb03b1ace881c8597c65cdee84cefb230f58df3c66212d256d39",
}
EXPECTED_DEPENDENCY_HASHES = {
    ...
    "artifacts/work_packages/Z01/attempts/0001/report.json": "6af4f1f37cc6e1bdb9d9fddaeb29e729ac3e31a5fd136c212f2771f6d28d8571",
}

JUNIT_PATHS = {
    "full_node_suite": ATTEMPT / "full-node-suite.junit.xml",
    "full_python_suite": ATTEMPT / "full-python-suite.junit.xml",
    "rollback_test": ATTEMPT / "rollback-test.junit.xml",
    "upgrade_matrix_test": ATTEMPT / "upgrade-matrix-test.junit.xml",
...
REQUIRED_CHECK_MAP = {
    ...
    "upgrade_matrix_test": "upgrade_matrix_test",
}
#: Maps each manifest exit-criterion key to its mechanism sentence.
EXIT_CRITERIA = {
    "failed_migration_rolls_back": "rollback_test proves a failed migration restores the exact prior state (restored source hash and state hash equal the retained originals) and refuses a rollback that discards the v3 source, mismatches the hash, loses migration records, rewrites promotion or effect history, or leaves records unresolved",
    "hook_re_trust_tested": "upgrade_matrix_test proves an upgraded host must re-establish hook trust and refuses silent inheritance of the prior trust state, with the positive re-trust path and the silent-inheritance refusal both exercised",
}
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        ...
        ("rollback_test", pytest_summary(JUNIT_PATHS["rollback_test"]), 14),
        ("upgrade_matrix_test", pytest_summary(JUNIT_PATHS["upgrade_matrix_test"]), 13),
        (
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "dependencies": {
            "Z01": _sealed_dependency("Z01", "Z01-0001", "E0357", "E0358"),
        },
        "next_action": "SEAL_Z03_0001_THEN_CONTINUE_DAG",
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "declaring_sources": {
            "matrix_fixture": (
                "tests/migration/fixtures/upgrade_rollback_matrix.yaml (Z03's own declaring source of upgrade/downgrade paths, rollback record fixtures and backfill batches)"
            ),
            "rollback_and_backfill_contract": (
                "migrations/contracts/compatibility-matrix.json rollback and backfill blocks (composed by citation, unmodified)"
            ),
            "v3_to_v4_migration": (
                "migrations/contracts/evolution-run-spec-v3-to-v4.migration.json (composed by citation, unmodified)"
            ),
        },
        "exit_criteria": {
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_z03_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "Z03_UPGRADE_DOWNGRADE_MIGRATION_ROLLBACK_MATRIX",
        ...
        "not_claimed": [
            "a real cross-version runtime migration, live upgrade or database mutation: all proofs are declared-fixture pure-function evaluations",
            "that the v4 plugin is executable, validated or production-ready (the matrix status stays UNVERIFIED_REFERENCE_MATRIX)",
            "hook re-trust or rollback verified against a live host or store: both are proven as declared-policy decisions, not runtime effects",
...
def build() -> dict[str, Any]:
    ...
    write_json("z03-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("Z03-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "z03-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored Z03-0001 report is not the deterministic document")
    return _summary()
