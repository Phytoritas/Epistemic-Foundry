#!/usr/bin/env python3
"""Append Z03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z03-0001"
WORK_PACKAGE_ID = "Z03"
EXPECTED_PARENT = "000358-47823318"
EXPECTED_PARENT_COUNT = 358
EXPECTED_PARENT_EVIDENCE = "E0360"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "c175bfc7605841ca3b71ffecae6f018c0c2c56a6ee7a6e5839e23ac70f3782e1"
)
CORE_EVIDENCE_ID = "E0361"
FINAL_EVIDENCE_ID = "E0362"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward Z04-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Z03-0001 upgrade, downgrade, migration and rollback matrix "
    "is evidence-sealed PASS (E0361/E0362). Z04-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "tests/migration").rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        ),
        "docs/migration_v2_to_v4.md",
    ]
    return {relative: sha256(ROOT / relative) for relative in relatives}


ATTEMPT_ARTIFACTS = (
    "build_z03_0001_evidence.py",
    "full-node-suite.junit.xml",
    "full-python-suite.junit.xml",
    "node-test-inventory.json",
    "rollback-test.junit.xml",
    "run_z03_0001_checks.py",
    "upgrade-matrix-test.junit.xml",
    "wire-literal-discipline.junit.xml",
    "z03_0001_rah_seal.py",
)
...
def core_summary() -> str:
    ...
    return (
        "Z03-0001 PASS core verification: upgrade, downgrade, migration and rollback matrix, "
        ...
        "and an S05-0001 regression baseline. the harness emits two content-addressed report records (z03-upgrade-matrix-report/v1, z03-rollback-report/v1) whose canonical JSON is hashed into a record_sha256, and it echoes the composed contract's rollback, backfill and compatibility_window blocks verbatim rather than restating them, with every failure a typed EF_Z03_* refusal whose reason exceeds fifty characters "
        "upgrade covers v2 to v4 (two-step) and v3 to v4 reconciliation, hook re-trust (positive and silent-inheritance refusal) and fail-closed downgrade, while rollback covers exact-prior-state restore plus five negative proofs (source discarded, hash mismatch, records lost, history rewritten, unresolved-fail-closed) and three backfill proofs (dry-run, unresolved, partial is not batch) "
        "every gate decision is a typed refusal code and each upgrade path is accepted only when its declared terminal exactly reconciles with the terminal recomputed from its per-step outcomes; the harness spawns no host and store, mutates nothing on disk, and never modifies the composed contracts "
        "Key review finding: The honesty boundary holds: no code path claims a real runtime migration, every report carries a honesty_note, and evaluations are pure functions with nothing written, restored or deleted on disk. "
        ...
        + ". Z04-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a real cross-version runtime migration, live upgrade or database mutation: all proofs are declared-fixture pure-function evaluations; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z03-0001 parent is not the sealed Z03 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z03 preflight requires exactly 358 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z03 core did not append exactly E0361")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z03 final did not append exactly E0362")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z03 core tail is not E0361")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 360:
        raise SystemExit(f"unexpected Z03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
