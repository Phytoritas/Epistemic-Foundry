#!/usr/bin/env python3
"""Run the K05-0001 corpus/holdout/prior-art boundary acceptance checks.

Required checks from manifests/development_manifest.yaml (K05):
...
report.  The boundaries ship under src/epistemic_foundry/evidence/v4_k05, so
...
``dependency-regression-firewall`` re-runs the Verifier Firewall suite and the
sealed S05 controls, because this package composes ``build_holdout_manifest``
and inherits the leakage boundary S05 audits.
"""
...
ATTEMPT_ID = "K05-0001"
COMPONENT = "artifacts/work_packages/K05/attempts/0001"
ENGINE = "src/epistemic_foundry/evidence/v4_k05"
# The firewall owns the holdout manifest this package seals through, and the
# sealed S05 controls own the leakage audit that reads the same boundary.
DEPENDENCY_TARGETS = {
    "dependency-regression-firewall": (
        "tests/test_verifier_firewall.py",
        "artifacts/work_packages/S05/attempts/0001",
    ),
}
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/evidence gained its package marker for this
    # package and evidence/v4_k05 is new, so both levels are proved
...
def targeted() -> int:
    return _pytest("targeted-k05-boundaries", COMPONENT)
