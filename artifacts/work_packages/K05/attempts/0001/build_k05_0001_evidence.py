#!/usr/bin/env python3
"""Build and verify K05-0001 evidence: Corpus/evidence snapshot pinning, hidden holdout and prior-art boundaries.
...
ATTEMPT = ROOT / "artifacts/work_packages/K05/attempts/0001"
ATTEMPT_ID = "K05-0001"
WORK_PACKAGE_ID = "K05"
RECORDED_AT = "2026-08-02T17:00:00.000Z"
ATTEMPT_DIR = "artifacts/work_packages/K05/attempts/0001"
AUTHORITY_DECISION = "HD-EF4-K05-SCOPE-20260802-001"

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/K05/attempts/0001/check_packaging.py": "b938e65e8119f190c4d494f2f52ce2f49f19e7666097f234698fa9eaeba3e50c",
    "artifacts/work_packages/K05/attempts/0001/fixtures.py": "08a28b13dc5dca26b20d1f14cf8ed725e772e0558fd4749ffdc606939cac703d",
    "artifacts/work_packages/K05/attempts/0001/pytest.ini": "3d96f3378e1a8326c6749b74e92a84609e522730178f708dda0f4d61405b0e65",
    "artifacts/work_packages/K05/attempts/0001/run_k05_0001_checks.py": "f7acc0f220be7f89b122d9fb58a65aa5a2c05c2236751b8577e15442cfc19535",
    "artifacts/work_packages/K05/attempts/0001/test_negative_adversarial.py": "50b9cee1010c55d39fdb7296484dafcc9ea0b7318956e2fcc58362ac2821e231",
    "artifacts/work_packages/K05/attempts/0001/test_provenance_receipts.py": "e59a5cb07ad104d68eb3ef19fb1e8db0962a60e6ad4ec73a32e81b017db60ffa",
    "artifacts/work_packages/K05/attempts/0001/test_schema_and_type.py": "7f47bae5719f969a6ae86c84647b12017ef275b01184721a5399d391646d7c8f",
    "artifacts/work_packages/K05/attempts/0001/test_unit_contract.py": "0e53cf81591063b962440b44659507d51bcbcfd4536233e02982a3f495997adc",
    "src/epistemic_foundry/evidence/__init__.py": "86830dc982df014fef3a418578c8fb16900d675c42c2551e181fc14297a89518",
    "src/epistemic_foundry/evidence/v4_k05/__init__.py": "1f507c09e5d85301fe626eb1c25c27a721fdce12e1bc1f0a3ba64ae6bdae4804",
    "src/epistemic_foundry/evidence/v4_k05/boundaries.py": "1216c8a434cbd1831279d28e07fcab2abc2017c587df4dd92617c70511ccce61",
}
EXPECTED_DEPENDENCY_HASHES = {
    "artifacts/authority_decisions/HD-EF4-K05-SCOPE-20260802-001.human-decision.json": "016fdc021e9f6e76231b4a1066ccd7cb3c282fbd4f5e75c6af06182106b474de",
...
JUNIT_PATHS = {
    "dependency_regression_firewall": ATTEMPT
    ...
    "targeted_k05_boundaries": ATTEMPT / "targeted-k05-boundaries.junit.xml",
...
OUTPUT_NAMES = (
    ...
    "k05-verification.json",
    "k05_0001_rah_seal.py",
    "negative-and-adversarial-tests.junit.xml",
    "node-test-inventory.json",
    "provenance-and-receipt-audit.junit.xml",
    "pytest.ini",
    "review.md",
    "run_k05_0001_checks.py",
    "schema-and-type-check.junit.xml",
    "targeted-k05-boundaries.junit.xml",
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        (
            "dependency_regression_firewall",
            pytest_summary(JUNIT_PATHS["dependency_regression_firewall"]),
            79,
        ),
        ...
        (
            "targeted_k05_boundaries",
            pytest_summary(JUNIT_PATHS["targeted_k05_boundaries"]),
            95,
        ),
        (
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        ...
        "dependencies": {
            ...
            "K04": _sealed_dependency("K04", "K04-0001", "E0038", "E0039"),
            "S05": _sealed_dependency("S05", "S05-0001", "E0175", "E0176"),
        },
        "next_action": "SEAL_K05_0001_THEN_CONTINUE_DAG",
...
def write_scope_verification() -> dict[str, Any]:
    ...
    return {
        "approved_scope": [
            "src/epistemic_foundry/evidence/v4_k05/**",
            "artifacts/work_packages/K05/**",
            "src/epistemic_foundry/evidence/__init__.py (HD-EF4-K05-SCOPE-20260802-001)",
        ],
        "attempt_id": ATTEMPT_ID,
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "attempt_id": ATTEMPT_ID,
        "declaring_sources": {
            "holdout": ("verifier_firewall.build_holdout_manifest (composed)"),
            "integrity_statuses": (
                "schemas/source-integrity-report.schema.json (positional, pinned)"
            ),
            "license_vocabulary": (
                "schemas/document-manifest.schema.json license_status enum (positional)"
            ),
            "novelty": (
                "schemas/novelty-assessment.schema.json dimensions and enum-property order"
            ),
        },
        "exit_criteria": {
            "all_completion_and_external_effects_resolve_to_immutable_receipts": {
                "mechanism": (
                    "every identifier is derived from content — snapshot, partition, boundary and assessment ids alike — so the module holds no clock, no randomness and no new_id at all"
                ),
                "status": "PASS",
            },
            "governing_schemas_authority_boundaries_failure_states_exact": {
                "mechanism": (
                    "license, integrity and novelty vocabularies are read positionally from their canonical schemas with every position pinned by an assertion, the novelty field NAMES are selected by declaration order because one of them is itself an enum value elsewhere, and every emitted canonical record passes validate_artifact"
                ),
                "status": "PASS",
            },
            "happy_negative_crash_resume_adversarial_coverage": {
                "mechanism": (
                    "snapshot identity is the sorted content hashes, drift is refused on re-validation, a document in two partitions is leakage by construction, holdout handles must be drawn from the pinned snapshot, and prior art outside the declared boundary or after the as-of bound is refused"
                ),
                "status": "PASS",
            },
            "no_candidate_model_prompt_backend_or_hook_acquires_authority": {
                "mechanism": (
                    "the holdout seal is delegated to the firewall's own builder with its refusals surfaced rather than re-implemented, and the reachable novelty ladder is capped below both canonical ladders' tops so a corpus-bounded search can never claim novelty review eligibility"
                ),
                "status": "PASS",
            },
        },
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_k05_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "K05_EVIDENCE_SNAPSHOT_HOLDOUT_PRIOR_ART",
        ...
        "next_package": "L06-0001",
        "not_claimed": [
            "search-completeness verification: the certificate id is accepted opaque and its coverage belongs to the search-completeness surface",
            "collision-proof holdout handles: 128-bit truncated digests assume collision resistance",
            "any novelty judgment above the capped ladder",
...
def build() -> dict[str, Any]:
    ...
    write_json("k05-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("K05-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "k05-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored K05-0001 report is not the deterministic document")
    return _summary()
