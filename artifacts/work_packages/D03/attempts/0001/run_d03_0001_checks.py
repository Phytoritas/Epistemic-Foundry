#!/usr/bin/env python3
"""Run the D03-0001 content-addressed artifact store acceptance checks.

Required checks from manifests/development_manifest.yaml (D03):
``artifact_hash_test`` and ``orphan_receipt_test``.  Both are Node test files
under ``packages/foundry-kernel/src/artifacts``, alongside the content-addressed
artifact store they exercise, so they also join the full Node inventory rather
than being targeted only.  The store composes its durable substrate on the
sealed D01 SQLite WAL canonical store; the whole upstream Node surface is the
regression that matters, so it is covered by the full Node suite rather than a
bespoke dependency-regression run.
"""
...
ATTEMPT_SCRIPTS = "artifacts/work_packages/D03/attempts/0001"
...
def ruff_check() -> int:
    # The D03 surface is Node; ruff still guards this attempt's own scripts.
    return run(
...
def artifact_hash_test() -> int:
    return _node("artifact-hash-test", [f"{COMPONENT}/artifact-hash.test.mjs"])


def orphan_receipt_test() -> int:
    return _node("orphan-receipt-test", [f"{COMPONENT}/orphan-receipt.test.mjs"])
...
def main() -> int:
    ...
    checks = {
        ...
        "artifact-hash-test": artifact_hash_test,
        "orphan-receipt-test": orphan_receipt_test,
        "targeted": targeted,
