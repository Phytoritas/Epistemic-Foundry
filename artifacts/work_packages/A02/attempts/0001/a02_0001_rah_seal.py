#!/usr/bin/env python3
"""Append A02-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed A01/R05/X03 templates do.

A02 depends on A01.  Its product bytes are the two documents it attests
(``manifests/product_invariants.yaml`` and ``docs/product_constitution.md``),
bound here by explicit path since they do not share a single directory; A02
makes no edit to them.
"""
...
#: The current committed generation (e.g. "000257-f15ea056"); its retained
...
EXPECTED_PARENT = "000258-c2f1262c"
EXPECTED_PARENT_COUNT = 258
EXPECTED_PARENT_EVIDENCE = "E0260"
EXPECTED_PARENT_MANIFEST_SHA256 = "6c569519ad15b16df4afa8b1a4d7a78c30b1d203dcfa4d3baf65859f3e199b02"
CORE_EVIDENCE_ID = "E0261"
FINAL_EVIDENCE_ID = "E0262"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_a02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after A02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} product invariants and non-goals is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "A02-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required A02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: two documents A02 attests do not share a single directory.
    relatives = sorted(
        (
...
def core_summary() -> str:
    ...
    return (
        "A02-0001 PASS core verification: product invariants and non-goals, "
        "attested by a bounded implementation agent (A02 maker) that authored "
        "two deterministic attestation harnesses under "
        "artifacts/work_packages/A02/** and reviewed by the sealing session as "
        "an independent contract-reviewer (actor-independent from the author). "
        "A02 attests the two documents manifests/product_invariants.yaml and "
        "docs/product_constitution.md exactly as they currently are and makes "
        "no edit to them. invariant_schema_check asserts the validation_contract "
        "anchors the atomic unit at invariant_id, the 64 invariants are unique "
        "and contiguous EF4-I01..EF4-I64 with every required binding present and "
        "every work-package reference well-formed, each invariant statement is "
        "byte-equal to MASTER_SPEC.md and docs/product_constitution.md, and the "
        "ten non-goals EF4-NG01..EF4-NG10 are guarded and disclaim production "
        "performance and a required search backend. forbidden_claim_scan asserts "
        "neither document carries an un-negated production-maturity overclaim "
        "while the guard is not inert. invariant_schema_check 3/3, "
        "forbidden_claim_scan 3/3, the full Python suite green, the live full "
        "Node suite green with zero failures, and git diff --check all pass. A02 "
        "depends on A01; the sealed A01-0001 attempt is the build dependency and "
        "U04-0001 is the live latest-sealed regression baseline. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"A02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. A02 attests the "
        "product invariants and non-goals; it does not re-author the product "
        "documents, makes no product-maturity, runtime-executability or "
        "release-readiness claim, and asserts only that the v4 invariants are "
        "atomic and testable and that the non-goals prevent overclaim and "
        "provider lock-in. Author/reviewer separation holds while external "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("A02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("A02-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"A02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("A02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("A02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("A02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"A02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("A02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("A02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("A02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("A02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored A02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("A02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"A02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("A02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("A02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"A02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected A02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with A02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored A02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
