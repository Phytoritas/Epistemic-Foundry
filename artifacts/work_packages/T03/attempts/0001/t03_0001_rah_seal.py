#!/usr/bin/env python3
"""Append T03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "T03-0001"
WORK_PACKAGE_ID = "T03"
EXPECTED_PARENT = "000136-b2c478c7"
EXPECTED_PARENT_COUNT = 136
EXPECTED_PARENT_EVIDENCE = "E0138"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "068129eb132fa0141b9653c6d9f0f6aa7c519e64b8b1592c4cbfa501e27879d3"
)
CORE_EVIDENCE_ID = "E0139"
FINAL_EVIDENCE_ID = "E0140"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_t03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Implement W04 replay, drift and audit export on the sealed W02/W03 workflow state.",
    "Run the W04 replay, drift and audit export gate on the sealed W02/W03 workflow state.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required T03-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "T03-0001 PASS core verification: the stable CLI JSON/error and "
        "PATH-less surfaces are implemented in packages/plugin-host/src/cli "
        "with an exact T01-0002 dependency and a P01-0001 regression baseline. "
        "All 22 composed commands are projected from the sealed catalogs and "
        "map back to their tools, so the CLI can neither expose a command the "
        "tool surface lacks nor miss one it has. Every envelope is proved to "
        "survive render, parse, and re-render byte-identically before any "
        "bytes are written; keys are sorted, nulls are kept, and an undefined "
        "field, a non-finite number, or a cycle is refused rather than "
        "silently dropped. The exit-code table is checked at load against the "
        "sealed error vocabulary, is distinct, stays inside 1..125, avoids 0, "
        "126 and 127, is reversible for diagnosis, and refuses an unmapped "
        "code rather than folding it into INTERNAL. PATH-lessness is enforced "
        "by using only process.execPath, by an allowlisted child environment "
        "that excludes PATH and cannot be overridden back in, and by scanning "
        "every CLI source and the shipped dispatcher for shell:true, exec by "
        "string, PATH reads, and bare interpreter names. Ruff lint/format, "
        "cli_contract_test 22/22, pathless_cli_test 16/16, targeted 38/38, "
        "MCP regression 27/27, dispatch regression 4/4, full Python "
        "1261/1261, full Node 941/941 across 89 files, and git diff --check "
        "pass. "
        ...
        "W04-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"T03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". P01-0001, T01-0002, and every prior RAH generation remain "
        "immutable. T03 delivers the CLI surface, not a built dist/cli.mjs "
        "payload; argument schema validation stays with the sealed tool "
        "catalogs; and the PATH-less guarantee covers the CLI and its "
        "dispatcher rather than arbitrary host code. The T-phase x01-x03 "
        "packages are PASS, but W04, R05, the x05 wave, source coverage, PRD, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("T03-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "T03-0001 stable CLI JSON/error and PATH-less surfaces is "
        "evidence-sealed PASS. W04-0001 is next; implementation_gate=fail and "
        "completion_ready=false."
    )
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T03-0001 parent is not the sealed P01-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("T03 preflight requires exactly 136 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("T03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("T03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("T03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("T03 core did not append exactly E0139")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("T03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("T03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("T03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored T03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("T03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("T03 final did not append exactly E0140")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("T03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("T03 core tail is not E0139")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 138:
        raise SystemExit(f"unexpected T03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with T03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored T03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
