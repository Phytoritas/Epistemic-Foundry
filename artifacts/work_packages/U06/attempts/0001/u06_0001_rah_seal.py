#!/usr/bin/env python3
"""Append U06-0001 PASS evidence while global completion remains false.

Seal-prep note (read before running).  This seal is authored in a seal-prep
session that was instructed not to touch ``.rah/``, so it cannot read the live
ledger to pin the parent generation, its evidence tail, its manifest hash, or
the two evidence identifiers this attempt will mint.  All SIX of those
ledger-tail values are therefore left as sentinels and the seal refuses to run
until the parent fills them from the live ledger tail (the generation committed
by the most recent sealed closeout).  Everything else — summaries, hashes,
flat-snapshot verification, generation preservation — is derived exactly as the
R05 template does.  The parent that actually seals must, before running:

  1. read the live ``.rah/ralph`` current pointer and set ``EXPECTED_PARENT``,
     ``EXPECTED_PARENT_COUNT``, ``EXPECTED_PARENT_EVIDENCE`` (the ledger tail),
     and ``EXPECTED_PARENT_MANIFEST_SHA256`` from the current generation, and
  2. set ``CORE_EVIDENCE_ID`` and ``FINAL_EVIDENCE_ID`` to the next two free
     evidence identifiers (parent tail + 1 and + 2).
"""
...
#: The generation the most recent sealed closeout committed; fill from
#: `.rah/ralph` current pointer before running, e.g. "0002NN-XXXXXXXX".
EXPECTED_PARENT = "000268-7dfee6d6"
#: The number of preserved generations at the ledger tail (an int > 0).
EXPECTED_PARENT_COUNT = 268
#: The last evidence identifier in the sealed ledger, e.g. "E0NNN".
EXPECTED_PARENT_EVIDENCE = "E0270"
EXPECTED_PARENT_MANIFEST_SHA256 = "44f11182d98eee97af0cd948422e67f1d9816852fe6d9e0c8349651e22d7fd1e"
#: The two identifiers this attempt will mint (parent tail + 1 and + 2).
CORE_EVIDENCE_ID = "E0271"
FINAL_EVIDENCE_ID = "E0272"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_u06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after U06-0001 seals and select the next "
...
GATE_NOTE = (
    f"U06-0001 honest degraded UI / operator usability integration gate is "
...
def assert_parent_pins() -> None:
    ...
    if PARENT_PIN_SENTINEL in string_pins or EXPECTED_PARENT_COUNT <= 0:
        raise SystemExit(
            "U06-0001 seal is not pinned: set the six ledger-tail pins "
            "(EXPECTED_PARENT, EXPECTED_PARENT_COUNT, EXPECTED_PARENT_EVIDENCE, "
            "EXPECTED_PARENT_MANIFEST_SHA256, CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID) "
            "from the live ledger tail before running."
        )
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required U06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  U06 owns only
    #: the v4_u06 gate; the parent console marker was sealed by U05.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/console/v4_u06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "U06-0001 PASS core verification: honest degraded UI / operator "
        "usability integration gate, implemented in "
        "src/epistemic_foundry/console/v4_u06 by a bounded implementation agent "
        "and reviewed by an independent contract-review session (a distinct "
        "actor, actor-independent from the author) under an X05-0001 regression "
        "baseline and the sealed U05-0001 console projection it composes. Each "
        "panel resolves to one of the four honest-UI states read from "
        "epistemic_foundry.observability.result_state (never named in the gate): "
        "a surface that is absent, malformed, or that U05 refuses becomes an "
        "UNAVAILABLE panel carrying the reason it cannot be trusted, and is "
        "never fabricated into a POPULATED or EMPTY_CONFIRMED panel. Every panel "
        "and dashboard carries readonly=true and grants_authority=false, any "
        "authority_request is refused before any surface is read, and a "
        "dashboard is complete only when every embedded panel is populated, a "
        "verdict independently re-derivable from the panels it embeds. Every "
        "decision resolves to a content-addressed receipt with no clock or "
        "random draw. Ruff lint/format, wire-literal discipline, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the U05 dependency regression, full Python and "
        "full Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by an independent contract-review session with "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"U06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. U06 composes "
        "read-only operator views and records honest UI state only: it does not "
        "score, select, promote or evaluate any candidate, makes no DSSAT or "
        "plant-model parity claim, and promotion remains a governance decision. "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("U06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U06-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"U06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("U06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("U06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"U06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("U06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("U06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("U06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored U06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("U06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"U06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("U06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"U06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected U06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with U06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored U06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
