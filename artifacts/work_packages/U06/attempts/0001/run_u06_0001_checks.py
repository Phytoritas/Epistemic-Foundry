#!/usr/bin/env python3
"""Run the U06-0001 honest degraded UI / operator usability gate checks.

Required checks from manifests/development_manifest.yaml (U06):
...
report.  The gate ships under src/epistemic_foundry/console/v4_u06, a package
authorized by the U06 write scope beneath the ``console`` marker U05 already
placed on the wheel, so ``packaging-discovery`` proves ``console.v4_u06``
...
its owner (never naming one) is the invariant most likely to break here.

U06 declares exactly one dependency, U05, whose sealed console projection this
gate composes; that dependency is a pure-pytest attempt-directory package, so a
single ``dependency-regression-u05`` re-runs its sealed suite and fails here if
its projection surface drifts, exactly as the R05 template regresses each of its
typed dependencies.
...
#: The U06 write scope authorizes only this package; the parent ``console``
#: marker was sealed by U05, so ruff scopes to the v4_u06 gate and this attempt,
#: not the whole console tree.
GATE = "src/epistemic_foundry/console/v4_u06"
...
# U05 owns the sealed console projection whose four surfaces this gate composes
# into honest operator panels; a change to that projection surface must fail
# here, so its sealed attempt suite runs its own regression.
DEPENDENCY_TARGETS = {
    "dependency-regression-u05": ("artifacts/work_packages/U05/attempts/0001",),
}
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/console/v4_u06 is a new package beneath the console
    # marker, so the wheel claim is proved here rather than assumed.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-u06-usability", ATTEMPT_DIR)


def dependency_regression_u05() -> int:
    return _pytest(
        "dependency-regression-u05", *DEPENDENCY_TARGETS["dependency-regression-u05"]
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "targeted": targeted,
        "dependency-regression-u05": dependency_regression_u05,
