#!/usr/bin/env python3
"""Append S06-0001 PASS evidence while global completion remains false.
...
which at authoring time is the sealed R06-0001 closeout, evidence E0224).
...
EXPECTED_PARENT = "000226-4e6345fa"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 226
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0228"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "7873ad445004799ca7aebb6d99df9e03aaedf0dfd248b6ff8812b7c9b3fa37a4"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0229"
FINAL_EVIDENCE_ID = "E0230"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_s06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after S06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} leakage, reward-hacking and evaluator-update governance "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "S06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required S06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: security namespace is pinned so a change to the composed S05 surface (or
    #: the parent marker or the new v4_s06 gate) is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/security").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "S06-0001 PASS core verification: leakage, reward-hacking and "
        "evaluator-update governance integration gate, implemented in "
        "src/epistemic_foundry/security/v4_s06 by a bounded implementation "
        "subagent and reviewed by the sealing agent (actor-independent from the "
        "author) under exact dependency bindings (S05 S05-0001, J05 J05-0001) "
        "and an R06-0001 regression baseline. The gate composes the sealed S05 "
        "leakage-audit surface and the J05 quarantine workflow node without "
        "restating them, then derives each decision from the composed records' "
        "own fields: a reward signal is refused unless its fitness vector "
        "already passed the hard gate and its routing receipt learns from a "
        "non-immediate-proxy basis with a clean leakage audit over the required "
        "surfaces, and an evaluator update is refused unless it is future-only "
        "and independently qualified against a bundle distinct from the current "
        "one; feedback carrying holdout material and a reachable holdout are "
        "refused, and no candidate, model, prompt, backend or hook acquires "
        "evaluator, holdout or promotion authority. Every declared finding code "
        "names an exact refusal. Every admit or refuse decision re-derives byte "
        "for byte from its own published fields with no clock or random draw, "
        "and nothing scores, ranks, selects, promotes or evaluates. Ruff lint/"
        "format, schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the S05 and J05 dependency regressions, full "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"S06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. S06 gates the reward "
        "and evaluator-update surfaces and records a deterministic admit/refuse "
        "receipt only: it does not score, rank, select, promote or evaluate any "
        "candidate, performs no retroactive evaluator update and admits only "
        "future-only updates, grants no authority at the gate, makes no DSSAT or "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("S06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"S06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("S06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("S06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"S06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("S06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("S06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("S06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored S06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("S06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"S06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("S06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"S06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected S06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with S06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored S06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
