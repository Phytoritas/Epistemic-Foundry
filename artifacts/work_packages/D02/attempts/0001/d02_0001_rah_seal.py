#!/usr/bin/env python3
"""Append D02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000290-1f7b1a0a"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 290
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0292"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "cccbc44e9a5fcc51449605374598ec64bf28a0d7b59aa6a2036de65c6e98f283"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0293"
FINAL_EVIDENCE_ID = "E0294"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_d02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after D02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} PostgreSQL team store and tenant isolation is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "D02-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required D02-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  Both D02
    #: write-scope roots are pinned so a change to any product, migration, or
    #: test file is caught here too.
    roots = (
        ROOT / "python/epistemic_foundry/storage/postgres",
...
def core_summary() -> str:
    ...
    return (
        "D02-0001 PASS core verification: PostgreSQL TEAM-profile team store and "
        "tenant isolation, implemented in "
        "python/epistemic_foundry/storage/postgres and migrations/postgres by a "
        ...
        "(D01); the adapter imports the standard library alone and injects no "
        "driver, so there is no composed code surface. The team store reuses the "
        "D01 logical identity (tenant_id, workspace_id, record_type, record_id) "
        "with revision genesis zero, one atomic compare-and-swap that increments "
        "by exactly one, stale/duplicate/missing refusals (STALE_REVISION, "
        "RECORD_ALREADY_EXISTS, RECORD_NOT_FOUND), REVISION_EXHAUSTED at the "
        "9007199254740991 ceiling, unbounded JavaScript-string identity via the "
        "reversible utf16be-lowerhex-v1 projection under deterministic "
        "pg_catalog.\"C\" collation, and PostgreSQL json (not jsonb) with "
        "fail-closed integrity validation so member order, control characters, "
        "and lone UTF-16 surrogates round-trip while non-finite numbers such as "
        "1e400 are rejected. Tenant/workspace isolation is enforced with ENABLE "
        "plus FORCE ROW LEVEL SECURITY binding even the non-superuser table owner: "
        "access requires the runtime role's administrative principal_scopes "
        "binding on session_user, the transaction-local "
        "epistemic_foundry.tenant_id/workspace_id GUCs, and scope_is_authorized, "
        "so a missing context is default-deny and a GUC set for an unauthorized "
        "scope raises 42501/InsufficientPrivilege; the four fixed SECURITY DEFINER "
        "functions pin search_path=pg_catalog with PUBLIC EXECUTE revoked, and "
        "superuser/BYPASSRLS principals or privilege, column, policy, "
        "role-membership and catalog drift enter fail-closed SAFE_MODE. The two "
        "required suites are green against a real postgres:16 server "
        "(postgres_contract_test 30/30, tenant_isolation_test 13/13; conftest "
        "provisions an ephemeral database and NOSUPERUSER NOBYPASSRLS roles from "
        "an environment admin DSN and refuses a mock-only run), ruff check and the "
        "EF4-I22 wire-literal gate (5/5) pass, packaging discovery proves the "
        "component is importable yet intentionally outside the wheel, and full "
        "Python (1261/1261), full Node (1702/1702 across the 136-file inventory) "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"D02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. D02 seals a PostgreSQL "
        "TEAM-profile revisioned-record store and tenant/workspace isolation only: "
        "it does not provide the content-addressed artifact store or receipts (D03 "
        "territory), does not score, rank, select, promote or evaluate any "
        "candidate, and refuses adversarial input, unauthorized scope, and catalog "
        "or privilege drift with an exact finding code or fail-closed SAFE_MODE "
        "rather than degrading silently. Author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("D02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D02-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"D02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("D02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("D02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"D02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("D02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("D02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("D02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored D02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("D02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"D02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("D02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"D02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected D02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with D02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored D02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
