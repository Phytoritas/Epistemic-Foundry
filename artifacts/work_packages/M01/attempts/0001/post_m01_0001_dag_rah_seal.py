#!/usr/bin/env python3
"""Append the verified post-M01-0001 DAG reconciliation to active RAH state."""
...
import m01_0001_rah_seal as m01_seal  # noqa: E402
import post_m01_0001_dag_reconciliation as reconciliation  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_PARENT = "000054-ada9bb64"
EXPECTED_PARENT_MANIFEST_HASH = (
    "5f23112d3fc68cdd1d9af8b598f98103d716b0626cd006be8b7ce7745d6f55b3"
)
EXPECTED_EVIDENCE_ID = "E0055"
NEXT_ACTIONS = [
    "Execute M02-0001 real baseline centrality and graph algorithms under its exact manifest scope.",
    "Run M02 centrality_reference_test and uniform_rank_regression with regressions and separate primary-session review.",
    "Seal M02 evidence and recompute the live 156-package DAG without setting completion_ready=true.",
]
...
def validate_reconciliation() -> tuple[dict[str, Any], str]:
    ...
    path = ATTEMPT / "post-m01-0001-dag-reconciliation.json"
    ...
    if not (
        ...
        and stored.get("completed_package_count") == 54
        and stored.get("waiting_package_count") == 97
        and stored.get("ready_packages_manifest_order")
        == ["M02", "M03", "N01", "T01", "A06"]
        and stored.get("next_package") == "M02"
    ...
    review_path = ATTEMPT / "post-m01-0001-dag-review.md"
    ...
    hashes = {
        ...
        "reconciler": sha256(ATTEMPT / "post_m01_0001_dag_reconciliation.py"),
        "sealer": sha256(Path(__file__)),
    }
    summary = (
        "Post-M01-0001 live DAG reconciliation PASS is append-only bound to sealed "
        "M01-0001 final evidence E0054 / 000054-ada9bb64. The canonical external "
        ...
        "no unknown dependency and no cycle; 54 packages are PASS, 5 are "
        "dependency-ready, and 97 wait on unmet dependencies. Manifest-order READY "
        "is M02, M03, N01, T01, A06, so M02 is next. Highest numeric attempt "
        ...
        f"sha256:{hashes['sealer']}. This selects work only; implementation_gate=fail "
...
def commit(summary: str) -> str:
    ...
    if sha256(parent_manifest) != EXPECTED_PARENT_MANIFEST_HASH:
        raise SystemExit("sealed M01 parent generation manifest hash changed")
    identifiers = base.evidence_ids(payloads)
    if identifiers[-1] != "E0054" or len(base.numbered_generations(ralph_root)) != 54:
        raise SystemExit("post-M01 DAG seal requires exact E0054 / 54-generation state")
    ...
    note = (
        "The sealed post-M01 DAG selects M02 as the earliest dependency-ready "
...
def main() -> int:
    ...
    identifiers = base.evidence_ids(payloads)
    if current == EXPECTED_PARENT and identifiers[-1] == "E0054":
        ...
        if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
            raise SystemExit("stored E0055 summary differs from current DAG artifacts")
    result = m01_seal.verify_generation_store(55)
