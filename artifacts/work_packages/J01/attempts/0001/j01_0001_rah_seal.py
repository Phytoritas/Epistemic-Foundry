#!/usr/bin/env python3
"""Append J01-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000340-946f3fb8"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 340
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0342"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "af0804b821de4707390c2ee021fcb78da4347d7192f74f5e9c58e99db9143727"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0343"
FINAL_EVIDENCE_ID = "E0344"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_j01_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after J01-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} parent skill router and trigger-boundary gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "J01-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required J01-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: The five J01 product bytes outside the attempt directory, bound by path.
    #: The downstream foundry/references/** progressive-reference files are J02
    #: work, not J01 product, so they are intentionally not hashed here.
    return {
        relative: sha256(ROOT / relative)
        for relative in sorted(evidence.EXPECTED_PRODUCT_HASHES)
    }
...
def core_summary() -> str:
    ...
    return (
        "J01-0001 PASS core verification: the deterministic metadata-only parent "
        "skill router, implemented in packages/plugin-host/src/skill-router with "
        "bounded foundry skill metadata by a bounded implementation agent and "
        "reviewed by this seal-prep session (a distinct actor, actor-independent "
        "from the author) on the sealed C04 content-addressed artifact store, the "
        "sealed G04 and H01 host contracts, and the sealed S03 remote-authorization "
        "brand with no new production dependency. Implicit invocation is bounded to "
        "a single unambiguous bundled candidate that explicitly permits it, is "
        "non-sensitive and non-side-effecting, matches a bounded trigger, and hits "
        "no exclusion; missing or unspecified policy, absent triggers, exclusions, "
        "remote source, and ties all abstain into mode none. Sensitive, "
        "side-effecting, administrative, and remote skills are explicit-only, a "
        "remote explicit route requires the exact S03-branded activation "
        "authorization, and an unknown explicit id fails closed. Full instructions, "
        "bodies, references, proxies, accessor getters, sparse arrays, duplicate "
        "ids, and invalid hashes fail closed; the decision hash binds the exact "
        "indexed skill content, candidate and phrase order do not change it, caller "
        "input is not mutated, and the returned decision is deeply frozen. "
        "skill_metadata_lint lints only this skill's own SKILL.md and "
        "agents/openai.yaml. Ruff lint/format, skill_routing_eval 15/15, "
        "skill_metadata_lint 4/4, targeted 19/19, full Python 1261/1261, full Node "
        "1291/1291 across 115 files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"J01-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". C04-0001, G04-0001, H01-0001, S03, and every prior RAH generation "
        "remain immutable. J01 provides the bounded parent skill router and "
        "trigger boundaries; progressive-reference loading, ContextCapsule "
        "assembly, the J02 context-budget gate, skill activation, remote "
        "installation, authorization issuance, source coverage, PRD, review, and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("J01-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J01-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"J01 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("J01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("J01-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"J01 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("J01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("J01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("J01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored J01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("J01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"J01 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("J01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"J01 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected J01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with J01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored J01 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
