#!/usr/bin/env python3
"""Run the S05-0001 evolution threat-control acceptance checks.

Required checks from manifests/development_manifest.yaml (S05):
...
report.  The controls ship under src/epistemic_foundry/security/v4_s05, so
...
ATTEMPT_ID = "S05-0001"
COMPONENT = "artifacts/work_packages/S05/attempts/0001"
ENGINE = "src/epistemic_foundry/security/v4_s05"
# S04's red-team and traceability suites are the Node face of the security
# surface, C05 owns the sealed search space and evaluator composites, and the
# Python regression covers the composed firewall, quarantine, budget and
# sandbox modules.
DEPENDENCY_TARGETS = {
    ...
    "dependency-regression-redteam": ("tests/security",),
}
...
def packaging_discovery() -> int:
    # src/epistemic_foundry/security already ships, so no new marker and no
    # scope decision exist; this check proves that claim instead of assuming.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-s05-threat-controls", COMPONENT)


def dependency_regression_c05() -> int:
    return _pytest(
        "dependency-regression-c05",
        *DEPENDENCY_TARGETS["dependency-regression-c05"],
    )


def dependency_regression_firewall() -> int:
    return _pytest(
        "dependency-regression-firewall",
        *DEPENDENCY_TARGETS["dependency-regression-firewall"],
    )


def dependency_regression_redteam() -> int:
    name = "dependency-regression-redteam"
    files = sorted(
    ...
    if not files:
        ...
        print("the red-team gate declares no Node test file", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    checks = {
        ...
        "dependency-regression-firewall": dependency_regression_firewall,
        "dependency-regression-redteam": dependency_regression_redteam,
