#!/usr/bin/env python3
"""Prove the S05 controls are discoverable by the project's own packaging config.

``src/epistemic_foundry/security`` already ships with the wheel, so unlike
E05, F05 and M05 no new package marker was needed and no scope decision
exists; this check exists to prove that claim rather than assume it.  The
...
EXPECTED = (
    "epistemic_foundry.security",
    "epistemic_foundry.security.v4_s05",
)
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the controls would import from a checkout but be absent "
                        "from the wheel"
                    ),
                    "status": "FAIL",
                },
