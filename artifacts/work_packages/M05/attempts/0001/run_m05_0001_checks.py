#!/usr/bin/env python3
"""Run the M05-0001 evolution cartography acceptance checks.

Required checks from manifests/development_manifest.yaml (M05):
...
report.  The mapper ships under src/epistemic_foundry/cartography/v4_m05, so
``wire-literal-discipline`` runs the repository's EF4-I22 gate as its own
check.  The D05 store regression drives a real PostgreSQL container;
mock-only substitution is forbidden by that suite itself.
"""
...
ATTEMPT_ID = "M05-0001"
COMPONENT = "artifacts/work_packages/M05/attempts/0001"
ENGINE = "src/epistemic_foundry/cartography/v4_m05"
# M04's map UI is the Node face of the cartography surface, D05 is the store
# the niches and lineage persist to, L05 owns the lineage memory this mapper
# composes, and the archive module owns the coverage summary it delegates to.
DEPENDENCY_TARGETS = {
    "dependency-regression-archive": ("tests/test_epistemic_species_archive.py",),
    "dependency-regression-map": ("web/src/features/map",),
...
def packaging_discovery() -> int:
    # Without the cartography package marker the mapper imports from a
    # checkout but is absent from the wheel (HD-EF4-M05-SCOPE-20260802-001).
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-m05-cartography", COMPONENT)
...
def dependency_regression_retention() -> int:
    return _pytest(
        ...
        *DEPENDENCY_TARGETS["dependency-regression-retention"],
    )
...
def dependency_regression_map() -> int:
    name = "dependency-regression-map"
    files = sorted(
    ...
    if not files:
        ...
        print("the map gate declares no Node test file", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    checks = {
        ...
        "dependency-regression-map": dependency_regression_map,
