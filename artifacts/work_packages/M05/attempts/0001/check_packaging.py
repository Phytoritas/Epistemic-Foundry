...
Without ``src/epistemic_foundry/cartography/__init__.py`` the mapper imports
...
variant (HD-EF4-M05-SCOPE-20260802-001).  This check reads the discovery mode
from pyproject instead of assuming it, so a change there fails here.
"""
...
EXPECTED = (
    ...
    "epistemic_foundry.cartography.v4_m05",
)
            ...
            json.dumps(
                {
                    ...
                    "missing": missing,
                    "note": (
                        "the mapper would import from a checkout but be absent "
                        "from the wheel"
                    ),
                    "status": "FAIL",
                },
