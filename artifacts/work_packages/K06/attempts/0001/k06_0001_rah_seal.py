#!/usr/bin/env python3
"""Append K06-0001 PASS evidence while global completion remains false.
...
or the evidence identifiers this attempt will mint.  Every ledger pin is
therefore left as the sentinel ``FILL_FROM_LEDGER`` and
``assert_parent_pins()`` refuses to run until the parent fills all of them from
the live ledger tail: ``EXPECTED_PARENT`` and ``EXPECTED_PARENT_COUNT`` and
``EXPECTED_PARENT_MANIFEST_SHA256`` describe the current tail, ``EXPECTED_PARENT_EVIDENCE``
is its latest evidence id, and ``CORE_EVIDENCE_ID`` / ``FINAL_EVIDENCE_ID`` are
the two identifiers this seal appends (the tail's latest id plus one, plus two).
Everything else — summaries, hashes, flat-snapshot verification, generation
accounting — is derived exactly as the templates do.
"""
...
#: parent fills all six from the current frontier before running this seal.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
EXPECTED_PARENT = "000218-3238ad83"
EXPECTED_PARENT_COUNT = 218
EXPECTED_PARENT_EVIDENCE = "E0220"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "c40c7bd6446c6b4bb9a7913e681b4373b98159c1e966bb18e32a35bab04e096a"
)
CORE_EVIDENCE_ID = "E0221"
FINAL_EVIDENCE_ID = "E0222"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_k06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after K06-0001 seals and select the next "
...
#: is built from the evidence pins so that, once the parent fills them, it names
#: this attempt's evidence automatically.
GATE_NOTE = (
    f"{ATTEMPT_ID} evidence/holdout version and leakage-prevention integration "
...
def assert_parent_pins() -> None:
    if PARENT_PIN_SENTINEL in (
        ...
        str(EXPECTED_PARENT_EVIDENCE),
        str(EXPECTED_PARENT_MANIFEST_SHA256),
        str(CORE_EVIDENCE_ID),
        str(FINAL_EVIDENCE_ID),
    ):
        raise SystemExit(
            "K06-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required K06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: evidence marker tree is covered so the pre-existing K05 marker is proven
    #: unchanged alongside the v4_k06 gate this attempt owns.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/evidence").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "K06-0001 PASS core verification: evidence/holdout version and "
        "leakage-prevention integration gate, implemented in "
        "src/epistemic_foundry/evidence/v4_k06 by an implementing agent under "
        "bounded delegation and reviewed by an independent reviewer "
        ...
        "bindings (K05 K05-0001, O05 O05-0001), composing the S05 verifier "
        "firewall, and a W05-0001 regression baseline. The gate binds one "
        "re-derivable evidence/holdout version over the K05 snapshot bytes, the "
        "concealed partition, the holdout manifest, the S05 evaluator bundle and "
        "the prior-art date, re-deriving every identity through the module that "
        "owns it and verifying the holdout against the sealed manifest and the "
        "firewall leakage set rather than a label. Three invariant refusals "
        "hold: a stale evidence/holdout version, a hidden-holdout exposure, and "
        "evaluator feedback intersecting the holdout, including against a lying "
        "firewall that reports the holdout reachable. Every decision re-derives "
        "byte for byte from its own published fields with no clock or random "
        ...
        "packaging-discovery, K05/O05 dependency regressions, full Python and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"K06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. K06 binds an "
        "evidence/holdout version and admits or refuses candidate-facing "
        "operations against it only: it does not score, select, promote or "
        "evaluate any candidate, makes no DSSAT or plant-model parity claim, and "
        "promotion remains a governance decision. Author/reviewer separation "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("K06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("K06-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"K06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("K06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("K06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("K06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"K06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("K06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("K06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("K06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored K06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("K06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"K06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("K06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"K06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected K06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with K06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored K06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
