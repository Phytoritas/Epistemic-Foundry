#!/usr/bin/env python3
"""Prove the K06 leakage gate is discoverable by the packaging config.

``src/epistemic_foundry/evidence`` was authorized as a package marker by K05 and
``evidence.v4_k06`` is the new marker this package adds, so there is a scope
decision to prove rather than assume: a marker that never reached the wheel would
let the gate import from a checkout and vanish from an installed distribution.
...
EXPECTED = ("epistemic_foundry.evidence", "epistemic_foundry.evidence.v4_k06")
