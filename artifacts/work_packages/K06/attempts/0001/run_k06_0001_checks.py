#!/usr/bin/env python3
"""Run the K06-0001 evidence/holdout version and leakage-gate acceptance checks.

Required checks from manifests/development_manifest.yaml (K06):
...
gate ships under src/epistemic_foundry/evidence/v4_k06 behind the ``evidence``
package marker (authorized by K05, extended here with the ``v4_k06`` marker), so
``packaging-discovery`` proves the marker actually reaches the wheel rather than
only importing from the checkout.

The dependency regressions cover the sealed surfaces this package composes: K05's
corpus/holdout/prior-art boundaries, whose snapshot, partition, holdout and
boundary identities the gate re-derives, and O05's evolution retrieval surface,
whose plan and lane-receipt identities the gate admits against a bound version.
Each is exercised through the dependency's own sealed test suite, so a regression
in either fails here rather than only inside the full suite.
...
# The K06 write scope is the v4_k06 gate package only.  ``evidence/__init__.py``
# is the pre-existing K05 marker and is deliberately outside the component the
# ruff/write-scope surface covers.
EVIDENCE = "src/epistemic_foundry/evidence/v4_k06"
PRODUCT_TESTS = {
...
# K05 owns the corpus/holdout/prior-art boundaries the gate re-derives snapshot,
# partition, holdout and boundary identity through; O05 owns the evolution
# retrieval surface whose plan and lane-receipt identities the gate admits
# against the bound version.  A change in either must fail here, so each runs its
# own sealed suite from its attempt directory.
DEPENDENCY_TARGETS = {
...
def ruff_check() -> int:
    return run(
        "ruff-check", ["uv", "run", "--locked", "ruff", "check", EVIDENCE, ATTEMPT_DIR]
    )
...
def ruff_format_check() -> int:
    return run(
        "ruff-format-check",
        ["uv", "run", "--locked", "ruff", "format", "--check", EVIDENCE, ATTEMPT_DIR],
    )


def packaging_discovery() -> int:
    # evidence.v4_k06 is a new package marker, so the wheel claim is proved here
    # rather than assumed.
    return run(
        "packaging-discovery",
        [
...
def targeted() -> int:
    return _pytest("targeted-k06-leakage-gate", ATTEMPT_DIR)
