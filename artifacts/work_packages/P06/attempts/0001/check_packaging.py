#!/usr/bin/env python3
"""Prove the P06 no-majority referral gate is discoverable by the packaging config.

``src/epistemic_foundry/parliament/v4_p06`` is a new package under the P06 write
scope (``src/epistemic_foundry/parliament/v4_p06/**``).  The enclosing
``parliament`` namespace marker already exists — the sealed P05 gate created and
owns it — so P06 adds no marker outside its own write scope; but a ``v4_p06``
package that never reached the wheel would let the gate import from a checkout and
vanish from an installed distribution, so its discoverability is a scope fact to
prove rather than assume.  The discovery mode is read from pyproject instead of
assumed, so a change there fails here.
"""
...
EXPECTED = ("epistemic_foundry.parliament", "epistemic_foundry.parliament.v4_p06")
