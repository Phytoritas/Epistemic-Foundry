#!/usr/bin/env python3
"""Append P06-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000278-7d3ceaaf"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 278
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0280"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "d365af0d3cbf08b0354b3597145c5f58f63608157a15bf0c14f78e6f86474487"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0281"
FINAL_EVIDENCE_ID = "E0282"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_p06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after P06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} no-majority promotion and sealed-candidate attestation "
    f"referral gate is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "P06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required P06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  P06's write
    #: scope is the v4_p06 package only (the enclosing parliament marker predates
    #: P06 and is owned by P05), so only that package is pinned so a change to the
    #: v4_p06 gate or its marker is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/parliament/v4_p06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "P06-0001 PASS core verification: no-majority promotion and "
        "sealed-candidate attestation referral gate, implemented in "
        "src/epistemic_foundry/parliament/v4_p06 by a bounded implementation "
        "subagent and reviewed by the sealing agent (actor-independent from the "
        "author) under exact dependency bindings (P05 P05-0001, V05 V05-0001) and "
        "an X05-0001 regression baseline. The gate decides whether a sealed "
        "candidate may be referred to the promotion authority, or must be "
        "withheld, and never promotes: gate_grants_promotion is always False and "
        "promotion authority lives in governance.promotion, which takes no score. "
        "It composes the sealed surfaces without restating them (EF4-I22) and "
        "refuses to let a promotion be reduced to a single number or a bare "
        "majority: two independent organs must both have cleared the candidate "
        "(the P05 Parliament convened the docket and the V05 cascade advanced the "
        "claim, each verified by its owning gate name — the V05 name pinned at the "
        "component boundary rather than imported, so no parliament<->validation "
        "cycle forms — and a re-derived hash), the convened docket must have "
        "preserved its dissent, and an independent sealed-candidate attestation "
        "must be schema-valid, re-derive its hash, pass, and attest over both "
        "organ receipts. The referral level is capped at the lower of the two "
        "replication-bounded ceilings. Every refer/withhold decision re-derives "
        ...
        "packaging-discovery, the P05/V05 dependency regressions, full Python and "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"P06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. P06 refers or withholds "
        "a sealed candidate to the promotion authority and records a replayable "
        "receipt only: it does not score, select, promote or evaluate any "
        "candidate, holds no promotion authority at the gate, makes no DSSAT or "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("P06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"P06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("P06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("P06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("P06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"P06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("P06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("P06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("P06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored P06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("P06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"P06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("P06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("P06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"P06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected P06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with P06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored P06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
