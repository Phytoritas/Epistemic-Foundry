#!/usr/bin/env python3
"""Append H05-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "H05-0001"
WORK_PACKAGE_ID = "H05"
EXPECTED_PARENT = "000194-a85aee3c"
EXPECTED_PARENT_COUNT = 194
EXPECTED_PARENT_EVIDENCE = "E0196"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "3a60130189bbd261de675035383858b0cb498df200edbdbe19d3f9fd50cb719e"
)
CORE_EVIDENCE_ID = "E0197"
FINAL_EVIDENCE_ID = "E0198"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_h05_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward U01-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "H05-0001 evolution/holdout observability hooks with explicit coverage limits "
    "is evidence-sealed PASS (E0197/E0198). U01-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required H05-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (
                ROOT / "plugin_blueprint/epistemic-foundry/hooks/v4_h05"
            ).rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "H05-0001 PASS core verification: evolution/holdout observability hooks with explicit coverage limits, "
        ...
        "session, under exact dependency bindings (G05 G05-0001, H04 H04-0001) "
        "and an S05-0001 regression baseline. every vocabulary is imported from the gateway or derived from the schema's own pinned-closed properties, the decision set must partition exactly into observer and control or the load refuses, and every emitted envelope passes the gateway's own validator "
        ...
        "Key review finding: Two coverage scopes deliberately differ: a registration's own disposition is scoped to its observed hosts while the report is scoped to every gateway host — claims are checked against the absolute scope. "
        ...
        + ". U01-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"H05-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "hook delivery guarantees: it observes events a host already delivered and hook coverage remains non-exhaustive per the gateway's own contract; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("H05-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H05-0001 parent is not the sealed H05 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("H05 preflight requires exactly 194 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("H05 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("H05-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H05 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("H05 core did not append exactly E0197")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H05 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("H05 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("H05 report does not bind current core generation")
    if (
        ...
        raise SystemExit("H05 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored H05 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("H05 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("H05 final did not append exactly E0198")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H05 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("H05 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("H05 core tail is not E0197")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 196:
        raise SystemExit(f"unexpected H05 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with H05 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored H05 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
