#!/usr/bin/env python3
"""Append W06-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000228-b2231735"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 228
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0230"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "928d08ab3c58b038e94402cf0440de9cb890d815c65cb8c371b501d9695b4e07"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0231"
FINAL_EVIDENCE_ID = "E0232"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_w06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after W06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} crash recovery, future-only evaluator update and replay "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "W06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required W06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: recovery namespace is pinned so a change to the composed W05 surface (or
...
def core_summary() -> str:
    ...
    return (
        "W06-0001 PASS core verification: crash recovery, future-only evaluator "
        "update and replay integration gate, implemented in "
        "src/epistemic_foundry/recovery/v4_w06 by a bounded implementation "
        "subagent and reviewed by the sealing agent (actor-independent from the "
        "author) under exact dependency bindings (W05 W05-0001, N06 N06-0001, D06 "
        "D06-0001 composed transitively through W05) and an R06-0001 regression "
        "baseline. The gate composes the sealed W05 resume/quarantine, EF4-I60 "
        "reconciliation, release replay and N06 schedule surfaces without "
        "restating them, then proves across the crash boundary that no candidate "
        "was lost or double-counted, that the resumed run replays byte for byte "
        "as the run it continues, that an evaluator update never re-scores the "
        "recovered run, and that the checkpoint, resume, replay and schedule all "
        "name one evolution run; no candidate, model, prompt, backend or hook "
        "acquires evaluator, holdout or promotion authority. Five finding codes "
        "name exact refusals and every composed-module refusal travels out under "
        "its own type and code. Every recovery re-derives byte for byte from its "
        "own published fields with no clock or random draw, and nothing scores, "
        "ranks, selects, promotes or evaluates. Ruff lint/format, "
        "schema-and-type-check, unit-and-contract-tests, "
        "negative-and-adversarial-tests, provenance-and-receipt-audit, "
        "packaging-discovery, the W05 and N06 dependency regressions, full Python "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"W06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. W06 accounts for a "
        "crash recovery and records a deterministic, re-derivable recovery "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("W06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("W06-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"W06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("W06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("W06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("W06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"W06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("W06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("W06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("W06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored W06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("W06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"W06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("W06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("W06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"W06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected W06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with W06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored W06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
