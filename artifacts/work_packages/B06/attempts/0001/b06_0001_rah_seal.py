#!/usr/bin/env python3
"""Append B06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "B06-0001"
WORK_PACKAGE_ID = "B06"
EXPECTED_PARENT = "000176-45d7314a"
EXPECTED_PARENT_COUNT = 176
EXPECTED_PARENT_EVIDENCE = "E0178"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "b3378614d91a555c8d86e2d45328c76104b8bd1f8473cebde84b519ba328483c"
)
CORE_EVIDENCE_ID = "E0179"
FINAL_EVIDENCE_ID = "E0180"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_b06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward E06-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "B06-0001 reproducible build and backend-pin integration gate "
    "is evidence-sealed PASS (E0179/E0180). E06-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required B06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: This package's product lives entirely in the attempt directory
    #: (plus gitignored generated build output bound by the report).
    return {}
...
def core_summary() -> str:
    ...
    return (
        "B06-0001 PASS core verification: reproducible build and backend-pin integration gate, "
        ...
        "session, under exact dependency bindings (B05 B05-0001, C05 C05-0001, S05 S05-0001) "
        "and an S05-0001 regression baseline. the pin ledger's field set is closed, every pin carries kind, subject, digest and source, a floating pin is refused by its own code, and the Shinka backend pin is recorded BLOCKED with digest null rather than invented "
        ...
        "Key review finding: The double build is bit-identical: wheel and sdist digests equal across two real uv builds, with environment pinning inherited from the toolchain lock (SOURCE_DATE_EPOCH cross-checked against CI). "
        ...
        + ". E06-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"B06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "Node workspace tarball reproducibility, which B02's build gate owns; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("B06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("B06-0001 parent is not the sealed B06 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("B06 preflight requires exactly 176 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("B06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("B06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("B06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("B06 core did not append exactly E0179")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("B06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("B06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("B06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored B06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("B06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("B06 final did not append exactly E0180")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("B06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("B06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("B06 core tail is not E0179")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 178:
        raise SystemExit(f"unexpected B06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with B06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored B06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
