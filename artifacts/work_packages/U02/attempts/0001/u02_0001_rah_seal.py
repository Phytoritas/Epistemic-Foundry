#!/usr/bin/env python3
"""Append U02-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000222-0bf2880b"
EXPECTED_PARENT_COUNT = 222
EXPECTED_PARENT_EVIDENCE = "E0224"
EXPECTED_PARENT_MANIFEST_SHA256 = "01c90f91ef67dcb2e9adbe2731ff9f62f739b40b0239efe6138c642bfe176aed"
CORE_EVIDENCE_ID = "E0225"
FINAL_EVIDENCE_ID = "E0226"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_u02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after U02-0001 seals and select the next "
...
GATE_NOTE = (
    f"U02-0001 dashboard shell, auth and explicit health states is "
...
def assert_parent_pins() -> None:
    ...
    if PARENT_PIN_SENTINEL in pins:
        raise SystemExit(
            "U02-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required U02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: two approved write-scope trees.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for root in ("web/src/app", "web/src/features/health")
...
def core_summary() -> str:
    ...
    return (
        "U02-0001 PASS core verification: dashboard shell, auth and explicit "
        "health states, implemented in web/src/app and web/src/features/health "
        "by a bounded implementation agent under the product owner's instruction "
        "and reviewed by a separate sealing agent (actor-independent from the "
        "author), under an exact U01 U01-0001 dependency binding and an R06-0001 "
        "regression baseline. The manifest declares exactly two required checks. "
        "ui_security_test (45/45): the local security posture, credential field "
        "vocabulary and auth machine are frozen data; secured views refuse in "
        "every non-authenticated state and credential material is refused "
        "wherever it appears; product modules read no clock, no random source and "
        "no environment (loopback auth/CSRF/CSP). degraded_state_test (37/37): "
        "read model state follows the receipt not the caller, a backend failure "
        "renders UNAVAILABLE and is never claimed as confirmed empty, and "
        "EMPTY_CONFIRMED, UNAVAILABLE and UNKNOWN stay three distinct first-class "
        "states with DEGRADED rendered with its non-passing checks. "
        "full-node-suite captured GREEN at 107 modules / 1192 tests; that "
        "repository-wide total is integration-owned and moved by concurrent "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"U02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        ...
        "auth state or health receipt, security beyond the loopback posture, or "
        "any conflation of EMPTY_CONFIRMED with UNAVAILABLE. Author/reviewer "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("U02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U02-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"U02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("U02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("U02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("U02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"U02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("U02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("U02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("U02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored U02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("U02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"U02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("U02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("U02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"U02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected U02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with U02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored U02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
