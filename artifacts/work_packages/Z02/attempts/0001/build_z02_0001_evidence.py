#!/usr/bin/env python3
"""Build and verify Z02-0001 evidence: SBOM, signing, provenance and deterministic bundle.
...
ATTEMPT_DIR = "artifacts/work_packages/Z02/attempts/0001"
AUTHORITY_DECISION = None

EXPECTED_PRODUCT_HASHES = {
    "artifacts/work_packages/Z02/attempts/0001/run_z02_0001_checks.py": "9768f103f05a7d7f745ef7de3e661cb825064dce6d17bb942e791b247e06aba0",
    "scripts/release/test_sbom.py": "57db7d33fb26bf563ad404ca4feaaded9d0aeb3130c5638b0936dc642dfeb794",
    "scripts/release/test_signature.py": "1fcaf61b1e8d92a91088cc36a8670b662938bd74440777ba39eb539d36ba320b",
    "scripts/release/test_zip_integrity.py": "d80ac3cc36e295d8989a0a61b02ce3b863d86f466cc674b6c62d79f34541e825",
    "scripts/release/z02_release_engine.py": "c3bd58ac4b5ed58cf8f12a51b2fb8ad342c16fdd559549dd9f8ffdb71d502b50",
}
...
JUNIT_PATHS = {
    ...
    "sbom_test": ATTEMPT / "sbom-test.junit.xml",
    "signature_test": ATTEMPT / "signature-test.junit.xml",
    "wire_literal_discipline": ATTEMPT / "wire-literal-discipline.junit.xml",
    "zip_integrity_test": ATTEMPT / "zip-integrity-test.junit.xml",
}
...
REQUIRED_CHECK_MAP = {
    ...
    "zip_integrity_test": "zip_integrity_test",
}
#: Maps each manifest exit-criterion key to its mechanism sentence.
EXIT_CRITERIA = {
    "bundle_clean_extracts": "zip_integrity_test builds a deterministic ZIP_STORED bundle from the in-repo payload and proves it clean-extracts byte-identically into a temp root, refusing absolute, parent-traversal, backslash and drive-colon members before extraction and refusing content-tampered or surplus members",
    "signatures_and_provenance_verify": "signature_test derives the signing status from the signatures actually present (fail-closed UNSIGNED and not-shippable for the reference bundle) and refuses any SIGNED over-claim, while the provenance record is built by the canonical runtime and validated against plugin-release-provenance.schema.json with every embedded digest re-derivable",
}
...
def regression_evidence() -> dict[str, Any]:
    ...
    for label, summary, expected in (
        ...
        ("sbom_test", pytest_summary(JUNIT_PATHS["sbom_test"]), 8),
        ("signature_test", pytest_summary(JUNIT_PATHS["signature_test"]), 9),
        (
            "wire_literal_discipline",
            pytest_summary(JUNIT_PATHS["wire_literal_discipline"]),
            5,
        ),
        ("zip_integrity_test", pytest_summary(JUNIT_PATHS["zip_integrity_test"]), 9),
    ):
        if (
            summary["collected"],
            summary["passed"],
...
def dependency_status() -> dict[str, Any]:
    ...
    status: dict[str, Any] = {
        ...
        "next_action": "SEAL_Z02_0001_THEN_CONTINUE_DAG",
...
def package_verification(regression: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "declaring_sources": {
            "build_check_names": (
                "manifests/product_invariants.yaml (EF4-I32) and workflows/plugin_release.workflow.yaml release nodes"
            ),
            "payload_identity": (
                "plugins/epistemic-foundry/.codex-plugin/plugin.json cross-checked against manifests/compatibility_matrix.yaml"
            ),
            "provenance_record": (
                "schemas/plugin-release-provenance.schema.json, built and validated through the canonical epistemic_foundry.release.provenance runtime (composed, not reimplemented)"
            ),
            "signing_status": (
                "src/epistemic_foundry/release/provenance.py derive_signing_status / release_is_shippable"
            ),
        },
        "exit_criteria": {
    ...
    records.append(
        {
            ...
            "command": [
                "python",
                "-B",
                f"{ATTEMPT_DIR}/build_z02_0001_evidence.py",
                "build",
            ],
            "exit_code": 0,
            "recorded_at_utc": RECORDED_AT,
...
def report_document(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "attempt_id": ATTEMPT_ID,
        "attempt_type": "Z02_SBOM_SIGNING_PROVENANCE_DETERMINISTIC_BUNDLE",
        ...
        "not_claimed": [
            "a real cryptographic signing: signatures are empty and status is derived UNSIGNED; the synthetic-signature control is explicitly labelled non-cryptographic",
            "a real supply-chain or transitive-dependency SBOM scan: the SBOM is a content-addressed bill of materials over the in-repo payload files only",
            "a production, executable or validated release: the bundle is describable but not shippable and the v4 plugin is not claimed executable or production-ready",
...
def build() -> dict[str, Any]:
    ...
    write_json("z02-verification.json", verification)
...
def bind_rah_state(
    *,
    ...
    if "rah_state" in stored:
        raise SystemExit("Z02-0001 report is already RAH-bound")
    ...
    verification = read_json(ATTEMPT / "z02-verification.json")
    report = report_document(
...
def verify() -> dict[str, Any]:
    ...
    if render(expected) != render(stored):
        raise SystemExit("stored Z02-0001 report is not the deterministic document")
    return _summary()
