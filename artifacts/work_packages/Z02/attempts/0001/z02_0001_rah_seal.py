#!/usr/bin/env python3
"""Append Z02-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "Z02-0001"
WORK_PACKAGE_ID = "Z02"
EXPECTED_PARENT = "000356-2dbd7499"
EXPECTED_PARENT_COUNT = 356
EXPECTED_PARENT_EVIDENCE = "E0358"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "09fffed656856eba86973274408850eb8620d95547016e5f17ca4e3947effd9b"
)
CORE_EVIDENCE_ID = "E0359"
FINAL_EVIDENCE_ID = "E0360"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_z02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward Z03-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "Z02-0001 sbom, signing, provenance and deterministic bundle "
    "is evidence-sealed PASS (E0359/E0360). Z03-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required Z02-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "scripts/release").rglob("*")
...
ATTEMPT_ARTIFACTS = (
    "build_z02_0001_evidence.py",
    "full-node-suite.junit.xml",
    "full-python-suite.junit.xml",
    "node-test-inventory.json",
    "run_z02_0001_checks.py",
    "sbom-test.junit.xml",
    "signature-test.junit.xml",
    "wire-literal-discipline.junit.xml",
    "z02_0001_rah_seal.py",
    "zip-integrity-test.junit.xml",
)
...
def core_summary() -> str:
    ...
    return (
        "Z02-0001 PASS core verification: sbom, signing, provenance and deterministic bundle, "
        ...
        "session, under exact dependency bindings (Z01 Z01-0001) "
        "and an S05-0001 regression baseline. the provenance record is built by the canonical runtime function and validated against plugin-release-provenance.schema.json (which fixes the checks[].status enum and the sha256 digest patterns), and every failure state is a typed EF_Z02_* refusal whose human reason exceeds fifty characters "
        "each module pairs happy-path proofs (deterministic re-derivation, full SBOM completeness, byte-identical clean extraction) with negative proofs (missing, extra or tampered SBOM component, signing over-claim, tampered provenance hash) and adversarial proofs (absolute-path and parent-traversal zip members refused before extraction), plus a labelled-synthetic genuineness control so the signing derivation is not vacuously always UNSIGNED "
        "the engine mutates nothing outside its write scope, derives signing status from the signatures actually present rather than accepting a claim, keeps the reference bundle fail-closed UNSIGNED and not-shippable, fabricates no signature, spawns no signing service, and defers the one real single-host lifecycle to the sealed G04-0001 gate "
        "Key review finding: The honesty boundary is load-bearing and test-enforced: the reference bundle derives UNSIGNED and release_is_shippable is False, signature-test asserts that fail-closed state and refuses any claimed SIGNED with EF_Z02_SIGNATURE_OVERCLAIM; no cryptographic signature is fabricated. "
        ...
        + ". Z03-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"Z02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "a real cryptographic signing: signatures are empty and status is derived UNSIGNED; the synthetic-signature control is explicitly labelled non-cryptographic; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("Z02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z02-0001 parent is not the sealed Z02 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("Z02 preflight requires exactly 356 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("Z02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("Z02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("Z02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("Z02 core did not append exactly E0359")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("Z02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("Z02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("Z02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored Z02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("Z02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("Z02 final did not append exactly E0360")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("Z02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("Z02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("Z02 core tail is not E0359")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 358:
        raise SystemExit(f"unexpected Z02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with Z02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored Z02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
