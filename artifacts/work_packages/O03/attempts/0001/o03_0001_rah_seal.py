#!/usr/bin/env python3
"""Append O03-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "O03-0001"
WORK_PACKAGE_ID = "O03"
EXPECTED_PARENT = "000108-6b3761d4"
EXPECTED_PARENT_COUNT = 108
EXPECTED_PARENT_EVIDENCE = "E0110"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "97dfbb52a63767c987ea2564011fa25e12612b208fdd0e0e8da68a38cb20d377"
)
CORE_EVIDENCE_ID = "E0111"
FINAL_EVIDENCE_ID = "E0112"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_o03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Resolve T01-0001 SPEC_GAP under a recorded product-owner decision and run T01-0002.",
    "Continue the dependency-ready frontier serially: T01, W01, A06, then recompute the DAG.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required O03-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "O03-0001 PASS core verification: dependency clusters and Evidence "
        "Pack assembly are implemented in "
        "python/epistemic_foundry/retrieval/evidence_pack with exact O01 "
        "PASS dependency and B04-0010 regression baseline bindings. Shared "
        "datasets, preprint/journal families, review chains, team series, "
        "model reuse, citation dependencies, and declared UNKNOWN links merge "
        "into typed clusters whose adjusted support is the independent-unit "
        "count; counter, null, boundary, and method lanes stay visible with "
        "typed exclusion reasons only; packs bind the recomputed O01 "
        "completeness certificate and reject unretrieved or metadata-only "
        "evidence. Ruff lint/format, targeted 45/45, full Python 1115/1115, "
        "full Node 819/819 across 79 files, and git diff --check pass. "
        ...
        "T01-0002 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"O03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". O01-0002, B04-0002 through B04-0010, and every prior RAH "
        "generation remain immutable. O03 is PASS, but T01, W01, A06, the "
        "wider 156-package DAG, source coverage, PRD, review, and global "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("O03-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "O03-0001 dependency clusters and Evidence Pack assembly are "
        "evidence-sealed PASS. T01-0002 SPEC_GAP resolution is next; "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("O03-0001 parent is not the sealed B04-0010 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("O03 preflight requires exactly 108 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("O03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("O03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("O03 core did not append exactly E0111")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("O03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("O03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("O03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored O03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("O03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("O03 final did not append exactly E0112")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("O03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("O03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("O03 core tail is not E0111")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 110:
        raise SystemExit(f"unexpected O03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with O03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored O03 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
