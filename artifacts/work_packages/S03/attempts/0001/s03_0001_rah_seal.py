#!/usr/bin/env python3
"""Append S03-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000316-66c70be0"
...
EXPECTED_PARENT_EVIDENCE = "E0318"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "87479d0d29197deff647101e4b91e7f9aa75a7e792e27b530ecd1a48f9db0d71"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0319"
FINAL_EVIDENCE_ID = "E0320"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_s03_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after S03-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} Skill Vault quarantine and SkillLockfile gate is evidence-sealed "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "S03-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required S03-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: skill-vault package is pinned so a change to the Skill Vault boundary, its
    #: tests, README, or package manifest is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "packages/skill-vault").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "S03-0001 PASS core verification: the Skill Vault boundary -- remote skill "
        "quarantine, non-executing static scanning, canonical SkillLockfile hash "
        "pinning, and disabled-install and explicit-activation gating -- implemented "
        "in packages/skill-vault by a bounded implementation agent and reviewed by "
        "this seal-prep session (a distinct actor, actor-independent from the author) "
        "on the sealed S01-0001 skill supply-chain package with no new production "
        "dependency. quarantineCandidate admits a remote candidate only into an inert "
        "QUARANTINED record (executable:false, active:false, authorityEligible:false) "
        "that never exposes raw files or content and fails closed on path traversal, "
        "absolute, backslash, or portable-reserved names (PATH_ESCAPE_DENIED), name "
        "collisions (PATH_COLLISION), accessor fields (ACCESSOR_FIELD_DENIED), and "
        "Proxy inputs (PROXY_INPUT_DENIED) without invoking any getter. scanCandidate "
        "asserts noScriptsExecuted:true while flagging install hooks, dynamic "
        "evaluation, self-authority claims, symlink content, a failed signature, and "
        "script content; any CRITICAL finding makes issueReviewDecision fail closed "
        "with CRITICAL_FINDING_BLOCKS_APPROVAL, a review must bind the exact source, "
        "revision, content hash, and inferred permissions (REVIEW_SUBJECT_MISMATCH, "
        "INFERRED_PERMISSION_MISSING, MISSING_FIELD), and a foreign or copied "
        "candidate cannot gain local approval (UNRECOGNIZED_CANDIDATE). "
        "createSkillLockfile pins each approved skill's source, revision, "
        "content_hash, license, sorted permissions, and sorted approvers under a "
        "deterministic lock_hash over canonical JSON; verifySkillLockfileSnapshot "
        "refuses drift (LOCK_HASH_MISMATCH, NON_CANONICAL_ORDER, ACCESSOR_FIELD_DENIED), "
        "a disabled install refuses a hash drift (INSTALL_HASH_MISMATCH) or an "
        "unapproved skill (SKILL_NOT_APPROVED), and activation refuses a policy, "
        "permission, or lockfile drift (POLICY_HASH_MISMATCH, "
        "UNVERIFIED_PERMISSION_DENIED, PERMISSION_EXPANSION_DENIED, "
        "UNRECOGNIZED_LOCKFILE) and reports effectPerformed:false. The lockfile "
        "schemaRef is a string only; no schema is loaded or validated and none is in "
        "scope. Ruff lint/format, malicious_skill_fixture_test 11/11, "
        "skill_lockfile_test 10/10, targeted 21/21, full Python 1261/1261, full Node "
        "1274/1274 across 113 files, and git diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"S03-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". S01-0001 and every prior RAH generation remain immutable. S03 provides "
        "skill quarantine, static scanning, the SkillLockfile, and disabled-install "
        "and activation gating; the S-phase threat model and red-team gate (S04), the "
        "wider runtime skill execution surface, source coverage, PRD, review, and "
        "global closeout remain; implementation_gate=fail and completion_ready=false."
    )
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("S03-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S03-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"S03 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("S03 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("S03-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("S03 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"S03 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S03 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("S03 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("S03 report does not bind current core generation")
    if (
        ...
        raise SystemExit("S03 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored S03 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("S03 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"S03 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("S03 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("S03 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"S03 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected S03 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with S03 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored S03 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
