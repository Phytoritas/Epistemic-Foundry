#!/usr/bin/env python3
"""Append K04-0001 core and closeout evidence to the active RAH state."""
...
import build_k04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


EXPECTED_INITIAL_PARENT = "000037-8b76b244"
EXPECTED_INITIAL_EVIDENCE = "E0037"
EXPECTED_INITIAL_GENERATION_COUNT = 37
NEXT_ACTIONS = [
    "Recompute and seal the live 156-package DAG after K04-0001 PASS.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required K04 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "K04-0001 PASS: PDF, web-page, and dataset instruction-like content "
        "remains untrusted data in the actual trust-boundary runtime and cannot "
        "gain instruction, approval, policy, execution, or capability authority. "
        "Malware, provenance, and tamper failures are QUARANTINE; passive "
        "malformed parser output is typed FAIL; neither path can be trusted, "
        "exported, or projected. Integrity scanning precedes all parser stages "
        "and the deterministic ingest quality gate is projection's sole direct "
        "predecessor. K04 targeted is Python 10/10 and Node 6/6, current "
        "predecessors are K01 85/85, K02/K03 76/76, and S01 17/17, full Python "
        "is 1064/1064, full Node is 476/476 over 55 files, and codegen is "
        "126/126. "
        f"Injection sha256:{hashes['corpus-injection-verification.json']}; quality "
        f"sha256:{hashes['ingest-quality-verification.json']}; workflow sha256:"
        f"{hashes['workflow-gate-verification.json']}; regression sha256:"
        ...
        f"{hashes['corpus-security-verification.artifact-receipt.json']}; targeted "
        f"Python JUnit sha256:{hashes['targeted-k04-python.junit.xml']}; targeted "
        f"Node JUnit sha256:{hashes['targeted-k04-node.junit.xml']}; full Python "
        f"JUnit sha256:{hashes['full-python-suite.junit.xml']}; full Node JUnit "
        f"sha256:{hashes['full-node-suite.junit.xml']}; inventory sha256:"
        f"{hashes['node-test-inventory.json']}; review sha256:{hashes['review.md']}; "
        f"builder sha256:{hashes['build_k04_0001_evidence.py']}; sealer sha256:"
        f"{hashes['k04_0001_rah_seal.py']}. Review is primary-session separate "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"K04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"{hashes['write-scope-verification.json']}; dependency sha256:"
        f"{hashes['dependency-status.json']}; receipt sha256:"
        f"{hashes['corpus-security-verification.artifact-receipt.json']}; RAH "
        f"integrity sha256:{hashes['rah-core-integrity.json']}; builder sha256:"
        f"{hashes['build_k04_0001_evidence.py']}; sealer sha256:"
        f"{hashes['k04_0001_rah_seal.py']}. Every prior report and generation is "
        "preserved. K04-0001 is immutable PASS and the live 156-package DAG must "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("K04 requires a contiguous evidence ledger")
    if not (
        ...
        raise SystemExit("K04 preflight requires active/fail/completion_ready=false")
...
def commit_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "K04-0001 corpus security and ingest-quality integration is "
        "evidence-sealed PASS. The live 156-package DAG and downstream packages "
...
def run_preflight() -> dict[str, Any]:
    ...
    if not (
        ...
        raise SystemExit("K04 preflight is not at the sealed post-K03 DAG state")
    return {
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("K04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, core_id]:
        raise SystemExit("K04 core did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("K04 core did not preserve every prior generation")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("K04 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("K04 evidence identifiers differ from the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("K04 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("K04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("K04 final did not append exactly one evidence row")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("K04 final evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
        raise SystemExit("K04 final did not preserve every prior generation")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if evidence_ids(payloads)[-2:] != [core_id, final_id]:
        raise SystemExit("RAH ledger does not end with K04 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("K04 final generation does not follow core")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored K04 final summary differs from artifacts")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
