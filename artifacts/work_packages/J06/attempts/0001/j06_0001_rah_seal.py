#!/usr/bin/env python3
"""Append J06-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This seal was authored as a sealing hand
that was instructed NOT to touch ``.rah/``, so it cannot read the live ledger to
pin the parent generation identifier, its count, its manifest hash, the parent's
tail evidence id, or this attempt's own core/final evidence ids.  ALL SIX are
therefore left as the sentinel ``FILL_FROM_LEDGER`` and the seal refuses to run
until the parent (Parent Architect / Research Governor) fills every one of them
from the live ledger tail before sealing:

  * ``EXPECTED_PARENT`` — the generation currently at the ledger tail.
  * ``EXPECTED_PARENT_COUNT`` — the number of preserved numbered generations.
  * ``EXPECTED_PARENT_MANIFEST_SHA256`` — sha256 of that generation's manifest.
  * ``EXPECTED_PARENT_EVIDENCE`` — the last evidence id already committed.
  * ``CORE_EVIDENCE_ID`` / ``FINAL_EVIDENCE_ID`` — the two ids this seal appends.

Everything else — summaries, artifact and product hashes, flat-snapshot
verification, generation accounting — is derived exactly as the R05/R04/H05
templates do.  ``assert_parent_pins`` guards every sentinel, so no mode does any
ledger arithmetic until the parent has pinned all six.
"""
...
#: Sentinel for the pins that can only be read from the live ledger tail.  The
#: parent fills every one of these from the live ledger before sealing; until
#: then ``assert_parent_pins`` refuses to run.
PARENT_PIN_SENTINEL = "FILL_FROM_LEDGER"
#: The generation currently at the ledger tail (fill from `.rah/ralph` current
#: pointer), e.g. "000215-XXXXXXXX".
EXPECTED_PARENT = "000216-38fa31bf"
#: The number of preserved numbered generations at the tail.
EXPECTED_PARENT_COUNT = 216
#: The last evidence id already committed at the tail.
EXPECTED_PARENT_EVIDENCE = "E0218"
#: sha256 (hex, no prefix) of the parent generation's manifest.
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "643f0941d83d765d10aed28817d2ab80bed8f6bf9b64d6b397533c13a02dc0ac"
)
#: The two evidence ids this seal appends: core then final closeout.
CORE_EVIDENCE_ID = "E0219"
FINAL_EVIDENCE_ID = "E0220"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_j06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after J06-0001 seals and select the next "
...
#: references the (sentinel) evidence ids by name so it stays consistent once
#: the parent pins them.
GATE_NOTE = (
    f"{ATTEMPT_ID} operator/prompt qualification and context-budget integration "
    f"gate is evidence-sealed PASS ({CORE_EVIDENCE_ID}/{FINAL_EVIDENCE_ID}). The "
    f"next package is {NEXT_PACKAGE}; implementation_gate=fail and "
...
def assert_parent_pins() -> None:
    ...
    if PARENT_PIN_SENTINEL in pins:
        raise SystemExit(
            "J06-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required J06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  J06 ships the
    #: v4_j06 subpackage of the ``operators`` package, so the whole operators
    #: tree is pinned; a change to any operator module fails the seal.
    relatives = sorted(
...
def core_summary() -> str:
    ...
    return (
        "J06-0001 PASS core verification: operator/prompt qualification and "
        "context-budget integration gate, implemented in "
        "src/epistemic_foundry/operators/v4_j06 by a bounded implementation agent "
        "and reviewed by the sealing agent (actor-independent from the author) "
        "under exact dependency bindings (J05 J05-0001, S05 S05-0001) and a "
        "W05-0001 regression baseline. The gate admits a mutation operator only "
        "when a prompt-affecting operator has been qualified out of quarantine "
        "for a future run (composed from J05's claim_active_prompt_operator, "
        "which runs the S05 inert-mutations gate, and build_activation_record) "
        "and when the work's context tokens, re-derived from the manifest's own "
        "components, fit under a budget the budget module confirms bounds spend. "
        "Refusals hold by code: unregistered operator, quarantined prompt "
        "(EF4-I55), missing qualification evidence, retroactive (same-run) "
        "application, inconsistent token accounting, unenforced or ceiling-less "
        "budget, over-budget work and a drifted receipt. Every admission "
        "re-derives its own receipt digest byte for byte with no clock or random "
        ...
        "packaging-discovery, J05/S05 dependency regressions, full Python and "
        "full Node suites, and git diff --check all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"J06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. J06 qualifies, meters "
        "and refuses, recording a qualification-and-budget verdict only: it does "
        "not score, select, promote, evaluate or execute any candidate, makes no "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("J06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J06-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"J06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("J06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("J06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("J06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"J06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("J06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("J06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("J06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored J06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("J06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"J06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("J06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("J06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"J06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected J06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with J06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored J06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
