#!/usr/bin/env python3
"""Prove the J06 integration gate is discoverable by the packaging config.

``src/epistemic_foundry/operators`` already ships (J05), and the J06 write scope
adds ``src/epistemic_foundry/operators/v4_j06``.  A subpackage that never reached
the wheel would let the gate import from a checkout and vanish from an installed
distribution, so there is a scope decision to prove rather than assume.  The
discovery mode is read from pyproject instead of assumed, so a change there fails
here.
"""
...
EXPECTED = ("epistemic_foundry.operators", "epistemic_foundry.operators.v4_j06")
