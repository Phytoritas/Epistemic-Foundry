#!/usr/bin/env python3
"""Append I04-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000296-4e0175c5"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 296
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0298"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "2dc74c2a263a8814501976fe07bf0a06139191e62153a66308ed07b65fbce46c"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0299"
FINAL_EVIDENCE_ID = "E0300"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_i04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after I04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} intake UX and export gate is evidence-sealed PASS "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "I04-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required I04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    #: approved intake write-scope tree, so a change to the projection, the
    #: export gate, or their tests is caught here too.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "web/src/features/intake").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "I04-0001 PASS core verification: the intake UX projection and canonical "
        "frame export/import gate, implemented in web/src/features/intake by a "
        "bounded implementation agent and reviewed by this seal-prep session (a "
        "distinct actor, actor-independent from the author) on the sealed I02-0001 "
        "inbox/council contract and the sealed I03-0001 ontology and measurement "
        "resolution with no new production dependency. The projection reconstructs "
        "the frame from authority inputs and renders inbox and blockers as the "
        "first visible aria-live section; council, scope, ontology, and "
        "measurement blockers keep their canonical I02 vocabulary and order, "
        "untrusted text is HTML-escaped, no confidence or verdict is invented, and "
        "a forged frame with cleared blockers is rejected. Canonical export "
        "round-trips byte-for-byte and is key-order independent while meaningful "
        "array order is preserved; its SHA-256 matches the Node oracle. Import "
        "fails closed on invalid UTF-8, non-canonical bytes, content tampering, "
        "unknown fields, a fabricated derived gate, and an Inbox frame. "
        "intake_ui_test 20/20, frame_roundtrip_test 12/12, targeted 32/32, full "
        "Python 1261/1261, full Node 1253/1253 across 111 files, and git "
        "diff --check pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"I04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". I02-0001, I03-0001, and every prior RAH generation remain immutable. "
        "I04 provides the component-local intake projection and export/import "
        "gate; a canonical intake or export schema, persistence, a remote "
        "service, ontology approval issuance, evidence search, the full "
        "coverage-before-confidence UI owned by O01/O04/U03, source coverage, "
        "PRD, review, and global closeout remain; implementation_gate=fail and "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("I04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I04-0001 parent is not the pinned sealed tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"I04 preflight requires exactly {EXPECTED_PARENT_COUNT} "
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("I04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("I04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("I04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"I04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("I04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("I04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("I04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored I04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("I04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"I04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("I04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("I04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"I04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected I04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with I04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored I04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
