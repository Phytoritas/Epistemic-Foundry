#!/usr/bin/env python3
"""Append S04-0003 PASS evidence while the global repair goal stays active."""
...
import build_s04_0003_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run B04-0007 projection and regression revalidation now that S04-0003 is evidence-sealed PASS.",
    "Resolve the C01-owned sample GateDecision canonical hash debt after B04-0007 and before C04-0002.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required S04-0003 artifact is missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "S04-0003 PASS under HD-EF4-B04-SG002-20260730-001: active binding "
        "DMB-EF4-20260730-002 validates current development manifest successor "
        "sha256:5dd99d2aae1e9ef662b9b8242b5fa921621434c6600c9c06e3a573d03079bb12; "
        "the three C03 replacement hashes, binding/patch/decision self-hashes, "
        "superseded S04-0002 evidence, and C01 reconciliation chain all pass. "
        "Targeted security is 67/67, full Node 460/460, and full Python 990/990; "
        "the earlier pytest console-entry failure is retained as a non-product "
        "Windows import-path diagnostic. "
        ...
        f"write-scope evidence sha256:{hashes['write-scope-verification.json']}; "
        f"targeted JUnit sha256:{hashes['targeted-security-suite.junit.xml']}; "
        f"Node JUnit sha256:{hashes['full-node-suite.junit.xml']}; Python JUnit "
        f"sha256:{hashes['full-python-suite.junit.xml']}; review sha256:"
        f"{hashes['review.md']}; builder sha256:{hashes['build_s04_0003_evidence.py']}; "
        f"sealer sha256:{hashes['s04_0003_rah_seal.py']}. Review is primary-session "
        "separate with actor_independence=false. B04-0007 is next; the C01-owned "
        "sample GateDecision hash debt and later C04/B04 gates remain, so "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"S04-0003 PASS closeout is hash-sealed after core generation {parent}: "
        ...
        f"targeted JUnit sha256:{hashes['targeted-security-suite.junit.xml']}; "
        f"Node JUnit sha256:{hashes['full-node-suite.junit.xml']}; Python JUnit "
        f"sha256:{hashes['full-python-suite.junit.xml']}; RAH core integrity sha256:"
        f"{hashes['rah-core-integrity.json']}; builder sha256:"
        f"{hashes['build_s04_0003_evidence.py']}; sealer sha256:"
        f"{hashes['s04_0003_rah_seal.py']}. S04-0003 is PASS and B04-0007 is "
        "dependency-ready. Every prior attempt and generation remains preserved; "
        "the C01 hash debt, C04, and final B04 packaging remain, so "
        "implementation_gate=fail and completion_ready=false."
    )
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "S04-0003 active development-manifest binding correction is evidence-sealed "
        "PASS. B04-0007 is next; the C01-owned GateDecision example hash debt, "
        "C04, and final packaging remain, so implementation_gate=fail and "
...
def assert_active_preflight(payloads: dict[str, Any]) -> None:
    ...
    if identifiers != expected:
        raise SystemExit("S04-0003 preflight requires a contiguous evidence ledger")
    if (
...
def run_preflight() -> dict[str, Any]:
    ...
    if "rah_state" in report:
        raise SystemExit("S04-0003 report is already RAH-bound; use verify")
    generations = numbered_generations(ralph_root)
    if generations[-1] != generation:
        raise SystemExit("S04-0003 preflight generation pointer mismatch")
    return {
        "mode": "preflight",
        "status": "PASS",
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("S04-0003 core generation pointer mismatch")
    ...
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("core evidence does not match the S04-0003 summary")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict) or rah.get("core_generation") != parent:
        raise SystemExit("S04-0003 report does not bind the current core generation")
    ...
    if evidence_ids(payloads)[-1] != core_id or next_evidence_id(payloads) != final_id:
        raise SystemExit("S04-0003 report evidence IDs do not match the live ledger")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("S04-0003 core summary changed before final sealing")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("S04-0003 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, final_id]:
        raise SystemExit("final seal did not append exactly one evidence entry")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("final evidence does not match the S04-0003 closeout seal")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or after[-1] != generation:
...
def run_verify() -> dict[str, Any]:
    ...
    if not isinstance(rah, dict):
        raise SystemExit("S04-0003 report is not RAH-bound")
    core_generation = str(rah.get("core_generation"))
    core_id = str(rah.get("core_evidence_id"))
    final_id = str(rah.get("final_closeout_evidence_id"))
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [core_id, final_id]:
        raise SystemExit("sealed ledger does not end with S04 core/final evidence")
    if generation_number(generation) != generation_number(core_generation) + 1:
        raise SystemExit("sealed S04 final generation does not follow the core generation")
    summary, hashes = final_summary(core_generation)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored S04 final evidence differs from closeout artifacts")
    return {
        "mode": "verify",
        "status": "PASS",
        "generation": generation,
