#!/usr/bin/env python3
"""Append V04-0001 PASS evidence while global completion remains false.
...
EXPECTED_PARENT = "000254-c79a54bb"
#: The count of preserved numbered generations at that tail.  Fill as an int.
EXPECTED_PARENT_COUNT = 254
#: The last evidence id in the ledger at that tail, e.g. "E02xx".
EXPECTED_PARENT_EVIDENCE = "E0256"
#: sha256 hex of that generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "17d79d12692dc58486408afc3364510a792fea846fcdc471c290d99b82520550"
#: The two evidence ids this seal mints (core, then final closeout).
CORE_EVIDENCE_ID = "E0257"
FINAL_EVIDENCE_ID = "E0258"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_v04_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after V04-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} V-phase result reconciliation and evidence-class gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "V04-0001 seal is not pinned: set "
            + ", ".join(unfilled)
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required V04-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = sorted(
        ...
        for path in (ROOT / "python/epistemic_foundry/validation/reconcile").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "V04-0001 PASS core verification: V-phase result reconciliation and "
        "evidence-class gate, implemented in "
        "python/epistemic_foundry/validation/reconcile by a bounded "
        "implementation agent under the parent architect's delegated V04 write "
        "scope and reviewed by an independent reviewer distinct from the author, "
        "under exact dependency bindings (V02 V02-0001, V03 V03-0001) and an "
        "X04-0001 regression baseline. Reconciliation copies the source evidence "
        "class forward verbatim and refuses the one transformation EF4-I11 "
        "forbids: a non-empirical source entered as an empirical candidate is "
        "REJECT (EVIDENCE_CLASS_OVERCLAIMED, non_empirical_guard_passed=false), "
        "with the empirical boundary read from the evidence-node and "
        "experiment-result schema enum markers at runtime, so simulation, formal "
        "and benchmark stay distinct from observation. A support role is admitted "
        "only when V03's execution gate was PASS, the result status was "
        "COMPLETED and the preregistered falsification outcome was NOT_FALSIFIED; "
        "a failed, incidented, denied, non-completed or falsified run makes the "
        "support role a REJECT (CONFIRMATION_WITHOUT_CLEAN_RUN) and PROMOTE "
        "requires that positive conjunction, never merely the absence of a "
        "refusal, so a broken pipeline never reads as confirmation. "
        "quality_adjustments are carried on the record but read after the "
        "categorical decision is fixed and never touch the branch taken, "
        "REQUIRE_HUMAN_REVIEW and QUARANTINE preserve unclean, falsified, "
        "inconclusive and untested results rather than dropping them, and the "
        "record re-derives its own hash over exactly the fields it publishes. "
        ...
        "whole-component targeted, the V02 dependency regression 86/86, the V03 "
        "dependency regression 117/117, full Python 1261/1261, full Node "
        "1682/1682 across the unified 134-file inventory, and git diff --check "
        "all pass. "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"V04-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. V04 reconciles a "
        "validation result across the V01/V02/V03 surfaces into one typed record "
        "and enforces the two evidence-class refusals: simulation, formal, "
        "benchmark and empirical stay distinct and a failed run is never a "
        "confirmation. It does not score, select, promote or evaluate any "
        "candidate on a score, makes no DSSAT or plant-model parity claim, and "
        "promotion remains a governance decision. Author/reviewer separation "
        "holds with actor independence while external actor-independent "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("V04-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V04-0001 parent is not the sealed frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"V04 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("V04 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("V04-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V04 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"V04 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V04 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("V04 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("V04 report does not bind current core generation")
    if (
        ...
        raise SystemExit("V04 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored V04 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("V04 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"V04 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V04 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("V04 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"V04 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected V04 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with V04 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored V04 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
