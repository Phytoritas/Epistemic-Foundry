#!/usr/bin/env python3
"""Append H02-0001 PASS evidence while global completion remains false.
...
derived exactly as the sealed A01/A02/A03/B01/B03 templates do.

H02 depends on H01. Its product bytes are the two session and prompt lifecycle
hook declarations ``plugins/epistemic-foundry/hooks/session.json`` and
``plugins/epistemic-foundry/hooks/prompt.json`` (byte-equal to the plugin
blueprint), which it attests without editing. The required checks prove the
STATIC declarations only; the hook runner, plugin-manifest hook registration and
host capability probing are deferred integration (H04 / X01 / G06).
"""
...
#: The current committed generation (e.g. "000340-abcd1234"); its retained
...
EXPECTED_PARENT = "000342-87d0162c"
EXPECTED_PARENT_COUNT = 342
EXPECTED_PARENT_EVIDENCE = "E0344"
EXPECTED_PARENT_MANIFEST_SHA256 = "351fa2102e732e50c5cd0f611089ed626b222a92cd7cba2d722ff1e98491f999"
CORE_EVIDENCE_ID = "E0345"
FINAL_EVIDENCE_ID = "E0346"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_h02_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402

#: The hook-declaration product bytes H02 attests, outside the attempt directory.
PRODUCT_FILES = (
...
NEXT_ACTIONS = [
    "Recompute the DAG frontier after H02-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} session and prompt lifecycle hook declarations are "
...
def assert_parent_pins() -> None:
    ...
    if any(pin == PARENT_PIN_SENTINEL for pin in pins):
        raise SystemExit(
            "H02-0001 seal is not pinned: set EXPECTED_PARENT, "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required H02-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory: the two session and prompt
    #: lifecycle hook declarations H02 attests (never edits).
...
def core_summary() -> str:
    ...
    return (
        "H02-0001 PASS core verification: session and prompt lifecycle hooks, "
        "attested by the bounded implementation agent(s) that authored the "
        "session and prompt hook declarations and the eight-case contract "
        "harness, and reviewed by the sealing session as an independent "
        "contract-reviewer (actor-independent from the author). H02 attests "
        "plugins/epistemic-foundry/hooks/session.json and "
        "plugins/epistemic-foundry/hooks/prompt.json exactly as they currently "
        "are (byte-equal to the plugin blueprint) and makes ZERO substantive "
        "change. session_hook_test asserts session.json declares only a "
        "SessionStart route (matcher startup|resume|clear|compact, timeout 15) "
        "and a PostCompact route (matcher manual|auto, timeout 15), every hook a "
        "type: command rooted at ${PLUGIN_ROOT} with a positive bounded integer "
        "timeout, with timeout expansion, an extra SessionEnd event and a direct "
        "transition/set-phase command each failing closed. prompt_hook_test "
        "asserts prompt.json declares only one bounded UserPromptSubmit "
        "classification route (timeout 8), with direct state mutation, authority "
        "fields and tool/completion/delegation events (PreToolUse/Stop/"
        "SubagentStart) each failing closed. The fourth session and fourth prompt "
        "case attest the fail-closed maturity boundary: the plugin manifest "
        "declares no hooks key and empty capabilities and dist/hook-runner.mjs is "
        "ABSENT -- a static declaration boundary with runtime integration deferred "
        "to H04/X01/G06, an honest SPECIFIED != IMPLEMENTED posture, not a "
        "weakening. Both required checks map to the shared eight-case harness "
        "(session_hook_test 4/4, prompt_hook_test 4/4, 8/8 zero failures); the "
        "full Python suite is green, the live full Node suite is green with zero "
        "failures, and git diff --check passes. H02 depends on H01; the sealed "
        "H01-0001 attempt is the build dependency and regression baseline. "
        ...
        "required checks prove the STATIC declarations only; the hook runner, "
        "plugin-manifest registration and host capability probing are deferred "
        "integration (H04/X01/G06). The DAG frontier recomputes next; "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"H02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. H02 attests the "
        "session and prompt lifecycle hook declarations; it does not re-author "
        "them, makes no product-maturity, runtime-executability or "
        "release-readiness claim, does not claim a hook runner, plugin-manifest "
        "hook registration or host capability probing (H04/X01/G06 integration "
        "scope), and asserts only that session bootstrap is bounded and prompt "
        "classification cannot change state directly. Author/reviewer separation "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("H02-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H02-0001 parent is not the sealed ledger tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"H02 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("H02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("H02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("H02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"H02 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("H02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("H02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("H02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored H02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("H02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"H02 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("H02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("H02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"H02 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected H02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with H02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored H02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
