#!/usr/bin/env python3
"""Append T01-0002 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "T01-0002"
WORK_PACKAGE_ID = "T01"
EXPECTED_PARENT = "000110-56a3a824"
EXPECTED_PARENT_COUNT = 110
EXPECTED_PARENT_EVIDENCE = "E0112"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "fb6038a05a6d426fa5681e02facf6fbfa492f69cea8bf6a8efdd6458c23bec17"
)
CORE_EVIDENCE_ID = "E0113"
FINAL_EVIDENCE_ID = "E0114"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_t01_0002_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Run W01-0001 workflow compiler and NodeContract validator next.",
    "Continue the dependency-ready frontier serially: W01, A06, then recompute the DAG.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required T01-0002 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "T01-0002 PASS core verification: the frozen 13-tool MCP surface "
        "(HD-EF4-T01-SG001-20260730-001) is implemented with the canonical "
        "catalog and envelope contracts under contracts/mcp/t01, "
        "provider-neutral handlers under src/epistemic_foundry/application, "
        "stateless STDIO and Streamable HTTP POST /mcp framing, plugin-host "
        "descriptor projection, and the declared Python/Node tests. Exact "
        "E04, G04, and S04-0005 PASS dependencies and the manifest and "
        "decision self-hashes are bound; nine PURE_READ tools are proven "
        "side-effect free and four DURABLE_PLAN_ARTIFACT tools are "
        "receipt-bound, idempotent, canonical-schema-validated, and "
        "non-executing. Ruff lint/format, targeted Python 41/41, targeted "
        "Node 15/15, full Python 1156/1156, full Node 834/834 across 82 "
        ...
        "W01-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"T01-0002 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". T01-0001 SPEC_GAP history, O03-0001, B04-0002 through B04-0010, "
        "and every prior RAH generation remain immutable. T01 is PASS, but "
        "W01, A06, the wider 156-package DAG, source coverage, PRD, review, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("T01-0002 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "T01-0002 MCP read/planning tools are evidence-sealed PASS under "
        "HD-EF4-T01-SG001-20260730-001. W01-0001 is next; "
...
def run_preflight() -> dict[str, Any]:
    ...
    if generation != EXPECTED_PARENT or evidence_ids(payloads)[-1] != EXPECTED_PARENT_EVIDENCE:
        raise SystemExit("T01-0002 parent is not the sealed O03-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("T01 preflight requires exactly 110 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("T01 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("T01-0002 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("T01 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("T01 core did not append exactly E0113")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T01 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("T01 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("T01 report does not bind current core generation")
    if (
        ...
        raise SystemExit("T01 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored T01 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    _, current, sealed = current_state()
    if current != generation or generation_number(current) != generation_number(parent) + 1:
        raise SystemExit("T01 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("T01 final did not append exactly E0114")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("T01 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("T01 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("T01 core tail is not E0113")
        return {"mode": "core-verify", "state_verification": integrity, "status": "PASS"}
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 112:
        raise SystemExit(f"unexpected T01 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with T01 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("stored T01 final evidence differs from current closeout bytes")
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
        "generation": generation,
