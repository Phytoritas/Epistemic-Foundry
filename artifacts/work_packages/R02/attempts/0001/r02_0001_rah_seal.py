#!/usr/bin/env python3
"""Append R02-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "R02-0001"
WORK_PACKAGE_ID = "R02"
EXPECTED_PARENT = "000128-ecffd603"
EXPECTED_PARENT_COUNT = 128
EXPECTED_PARENT_EVIDENCE = "E0130"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "a35c3668c1edb55adef85ffbd63b0f1cb209268cd9869dad05558f5647b4fae1"
)
CORE_EVIDENCE_ID = "E0131"
FINAL_EVIDENCE_ID = "E0132"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_r02_0001_evidence as evidence  # noqa: E402
...
NEXT_ACTIONS = [
    ...
    "Build R03 abduction and contradiction on the sealed R01/R02 reasoning before the R04 gate.",
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required R02-0001 artifact missing: {name}")
...
def core_summary() -> str:
    ...
    return (
        "R02-0001 PASS core verification: the deductive proof trace and "
        "assumption ledger are implemented in "
        "python/epistemic_foundry/reasoning/deduction with an exact R01-0001 "
        "dependency and regression baseline. A premise must cite evidence, an "
        "unevidenced statement may stand only as an assumption, and the engine "
        "derives the load-bearing assumptions itself and requires the graph's "
        "declared hidden_assumption_ids to equal them exactly, so neither an "
        "undeclared nor an over-declared assumption can be sealed. A deduction "
        "may narrow a ScopeVector but may never drop a scalar boundary, an "
        "inclusion or exclusion criterion, or a condition any premise carried, "
        "nor move to a value no premise covers; an unconstrained assumption "
        "cannot launder a scope the evidence never had. The supporting graph "
        "must be acyclic with resolvable endpoints, a deductive edge must name "
        "its rule, a conclusion on rejected support cannot be accepted, and a "
        "standing objection must be declared. Fixtures validate against the "
        "canonical schemas/argument-graph.schema.json. Ruff lint/format, "
        "deduction_trace_test 24/24, scope_widening_test 19/19, targeted "
        "43/43, dependency component regression 44/44, full Python 1261/1261, "
        "full Node 903/903 across 87 files, and git diff --check pass. "
        ...
        "R03-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    hashes = artifact_hashes(names)
    summary = (
        f"R02-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". R01-0001 and every prior RAH generation remain immutable. R02 "
        "checks a declared ArgumentGraph rather than constructing proofs, and "
        "rule_ref is an identifier rather than a checked derivation. R03, T03, "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("R02-0001 seal requires active/fail/completion_ready=false")
...
def commit_active_failed_generation(
    *, payloads: dict[str, Any], summary: str, expected_evidence_id: str
    ...
    note = (
        "R02-0001 proof trace and assumption ledger is evidence-sealed PASS. "
        "R03-0001 is next; implementation_gate=fail and "
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R02-0001 parent is not the sealed T02-0001 tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("R02 preflight requires exactly 128 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("R02 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("R02-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("R02 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("R02 core did not append exactly E0131")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R02 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("R02 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("R02 report does not bind current core generation")
    if (
        ...
        raise SystemExit("R02 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored R02 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("R02 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("R02 final did not append exactly E0132")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("R02 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("R02 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("R02 core tail is not E0131")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 130:
        raise SystemExit(f"unexpected R02 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with R02 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored R02 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
