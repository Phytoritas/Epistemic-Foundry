#!/usr/bin/env python3
"""Append D06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "D06-0001"
WORK_PACKAGE_ID = "D06"
EXPECTED_PARENT = "000192-81652605"
EXPECTED_PARENT_COUNT = 192
EXPECTED_PARENT_EVIDENCE = "E0194"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "d3cd329ab5314875969c34c203c409bddb397341dc97953e8564bca4eabb6ac9"
)
CORE_EVIDENCE_ID = "E0195"
FINAL_EVIDENCE_ID = "E0196"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_d06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward H05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "D06-0001 archive migration, crash recovery and atomic checkpoint integration gate "
    "is evidence-sealed PASS (E0195/E0196). H05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required D06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "migrations/v4_d06").rglob("*")
...
ATTEMPT_ARTIFACTS = (
    "build_d06_0001_evidence.py",
    "conftest.py",
    "d06_0001_rah_seal.py",
    ...
    "pytest.ini",
    "run_d06_0001_checks.py",
    "schema-and-type-check.junit.xml",
    "targeted-d06-recovery-gate.junit.xml",
...
def core_summary() -> str:
    ...
    return (
        "D06-0001 PASS core verification: archive migration, crash recovery and atomic checkpoint integration gate, "
        ...
        "session, under exact dependency bindings (D05 D05-0001, E05 E05-0001) "
        "and an S05-0001 regression baseline. the migration is one transaction with a preflight that refuses an absent D05 or a second apply, reuses D05's own domains and append-only guard, and journals both migrations with catalog-inventory content hashes that re-derive on the live server "
        "crash scenarios are proven with real aborted transactions against real PostgreSQL: a mid-work crash leaves a visible open attempt with observed-vs-expected counts, a crash during sealing leaves no checkpoint, tampering with constraints, functions or the journal is detected by digest mismatch, and recovery's only exits are seal-and-close or abandon-with-reason "
        "seal_and_close_attempt reads run and generation from the attempt so an attempt cannot seal a generation it never covered, and it refuses unless the expected candidate count equals the lineage rows actually persisted "
        "Key review finding: The recovery surface lives in its own schema because D05's sealed gate asserts an exact table set in the store schema — the design respects the sealed surface instead of rewriting it. "
        ...
        + ". H05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"D06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "WAL-replay crash fidelity: crashes are aborted transactions, not SIGKILL of the server; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("D06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D06-0001 parent is not the sealed D06 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("D06 preflight requires exactly 192 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("D06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("D06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("D06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("D06 core did not append exactly E0195")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("D06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("D06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("D06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored D06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("D06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("D06 final did not append exactly E0196")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("D06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("D06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("D06 core tail is not E0195")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 194:
        raise SystemExit(f"unexpected D06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with D06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored D06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
