#!/usr/bin/env python3
"""Run the E06-0001 concurrent effect and idempotency gate acceptance checks.

Required checks from manifests/development_manifest.yaml (E06):
...
report.  The gate ships under src/epistemic_foundry/effects/v4_e06, so
...
E06 composes the sealed E05 engine rather than reimplementing it, so
``dependency-regression-effects`` re-runs E05's own attempt suite together with
the repository's checkpoint and reconciliation gate.  A change to this package
that quietly altered what E05 accepts has to fail there.
"""
...
ATTEMPT_ID = "E06-0001"
COMPONENT = "artifacts/work_packages/E06/attempts/0001"
# E05's sealed attempt suite is the regression that proves the composition still
# holds; the checkpoint gate is the repository-level reconciliation surface the
# settled ledger is eventually handed to.
DEPENDENCY_TARGETS = {
    "dependency-regression-effects": (
        "artifacts/work_packages/E05/attempts/0001",
        "tests/test_checkpoint_and_reconciliation.py",
    ),
}
...
def packaging_discovery() -> int:
    # The gate imports through two package levels, so being absent from the
    # wheel is a two-file failure rather than a one-file one.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-e06-concurrency", COMPONENT)
