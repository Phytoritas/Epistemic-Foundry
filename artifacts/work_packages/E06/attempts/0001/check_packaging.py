#!/usr/bin/env python3
"""Prove the E06 gate is discoverable by the project's own packaging config.

``src/epistemic_foundry/effects`` already carries the package marker E05 added
(HD-EF4-E05-SCOPE-20260802-001), so this package adds only its own ``__init__``.
That makes the claim "the gate ships with the wheel" depend on two levels rather
than one, which is exactly the kind of thing that is assumed instead of checked.
The discovery mode is read from pyproject rather than assumed, so a change there
...
EXPECTED = ("epistemic_foundry.effects", "epistemic_foundry.effects.v4_e06")
