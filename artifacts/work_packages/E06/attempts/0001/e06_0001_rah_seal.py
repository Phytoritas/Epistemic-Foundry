#!/usr/bin/env python3
"""Append E06-0001 PASS evidence while global completion remains false."""
...
ATTEMPT_ID = "E06-0001"
WORK_PACKAGE_ID = "E06"
EXPECTED_PARENT = "000178-45876e13"
EXPECTED_PARENT_COUNT = 178
EXPECTED_PARENT_EVIDENCE = "E0180"
EXPECTED_PARENT_MANIFEST_SHA256 = (
    "5264119ab65aefed1f27fc29db0f34dc1e88763ab977a3abe676e8aba70bf99f"
)
CORE_EVIDENCE_ID = "E0181"
FINAL_EVIDENCE_ID = "E0182"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_e06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Continue the parallel-wave seal chain toward I05-0001 and the remaining integration layers.",
...
GATE_NOTE = (
    "E06-0001 concurrent candidate effect and idempotency integration gate "
    "is evidence-sealed PASS (E0181/E0182). I05-0001 is next; "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required E06-0001 artifact missing: {name}")
...
def product_hashes() -> dict[str, str]:
    ...
    relatives = [
        *sorted(
            ...
            for path in (ROOT / "src/epistemic_foundry/effects/v4_e06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "E06-0001 PASS core verification: concurrent candidate effect and idempotency integration gate, "
        ...
        "session, under exact dependency bindings (E05 E05-0001) "
        "and an S05-0001 regression baseline. effect statuses and the tri-state landed projection are read from the declaring ports module, receipts in fixtures are minted by the runtime's own builders, and the module holds no canonical enum value as a literal (package-scoped scan plus the repository gate) "
        ...
        "Key review finding: The settled ledger is keyed by idempotency key, not action id — found when action-id keying made a legitimate retry diverge between orderings; identical retries are interchangeable, and one key spanning two targets remains a caught divergence. "
        ...
        + ". I05-0001 is next; implementation_gate=fail and completion_ready=false."
    )
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"E06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. Not claimed: "
        "thread-level race coverage: interleavings are declared by the caller, and an undeclared interleaving is outside any PASS here; author/reviewer separation holds while "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("E06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E06-0001 parent is not the sealed E06 parent tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit("E06 preflight requires exactly 178 preserved generations")
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("E06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("E06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("E06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit("E06 core did not append exactly E0181")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("E06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("E06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("E06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored E06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("E06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit("E06 final did not append exactly E0182")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("E06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("E06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit("E06 core tail is not E0181")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
        }
    if count != EXPECTED_PARENT_COUNT + 2 or generation_number(generation) != 180:
        raise SystemExit(f"unexpected E06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with E06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored E06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
