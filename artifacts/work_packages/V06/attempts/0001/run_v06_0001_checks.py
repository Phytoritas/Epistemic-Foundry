#!/usr/bin/env python3
"""Run the V06-0001 experiment/replication end-to-end integration-gate checks.

Required checks from manifests/development_manifest.yaml (V06):
...
gate ships under src/epistemic_foundry/validation/v4_v06, one package below the
``validation`` marker V05 already sealed, so ``packaging-discovery`` proves the
new ``validation.v4_v06`` marker actually reaches the wheel rather than only
importing from the checkout, and ``wire-literal-discipline`` runs the
repository's EF4-I22 gate as its own check: the gate's INTEGRATE/REFUSE tokens
are not canonical schema enum values, so a new runtime module holding canonical
vocabulary must fail here.

The dependency regressions cover the sealed surfaces this package composes:
Q05's statistical-admissibility gate, whose receipt this gate verifies and
threads through both downstream gates; V05's validation-cascade advancement
gate, whose receipt this gate composes and cross-binds to the Q05 clearance; and
P05's promotion Parliament gate, whose convening receipt this gate composes and
re-checks for a promotion-authority claim.  Each is exercised through the
dependency's own sealed test suite, so a regression in any of them fails here
...
VALIDATION_V06 = "src/epistemic_foundry/validation/v4_v06"
PRODUCT_TESTS = {
...
# Q05 owns the statistical-admissibility gate whose receipt this gate verifies
# and threads through both downstream gates; V05 owns the validation-cascade
# advancement gate whose receipt this gate composes; P05 owns the promotion
# Parliament gate whose convening receipt this gate composes.  A change in any of
# the three must fail here, so each runs its own sealed suite.
DEPENDENCY_TARGETS = {
...
def ruff_check() -> int:
    return run(
        "ruff-check",
        ["uv", "run", "--locked", "ruff", "check", VALIDATION_V06, ATTEMPT_DIR],
    )
...
def wire_literal_discipline() -> int:
    ...
    # INTEGRATE/REFUSE decision tokens must not shadow any canonical schema enum.
    return _pytest("wire-literal-discipline", "tests/test_wire_literal_discipline.py")


def packaging_discovery() -> int:
    # src/epistemic_foundry/validation/v4_v06 is a new package marker below the
    # existing validation marker, so the wheel claim is proved here not assumed.
    return run(
        "packaging-discovery",
        [
            "uv",
...
def targeted() -> int:
    return _pytest("targeted-v06-integration-gate", ATTEMPT_DIR)
