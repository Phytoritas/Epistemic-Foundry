#!/usr/bin/env python3
"""Append V06-0001 PASS evidence while global completion remains false.

Authoring note (read before running).  This is a seal-PREP artifact.  It was
authored by the seal-prep session under an instruction not to touch ``.rah/``,
so it does not read the live ledger to pin the parent generation identifier, its
manifest hash, or the exact evidence identifiers this seal will issue.  ALL SIX
ledger-derived pins are therefore left as the sentinel ``FILL_FROM_LEDGER`` and
the seal refuses to run until the sealing session fills them from the live
ledger tail:

* ``EXPECTED_PARENT`` - the current committed RAH generation (e.g.
  ``000266-XXXXXXXX``), the tail the V06 core generation will branch from.
...
* ``EXPECTED_PARENT_EVIDENCE`` - the latest evidence id at that tail (the final
  closeout evidence of the package sealed immediately before V06).
* ``EXPECTED_PARENT_MANIFEST_SHA256`` - the sha256 of that tail generation's
  ``generation-manifest.json``.
* ``CORE_EVIDENCE_ID`` - the next contiguous evidence id the V06 core commits.
* ``FINAL_EVIDENCE_ID`` - the following evidence id the V06 closeout commits.
...
exactly as the R04/H05/R05/V05 templates do.  Do not run any mode until the six
pins are filled; ``assert_parent_pins`` fails closed while any remains a
sentinel.
"""
...
#: The current committed RAH generation the V06 core will branch from.
#: Fill from `.rah/ralph` current pointer before running, e.g. "000266-XXXXXXXX".
EXPECTED_PARENT = "000270-20ec9615"
#: The number of preserved generations at that tail.
EXPECTED_PARENT_COUNT = 270
#: The latest evidence id at that tail (final closeout of the prior package).
EXPECTED_PARENT_EVIDENCE = "E0272"
#: The sha256 of that tail generation's generation-manifest.json.
EXPECTED_PARENT_MANIFEST_SHA256 = "99d125df1e184e9642fe34965a90a20fe1f3acbe13cb5c579984de3f127fcc24"
#: The next contiguous evidence id the V06 core commits.
CORE_EVIDENCE_ID = "E0273"
#: The following evidence id the V06 closeout commits.
FINAL_EVIDENCE_ID = "E0274"
sys.path.insert(0, str(ATTEMPT))
sys.path.insert(0, str(AUTOMATION))

import build_v06_0001_evidence as evidence  # noqa: E402
import ralph_harness as rh  # noqa: E402
import state_store  # noqa: E402


NEXT_ACTIONS = [
    "Recompute the DAG frontier after V06-0001 seals and select the next "
...
GATE_NOTE = (
    f"{ATTEMPT_ID} experiment/replication end-to-end integration gate is "
...
def assert_parent_pins() -> None:
    ...
    if unfilled:
        raise SystemExit(
            "V06-0001 seal is not pinned: set "
            + ", ".join(unfilled)
            + " from the live ledger tail (the generation and evidence ids "
            "committed by the package sealed immediately before V06) before "
...
def artifact_hashes(names: tuple[str, ...]) -> dict[str, str]:
    ...
    for name in names:
        path = ATTEMPT / name
        if not path.is_file():
            raise SystemExit(f"required V06-0001 artifact missing: {name}")
        result[name] = sha256(path)
    return result


def product_hashes() -> dict[str, str]:
    #: Product bytes outside the attempt directory, bound by path.  V06 owns only
    #: the v4_v06 package; the validation marker and v4_v05 below it were sealed
    #: by V05 and are outside this attempt's write scope.
    relatives = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "src/epistemic_foundry/validation/v4_v06").rglob("*")
...
def core_summary() -> str:
    ...
    return (
        "V06-0001 PASS core verification: the experiment/replication end-to-end "
        "integration gate, implemented in "
        "src/epistemic_foundry/validation/v4_v06 by a bounded implementation "
        "agent under the primary session's brief and reviewed by this seal-prep "
        "session (actor-independent from the author) under exact dependency "
        "bindings (V05 V05-0001, P05 P05-0001, Q05 Q05-0001) and an X05-0001 "
        "regression baseline. The gate integrates a candidate's path only when "
        "Q05's statistical-admissibility receipt is authentic and admitted the "
        "candidate, V05's validation-cascade advancement receipt is authentic and "
        "advanced the claim, and P05's promotion Parliament receipt is authentic "
        "and convened the docket. The end-to-end binding no single gate can see is "
        "enforced here: every receipt names the one candidate, and the V05 and "
        "P05 receipts both reference the same Q05 clearance hash this gate was "
        "handed, so a path stitched from a candidate's advancement over here and a "
        "different clearance over there is refused. A composed Parliament receipt "
        "that reports itself as granting promotion is refused, and a "
        "candidate-generating requesting role is refused with the set the verifier "
        "firewall declares. Every integrate-or-refuse decision re-derives byte for "
        "byte from its own published fields with no clock or random draw, no input "
        "is mutated, integration_grants_promotion is always False, and nothing "
        "scores, selects, promotes or evaluates. Ruff lint/format, "
        ...
        "packaging-discovery, Q05/V05/P05 dependency regressions, full Python and "
        "full Node suites, and git diff --check all pass. "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Review is by this seal-prep session with actor_independence=true "
...
def final_summary(parent: str) -> tuple[str, dict[str, str]]:
    ...
    summary = (
        f"V06-0001 PASS closeout is hash-sealed after core generation {parent}: "
        + "; ".join(f"{name}=sha256:{digest}" for name, digest in hashes.items())
        + ". Every prior RAH generation remains immutable. V06 composes sealed "
        "verdicts and records a replayable integration decision only: it does "
...
def assert_active(payloads: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise SystemExit("V06-0001 seal requires active/fail/completion_ready=false")
...
def run_preflight() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V06-0001 parent is not the sealed frontier tail")
    generations = numbered_generations(ralph_root)
    if len(generations) != EXPECTED_PARENT_COUNT or generations[-1] != generation:
        raise SystemExit(
            f"V06 preflight requires exactly {EXPECTED_PARENT_COUNT} preserved generations"
        )
    ...
    if sha256(manifest_path) != EXPECTED_PARENT_MANIFEST_SHA256:
        raise SystemExit("V06 parent generation manifest hash mismatch")
    if "rah_state" in read_json(ATTEMPT / "report.json"):
        raise SystemExit("V06-0001 report is already RAH-bound; use verify")
    return {
        **checked,
        "completion_ready": False,
...
def run_core() -> dict[str, Any]:
    ...
    if (
        ...
        raise SystemExit("V06 core generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, CORE_EVIDENCE_ID]:
        raise SystemExit(f"V06 core did not append exactly {CORE_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V06 core evidence summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 1:
        raise SystemExit("V06 core did not preserve all prior generations")
...
def run_final() -> dict[str, Any]:
    ...
    if not isinstance(rah_state, dict) or rah_state.get("core_generation") != parent:
        raise SystemExit("V06 report does not bind current core generation")
    if (
        ...
        raise SystemExit("V06 report and live ledger evidence IDs disagree")
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != core_summary():
        raise SystemExit("stored V06 core summary changed before final seal")
    before = numbered_generations(ralph_root)
    prior_ids = evidence_ids(payloads)
    ...
    if (
        ...
        raise SystemExit("V06 final generation pointer mismatch")
    if evidence_ids(sealed) != [*prior_ids, FINAL_EVIDENCE_ID]:
        raise SystemExit(f"V06 final did not append exactly {FINAL_EVIDENCE_ID}")
    if sealed["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit("V06 final closeout summary mismatch")
    after = numbered_generations(ralph_root)
    if after[:-1] != before or len(after) != EXPECTED_PARENT_COUNT + 2:
        raise SystemExit("V06 final did not preserve all prior generations")
    return {
...
def run_verify() -> dict[str, Any]:
    ...
    if count == EXPECTED_PARENT_COUNT + 1:
        ...
        if evidence_ids(payloads)[-1] != CORE_EVIDENCE_ID:
            raise SystemExit(f"V06 core tail is not {CORE_EVIDENCE_ID}")
        return {
            "mode": "core-verify",
            "state_verification": integrity,
            "status": "PASS",
    ...
    if (
        ...
        raise SystemExit(f"unexpected V06 generation count: {count}")
    identifiers = evidence_ids(payloads)
    if identifiers[-2:] != [CORE_EVIDENCE_ID, FINAL_EVIDENCE_ID]:
        raise SystemExit("sealed ledger does not end with V06 core/final evidence")
    report = read_json(ATTEMPT / "report.json")
    parent = str(report["rah_state"]["core_generation"])
    summary, hashes = final_summary(parent)
    if payloads["evidence_ledger.json"]["entries"][-1].get("summary") != summary:
        raise SystemExit(
            "stored V06 final evidence differs from current closeout bytes"
        )
    return {
        "artifact_hashes": hashes,
        "completion_ready": False,
