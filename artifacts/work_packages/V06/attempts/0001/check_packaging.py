#!/usr/bin/env python3
"""Prove the V06 experiment/replication integration gate is discoverable by packaging.

``src/epistemic_foundry/validation`` is an existing package marker (V05 ships
under it) and ``validation.v4_v06`` is the new marker this package adds, so there
...
EXPECTED = ("epistemic_foundry.validation", "epistemic_foundry.validation.v4_v06")
