...
SCOPE = {"domain": "learning_science", "population": "adults"}
...
def receipt_proposal(
    ...
    *,
    ...
    return {
        ...
        "query_text": None if sentinel else f"query for {lane}",
...
def complete_receipts(plan, overrides: dict[str, str] | None = None):
    ...
    for lane in CANONICAL_LANES:
        ...
        result.append(seal_search_lane_receipt(receipt_proposal(plan, lane, overrides.get(lane, default))))
    return result
...
def test_receipt_completeness_test_all_six_states_are_typed(state: str) -> None:
    ...
    receipt = seal_search_lane_receipt(receipt_proposal(plan, lane, state))
...
def test_receipt_completeness_test_unsearched_execution_fields_are_null() -> None:
    plan = plan_for("E1")
    receipt = seal_search_lane_receipt(receipt_proposal(plan, "mechanism", "UNSEARCHED")).payload
...
def test_receipt_completeness_test_zero_results_are_not_unsearched() -> None:
    plan = plan_for("E1")
    searched = seal_search_lane_receipt(receipt_proposal(plan, "lexical", "SEARCHED_NONE")).payload
...
def test_receipt_completeness_test_result_count_and_query_hash_fail_closed() -> None:
    plan = plan_for("E1")
    receipt = seal_search_lane_receipt(receipt_proposal(plan, "lexical", "SEARCHED_WITH_RESULTS")).payload
    ...
    receipt = seal_search_lane_receipt(receipt_proposal(plan, "lexical", "SEARCHED_NONE")).payload
...
def test_receipt_completeness_test_query_hash_binds_exact_persisted_whitespace() -> None:
    ...
    proposal_value["query_text"] = "  exact query text  "

    receipt = seal_search_lane_receipt(proposal_value).payload

    assert receipt["query_text"] == "  exact query text  "
    validate_search_lane_receipt(receipt)

    receipt["query_text"] = "exact query text"
    with pytest.raises(PlanningContractError) as raised:
        validate_search_lane_receipt(receipt)
    assert raised.value.code == "QUERY_HASH_MISMATCH"
...
def test_receipt_completeness_test_duplicate_unsearched_sentinel_fails() -> None:
    ...
    extra = seal_search_lane_receipt(receipt_proposal(plan, "mechanism", "UNSEARCHED", suffix="2"))
...
def test_receipt_completeness_test_unselected_execution_conflicts_with_sentinel() -> None:
    plan = plan_for("E1")
    receipts = complete_receipts(plan)
    execution = seal_search_lane_receipt(receipt_proposal(plan, "mechanism", "SEARCHED_NONE", suffix="2"))
