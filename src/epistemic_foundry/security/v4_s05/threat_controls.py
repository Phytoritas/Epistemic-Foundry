...
from ...budgets.envelope import BudgetViolation, normalize_hard_limits
from ...contracts import default_registry, repo_root, validate_artifact
...
def qualify_candidate_execution(
    *,
    ...
    try:
        limits = normalize_hard_limits(
            dict(_require_mapping(hard_limits, "hard_limits"))
        )
    except BudgetViolation as error:
        _fail("QUOTA_MISSING", str(error), {"hard_limits": dict(hard_limits)})
