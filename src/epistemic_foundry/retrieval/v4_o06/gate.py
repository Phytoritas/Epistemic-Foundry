...
FINDING_CODES: dict[str, str] = {
    ...
    "WORK_CLASS_LANE_RULE_VIOLATED": (
        ...
        "exempt class demanded lanes, or a graded class required none"
    ),
    "CERTIFICATE_REFUSED": (
    ...
    "CLAIM_REFUSED": (
        "the canonical schema refused the novelty assessment, so the claim the "
        "gate would forward is not a valid assessment"
    ),
...
def build_search_completeness_certificate(
    *,
    ...
    class must name at least one required lane, each of which must have been
    selected and conclusively searched for the run to complete.
    """
