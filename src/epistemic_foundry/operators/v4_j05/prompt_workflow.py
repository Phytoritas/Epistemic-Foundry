...
def _text_list(value: object, label: str) -> list[str]:
    ...
    if any(not item.strip() for item in items):
        _fail("INPUT_INVALID", f"{label} carries an empty entry", {"label": label})
    return items
    ...
    try:
        gate = require_inert_mutations(
            ...
            active_prompt_genome_ids=_text_list(
                list(active_prompt_genome_ids), "active_prompt_genome_ids"
            ),
...
def build_activation_record(
    *,
    ...
    evidence = _text_list(
        list(qualification_evidence_ids), "qualification_evidence_ids"
    )
    if not evidence:
        _fail(
