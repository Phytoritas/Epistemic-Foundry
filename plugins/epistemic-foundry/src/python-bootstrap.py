...
def main(argv: list[str]) -> int:
    _force_utf8_streams()
    # The runtime closure is immutable by inventory contract. Never create
    # __pycache__ entries inside the installed payload.
    sys.dont_write_bytecode = True
    if sys.version_info < (3, 12):
        return _fail(
            "PYTHON_VERSION_UNSUPPORTED",
            "the bundled runtime requires Python 3.12 or newer, "
    ...
    resolved = Path(
        getattr(sys.modules["epistemic_foundry"], "__file__", "")
    ).resolve()
    try:
