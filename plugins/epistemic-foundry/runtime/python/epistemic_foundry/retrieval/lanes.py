...
#: Why this backend cannot serve each lane. The reason is the same whether the
#: plan declined the lane (sentinel) or selected it and got nothing (blocked);
#: only the receipt state differs.
ABSENT_LANE_REASONS: dict[str, str] = {
    LANE_SEMANTIC: (
        "no vector backend is configured; the workflow forbids vector similarity as "
        "a sole retrieval channel, and approximating this lane lexically would "
        "report a different search than the one the lane names"
    ),
    LANE_MECHANISM: (
...
def _family_query_batch(
    lane: str, family: str, queries: Sequence[str]
) -> dict[str, Any]:
    """One lane's canonical batch for exactly one query family."""
    ordered: list[str] = []
    for item in queries:
        text = str(item).strip()
    ...
    return {"lane": lane, "families": [{"family": family, "queries": ordered}]}


def _query_batch(lane: str, queries: Sequence[str]) -> dict[str, Any]:
    families = LANE_QUERY_FAMILIES.get(lane)
    if not families:
        raise LaneContractError(f"{lane!r} is not a canonical lane")
    # The workflow fixes which families each lane accepts. A lane that takes
    # more than one — counterevidence needs FORWARD and REVERSE — cannot be
    # served by repeating the same query text in both directions, because a
    # reverse search is a different search, not the same one relabelled.
    if len(families) > 1:
        raise LaneContractError(
            f"lane {lane!r} accepts the query families {list(families)}; this "
            "backend has no way to derive them from a single query batch"
        )
    return _family_query_batch(lane, families[0], queries)
...
def _backend_request(
    ...
    return {
        ...
        "lane": lane,
        "query_families": list(LANE_QUERY_FAMILIES[lane]),
...
def blocked_lane_receipt(
    context: LaneContext,
    lane: str,
    *,
    queries: Sequence[str],
    stop_reason: str = STOP_REASON_BACKEND_UNAVAILABLE,
    errors: Sequence[str] = (),
) -> dict[str, Any]:
    """Seal the BLOCKED execution receipt a selected but unservable lane owes.

    A lane the plan selected was promised a search. When no backend can run it,
    the truthful report is that the search was attempted and blocked, not that
    the lane was never required. `UNSEARCHED` is reserved for lanes the plan
    itself declined; using it here would let an unserved obligation read as a
    deliberate scope decision, and the certificate would then reconcile a lane
    that nothing ever answered.

    ``queries`` is the query batch the lane would have executed, in the lane's
    own query family. It is required: a blocked receipt that named no query
    would say a search failed without saying which search, and nothing
    downstream could tell what went unanswered.

    A lane that accepts several families, such as counterevidence, is refused
    outright rather than sealed from one family's text. Its canonical query
    binding covers every family, so a one-family batch would not merely be
    incomplete — it would hash to a different search than the one the lane owes.
    """
    if stop_reason not in BLOCKED_STOP_REASONS:
        raise LaneContractError(
            f"{stop_reason!r} is not a blocked stop reason; a BLOCKED receipt "
            f"must name one of {sorted(BLOCKED_STOP_REASONS)}"
        )
    families = LANE_QUERY_FAMILIES.get(lane)
    if not families:
        raise LaneContractError(f"{lane!r} is not a canonical lane")
    if len(families) > 1:
        raise LaneContractError(
            f"lane {lane!r} binds the query families {list(families)}; a blocked "
            "receipt sealed from one family would name a different search than "
            "the lane owes, so this backend cannot seal one for it"
        )
    batch = _family_query_batch(lane, families[0], queries)
    query_text, query_hash = _query_text_and_hash(batch)
    # The schema requires unique errors; preserve first-occurrence order.
    reported: list[str] = []
    for error in errors:
    ...
    return _seal_receipt(
        {
            ...
            "sentinel_reason": None,
            "lane_decision_evidence_ids": list(context.lane_decision_evidence_ids),
            # The lane was selected, so the query it would have run is known and
            # bound; only the execution is missing.
            "query_text": query_text,
            "query_hash": query_hash,
            "scope_filter": copy.deepcopy(dict(context.scope_filter)),
            "corpus_snapshot_hash": context.corpus_snapshot_hash,
            "index_versions": dict(context.index_versions),
            # Null, not zero: no result set was ever produced, and a count of
            # zero would be indistinguishable from a completed empty search.
            "result_ids": None,
            "result_count": None,
            "excluded_count": None,
            "stop_reason": stop_reason,
            "search_state": RECEIPT_STATE_BLOCKED,
            "recall_proxy": None,
            "errors": reported,
            "started_at": context.started_at,
            "finished_at": context.finished_at,
...
def blocked_lane_result(
    ...
    *,
    ...
    """A selected lane no backend could serve, reported as blocked."""
    receipt = blocked_lane_receipt(
        context, lane, queries=queries, stop_reason=stop_reason, errors=errors
    )
    return LaneResult(
        lane=lane,
        # Coverage-wise a blocked lane is inconclusive, exactly like a failed
        # one; the receipt carries the finer BLOCKED distinction on the wire.
        search_state=SearchState.SEARCH_FAILED,
        candidates=(),
        receipt=receipt,
        documents=(),
        duplicate_count=0,
        ...
        diagnostics={
            "stop_reason": stop_reason,
            "blocked_explanation": receipt["errors"][0],
        },
    )
...
def reconcile_lanes(
    ...
    *,
    ...
    A lane the caller *did* run but no backend could serve arrives as a blocked
    result and keeps its `BLOCKED` receipt state here. Only a lane nobody asked
    for becomes a sentinel.
    """
    ...
    receipt_states = {
        result.lane: str(result.receipt["search_state"]) for result in ordered
    }
    observed_channels = {
        channel
    ...
    return {
        ...
        # The wire vocabulary, which keeps BLOCKED and PARTIAL distinct from the
        # four-state coverage projection above.
        "receipt_states": dict(receipt_states),
        ...
        "blocked_lanes": [
            lane
            for lane in CANONICAL_LANES
            if receipt_states[lane] == RECEIPT_STATE_BLOCKED
        ],
