...
def _validate_receipt(
    record: dict[str, Any], schema_name: str, label: str
) -> None:
    try:
        validate_artifact(schema_name, record)
    except ContractViolation as error:
        _fail(
            "INPUT_INVALID",
            f"{label} does not satisfy the canonical {schema_name} schema",
            {"schema_errors": list(error.errors)},
        )
    expected_hash = hash_excluding(record, "receipt_hash")
    if record.get("receipt_hash") != expected_hash:
        _fail(
            "INPUT_INVALID",
            f"{label} does not re-derive its own receipt hash",
            {
                "expected_receipt_hash": expected_hash,
                "recorded_receipt_hash": record.get("receipt_hash"),
            },
        )
...
def _index_effects(
    ...
    for position, entry in enumerate(effect_receipts):
        ...
        _validate_receipt(record, "effect-receipt", f"effect_receipts[{position}]")
...
def _index_mutations(
    ...
    for position, entry in enumerate(mutation_receipts):
        ...
        _validate_receipt(
            record, "mutation-receipt", f"mutation_receipts[{position}]"
        )
