...
class PromotionCommitter:
    ...
    def commit(
        ...
        *,
        ...
        effect_receipt_record = dict(effect_receipt)
        try:
            validate_artifact("effect-receipt", effect_receipt_record)
        except ContractViolation as exc:
            raise MissingEffectReceipt(str(exc)) from exc
        claimed_receipt_hash = effect_receipt_record.get("receipt_hash")
        derived_receipt_hash = hash_excluding(effect_receipt_record, "receipt_hash")
        if claimed_receipt_hash != derived_receipt_hash:
            raise MissingEffectReceipt(
                "EffectReceipt receipt_hash mismatch: "
                f"claimed {claimed_receipt_hash!r}; derived {derived_receipt_hash!r}"
            )
        if effect_receipt_record.get("receipt_id") != decision.get("effect_receipt_id"):
            raise MissingEffectReceipt("EffectReceipt ID does not resolve PromotionDecision")
        if effect_receipt_record.get("intent_id") != decision.get(
            "commit_action_intent_id"
        ):
            raise MissingEffectReceipt("EffectReceipt does not resolve commit ActionIntent")
        if effect_receipt_record.get("idempotency_key") != key:
        ...
        if effect_receipt_record.get("status") not in allowed_statuses:
            raise MissingEffectReceipt(
                f"EffectReceipt status {effect_receipt_record.get('status')!r} "
                "does not resolve commit"
            )
        ...
        if state_changed:
            ...
            new_candidate["promotion_history"] = list(
            ...
            ) + [
                {
                    ...
                    "effect_receipt_id": effect_receipt_record["receipt_id"],
                }
            ]
...
def promoted(decision: Mapping[str, Any]) -> bool:
    ...
    try:
        validate_artifact("promotion-decision", decision_record)
    except ContractViolation as exc:
        raise PromotionRefused(str(exc)) from exc
    claimed_decision_hash = decision_record.get("decision_hash")
    derived_decision_hash = hash_excluding(decision_record, "decision_hash")
    if claimed_decision_hash != derived_decision_hash:
        raise PromotionRefused(
            "PromotionDecision decision_hash mismatch: "
            f"claimed {claimed_decision_hash!r}; derived {derived_decision_hash!r}"
        )
    return decision_record.get("decision") in {
        "PROMOTE",
        "CONDITIONAL",
