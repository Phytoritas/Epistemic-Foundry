...
def _deep_snapshot(
    value: object, *, label: str = "value", active: set[int] | None = None
) -> Any:
    """Detach one JSON tree without invoking caller-defined copy callbacks."""
    if active is None:
        active = set()
    ...
    if isinstance(value, Mapping):
        ...
        if identity in active:
            _fail("INPUT_INVALID", f"{label} contains a cycle", {"label": label})
        active.add(identity)
        try:
            try:
            ...
            for position, entry in enumerate(entries):
                try:
                    ...
                    _fail(
                        "INPUT_INVALID",
                        f"{label} contains a malformed mapping entry",
                        {
                ...
                if key in copied:
                    _fail(
                        "INPUT_INVALID",
                        f"{label} contains a duplicate mapping key",
                        {"key": key, "label": label},
                    )
                copied[key] = _deep_snapshot(
    ...
    if isinstance(value, Sequence) and not isinstance(
        ...
        try:
            ...
            return [
                _deep_snapshot(
                    item, label=f"{label}[{position}]", active=active
                )
...
class MutationOperatorRegistry:
    ...
    def record(self, operator_id: str) -> dict[str, Any]:
        ...
        return _deep_snapshot(self._records[identifier])

    def records(self) -> tuple[dict[str, Any], ...]:
        """Every record, in id order."""
        return tuple(_deep_snapshot(self._records[key]) for key in self.operator_ids())
    ...
    def parameter_contract(self, operator_id: str) -> dict[str, Any]:
        ...
        return _deep_snapshot(self._parameters[str(operator_id)])
    ...
    def proposal(self, operator_id: str) -> dict[str, Any] | None:
        ...
        return None if held is None else _deep_snapshot(held)
    ...
    def register(
        ...
        *,
        ...
        """
        operator_contract()
        document = _deep_snapshot(_require_mapping(spec, "spec"), label="spec")
        try:
        ...
        require_sealed_digest(document, OPERATOR_HASH_FIELD, OPERATOR_SPEC_KIND)
    ...
    def _validated_parameters(
        ...
        enforce.
        """
        declared = _deep_snapshot(
            _require_mapping(declared_parameters, "declared_parameters"),
            label="declared_parameters",
        )
    ...
    def _validated_proposal(
        ...
        document = _deep_snapshot(
            _require_mapping(proposal, "proposal"), label="proposal"
        )
        prompt_proposal_contract()
        proposal_status_vocabulary()
        try:
    ...
    def bind_parameters(
        ...
        supplied = _deep_snapshot(
            _require_mapping(arguments, "arguments"), label="arguments"
        )
    ...
    def claim_active_prompt_operator(
        ...
        try:
            gate = require_inert_mutations(
                ...
                proposals=(_deep_snapshot(proposal),),
            )
