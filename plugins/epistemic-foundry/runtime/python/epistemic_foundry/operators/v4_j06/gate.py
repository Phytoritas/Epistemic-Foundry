...
def admit_operator_application(
    *,
    ...
    if isinstance(qualification_evidence_ids, (str, bytes, Mapping)) or not isinstance(
        qualification_evidence_ids, Sequence
    ):
        _fail(
            "INPUT_INVALID",
            "qualification_evidence_ids must be a sequence of identifiers",
            {"submitted_type": type(qualification_evidence_ids).__name__},
        )
    evidence = [
        _require_text(item, f"qualification_evidence_ids[{index}]")
        for index, item in enumerate(qualification_evidence_ids)
    ]
