...
def build_niche(
    *,
    ...
    if not isinstance(occupant_ids, Sequence) or isinstance(
        occupant_ids, (str, bytes, bytearray, Mapping)
    ):
        _fail(
            "INPUT_INVALID",
            "occupant_ids must be a sequence of non-empty strings",
            {"actual_type": type(occupant_ids).__name__},
        )
    occupants: list[str] = []
    for position, occupant in enumerate(occupant_ids):
        if not isinstance(occupant, str) or not occupant:
            _fail(
                "INPUT_INVALID",
                f"occupant_ids[{position}] must be a non-empty string",
                {"index": position, "occupant_id": occupant},
            )
...
class NicheMap:
    ...
    def __init__(self, niches: Sequence[Mapping[str, Any]]) -> None:
        ...
        for position, candidate_niche in enumerate(niches):
            ...
            if len(set(niche["occupant_ids"])) != len(niche["occupant_ids"]):
                _fail(
                    "INPUT_INVALID",
                    "a niche cannot hold the same occupant twice",
                    {"occupant_ids": list(niche["occupant_ids"])},
                )
            if len(niche["occupant_ids"]) > niche["capacity"]:
                _fail(
                    "NICHE_OVERFULL",
                    f"{len(niche['occupant_ids'])} occupants exceed the declared capacity {niche['capacity']}",
                    {
                        "capacity": niche["capacity"],
                        "occupant_count": len(niche["occupant_ids"]),
                    },
                )
