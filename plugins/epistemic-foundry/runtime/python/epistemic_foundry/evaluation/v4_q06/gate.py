...
* **Validation advancement (V05).**  The V05 receipt is composed *without*
  ...
  artifact — it must name the V05 advancement gate, re-derive its own hash, name
  this candidate, and carry a boolean advancement verdict — and it is *bound*
  to this exact Q05 clearance:
...
#: The opaque V05 component identity this gate composes.  Importing V05 would
#: close a forbidden top-level ``evaluation``↔``validation`` cycle, so the
#: issuer name is pinned at this boundary exactly as P06 pins the same receipt.
#: It is a component identity, not a canonical schema vocabulary token.
COMPOSED_VALIDATION_GATE_NAME = "validation-cascade-advancement"
COMPOSED_VALIDATION_GATE_ID_PREFIX = "VCA-"
...
FINDING_CODES: dict[str, str] = {
    ...
    "ADVANCEMENT_RECEIPT_UNVERIFIED": (
        "the validation-advancement receipt does not name the owning gate, "
        "re-derive its own hash or carry a boolean advancement verdict, so it "
        "is not the self-sealing advancement record this gate composes"
    ),
...
def _verify_advancement(
    ...
    *,
    ...
    self-sealing artifact: it must name the owning V05 gate, re-derive its own
    hash, carry a boolean advancement verdict, and name this candidate.  It is
    then bound to this exact Q05 clearance — its recorded
    statistical-admissibility hash must equal the Q05 receipt's own hash — so a
    foreign advancement record cannot claim a clearance it never composed.
    Whether the receipt *advanced* is decided in the governance phase, not here.
    """
    record = _require_mapping(advancement_receipt, "advancement_receipt")
    if str(record.get("gate")) != COMPOSED_VALIDATION_GATE_NAME:
        _fail(
            "ADVANCEMENT_RECEIPT_UNVERIFIED",
    ...
    expected_gate_id = (
        COMPOSED_VALIDATION_GATE_ID_PREFIX
        + sha256_hex(
            canonical_json(
                {
                    "candidate_id": record.get(CANDIDATE_FIELD),
                    "cascade_plan_id": record.get("cascade_plan_id"),
                    "created_at": record.get("created_at"),
                    "decision": record.get("decision"),
                    "statistical_admissibility_receipt_hash": record.get(
    ...
    supplied_gate_id = record.get("gate_id")
    if not isinstance(supplied_gate_id, str) or supplied_gate_id != expected_gate_id:
        _fail(
            "ADVANCEMENT_RECEIPT_UNVERIFIED",
            "the advancement receipt does not re-derive its V05 gate id",
            {"recorded": supplied_gate_id, "recomputed": expected_gate_id},
        )
    if hash_excluding(dict(record), "receipt_hash") != record.get("receipt_hash"):
        _fail(
            "ADVANCEMENT_RECEIPT_UNVERIFIED",
...
def _verify_calibration(
    ...
    claimed_report_hash = record.get("report_hash")
    derived_report_hash = hash_excluding(record, "report_hash")
    if derived_report_hash != claimed_report_hash:
        _fail(
            "CALIBRATION_CONTRACT_VIOLATED",
            "the calibration report does not re-derive its own hash",
            {
                "claimed_report_hash": claimed_report_hash,
                "derived_report_hash": derived_report_hash,
            },
        )
