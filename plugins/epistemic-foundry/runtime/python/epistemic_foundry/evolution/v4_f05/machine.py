...
def evaluate_run(
    ...
    *,
    ...
    if stop_certificate is not None:
        ...
        try:
            validate_artifact("evolution-stop-certificate", record)
            derived_certificate_hash = hash_excluding(record, "certificate_hash")
            if record["certificate_hash"] != derived_certificate_hash:
                certificate_findings["certificate_hash_mismatch"] = {
                    "declared": record["certificate_hash"],
                    "derived": derived_certificate_hash,
                }
        except ContractViolation as error:
            certificate_findings["schema_errors"] = list(error.errors)
        else:
            ...
            elif certified_checkpoint not in {
