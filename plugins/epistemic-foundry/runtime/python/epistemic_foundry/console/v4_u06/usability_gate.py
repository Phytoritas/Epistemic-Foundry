...
def audit_dashboard_completeness(dashboard: Mapping[str, Any]) -> dict[str, Any]:
    ...
    mismatches = {
        ...
        if (
            sha256_of_payload(record.get(field))
            != sha256_of_payload(recomputed[field])
        )
    }
    if mismatches:
        _fail(
...
def build_operator_dashboard_from_payloads(
    *,
    surface_payloads: Mapping[str, Mapping[str, Any] | None],
    created_at: str,
    source_revisions: Mapping[str, str | None] | None = None,
    current_revisions: Mapping[str, str | None] | None = None,
    requesting_role: str = DEFAULT_REQUESTING_ROLE,
    authority_request: object | None = None,
) -> MappingProxyType:
    """Compose requested U05 payloads into one audited U06 dashboard.

    The keys of ``surface_payloads`` are the complete requested-surface set. A
    present ``None`` payload therefore becomes one honest ``UNAVAILABLE`` panel,
    while an absent key creates no panel.  Construction follows the sealed U05
    surface order rather than caller insertion order.
    """
    role = _require_text(requesting_role, "requesting_role")
    if authority_request is not None:
        _fail(
    ...
    timestamp = _require_text(created_at, "created_at")
    payloads = _require_mapping(surface_payloads, "surface_payloads")
    declared_order = declared_panel_surfaces()
    ...
    for surface, payload in payloads.items():
        if not isinstance(surface, str):
            _fail(
                "INPUT_INVALID",
                "surface_payloads keys must be strings",
                {"key_type": type(surface).__name__},
            )
        if surface not in declared:
            _fail(
                "SURFACE_UNDECLARED",
                "the sealed console does not project the requested surface",
                {"declared": list(declared_order), "surface": surface},
            )
        if payload is not None and not isinstance(payload, Mapping):
            _fail(
                "INPUT_INVALID",
                "surface payloads must be mappings or None",
                {"surface": surface, "value_type": type(payload).__name__},
            )
        requested.add(surface)

    def normalize_revisions(
        candidate: Mapping[str, str | None] | None, label: str
    ) -> dict[str, str | None]:
        if candidate is None:
            return {}
        revisions = _require_mapping(candidate, label)
        normalized: dict[str, str | None] = {}
        for surface, revision in revisions.items():
            if not isinstance(surface, str):
                _fail(
                    "INPUT_INVALID",
                    f"{label} keys must be strings",
                    {"key_type": type(surface).__name__, "label": label},
                )
            if surface not in requested:
                _fail(
                    "INPUT_INVALID",
                    f"{label} may name only requested surfaces",
                    {"label": label, "surface": surface},
                )
            if revision is None:
                normalized[surface] = None
            elif isinstance(revision, str):
                normalized[surface] = _require_text(
                    revision, f"{label}[{surface!r}]"
                )
            else:
                _fail(
                    "INPUT_INVALID",
                    f"{label} values must be strings or None",
                    {
                        "label": label,
                        "surface": surface,
                        "value_type": type(revision).__name__,
                    },
                )
        return normalized

    sources = normalize_revisions(source_revisions, "source_revisions")
    currents = normalize_revisions(current_revisions, "current_revisions")
    panels = [
        build_operator_panel(
            surface=surface,
            payload=payloads[surface],
            created_at=timestamp,
            source_revision=sources.get(surface),
            current_revision=currents.get(surface),
            requesting_role=role,
            authority_request=None,
        )
        for surface in declared_order
        if surface in requested
    ]
    dashboard = compose_operator_dashboard(
        panels=panels,
        created_at=timestamp,
        requesting_role=role,
        authority_request=None,
    )
    require_dashboard_identity(dashboard)
    audit_dashboard_completeness(dashboard)
    return dashboard
...
__all__ = [
    ...
    "build_console_projection",
    "build_operator_dashboard_from_payloads",
    "build_operator_panel",
