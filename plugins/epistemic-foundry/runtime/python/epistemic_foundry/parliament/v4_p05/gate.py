...
FINDING_CODES: dict[str, str] = {
    ...
    "CANDIDATE_IDENTITY_MISMATCH": (
        "the lineage, adjudication, statistical clearance, replication plan or "
        "replication result does not describe the one candidate this docket is "
        "convened about"
    ),
    ...
    "REPLICATION_RESULT_CONTRACT_VIOLATED": (
        "a replication result does not satisfy its canonical schema or re-derive "
        "its own hash, or the schema declares no promotion-effect vocabulary to "
        "read the blocking effect from"
    ),
...
def _resolve_replication(
    ...
    for position, result in enumerate(replication_results):
        ...
        claimed_hash = document["result_hash"]
        derived_hash = hash_excluding(document, "result_hash")
        if derived_hash != claimed_hash:
            _fail(
                "REPLICATION_RESULT_CONTRACT_VIOLATED",
                "a replication result does not re-derive its own hash",
                {
                    "position": position,
                    "claimed_result_hash": claimed_hash,
                    "derived_result_hash": derived_hash,
                },
            )
        if str(document.get(CANDIDATE_FIELD)) != expected:
            _fail(
...
def _resolve_red_queen(
    ...
    for position, result in enumerate(red_queen_results):
        document = _require_mapping(result, f"red_queen_results[{position}]")
        try:
            validate_artifact(CHALLENGE_RESULT_KIND, document)
        except ContractViolation as error:
            _fail(
                "INPUT_INVALID",
                "a Red Queen challenge result does not satisfy its canonical schema",
                {"position": position, "schema_errors": list(error.errors)},
            )
        claimed_hash = document["result_hash"]
        derived_hash = hash_excluding(document, "result_hash")
        if derived_hash != claimed_hash:
            _fail(
                "INPUT_INVALID",
                "a Red Queen challenge result does not re-derive its own hash",
                {
                    "position": position,
                    "claimed_result_hash": claimed_hash,
                    "derived_result_hash": derived_hash,
...
def derive_promotion_parliament(
    *,
    ...
    replication_plan_hash: str | None = None
    ...
    if replication_plan is not None:
        plan = _require_mapping(replication_plan, "replication_plan")
        try:
            validate_artifact(REPLICATION_PLAN_KIND, plan)
        except ContractViolation as error:
            _fail(
                "INPUT_INVALID",
                "the replication plan does not satisfy its canonical schema",
                {"schema_errors": list(error.errors)},
            )
        claimed_hash = plan["plan_hash"]
        derived_hash = hash_excluding(plan, "plan_hash")
        if derived_hash != claimed_hash:
            _fail(
                "INPUT_INVALID",
                "the replication plan does not re-derive its own hash",
                {
                    "claimed_plan_hash": claimed_hash,
                    "derived_plan_hash": derived_hash,
                },
            )
        if plan["candidate_id"] != expected:
            _fail(
                "CANDIDATE_IDENTITY_MISMATCH",
                "the replication plan describes a different candidate",
                {"expected": expected, "found": plan.get("candidate_id")},
            )
        replication_plan_id = plan["replication_plan_id"]
        replication_plan_hash = plan["plan_hash"]
    ...
    receipt: dict[str, Any] = {
        ...
        "replication_plan_hash": replication_plan_hash,
