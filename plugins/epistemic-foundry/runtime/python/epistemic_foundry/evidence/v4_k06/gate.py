...
FINDING_CODES: dict[str, str] = {
    ...
    "RECEIPT_REFUSED": (
        "the lane receipt does not validate against its canonical schema or "
        "re-derive its content-bound identity and self-hash, so whether it "
        "escaped the evidence boundary is undecidable"
    ),
    "SNAPSHOT_REFUSED": (
...
def admit_search_results_against_version(
    *, version: Mapping[str, Any], receipt: Mapping[str, Any]
    ...
    The receipt must validate against its canonical schema, re-derive its
    content-bound identity and self-hash, pin the bound snapshot when it pins
    one at all, and not return a document the partition conceals — the case of
    a search escaping into the hidden holdout while it ran.
    """
    bound = require_version_identity(version)
    ...
    try:
        derived_receipt_id, derived_receipt_hash = (
            derive_content_addressed_record_identity(
                lane_receipt, RECEIPT_ID_PREFIX, "receipt_id", "receipt_hash"
            )
        )
    except (TypeError, ValueError) as error:
        _fail(
            "RECEIPT_REFUSED",
            "the lane receipt cannot be canonically hashed",
            {"error": str(error), "lane": lane_receipt.get("lane")},
        )
        raise  # pragma: no cover - _fail always raises
    if lane_receipt.get("receipt_id") != derived_receipt_id or (
        lane_receipt.get("receipt_hash") != derived_receipt_hash
    ):
        _fail(
            "RECEIPT_REFUSED",
            "the lane receipt does not re-derive its content-bound identity",
            {
                "derived_receipt_hash": derived_receipt_hash,
                "derived_receipt_id": derived_receipt_id,
                "lane": lane_receipt.get("lane"),
                "stated_receipt_hash": lane_receipt.get("receipt_hash"),
                "stated_receipt_id": lane_receipt.get("receipt_id"),
            },
        )
...
def admit_candidate_execution_against_version(
    *,
    ...
    try:
        qualification = qualify_candidate_execution(
            ...
            budget_envelope=dict(
                _require_mapping(budget_envelope, "budget_envelope")
            ),
    ...
    admission: dict[str, Any] = {
        "budget_enforcement": qualification["budget_enforcement"],
        "budget_hash": qualification["budget_hash"],
        "budget_id": qualification["budget_id"],
...
def admit_evaluator_feedback_against_version(
    *,
    ...
    normalized_feedback_ids = sorted(
        [
    ...
    normalized_surfaces = sorted(
        {
            _require_text(value, f"surfaces_checked[{position}]")
            for position, value in enumerate(
                _require_sequence(surfaces_checked, "surfaces_checked")
            )
        }
    )
    normalized_access_log_id = _require_text(
        access_log_artifact_id, "access_log_artifact_id"
    )
    normalized_similarity_alerts = sorted(
        [
            _require_text(value, f"similarity_alerts[{position}]")
            for position, value in enumerate(
                _require_sequence(similarity_alerts, "similarity_alerts")
            )
        ]
    )
    normalized_run_or_bundle_id = _require_text(
    ...
    audit_id = LEAKAGE_AUDIT_ID_PREFIX + _digest_body(
        {
            "access_log_artifact_id": normalized_access_log_id,
            "feedback_artifact_ids": normalized_feedback_ids,
            "run_or_bundle_id": normalized_run_or_bundle_id,
            "similarity_alerts": normalized_similarity_alerts,
            "surfaces_checked": normalized_surfaces,
            "version_id": str(bound["version_id"]),
        }
    )
    try:
        audit = build_leakage_audit(
            ...
            run_or_bundle_id=normalized_run_or_bundle_id,
            surfaces_checked=normalized_surfaces,
            observed_artifact_ids=normalized_feedback_ids,
            access_log_artifact_id=normalized_access_log_id,
            similarity_alerts=normalized_similarity_alerts,
    ...
    admission: dict[str, Any] = {
        "admitted_feedback_artifact_ids": normalized_feedback_ids,
