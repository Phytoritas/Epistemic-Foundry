...
def _require_text_sequence(
    value: object, label: str, *, allow_empty_items: bool = False
) -> tuple[str, ...]:
    if isinstance(value, (str, bytes, bytearray, Mapping)) or not isinstance(
        ...
        _fail(
            ...
            {"label": label, "submitted_type": type(value).__name__},
        )
    items: list[str] = []
    for position, item in enumerate(value):  # type: ignore[union-attr]
        if not isinstance(item, str) or (not allow_empty_items and not item.strip()):
            requirement = "a string" if allow_empty_items else "a non-empty string"
            _fail(
                "INPUT_INVALID",
                f"{label}[{position}] must be {requirement}",
                {"label": label, "position": position},
            )
        items.append(item if allow_empty_items else item.strip())
    return tuple(items)
...
def assess_novelty_within_boundary(
    *,
    ...
    dimensions = sorted(
        set(_require_text_sequence(novelty_dimensions, "novelty_dimensions"))
    )
    ...
    refs = sorted(
        set(_require_text_sequence(closest_prior_art_refs, "closest_prior_art_refs"))
    )
    ...
    features = list(
        _require_text_sequence(distinguishing_features, "distinguishing_features")
    )
    unsearched = [str(value) for value in record.get("unsearched_sources") or []]
    if refs and not features:
        status_position = PRIOR_ART_FOUND_POSITION
    elif refs and unsearched:
    ...
    assessment: dict[str, Any] = {
        ...
        "limitations": [
            *_require_text_sequence(
                limitations, "limitations", allow_empty_items=True
            ),
