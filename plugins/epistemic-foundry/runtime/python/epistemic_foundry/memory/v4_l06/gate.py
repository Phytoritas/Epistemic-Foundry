...
class LegalHoldRegister:
    ...
    def __init__(self, records: Sequence[Mapping[str, Any]] = ()) -> None:
        ...
        for index, given in enumerate(records):
            record = self._verified_copy(given, f"holds[{index}]")
    ...
    @staticmethod
    def _verified_copy(given: Mapping[str, Any], label: str) -> dict[str, Any]:
        record = deepcopy(dict(_require_mapping(given, label)))
        identifier = _require_id(record.get("hold_id"), f"{label}.hold_id")
        ...
        placed_authority = _hold_authority(
            record.get("hold_authority"), f"{label}.authority"
        )
        release_fields = ("release_authority", "released_at", "release_hash")
        present_release_fields = tuple(
            field for field in release_fields if field in record
        )
        if present_release_fields:
            missing_release_fields = tuple(
                field for field in release_fields if field not in record
            )
            if missing_release_fields:
                _fail(
                    "INPUT_INVALID",
                    f"hold {identifier} has an incomplete release record",
                    {
                        "hold_id": identifier,
                        "missing": list(missing_release_fields),
                        "present": list(present_release_fields),
                    },
                )
            _require_id(record.get("released_at"), f"{label}.released_at")
            release_authority = _hold_authority(
                record.get("release_authority"), f"{label}.release_authority"
            )
            if (
                release_authority["legal_matter_id"]
                != placed_authority["legal_matter_id"]
            ):
                _fail(
                    "HOLD_AUTHORITY_MISMATCHED",
                    f"hold {identifier} was released under a different matter",
                    {
                        "hold_id": identifier,
                        "placed_under": placed_authority["legal_matter_id"],
                        "released_under": release_authority["legal_matter_id"],
                    },
                )
            declared_release = str(record.get("release_hash") or "")
            derived_release = hash_excluding(record, "release_hash")
            if declared_release != derived_release:
                _fail(
                    "INPUT_INVALID",
                    f"hold {identifier} does not re-derive its release hash",
                    {
                        "declared": declared_release,
                        "derived": derived_release,
                        "hold_id": identifier,
                    },
                )
        return record
    ...
    def hold(self, hold_id: str) -> Mapping[str, Any]:
        ...
        return self._verified_copy(self._holds[identifier], f"holds[{identifier}]")

    def standing(self) -> tuple[dict[str, Any], ...]:
        """The holds that have not been released, in id order."""
        verified = tuple(
            self._verified_copy(record, f"holds[{identifier}]")
            for identifier, record in sorted(self._holds.items())
        )
        return tuple(
            record
...
def release_legal_hold(
    *,
    ...
    timestamp = _require_id(released_at, "released_at")
