...
authority it does not hold.  A candidate whose hard gates have not passed cannot
have its multi-objective proxy dimensions routed as reward (EF4-I45: a combined score may
...
FINDING_CODES: dict[str, str] = {
    ...
    "REWARD_HACKING_HARD_GATE_FAILED": (
        "the candidate has not passed its hard gates, so routing its proxy "
        "dimensions as reward would let a score the gates rejected acquire search authority — "
    ...
    "EVALUATOR_CURRENT_BUNDLE_MISMATCH": (
        "the proposal names a current evaluator bundle other than the one sealed "
        "by the verifier firewall, so future/current qualification comparisons "
        "would be made against a forged identity"
    ),
    "HOLDOUT_REACHABLE": (
...
def refuse_reward_hacking(
    *,
    ...
    Three boundaries are composed: a candidate that has not passed its hard gates
    cannot have its proxy dimensions rewarded (EF4-I45), a reward that learns only from
    ...
    if hash_excluding(routing, "receipt_hash") != routing["receipt_hash"]:
        _fail(
            "ROUTING_CONTRACT_VIOLATED",
            "the model-routing receipt self-hash does not match its content; "
            "refusing a tampered routing receipt",
            {"receipt_id": str(routing["receipt_id"])},
        )

    if str(fitness["hard_gate_status"]) != vocab["hard_gate_passed"]:
        _fail(
            "REWARD_HACKING_HARD_GATE_FAILED",
            "a candidate without a hard-gate PASS may not have its proxy "
            "dimensions rewarded",
            {
...
def govern_evaluator_update(
    *,
    ...
    if document["current_evaluator_bundle_id"] != firewall.bundle_id:
        _fail(
            "EVALUATOR_CURRENT_BUNDLE_MISMATCH",
            "the proposal's current evaluator bundle does not match the firewall's "
            "sealed bundle",
            {
                "firewall_bundle_id": firewall.bundle_id,
                "proposal_current_evaluator_bundle_id": document[
