...
def _resolve_cascade_status(
    ...
    *,
    ...
    if hash_excluding(plan_record, "plan_hash") != plan_record.get("plan_hash"):
        _fail(
            "INPUT_INVALID",
            "the cascade plan does not re-derive its own hash",
            {
                "cascade_plan_id": str(
                    plan_record.get("cascade_plan_id") or ""
                )
            },
        )
    ...
    for position, result in enumerate(stage_results):
        ...
        claimed_hash = record["result_hash"]
        derived_hash = hash_excluding(record, "result_hash")
        if derived_hash != claimed_hash:
            _fail(
                "INPUT_INVALID",
                "a stage result does not re-derive its own hash",
                {
                    "stage_result_id": str(record.get("stage_result_id") or ""),
    ...
    if replication_plan is not None:
        ...
        if hash_excluding(plan_record, "plan_hash") != plan_record.get("plan_hash"):
            _fail(
                ...
                {
                    "replication_plan_id": str(
                        plan_record.get("replication_plan_id") or ""
                    )
                },
            )
