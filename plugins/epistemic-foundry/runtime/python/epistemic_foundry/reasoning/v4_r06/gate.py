...
def scope_scalar_fields() -> tuple[str, ...]:
    ...
    The nullable-string scalars are compared directly.  The set is filtered out
    of the canonical schema so a field rename tracks automatically.
    """
...
def _scope_structured_fields(scalar_fields: tuple[str, ...]) -> tuple[str, ...]:
    """The schema-owned non-scalar scope boundaries compared canonically."""
    document = default_registry().document(SCOPE_KIND)
    properties = document.get("properties", {})
    if not isinstance(properties, Mapping) or not properties:
        _fail(
    ...
    return tuple(sorted(set(properties) - set(scalar_fields)))


def _structured_scope_is_declared(value: object) -> bool:
    """True when a structured boundary carries content rather than its empty form."""
    if value is None:
        return False
...
def _resolve_parent(
    ...
    try:
        ...
        _fail(
            "SCOPE_VECTOR_CONTRACT_VIOLATED",
            "a scope vector is not canonical JSON",
            {"error_type": type(error).__name__, "scope_vector_id": scope_id},
        )
...
def _scope_is_empty(
    ...
    for field in structured_fields:
        if _structured_scope_is_declared(scope.get(field)):
            return False
    return True
...
def _derive_scope(left: _Parent, right: _Parent) -> tuple[str, list[str]]:
    ...
    structured_fields = _scope_structured_fields(scalar_fields)
    left_scope = left.scope_vector
    right_scope = right.scope_vector
    if _scope_is_empty(left_scope, scalar_fields, structured_fields):
        return "unknown", [f"{left.genome_id}:no-declared-scope"]
    if _scope_is_empty(right_scope, scalar_fields, structured_fields):
    ...
    for field in structured_fields:
        left_value = left_scope.get(field)
        right_value = right_scope.get(field)
        left_declared = _structured_scope_is_declared(left_value)
        right_declared = _structured_scope_is_declared(right_value)
        if not left_declared or not right_declared:
            continue
        try:
            values_differ = canonical_json(left_value) != canonical_json(right_value)
        except (TypeError, ValueError, OverflowError) as error:
            _fail(
                "SCOPE_VECTOR_CONTRACT_VIOLATED",
                "a structured scope boundary is not canonical JSON",
                {"error_type": type(error).__name__, "field": field},
            )
        if values_differ:
            conflicts.append(field)
    vocab = _vocab()
...
def _bind_crossover_report(
    ...
    named = {str(item) for item in candidate_ids}
    parents = {left.genome_id, right.genome_id}
    if len(candidate_ids) != 2 or len(named) != 2 or named != parents:
        _fail(
            "CROSSOVER_REPORT_MISBOUND",
