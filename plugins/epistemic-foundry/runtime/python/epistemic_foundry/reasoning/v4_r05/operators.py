...
from ...evolution_chamber.crossover import (
    CrossoverRejected,
...
FINDING_CODES: dict[str, str] = {
    ...
    "CROSSOVER_REPORT_MISMATCH": (
        "the compatibility report is not the canonical re-derivable assessment "
        "of both parents, so it says nothing authoritative about this splice"
    ),
...
def require_aporia_citation(
    ...
    cited_ids = {_require_text(item, "open_question_id") for item in cited}
    unmatched = sorted(cited_ids - set(left_open))
    if unmatched:
        _fail(
            "APORIA_CITATION_NOT_OPEN",
    ...
    return graph, tuple(sorted(cited_ids))
...
def _validated_compatibility_report(value: object) -> dict[str, Any]:
    report = dict(_require_mapping(value, "compatibility_report"))
    try:
        validate_artifact("crossover-compatibility-report", report)
    except ContractViolation as error:
        _fail(
            "CROSSOVER_REPORT_MISMATCH",
            "the compatibility report does not satisfy its canonical schema",
            {"schema_errors": list(error.errors)},
        )
    try:
        rebuilt = build_crossover_report(
            candidate_ids=report["candidate_ids"],
            scope_compatibility=report["scope_compatibility"],
            measurement_compatibility=report["measurement_compatibility"],
            causal_compatibility=report["causal_compatibility"],
            unit_compatibility=report["unit_compatibility"],
            conflicts=report["conflicts"],
            required_repairs=report["required_repairs"],
            report_id=report["report_id"],
        )
    except CrossoverRejected as error:
        _fail(
            "CROSSOVER_REPORT_MISMATCH",
            "the compatibility report cannot be re-derived from its own axes",
            {"reason": str(error)},
        )
    if rebuilt != report:
        _fail(
            "CROSSOVER_REPORT_MISMATCH",
            "the compatibility report differs from its canonical derivation",
            {REPORT_IDENTITY_FIELD: report.get(REPORT_IDENTITY_FIELD)},
        )
    return rebuilt


def _resolve_child_id(value: str | None, *, id_prefix: str) -> str:
    if value is None:
...
def apply_scientific_mutation(
    *,
    ...
    child_id = _resolve_child_id(child_genome_id, id_prefix=contract.id_prefix)
...
def apply_typed_crossover(
    *,
    ...
    report = _validated_compatibility_report(compatibility_report)
    named = {
        _require_text(item, REPORT_PARENTS_FIELD)
        for item in _require_sequence(
            report[REPORT_PARENTS_FIELD], REPORT_PARENTS_FIELD
        )
    }
